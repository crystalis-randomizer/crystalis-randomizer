...
module$js$util.Multiset=Multiset$$module$js$util;module$js$util.assertNever=assertNever$$module$js$util;module$js$util.isNonNull=isNonNull$$module$js$util;const REPEATABLE_FLAGS$$module$js$flagset=new Set(["S"]),PRESETS$$module$js$flagset=[{title:"Casual",descr:"Basic flags for a relatively easy playthrough.\n            Note that statue glitch may be required.",flags:"Ds Edmrsx Fw Gt Mr Rpt Sct Skm Tab"},{title:"Standard",descr:"Standard flags with full item shuffle.",flags:"Ds Emrs Fcprsw Gt Hd Mr Ps Ropst Sckmt Tab",default:!0},{title:"Advanced",descr:"Advanced flags with all logic glitches in play.",flags:"Em Fsw Gcfprt Hbdw Mert Ps Ropst Sckmt Tab Wtu"},
{title:"Full Shuffle",descr:"Randomizes as much as possible.",flags:"Emr Fcprsw Gt Hd Mert Ps Ropst Sckmt Tabmp Wmtuw"},{title:"Wild Warp",descr:"Opens up the whole world from the start.\n            Progression could be anywhere.",flags:"Emr Fcprs Gtw Hd Mert Ps Ropst Sckmt Tab"},{title:"Hardcore",descr:"Not for the faint of heart. Good luck...",flags:"Fcprsw Gt Hbdhwx Mert Ps Ropst Sckmt Tab Wmtuw"},{title:"The Full Stupid",descr:"Nobody has ever completed this.",flags:"Fcprsw Gt Hbdhmwxz Mert Ps Ropst Sckmt Tab Wmtuw"},
...
Condition$$module$js$graph$condition(688);a.OPENED_SWAN=Condition$$module$js$graph$condition(691);a.OPENED_PRISON=Condition$$module$js$graph$condition(728);a.OPENED_SEALED_CAVE=Condition$$module$js$graph$condition(750);a.START=Condition$$module$js$graph$condition(752);a.WARP_OAK=Condition$$module$js$graph$condition(758);a.WARP_JOEL=Condition$$module$js$graph$condition(762)})(Event$$module$js$graph$condition||(Event$$module$js$graph$condition={}));var Capability$$module$js$graph$condition;
...
d],[module$js$graph$condition.Capability.MONEY,module$js$graph$condition.Capability.SWORD],[module$js$graph$condition.Capability.CLIMB_WATERFALL,module$js$graph$condition.Magic.FLIGHT],[module$js$graph$condition.Capability.SHOOTING_STATUE,module$js$graph$condition.Magic.BARRIER],[module$js$graph$condition.Capability.CLIMB_SLOPE,module$js$graph$condition.Item.RABBIT_BOOTS,module$js$graph$condition.Magic.FLIGHT],[module$js$graph$condition.Event.GENERALS_DEFEATED,module$js$graph$condition.Item.IVORY_STATUE],
[module$js$graph$condition.Event.OPENED_SEALED_CAVE,module$js$graph$condition.Event.STARTED_WINDMILL]];this.flags.assumeGhettoFlight()&&a.push([module$js$graph$condition.Capability.CLIMB_WATERFALL,and$$module$js$graph$condition(module$js$graph$condition.Event.RIDE_DOLPHIN,module$js$graph$condition.Item.RABBIT_BOOTS)]);this.flags.guaranteeBarrier()||a.push([module$js$graph$condition.Capability.SHOOTING_STATUE,and$$module$js$graph$condition(module$js$graph$condition.Capability.MONEY,module$js$graph$condition.Capability.BUY_HEALING),
and$$module$js$graph$condition(module$js$graph$condition.Capability.MONEY,module$js$graph$condition.Item.SHIELD_RING),and$$module$js$graph$condition(module$js$graph$condition.Capability.MONEY,module$js$graph$condition.Magic.REFRESH)]);this.flags.leatherBootsGiveSpeed()&&a.push([module$js$graph$condition.Capability.CLIMB_SLOPE,module$js$graph$condition.Item.LEATHER_BOOTS]);for(const b of this.rom.bosses)null!=b.kill&&null!=b.drop&&a.push([module$js$graph$condition.Item(b.drop),module$js$graph$condition.Boss(b.kill)]);
a.push([module$js$graph$condition.Item.ORB_OF_WATER,module$js$graph$condition.Boss.RAGE]);this.flags.guaranteeGasMask()?a.push([module$js$graph$condition.Capability.TRAVEL_SWAMP,module$js$graph$condition.Item.GAS_MASK]):a.push([module$js$graph$condition.Capability.TRAVEL_SWAMP,or$$module$js$graph$condition(module$js$graph$condition.Item.GAS_MASK,and$$module$js$graph$condition(module$js$graph$condition.Capability.MONEY,module$js$graph$condition.Item.MEDICAL_HERB),and$$module$js$graph$condition(module$js$graph$condition.Capability.MONEY,
module$js$graph$condition.Magic.REFRESH))]);return a.map((a)=>{var [b,...c]=a;return{capability:b,condition:or$$module$js$graph$condition(...c)}})}wallCapability(a){return{flag:[module$js$graph$condition.Capability.BREAK_STONE,module$js$graph$condition.Capability.BREAK_ICE,module$js$graph$condition.Capability.FORM_BRIDGE,module$js$graph$condition.Capability.BREAK_IRON][a][0][0]}}}
const TERRAINS$$module$js$graph$overlay=(()=>{function a(a){9===(a&9)&&(a&=-2);if(!(a&4)){var b={};18===(a&18)?(a&32&&(b.exit=module$js$graph$condition.Capability.CLIMB_WATERFALL),b.enter=or$$module$js$graph$condition(module$js$graph$condition.Event.RIDE_DOLPHIN,module$js$graph$condition.Magic.FLIGHT)):(a&64?b.exit=module$js$graph$condition.Capability.CLIMB_SLOPE:a&32&&(b.exit=module$js$graph$condition.Magic.FLIGHT),a&2&&(b.enter=module$js$graph$condition.Magic.FLIGHT));a&8&&(b.enter=(b.enter||[[]]).map((a)=>
module$js$graph$condition.Capability.TRAVEL_SWAMP[0].concat(a)));a&1&&(b.enter=(b.enter||[[]]).map((a)=>module$js$graph$condition.Capability.SHOOTING_STATUE[0].concat(a)));return b}}const b=[];for(let c=0;128>c;c++)b[c]=a(c);return b})();var module$js$graph$overlay={};module$js$graph$overlay.Overlay=Overlay$$module$js$graph$overlay;class Routes$$module$js$graph$routes{constructor(){this.routes=new DefaultMap$$module$js$util(()=>new MutableRequirement$$module$js$graph$condition);this.edges=new DefaultMap$$module$js$util(()=>new Map)}addEdge(a,b,c){const d=LabeledRoute$$module$js$graph$routes(a,c);this.edges.get(b).set(d.label,d);for(const d of this.routes.get(b))this.addRoute(a,[...d,...c])}addRoute(a,b){const c=new Map,d=new Set;a=LabeledRoute$$module$js$graph$routes(a,b);c.set(a.label,a);for(a=c.values();;){const {value:b,
...
function deterministic$$module$js$pass$deterministic(a,b){addMezameTrigger$$module$js$pass$deterministic(a);addZombieWarp$$module$js$pass$deterministic(a);normalizeSwords$$module$js$pass$deterministic(a,b);fixCoinSprites$$module$js$pass$deterministic(a);fixMimics$$module$js$pass$deterministic(a);makeBraceletsProgressive$$module$js$pass$deterministic(a);addTowerExit$$module$js$pass$deterministic(a);closeCaveEntrances$$module$js$pass$deterministic(a,b);reversibleSwanGate$$module$js$pass$deterministic(a);
...
noTeleportOnThunderSword$$module$js$pass$deterministic(a);undergroundChannelLandBridge$$module$js$pass$deterministic(a);b.connectLimeTreeToLeaf()&&connectLimeTreeToLeaf$$module$js$pass$deterministic(a);simplifyInvisibleChests$$module$js$pass$deterministic(a);addCordelWestTriggers$$module$js$pass$deterministic(a,b);b.disableRabbitSkip()&&fixRabbitSkip$$module$js$pass$deterministic(a);fixReverseWalls$$module$js$pass$deterministic(a);b.chargeShotsOnly()&&disableStabs$$module$js$pass$deterministic(a);
b.orbsOptional()&&orbsOptional$$module$js$pass$deterministic(a)}function addMezameTrigger$$module$js$pass$deterministic(a){const b=a.nextFreeTrigger();b.used=!0;b.conditions=[-753];b.message=MessageId$$module$js$rom$messageid.of({action:4});b.flags=[752];a.locations.MezameShrine.spawns.push(Spawn$$module$js$rom$location.of({tile:136,type:2,id:b.id}))}
...
71;d(19)[2].flags=[169];d(19)[3].flags=[169];e(20,14)[1]=-137;c(e(22,87),-82);c(e(136,87),-82);var g=d(29);g[0].message.action=3;g[1].message.action=3;g[2].message.action=3;g[3].message.action=3;const h=d(30);(()=>{const [b,c,d,e]=h;e.condition=-70;c.condition=-1;a.npcs[30].localDialogs.set(-1,[e,d,b,c])})();for(const a of[32,33,34,124,125])f(d(a));g=d(31);g.unshift(...g.splice(1,1));a.npcs[51].spawnConditions.set(223,[-33,-28]);d(52)[1].condition=27;d(56)[3].condition=514;d(56)[3].message.action=
...
c(f)),d("09:05","[35:Fog Lamp]",c(f)),d("09:06","lamp",commonNoun$$module$js$pass$fixdialog(f)));d("0a:0c","[28:Flute of Lime]",c(a.npcs[56].data[0]));f=a.prg[250361];65>f&&b("0b:01");d("0b:01","[45:Recover]",c(f));f=a.prg[251609];65>f&&(d("0b:01","teach you\n the magic of","give you\n "),b("1d:12"));d("0b:01","[46:Barrier]",c(f));d("1d:12","[46:Barrier]",c(f));f=function(...b){var c=[(a)=>BOWS$$module$js$pass$fixdialog.has(a),(a)=>SWORD_OR_MAGIC$$module$js$pass$fixdialog.has(a),(b)=>a.items[a.itemGets[b].itemId].unique];
for(const d of c)for(const e of b){c=a.locations[e];for(const a of c.spawns)if(a.isChest()&&72>=a.id&&d(a.id))return a.id}return-1}(79,78,77,76,71,70,69,68,75,74,73,72);0<=f?d("0d:00","[35:Fog Lamp]",c(f)):d("0d:00","that a [35:Fog Lamp] was","there was treasure");f=a.npcs[195].localDialogs.get(-1)[0].condition&255;g=a.prg[250679];d("0e:03","[02:Sword of Water]",c(f));d("0e:03","[09:Ball of Water]",c(g));d("10:0c","that's","is");d("10:0c",/, is in the\+lighthouse/,"");f=e.get(35);null!=f&&(d("12:05",
"[3c:Kirisa Plant]",c(f)),d("12:10","the plant",`the ${commonNoun$$module$js$pass$fixdialog(f)}`),d("12:10","[3c:Kirisa Plant]",c(f)),f=`Our illustrious chief seeks ${vague$$module$js$pass$fixdialog(f)}.`,d("12:09",/[^]*/,f),d("12:0a",/[^]*/,f));f=e.get(116);null!=f&&(d("13:02","[3b:Love Pendant]",c(f)),d("13:00","pendant",commonNoun$$module$js$pass$fixdialog(f)));f=a.prg[251614];65>f&&d("13:02","teachs+you the magic of","bestow\n upon you the");d("13:02","[47:Change]",c(f));e=e.get(117);null!=e&&
(d("18:06","[3d:Ivory Statue]",c(e)),d("18:07","[3d:Ivory Statue]",c(e)));d("18:06","It's in a room","{0b:Karmine} is");e=a.prg[250255];65>e&&d("18:07","teach","give");d("18:07","[48:Flight]",c(e));e=a.prg[251477];65>e&&b("1c:10");d("1c:10","[42:Paralysis]",c(e));d("20:06","Statue of Gold",c(58))}const BOWS$$module$js$pass$fixdialog=new Set([62,63,64]),SWORD_OR_MAGIC$$module$js$pass$fixdialog=new Set([0,1,2,3,65,66,67,68,69,70,71,72]);
function findTornelTradeIn$$module$js$pass$fixdialog(a){for(const b of a.localDialogs.values())for(a=2;a<b.length;a++){const c=~b[a].condition;if(516<c&&524>=c&&!(c&1))return c&255}return 6}function vague$$module$js$pass$fixdialog(a){switch(a){case 37:return"an unusual statue";case 40:return"a rare instrument";case 53:return"a brilliant lamp";case 59:return"a beautiful charm";case 60:return"a fragrant plant";case 61:return"an exotic statue"}fail$$module$js$assert();return"a valuable item"}
...
_REQUIRE_HEALED_DOLPHIN_TO_RIDE:c.requireHealedDolphinToRide(),_REVERSIBLE_SWAN_GATE:!0,_SAHARA_RABBITS_REQUIRE_TELEPATHY:c.saharaRabbitsRequireTelepathy(),_SIMPLIFY_INVISIBLE_CHESTS:!0,_TELEPORT_ON_THUNDER_SWORD:c.teleportOnThunderSword(),_TRAINER:c.trainer(),_TWELVTH_WARP_POINT:!0,_UNIDENTIFIED_ITEMS:c.unidentifiedItems()},l=new Assembler$$module$js$6502;deterministicPreParse$$module$js$pass$deterministic(a.subarray(16));var m=Object.keys(k).filter((a)=>k[a]).map((a)=>`define ${a} 1\n`).join("");
l.assemble(m,"flags.s");yield g("preshuffle.s");h=new Random$$module$js$random(h);m=new Rom$$module$js$rom(a);"object"==typeof window&&(window.rom=m);m.spoiler=new Spoiler$$module$js$rom$spoiler(m);e&&(e.spoiler=m.spoiler);deterministic$$module$js$pass$deterministic(m,c);yield g("postparse.s");m.scalingLevels=48;m.uniqueItemTableAddress=l.expand("KeyItemData");c.shuffleShops()&&shuffleShops$$module$js$patch(m,c,h);randomizeWalls$$module$js$patch(m,c,h);crumblingPlatforms$$module$js$pass$crumblingplatforms(m,
h);c.randomizeWildWarp()&&shuffleWildWarp$$module$js$patch(m,c,h);rescaleMonsters$$module$js$patch(m,c,h);unidentifiedItems$$module$js$pass$unidentifieditems(m,c,h);shuffleTrades$$module$js$pass$shuffletrades(m,c,h);c.randomizeMaps()&&shuffleMazes$$module$js$pass$shufflemazes(m,h);const n=World$$module$js$graph$world.build(m,c),p=yield(new AssumedFill$$module$js$graph$shuffle(m,c)).shuffle(n.graph,h,f);if(p)n.traverse(n.graph,p),update$$module$js$rom$slots(m,p.slots);else return[a,-1];rescaleShops$$module$js$patch(m,
l,c.bargainHunting()?h:void 0);c.shuffleMonsters()&&shuffleMonsters$$module$js$patch(m,c,h);identifyKeyItemsForDifficultyBuffs$$module$js$patch(m);c.doubleBuffMedicalHerb()?(a[115996]*=2,a[115962]*=3):c.buffMedicalHerb()&&(a[115996]+=16,a[115962]*=2);c.storyMode()&&storyMode$$module$js$patch(m);shuffleMusic$$module$js$patch(m,c,h);shufflePalettes$$module$js$pass$shufflepalettes(m,c,h);c.blackoutMode()&&blackoutMode$$module$js$patch(m);misc$$module$js$patch(m,c,h);fixDialog$$module$js$pass$fixdialog(m);
c.buffDyna()&&buffDyna$$module$js$patch(m,c);c.trainer()&&(m.wildWarp.locations=[10,26,53,72,109,110,140,170,172,176,182,159,166,88,92,0]);yield m.writeData();buffDyna$$module$js$patch(m,c);h=yield postParsedShuffle$$module$js$patch(a,h,b,c,l,g);EXPAND_PRG$$module$js$patch&&(m=a.subarray(16),m.subarray(507904,524288).set(m.subarray(245760,262144)));return[a,h]})}
...
voiceReplacements$$module$js$tracker=[[/\bketostix\b/,"key 2 styx"],[/\bketo\b/,"key 2"],[/\b(sticks|stick c|stix e|stick sea|dixie|sticks e|stick see|sexy|60|sixty)\b/,"styx"],[/\b(arraylist|gorilla[sz]|aurelius|h?airless|a realist|arl[eiy]ss?)\b/,"aryllis"],[/\b(amazon us|amazon[ae]?ss?|amazon)\b/,"amazones"],[/\bamazones basement\b/,"aryllis basement"],[/\b(deal|dil|diehl)\b/,"deo"],[/\b(ds|deals|diaz|delos|drose|theos)\b/,"deos"],[/\bone\b/,"1"],[/\b(two|ii|to|too)\b/,"2"],[/\bthree\b/,"3"],[/\bfour\b/,
"4"],[/\bar[ck]s tom\b/,"ark stom"],[/\borbit\b/,"orb of"],[/^(contract|amtrack|ontra(c|k|ck))\b/,"untrack"],[/^((ch|tr)[eu](c|k|ck))\b/,"track"],[/^track ?suit\b/,"track flute"],[/\b(lyme)\b/,"lime"],[/marked\b/,"mark"],[/^(marc|mach|smart|[bp]ark)\b/,"mark"],[/\blee felder\b/,"leaf elder"],[/^markley f/,"mark leaf "],[/^(mark of|market)\b/,"mark"],[/\bleif\b/,"leaf"],[/\beldar\b/,"elder"],[/\blight\b/,"flight"],[/\bmann\b/,"moon"],[/\bbosemann?\b/,"bow of moon"],[/\b(certifier|start a fire)\b/,
"sword of fire"],[/\bwhen milky\b/,"windmill key"],[/^(4 clear|faux clear|folklore|so ?clear|pho clear)\b/,"full clear"],[/\b(mayo|meto|nato|mader|meadow|matter|ma[dt]er|motto|model)\b/,"mado"],[/\bcalvey\b/,"kelby"],[/\b(kelbyone|lv ?1)\b/,"kelby 1"],[/\blv ?2\b/,"kelby 2"],[/\b(carmine|carmen|combine)\b/,"karmine"],[/\b(dra[gk][ao]n)\b/,"draygon"],[/\b([cs][ae][bdgv][aeiu]*r[ae]|xavier)\b/,"sabera"],[/\b(wright|rite|write)\b/,"right"],[/\b([ck]or[bd]el[le]?|quadrel)\b/,"cordel"],[/\b(hotel desk|(bel)?l desk|kill basque|kehl basc|caleb ask(ed)?)\b/,
"kelby"],[/\b(porter|port[eo]la|porto a)\b/,"portoa"],[/\b(athena|cena|tina|isina|esquina)\b/,"asina"],[/\b((at the|ec[hk]o) hann?ah?|alcohol(ic)?)\b/,"akahana"],[/\b((roca|broke[nr]|brokaw|barr?oca) hann?ah?|bro k[ao]h[ao]na|pokehana)\b/,"brokahana"],[/\b(stoned)\b/,"stone"],[/\b(guards)\b/,"guard"],[/\b(window)\b/,"windmill"],[/\b(sa[bv][eo]r)\b/,"sabre"],[/\bsabre south\b/,"sabre west"],[/\b(hebrew|cebu|sebba|zabel)\b/,"zebu"],[/\b([bct]([aou]r|r[aou])nn?ell?)\b/,"tornel"],[/\b(clarke)\b/,"clark"],
[/\bsabera 1 left\b/,"sabera fortress left"],[/\bsabera 1 right\b/,"sabera fortress right"],[/\b(cantu|cancel|can sue|kincer|kenzo|cancer)\b/,"kensu"],[/\b(light house)\b/,"lighthouse"],[/\bkensu(?: in)? (lighthouse|swan)\b/,"$1"],[/\b(kensu slime|slime kensu)\b/,"slime"],[/\bunderground channel\b/,"channel"],[/\b(?:mount|mt) (sabre|hydra)\b/,"$1"],[/\b([ck]h?[au]r[iu]ss?[aou])\b/,"kirisa"],[/\bmado (lower|upper)\b/,"mado 2 $1"],[/\bsabera( 2)? (level|chest|sewer)\b/,"sabera 2 level"],[/\b(script|richt)\b/,
"crypt"],[/\bdraygon 1\b/,"draygon"],[/\besi\b/,"evil spirit island"],[/\bsabre north summit\b/,"sabre summit"],[/\bkirisa plant cave\b/,"kirisa cave"],[/\bvampire cave\b/,"sealed cave"],[/^((?:un)?)ma[urxkcs ]*t[io]me?(?: fight)?/,"$1mark stom"],[/^((?:un)?)mark telepathy/,"$1mark stom"],[/^((?:un)?)mark battle (armor|suit)/,"$1mark oasis cave flight"],[/^((?:un)?)mark power ring/,"$1mark oasis cave center"],[/^((?:un)?)mark grass/,"$1mark cordel grass"]],fullClears$$module$js$tracker=new Map([["sealed cave",
["sealed cave front","sealed cave back","vampire 1"]],["styx",["styx left","styx right"]],["oak",["oak elder","oak mother"]],["sabre west",["sabre west slope","sabre west","tornel"]],["sabre north",["sabre north","kelby 1","sabre summit"]],["waterfall cave",["waterfall cave","stone akahana"]],["fog lamp",["fog lamp front","fog lamp back"]],["kirisa plant",["kirisa cave","kirisa meadow"]],["karmine",["karmine basement","karmine","behind karmine","slime"]],["amazones",["aryllis","aryllis basement"]],
["mado 2",["mado 2","mado 2 upper","mado 2 lower"]],["pyramid",["pyramid","draygon"]],["hydra",["hydra front","hydra back","hydra summit"]],["sabera 1",["sabera fortress left","sabera fortress right","vampire 2","sabera 1","clark"]],["oasis cave",["oasis cave","oasis cave flight","oasis cave center"]],["sabera 2",["sabera 2","sabera 2 level"]]]),SLOTS$$module$js$tracker=[[0,121,192,"leaf elder"],[1,274,176,"oak elder"],[2,335,123,"waterfall cave"],[3,77,10,"styx left"],[5,89,107,"sealed cave front"],
[6,115,224,"sabre west slope"],[7,282,187,"insect"],[8,47,182,"kelby 1"],[9,251,232,"rage"],[10,206,249,"aryllis basement"],[11,83,63,"mado 1"],[12,23,9,"behind karmine"],[18,49,48,"mado 2"],[20,77,2,"styx right"],[118,70,3,"styx right"],[119,84,3,"styx right"],[27,168,96,"oasis cave flight"],[28,199,110,"draygon"],[29,82,95,"sealed cave back"],[30,82,101,"sealed cave back"],[31,346,147,"fog lamp front"],[112,346,153,"fog lamp front"],[113,346,159,"fog lamp front"],[32,126,52,"hydra front"],[33,227,
97,"sabera fortress left"],[34,256,73,"evil spirit island"],[35,58,115,"sabera 2"],[36,82,113,"sealed cave front"],[37,189,180,"cordel grass"],[38,18,172,"kelby 2"],[39,267,185,"oak mother"],[40,275,147,"portoa queen"],[41,147,206,"akahana"],[42,172,104,"oasis cave center"],[43,203,5,"brokahana"],[44,249,69,"evil spirit island"],[45,191,110,"deo"],[46,89,99,"vampire 1"],[47,164,104,"oasis cave"],[48,319,123,"stone akahana"],[114,320,130,"waterfall cave"],[50,105,94,"windmill guard"],[51,64,198,"sabre north"],
[52,83,71,"zebu"],[53,345,140,"fog lamp back"],[54,301,119,"dolphin"],[55,233,118,"clark"],[56,234,88,"sabera 1"],[57,295,92,"lighthouse"],[59,274,117,"channel"],[60,338,226,"kirisa meadow"],[61,23,17,"karmine"],[62,206,241,"aryllis"],[63,101,6,"hydra summit"],[64,207,110,"draygon"],[65,92,117,"windmill"],[66,279,126,"sabre summit"],[67,202,138,"stom"],[68,124,202,"tornel"],[69,304,128,"asina"],[70,248,35,"whirlpool"],[71,277,3,"swan"],[72,15,25,"slime"],[80,82,107,"sealed cave front"],[82,134,219,
"sabre west"],[83,59,219,"sabre north"],[84,52,55,"mado 2 upper"],[85,241,97,"sabera fortress right"],[86,123,23,"hydra front"],[116,115,3,"hydra back"],[87,70,9,"styx left"],[117,84,9,"styx left"],[88,32,38,"karmine basement"],[121,32,16,"karmine basement"],[122,40,16,"karmine basement"],[123,40,38,"karmine basement"],[90,161,97,"fortress exit"],[16,327,123,"waterfall cave"],[92,256,79,"evil spirit island"],[93,36,139,"sabera 2 level"],[94,14,229,"sabre north"],[95,345,225,"kirisa cave"],[96,18,
94,"mado 2 upper"],[97,234,96,"vampire 2"],[98,18,118,"sabera 2 level"],[99,36,54,"mado 2 upper"],[100,175,97,"oasis cave"],[101,139,40,"hydra back"],[102,66,160,"sabera 2 level"],[105,131,201,"sabre west"],[106,76,226,"sabre west"],[107,18,100,"mado 2 upper"],[108,193,103,"pyramid"],[120,199,103,"crypt"],[109,205,103,"crypt"],[115,256,67,"evil spirit island"],[110,24,38,"karmine basement"],[111,44,97,"mado 2 lower"]],KEY$$module$js$tracker=new Set([16,18,35,38,97]),BOSSES$$module$js$tracker=new Map([[-257,
46],[-258,7],[-259,8],[-260,9],[-261,56],[-262,11],[-263,38],[-264,35],[-265,18],[-266,61],[-267,28],[-269,97]]),TRADES$$module$js$tracker=new Set([41,62,68,71,72]);
class Graph$$module$js$tracker{constructor(a,b,c){this.rom=a;this.world=b;this.flags=c;this.slots=new Map;this.items=new Map;this.slotElts=new Map;this.has=Bits$$module$js$bits.of();this.tracks=new Map;this.marks=new Map;this.voice=!1;window.GRAPH=this;this.grid=document.getElementsByClassName("grid")[0];this.map=document.getElementsByClassName("map")[0];c=Bits$$module$js$bits.of();for(var d of b.graph.slots){const {item:a,index:b}=d;null!=a&&this.slots.set(a,b)}for(const e of b.graph.items){const {item:b,
index:g}=e;null!=b&&(this.items.set(b,g),(d=a.items[b])&&!d.unique&&(c=Bits$$module$js$bits.with(c,g)))}this.unusedItems=b.graph.items.length;this.has=this.always=c;this.grid.addEventListener("click",(a)=>{let b=a.target;for(;b&&!b.dataset.index;)b=b.parentElement;b&&(this.toggle(b),a.preventDefault())})}toggle(a,b){b=Number(a.dataset.index);const c=a.classList.toggle("got");a.dataset.item&&(this.has=c?Bits$$module$js$bits.with(this.has,b):Bits$$module$js$bits.without(this.has,b));this.update()}addSlot(a,
b,c,d){const e=this.slots.get(a);if(null==e){debugger;throw Error();}const f=document.createElement("div"),g=this.rom.itemGets[a],h=g&&this.rom.items[g.itemId];if(h&&h.unique||KEY$$module$js$tracker.has(a))f.classList.add("key"),b--,c--;b--;c--;f.dataset.index=String(e);f.style.left=b+"px";f.style.top=c+"px";b=document.createElement("div");f.appendChild(b);b.textContent=112<=a?"Mimic":this.rom.items[g.itemId].messageName.replace(" ","\u00a0");this.flags.randomizeTrades()&&TRADES$$module$js$tracker.has(a)&&
f.classList.add("boss");this.slotElts.set(e,f);a=this.marks.get(d);null==a&&this.marks.set(d,a=[]);a.push(f);this.map.appendChild(f)}addItem(a,b,...c){b=Number.parseInt(b.substring(1),16);const d=document.getElementsByClassName(a)[0];var e=document.createElement("div");d.appendChild(e);e=this.items.get(b);null==e&&this.items.set(b,e=this.unusedItems++);d.dataset.index=String(e);d.dataset.item=String(e);this.tracks.set(a.replace(/-/g," "),d);for(const a of c)this.tracks.set(a.replace(/-/g," "),d)}addExtraFlags(){var a=
this.world.graph;for(const b of a.slots.slice(a.fixed))if(a=(a=BOSSES$$module$js$tracker.get(b.condition))&&this.slots.get(a),null!=a){a=this.slotElts.get(a);if(null==a)throw Error("expected");this.slotElts.delete(Number(a.dataset.index));this.slotElts.set(b.index,a);a.classList.add("boss");a.dataset.index=String(b.index);a.dataset.item=String(b.index)}}update(){for(var a of this.slotElts.values())a.dataset.state=a.classList.contains("got")?"":"blocked";for(const b of traverse$$module$js$graph$shuffle(this.world.graph,
newFill$$module$js$graph$shuffle(),this.has))(a=this.slotElts.get(b))&&!a.classList.contains("got")&&(a.dataset.state="available")}addVoiceRecognition(){try{const a=this.recognition=new SpeechRecognition,b=new SpeechGrammarList;b.addFromString(`
          #JSGF V1.0;
          grammar command;
      ...
      `,1);a.lang="en-US";a.grammars=b;a.interimResults=!1;a.maxAlternatives=10;a.onstart=()=>{this.voice=!0};a.onresult=(b)=>{const c=new Set;var e=b.results[b.results.length-1];if(e.isFinal){b=!1;for(var f of e){let a=f.transcript.toLowerCase().replace(/[^a-z ]/g,"");c.add(a);"stop listening"===a&&(b=!0,this.voice=!1);for(const b of voiceReplacements$$module$js$tracker){const [d,e]=b;a=a.replace(d,e);c.add(a)}if(e=/^(track|untrack|clear|unclear|full clear) (.*)/.exec(a)){if(e[1].endsWith("track")){var g=
this.tracks.get(e[2]);if(!g)continue;this.toggle(g,!e[1].startsWith("un"))}else if(e[1].endsWith("clear")){g=fullClears$$module$js$tracker.get(e[2]);if(!g){if(!this.marks.has(e[2]))continue;g=[e[2]]}for(const a of g)for(const b of this.marks.get(a))b.classList.toggle("got",!e[1].startsWith("un"))}b=!0;break}if(e=/^(?:un)?mark(?: (\d+)(?: che(?:st|ck)s?)?(?: in))? (.*)/.exec(a))if(g=this.marks.get(e[2]),e=Number(e[1]||"1"),g&&e){f=!a.startsWith("un");for(const a of g)if(a.classList.contains("got")!==
f&&(a.classList.toggle("got",f),0===--e))break;b=!0;break}}b||console.log(`No match: ${[...c].join(", ")}`);a.stop()}};a.onend=()=>{this.voice&&a.start()};return!0}catch(a){return console.error(a),!1}}startVoice(){if(!this.recognition&&!this.addVoiceRecognition())return!1;this.voice=!0;this.recognition.start();return!0}stopVoice(){this.voice=!1;this.recognition&&this.recognition.stop()}}
function polyfill$$module$js$tracker(...a){const b=window;for(let c of a)if("function"===typeof b[c]){b[a[0]]=b[c];return}console.error(`Could not polyfill ${a[0]}`)}polyfill$$module$js$tracker("SpeechRecognition","webkitSpeechRecognition");polyfill$$module$js$tracker("SpeechGrammarList","webkitSpeechGrammarList");let flags$$module$js$tracker="Rlpt Tb";for(const a of location.hash.substring(1).split("&")){const [b,c]=a.split("=");"flags"===b&&(flags$$module$js$tracker=decodeURIComponent(c))}
...
d.addExtraFlags();d.update();document.getElementById("toggle-map").addEventListener("click",()=>{d.map.classList.toggle("hidden")});document.getElementById("clear-all").addEventListener("click",()=>{for(const a of d.grid.querySelectorAll(".got"))a.classList.remove("got");d.has=d.always;d.update()});const e=document.getElementById("voice");e.addEventListener("click",()=>{d.voice?(d.stopVoice(),e.textContent="enable voice"):d.startVoice()&&(e.textContent="disable voice")});window.graph=d})}main$$module$js$tracker();
