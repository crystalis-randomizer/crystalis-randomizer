...
"Early flight")).option(kb).option(lb),Wa=(new Condition$$module$nodes(a,"Sword charge glitch")).option(Id).option(Ca),pe=(new Condition$$module$nodes(a,"Rabbit skip")).option(p).option(Ca),Ab=(new Condition$$module$nodes(a,"Talk glitch")).option(k).option(Ca),qe=(new Condition$$module$nodes(a,"Ghetto flight")).option(h).option(Ca),re=(new Condition$$module$nodes(a,"Wild warp")).option(Gd).option(Ca,Hd),Xa=(new Condition$$module$nodes(a,"Any level 2 sword")).option(M,Ka).option(M,Da).option(S,La).option(S,
...
new FlagSet$$module$flagset("Sbkm Sct"));var g={};for(var h of f.nodes)if(h instanceof Slot$$module$nodes&&h.slots){var k=h.slotType?h.slotType[0]:"c";(g[k]=g[k]||[]).push(h)}h=[];k=[];var p={};for(var m of d.flags.S){var q=[];for(var n of m){p[n]=!0;for(var u of g[n])q.push(u),k.push([u,h.length])}h.push(q)}for(var l of"bc")if(p[l])for(m=g[l],n=m.map((a)=>[a.item,a.itemIndex]),a.shuffle(n),u=0;u<m.length;u++)m[u].set(...n[u]);q=(a)=>f.nodes.find((b)=>b instanceof Slot$$module$nodes&&b.name===a);
g=q("Sword of Wind");l=q("Sword of Fire");m=q("Ball of Wind");n=q("Ball of Fire");u=q("Statue of Onyx");q=q("Gas Mask");p.k&&a.nextInt(2)&&(l.swap(g),n.swap(m));p.k&&!a.nextInt(3)&&u.swap(q);p=[1,1,1,1,2,2,2,3,3,3,5,5,8,13];g=[];e&&e.addTasks(10);for(l=0;1E3>l;l++){e&&0==l%100&&(yield new Promise(requestAnimationFrame),e.addCompleted(1));a.shuffle(k);for(var y of h)a.shuffle(y);m=h.map(()=>-1);n=p[a.nextInt(p.length)];u=[];const b=(a)=>(a instanceof Chest$$module$nodes||a instanceof BossDrop$$module$nodes)&&
...
Slot$$module$nodes&&a.write(b);if(c){c.items=[];c.route=[];for(const a of g){const [b,d]=a;c.route.push(d);null!=b.slotName&&(y=b.slotName,0>y.indexOf(b.orig)&&(y+=` (normally ${b.orig})`),c.items.push({slotIndex:b.slotIndex,itemIndex:b.itemIndex,origName:b.vanillaItemName,slotName:b.slotName,itemName:b.item.name,text:`${b.item.name}: ${y}`}))}}}),shuffle2$$module$depgraph=(b,a,c,d,e)=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){const f=generate$$module$depgraph(d),g=f.integrate();e&&
e.addTasks(1E3);for(let h=0;1E3>h;h++){try{yield shuffle3$$module$depgraph(f,g,b,a,c,d,e)}catch(k){e&&(e.addCompleted(1),0===h%50&&(yield new Promise(requestAnimationFrame)));continue}console.log(`success after ${h} attempts`);return}throw Error("failed");}),shuffle3$$module$depgraph=(b,a,c,d,e,f,g)=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){var g=b.nodes.filter((a)=>a instanceof Slot$$module$nodes&&a.slots&&a.slotName),k=new Map(d.shuffle(g.map((a)=>[a,[a.item,a.itemIndex]])));const p=
new Set(d.shuffle(g)),m=new Map,q={};f||(f=new FlagSet$$module$flagset("Sbkm Sct"));f.flags.S.forEach((a,b)=>{for(const c of a)q[c]=b});for(var n of p)m.set(n.item,n);const u=(a)=>13<=a&&36>=a||38===a||40===a||49===a||72<a&&91!=a,l=(a,b)=>q[a.slotType?a.slotType[0]:"c"]==q[b.slotType?b.slotType[0]:"c"]&&(112!==b.itemIndex||a instanceof Chest$$module$nodes&&!a.isInvisible)&&(!u(b.itemIndex)||(a instanceof Chest$$module$nodes||a instanceof BossDrop$$module$nodes)&&9!==a.origIndex)?!0:!1;n=a.assumedFill(d,
(a,b)=>l(a,m.get(b)));if(null==n)throw Error("Could not fill!");const y=new Map;for(let b=0;b<n.length;b++){var z=n[b];if(null!=z){var ja=m.get(a.item(z));z=k.get(ja);k.delete(ja);ja=a.location(b);y.set(ja,z);p.delete(ja)}}n=(a,b)=>{for(const c of p)if(l(c,a)){y.set(c,b);p.delete(c);return}throw Error("Could not fill extra items");};for(const a of k){const [b,c]=a;112===b.itemIndex&&(n(b,c),k.delete(b))}for(const a of k){const [b,c]=a;u(b.itemIndex)&&(n(b,c),k.delete(b))}for(const a of k){const [b,
c]=a;n(b,c)}for(const a of y){const [b,c]=a;b.set(...c)}const {win:kb,path:Ba}=b.traverse({wanted:g,dfs:!1});if(!kb)throw Error("FAIL");for(const a of b.nodes)a instanceof Slot$$module$nodes&&a.write(c);if(e){e.items=[];e.route=[];for(const a of Ba){const [c,d]=a;e.route.push(d);g=b.nodes[c];null!=g.slotName&&(k=g.slotName,0>k.indexOf(g.orig)&&(k+=` (normally ${g.vanillaItemName})`),e.items.push({slotIndex:g.slotIndex,itemIndex:g.itemIndex,origName:g.vanillaItemName,slotName:g.slotName,itemName:g.item.name,
text:`${g.item.name}: ${k}`}))}}});var module$depgraph={};module$depgraph.generate=generate$$module$depgraph;module$depgraph.shuffle=shuffle$$module$depgraph;module$depgraph.shuffle2=shuffle2$$module$depgraph;module$depgraph.shuffle3=shuffle3$$module$depgraph;class Deque$$module$util{constructor(){this.buffer=Array(16);this.mask=15;this.size=this.end=this.start=0}get length(){return this.size}upsize(b){for(;this.mask<b;)this.end<=this.start&&(this.start+=this.mask+1),this.mask=this.mask<<1|1,this.buffer=this.buffer.concat(this.buffer);this.size=b}push(...b){this.upsize(this.size+b.length);for(const a of b)this.buffer[this.end]=a,this.end=this.end+1&this.mask}pop(){if(this.size)return this.end=this.end-1&this.mask,this.size--,this.buffer[this.end]}peek(){if(this.size)return this.buffer[this.end-
...
module$graph.SparseDependencyGraph=SparseDependencyGraph$$module$graph;class TrackerNode$$module$nodes extends Node$$module$graph{constructor(b,a){super(b,a)}get nodeType(){return"Tracker"}}class Option$$module$nodes extends Node$$module$graph{constructor(b,a,c){super(b,a);this.value=c}get nodeType(){return"Option"}edges(){return this.value?[Edge$$module$graph.of(this)]:[]}}
class Slot$$module$nodes extends Node$$module$graph{constructor(b,a,c,d,e=[]){super(b,a);this.slotName=a;this.slotIndex=d;this.slotType=c instanceof Magic$$module$nodes?"magic":null;this.vanillaItemName=c.name;this.slots=e;this.item=c;this.itemIndex=d}get nodeType(){return"Slot"}toString(){return`${super.toString()} [${this.vanillaItemName} $${this.slotIndex.toString(16).padStart(2,0)}]`}edges(){return null!=this.item&&null!=this.itemIndex?[Edge$$module$graph.of(this.item,this)]:[]}get name2(){return this.item==
...
class Location$$module$nodes extends Node$$module$graph{constructor(b,a,c,d){super(b,c.name+": "+d);this.simpleName=d;this.id=a;this.area=c;this.connections=[];this.chests=[];this.type=this.bossNode=null;this.isStart=!1}get nodeType(){return"Location"}toString(){return`Location ${this.id.toString(16).padStart(2,0)} ${this.name}`}edges(){const b=[];for(const a of this.connections)b.push(Edge$$module$graph.of(a.to,a.from,...a.deps,...a.from.bossNode?[a.from.bossNode]:[])),a.bidi&&b.push(Edge$$module$graph.of(a.from,
...
a.item:a);b.reqs.push(Edge$$module$graph.of(b,this,...a));return this}boss(b){this.bossNode=b;b.reqs.push(Edge$$module$graph.of(b,this,...b.deps));return this}overworld(){this.type="overworld";return this}town(){this.type="town";return this}cave(){this.type="cave";return this}sea(){this.type="sea";return this}fortress(){this.type="fortress";return this}house(){this.type="house";return this}shop(){this.type="house";return this}misc(){this.type="misc";return this}start(){this.isStart=!0;return this}fullName(){const b=
[this.simpleName];this.bossNode&&b.push(this.bossNode.name);return b.join("\\n")}}const checkBounds$$module$nodes=(b,a,...c)=>{if(b>a.length)throw Error("never found: "+c.map((a)=>"number"==typeof a?a.toString(16):a).join(" "));};
class WorldGraph$$module$nodes extends Graph$$module$graph{integrate(b={}){const {removeConditions:a=!0,removeTriggers:c=!0,removeOptions:d=!0,removeTrackers:e=!0}=b,f=new SparseDependencyGraph$$module$graph(this.nodes.length),g=[],h=new Set,k=new Map;var p=[],m=[];const q=[];var n=[];const u=[];b=new Set;for(var l of this.nodes)if(l instanceof Location$$module$nodes){if(l.isStart){const [a]=f.addRoute([l.uid]);k.set(a.label,a)}g[l.uid]=[];for(var y of l.connections)h.has(y)||(h.add(y),y.bidi&&h.add(y.reverse()));
for(const a of l.chests)f.addRoute([a.uid,l.uid])}else if(l instanceof Slot$$module$nodes&&"Alarm Flute"===l.item.name)f.addRoute([l.item.uid]),f.finalize(l.item.uid);else if(!(l instanceof Slot$$module$nodes&&null==l.slotName))if(l instanceof ItemGet$$module$nodes&&"Medical Herb"===l.name&&(f.addRoute([l.uid]),f.finalize(l.uid)),l instanceof Option$$module$nodes)p.push(l);else if(l instanceof TrackerNode$$module$nodes)m.push(l);else if(l instanceof Condition$$module$nodes)q.push(l);else if(l instanceof
Trigger$$module$nodes)n.push(l);else if(l instanceof ItemGet$$module$nodes)u.push(l);else if(l instanceof Slot$$module$nodes)b.add(l);else if(!(l instanceof Area$$module$nodes))throw Error(`Unknown node type: ${l.nodeType}`);for(const a of h)g[a.from.uid].push(a);l=(a)=>{for(const b of a){for(const a of b.edges())f.addRoute(a);f.finalize(b.uid)}};d&&l(p);e&&l(m);c&&l(n);for(p=k.values();!(m=p.next()).done;){m=m.value;n=m.target;for(var z of g[n]){n=[z.to.uid,...m.deps];for(y=z.deps.length-1;0<=y;y--)n.push(z.deps[y].uid);
z.from.bossNode&&n.push(z.from.bossNode.uid);for(const a of f.addRoute(n))k.has(a.label)||k.set(a.label,a)}}a&&l(q);for(z=0;z<this.nodes.length;z++)this.nodes[z]instanceof ItemGet$$module$nodes||f.finalize(z);z=new LocationList$$module$nodes(this);for(const a of b)for(const b of f.nodes[a.uid].values())z.addRoute([a.uid,...b]);return z}}
...
RNMX$$module$random)}nextInt(b){return Math.floor(this.next()*b)}shuffle(b){for(let a=b.length;a;){const c=this.nextInt(a--);[b[a],b[c]]=[b[c],b[a]]}return b}}var module$random={};module$random.Random=Random$$module$random;const STATUS$$module$version="unstable",VERSION$$module$version="assumed",LABEL$$module$version="assumed 956f677",HASH$$module$version="956f677e0672325716f60bd5e8c257b611fe9edd",DATE$$module$version=new Date(1551217271E3);var module$version={};module$version.STATUS=STATUS$$module$version;module$version.VERSION=VERSION$$module$version;module$version.LABEL=LABEL$$module$version;module$version.HASH=HASH$$module$version;module$version.DATE=DATE$$module$version;var $jscompDefaultExport$$module$patch={apply(b,a){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){let c;a.seed||(a.seed=parseSeed$$module$patch("").toString(16),window.location.hash+="&seed="+a.seed);c=a.flags?new FlagSet$$module$flagset(a.flags):new FlagSet$$module$flagset("Em Gt Mr Rlpt Sbk Sct Sm Tasd");for(const b in a)"false"===a[b]&&(a[b]=!1);yield shuffle$$module$patch(b,parseSeed$$module$patch(a.seed),c)})}};
...
,,,8,,,],[247,"p","skeleton shot",,,,11,,,],[248,"p","lavaman shot",,,,14,,,],[249,"p","black knight flail",,,,18,,,],[250,"p","lizardman flail",,,,21,,,],[252,"p","mado shuriken",,,,36,,,],[253,"p","guardian statue missile",,,,23,,,],[254,"p","demon wall fire",,,,23,,,]].map((b)=>{var [a,c,d,e=0,f=0,g=0,h=0,k=0,p=0]=b;return[a,{id:a,type:c,name:d,sdef:e,swrd:f,hits:g,satk:h,dgld:k,sexp:p}]}));
...
this.report["$"+d.id.toString(16).padStart(2,0)]=[];const {maxFlyers:g,nonFlyers:h={},fixedSlots:k={}}=MONSTER_ADJUSTMENTS$$module$patch[d.id]||{};let p=k.pat0||null,m=k.pat1||null,q=k.pal2||null,n=k.pal3||null,u=g,l=!1;for(const b of d.objects||[])if(2==(b[2]&7)&&(l=!0),!(b[2]&7)){var a=b[3]+80;126==a||127==a||159==a?m=98:143==a&&(p=97)}if(21==d.id||71==d.id)l=!1;f.push(`Initial pass: ${[l,p,m,q,n].join(", ")}`);a=(a)=>{var b=FLYERS$$module$patch.has(a.id),c=MOTHS_AND_BATS$$module$patch.has(a.id);
if(b){if(!u)return!1;--u}if(null!=q&&null!=a.pal2&&q!=a.pal2||null!=n&&null!=a.pal3&&n!=a.pal3)return!1;var g=!l||TREASURE_CHEST_BANKS$$module$patch.has(a.pat);let k;if(d.rom.objects[a.id].child||RETAIN_SLOTS$$module$patch.has(a.id)){const b=(k=a.patSlot?128:0)?m:p;if(null!=b&&b!=a.pat)return!1;if(k)m=a.pat;else if(g)p=a.pat;else return!1;f.push(`  Adding ${a.id.toString(16)}: pat(${k}) <-  ${a.pat.toString(16)}`)}else if(null==p&&g||p==a.pat)p=a.pat,k=0,f.push(`  Adding ${a.id.toString(16)}: pat0 <-  ${a.pat.toString(16)}`);
...
e.dataset.flag=c;b.parentElement.insertBefore(e,b);const f=document.createElement("label");f.textContent=c;f.htmlFor=e.id;b.parentElement.insertBefore(f,b);const g=document.createElement("div");b.parentElement.insertBefore(g,b);const h=document.createElement("label");h.textContent=d;h.htmlFor=e.id;g.appendChild(h);a&&g.appendChild(a);b.classList.contains("hard")&&(f.classList.add("hard"),h.classList.add("hard"),h.textContent+=" *");b.remove();e.addEventListener("change",()=>{window.FLAGS=flags$$module$main;
