...
e.addTasks(100);for(let h=0;1E3>h;h++){try{yield shuffle3$$module$depgraph(f,g,b,a,c,d,e)}catch(k){e&&(e.addCompleted(1),0===h%5&&(yield new Promise(requestAnimationFrame)));continue}console.log(`success after ${h} attempts`);return}throw Error("failed");}),shuffle3$$module$depgraph=(b,a,c,d,e,f,g)=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){var g=b.nodes.filter((a)=>a instanceof Slot$$module$nodes&&a.slots&&a.slotName),k=new Map(d.shuffle(g.map((a)=>[a,[a.item,a.itemIndex]])));const p=
...
RNMX$$module$random)}nextInt(b){return Math.floor(this.next()*b)}shuffle(b){for(let a=b.length;a;){const c=this.nextInt(a--);[b[a],b[c]]=[b[c],b[a]]}return b}}var module$random={};module$random.Random=Random$$module$random;const STATUS$$module$version="unstable",VERSION$$module$version="assumed",LABEL$$module$version="assumed cf33553",HASH$$module$version="cf33553af95d0f33fe3dce7fc824f2dc2e465776",DATE$$module$version=new Date(1551217183E3);var module$version={};module$version.STATUS=STATUS$$module$version;module$version.VERSION=VERSION$$module$version;module$version.LABEL=LABEL$$module$version;module$version.HASH=HASH$$module$version;module$version.DATE=DATE$$module$version;var $jscompDefaultExport$$module$patch={apply(b,a){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){let c;a.seed||(a.seed=parseSeed$$module$patch("").toString(16),window.location.hash+="&seed="+a.seed);c=a.flags?new FlagSet$$module$flagset(a.flags):new FlagSet$$module$flagset("Em Gt Mr Rlpt Sbk Sct Sm Tasd");for(const b in a)"false"===a[b]&&(a[b]=!1);yield shuffle$$module$patch(b,parseSeed$$module$patch(a.seed),c)})}};
