...
e&&e.addTasks(100);for(let h=0;1E3>h;h++){try{yield shuffle3$$module$js$depgraph(f,g,b,a,c,d,e)}catch(k){e&&(e.addCompleted(1),0===h%5&&(yield new Promise(requestAnimationFrame)));continue}console.log(`success after ${h} attempts`);return}throw Error("failed");}),shuffle3$$module$js$depgraph=(b,a,c,d,e,f,g)=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){var g=b.nodes.filter((a)=>a instanceof Slot$$module$js$nodes&&a.slots&&a.slotName),k=new Map(d.shuffle(g.map((a)=>[a,[a.item,a.itemIndex]])));
...
return Math.min(AM$$module$js$random*this.iy,RNMX$$module$js$random)}nextInt(b){return Math.floor(this.next()*b)}shuffle(b){for(let a=b.length;a;){const c=this.nextInt(a--);[b[a],b[c]]=[b[c],b[a]]}return b}}var module$js$random={};module$js$random.Random=Random$$module$js$random;const STATUS$$module$js$version="unstable",VERSION$$module$js$version="assumed",LABEL$$module$js$version="assumed cf33553",HASH$$module$js$version="cf33553af95d0f33fe3dce7fc824f2dc2e465776",DATE$$module$js$version=new Date(1551217183E3);var module$js$version={};module$js$version.STATUS=STATUS$$module$js$version;module$js$version.VERSION=VERSION$$module$js$version;module$js$version.LABEL=LABEL$$module$js$version;module$js$version.HASH=HASH$$module$js$version;module$js$version.DATE=DATE$$module$js$version;var $jscompDefaultExport$$module$js$patch={apply(b,a){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){let c;a.seed||(a.seed=parseSeed$$module$js$patch("").toString(16),window.location.hash+="&seed="+a.seed);c=a.flags?new FlagSet$$module$js$flagset(a.flags):new FlagSet$$module$js$flagset("Em Gt Mr Rlpt Sbk Sct Sm Tasd");for(const b in a)"false"===a[b]&&(a[b]=!1);yield shuffle$$module$js$patch(b,parseSeed$$module$js$patch(a.seed),c)})}};
