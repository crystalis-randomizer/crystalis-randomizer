...
ZeroPageX:148,Absolute:140},tax:{Implied:170},tay:{Implied:168},tsx:{Implied:186},txa:{Implied:138},txs:{Implied:154},tya:{Implied:152}};var module$js$6502={};module$js$6502.assemble=assemble$$module$js$6502;module$js$6502.buildRomPatch=buildRomPatch$$module$js$6502;const Bits$$module$js$bits_bigint={of:(...b)=>{let a=ZERO$$module$js$bits_bigint;for(const c of b)a|=ONE$$module$js$bits_bigint<<BigInt(c);return a},from:(b)=>{let a=ZERO$$module$js$bits_bigint;for(const c of b)a|=ONE$$module$js$bits_bigint<<BigInt(c);return a},containsAll:(b,a)=>!(a&~b),with:(b,a)=>b|ONE$$module$js$bits_bigint<<BigInt(a),without:(b,a)=>b&~(ONE$$module$js$bits_bigint<<BigInt(a)),has:(b,a)=>!!(b&ONE$$module$js$bits_bigint<<BigInt(a)),bits:(b)=>{const a=[];let c=0;for(;b;){let d=Number(b&
MAX_UINT32$$module$js$bits_bigint),e=32;for(;d;){const b=Math.clz32(d)+1;e-=b;d<<=b;32==b&&(d=0);a.push(c|e)}b>>=THIRTY_TWO$$module$js$bits_bigint;c+=32}return a},clone:(b)=>b,empty:(b)=>!b};Bits$$module$js$bits_bigint.BIGINT_OK="function"===typeof BigInt&&"bigint"===typeof BigInt(0);
const ZERO$$module$js$bits_bigint=Bits$$module$js$bits_bigint.BIGINT_OK&&BigInt(0),ONE$$module$js$bits_bigint=Bits$$module$js$bits_bigint.BIGINT_OK&&BigInt(1),MAX_UINT32$$module$js$bits_bigint=Bits$$module$js$bits_bigint.BIGINT_OK&&BigInt(4294967295),THIRTY_TWO$$module$js$bits_bigint=Bits$$module$js$bits_bigint.BIGINT_OK&&BigInt(32);var module$js$bits_bigint={};const Bits$$module$js$bits_array={of:(...b)=>{let a=[];for(const c of b)a[c>>>5]=(a[c>>>5]||0)|1<<c;return a},from:(b)=>{let a=[];for(const c of b)a[c>>>5]=(a[c>>>5]||0)|1<<c;return a},containsAll:(b,a)=>{for(let c=Math.max(b.length,a.length)-1;0<=c;c--)if((a[c]||0)&~(b[c]||0))return!1;return!0},with:(b,a)=>{b[a>>>5]=(b[a>>>5]||0)|1<<a;return b},without:(b,a)=>{b[a>>>5]=(b[a>>>5]||0)&~(1<<a);return b},has:(b,a)=>!!((b[a>>>5]||0)&1<<a),bits:(b)=>{const a=[];for(let c=0;c<b.length;c++){let d=b[c],e=
32;for(;d;){const b=Math.clz32(d)+1;e-=b;d<<=b;32==b&&(d=0);a.push(c<<5|e)}}return a},clone:(b)=>[...b],empty:(b)=>b.every((a)=>!a)};var module$js$bits_array={};const b$$module$js$bits=module$js$bits_bigint.Bits.BIGINT_OK?module$js$bits_bigint.Bits:module$js$bits_array.Bits;class Bits$$module$js$bits{}Bits$$module$js$bits.of=b$$module$js$bits.of;Bits$$module$js$bits.from=b$$module$js$bits.from;Bits$$module$js$bits.containsAll=b$$module$js$bits.containsAll;Bits$$module$js$bits.with=b$$module$js$bits.with;Bits$$module$js$bits.without=b$$module$js$bits.without;Bits$$module$js$bits.has=b$$module$js$bits.has;Bits$$module$js$bits.bits=b$$module$js$bits.bits;
...
d(62,"Bow of Moon").fromPerson("Aryllis",35).direct(251624).dialog(35,null,1).key(),hc=d(63,"Bow of Sun").chest().key(),ic=d(64,"Bow of Truth").fromPerson("Azteca",131).npcSpawn(131,156,1).dialog(131,null,0).key(),tb=(new Magic$$module$js$nodes(a,65,"Refresh",65,null)).fromPerson("Zebu at windmill",94).direct(251665).dialog(94,16,3).trigger(180,1),Ta=(new Magic$$module$js$nodes(a,66,"Paralysis",66,null)).direct("Zebu at Mt. Sabre summit",251477).trigger(141).trigger(178),Z=(new Magic$$module$js$nodes(a,
67,"Telepathy",67,null)).direct("Tornel at Stom's house",223220).npcSpawn(95,30,1).trigger(133,1),jc=(new Magic$$module$js$nodes(a,68,"Teleport",68,null)).fromPerson("Tornel on Mt. Sabre",95).dialog(95,33,0),Sd=(new Magic$$module$js$nodes(a,69,"Recover",69,null)).direct("Asina in Portoa",250361),kc=(new Magic$$module$js$nodes(a,70,"Barrier",70,null)).direct("Asina on Angry sea",251609).trigger(132,0),pa=(new Magic$$module$js$nodes(a,71,"Change",71,null)).direct("Kensu in Swan",251614).npcSpawn(116,
241,1),D=(new Magic$$module$js$nodes(a,72,"Flight",72,null)).weight(5).direct("Kensu in Draygonia Fortress",250255),ub=d(40,"Flute of Lime").chest(void 0,91).key(),Td=Fa.bossDrop("Vampire 2",12,97).npcSpawn(204).key(),H=d(112,"Mimic"),lc=(new Trigger$$module$js$nodes(a,"Talked to Leaf Elder")).get(M),mc=new Trigger$$module$js$nodes(a,"Talked to Leaf Student"),nc=(new Trigger$$module$js$nodes(a,"Buy alarm flute")).get(qb),vb=new Trigger$$module$js$nodes(a,"Talked to Zebu in cave"),Ud=(new Trigger$$module$js$nodes(a,
"Woke up Windmill Guard")).get(Yb),Ua=new Trigger$$module$js$nodes(a,"Started Windmill"),oc=(new Trigger$$module$js$nodes(a,"Learned Refresh")).get(tb),Vd=(new Trigger$$module$js$nodes(a,"Gave Statue to Akahana")).get(Sa),Wd=(new Trigger$$module$js$nodes(a,"Fought Stom")).get(Z),pc=new Trigger$$module$js$nodes(a,"Visited Oak"),qc=new Trigger$$module$js$nodes(a,"Talked to Oak Mother"),wb=new Trigger$$module$js$nodes(a,"Rescued Oak Child"),Xd=(new Trigger$$module$js$nodes(a,"Talked to Oak Mother Again")).get(Ub),
Yd=(new Trigger$$module$js$nodes(a,"Talked to Oak Elder")).get(S),Zd=(new Trigger$$module$js$nodes(a,"Talked to Tornel on Mt Sabre")).get(jc),rc=new Trigger$$module$js$nodes(a,"Villagers Abducted"),sc=new Trigger$$module$js$nodes(a,"Talked to Rabbit in Leaf"),$d=(new Trigger$$module$js$nodes(a,"Learned Paralysis")).get(Ta),tc=new Trigger$$module$js$nodes(a,"Talked to Portoa Queen"),uc=new Trigger$$module$js$nodes(a,"Talked to Fortune Teller"),vc=new Trigger$$module$js$nodes(a,"Visited Underground Channel"),
ae=(new Trigger$$module$js$nodes(a,"Sent to Waterfall Cave")).get(Ra),be=(new Trigger$$module$js$nodes(a,"Cured Akahana")).get(Xb),wc=(new Trigger$$module$js$nodes(a,"Talked to Rage")).get(Ea),xc=new Trigger$$module$js$nodes(a,"Mesia recording played"),ce=(new Trigger$$module$js$nodes(a,"Talked to Asina")).get(Sd),yc=(new Trigger$$module$js$nodes(a,"Healed Dolphin")).get(rb),xb=new Trigger$$module$js$nodes(a,"Returned Fog Lamp"),zc=new Trigger$$module$js$nodes(a,"Talked to Kensu in Cabin"),Ac=new Trigger$$module$js$nodes(a,
"Talked to Joel Elder"),de=(new Trigger$$module$js$nodes(a,"Talked to Clark")).get(bc),ee=(new Trigger$$module$js$nodes(a,"Talked to Kensu in Lighthouse")).get(dc),yb=new Trigger$$module$js$nodes(a,"Calmed the Angry Sea"),fe=(new Trigger$$module$js$nodes(a,"Learned Barrier")).get(kc),Bc=new Trigger$$module$js$nodes(a,"Talked to Stom in Swan Hut"),Cc=new Trigger$$module$js$nodes(a,"Talked to Kensu in Swan tavern"),Dc=new Trigger$$module$js$nodes(a,"Talked to Kensu at Swan dance"),ge=(new Trigger$$module$js$nodes(a,
"Returned Kensu's love pendant")).get(pa),he=(new Trigger$$module$js$nodes(a,"Talked to Amazones Queen")).get(gc),zb=new Trigger$$module$js$nodes(a,"Entered Shyron"),ie=(new Trigger$$module$js$nodes(a,"Talked to Zebu in Shyron")).get($b),Ec=new Trigger$$module$js$nodes(a,"Shyron Massacre"),je=(new Trigger$$module$js$nodes(a,"Saved Kensu")).get(D),ke=(new Trigger$$module$js$nodes(a,"Talked to Deo")).get(Rd),le=(new Trigger$$module$js$nodes(a,"Talked to Akahana's Friend")).get(Pd),me=(new Trigger$$module$js$nodes(a,
"Get Bow of Truth")).get(ic),ne=(new Trigger$$module$js$nodes(a,"Forged Crystalis")).get(mb),oe=new Trigger$$module$js$nodes(a,"Win"),Va=(new Condition$$module$js$nodes(a,"Early flight")).option(kb).option(lb),Wa=(new Condition$$module$js$nodes(a,"Sword charge glitch")).option(Id).option(Ca),pe=(new Condition$$module$js$nodes(a,"Rabbit skip")).option(n).option(Ca),Ab=(new Condition$$module$js$nodes(a,"Talk glitch")).option(k).option(Ca),qe=(new Condition$$module$js$nodes(a,"Ghetto flight")).option(h).option(Ca),
re=(new Condition$$module$js$nodes(a,"Wild warp")).option(Gd).option(Ca,Hd),Xa=(new Condition$$module$js$nodes(a,"Any level 2 sword")).option(M,Ka).option(M,Da).option(S,La).option(S,Ma).option(N,Ea).option(N,Na).option(K,Oa).option(K,Pa),v=(new Condition$$module$js$nodes(a,"Destroy stone")).option(M,Ka).option(M,Da).option(Wa,M,Xa),w=(new Condition$$module$js$nodes(a,"Destroy ice")).option(S,La).option(S,Ma).option(Wa,S,Xa),r=(new Condition$$module$js$nodes(a,"Cross rivers")).option(N,Ea).option(N,
Na).option(D,Va).option(Wa,N,Xa),qa=(new Condition$$module$js$nodes(a,"Destroy iron")).option(K,Oa).option(K,Pa).option(Wa,K,Xa),I=(new Condition$$module$js$nodes(a,"Any sword")).option(M).option(S).option(N).option(K),se=(new Condition$$module$js$nodes(a,"Match insect sword (fire/water/thunder)")).option(S).option(N).option(K).option(Sa,q,M).option(Sa,G,M),te=(new Condition$$module$js$nodes(a,"Speed boots")).option(Wb,g),Fc=(new Condition$$module$js$nodes(a,"Climb slopes")).option(Vb).option(D,Va).option(te),
ue=(new Condition$$module$js$nodes(a,"Enter Mt Sabre North")).option(sc).option(pe),Gc=(new Condition$$module$js$nodes(a,"Asina in her room")).option(xc),ve=(new Condition$$module$js$nodes(a,"Paralysis or Ball of Water")).option(Ta).option(Gc).option(Ab),we=(new Condition$$module$js$nodes(a,"Ride dolphin")).option(rb,zc),T=(new Condition$$module$js$nodes(a,"Cross sea")).option(we).option(D,Va),xe=(new Condition$$module$js$nodes(a,"Cross whirlpool")).option(yb).option(D,Va).option(qe),U=(new Condition$$module$js$nodes(a,
"Refresh if needed")).option(G).option(ja).option(tb),Hc=(new Condition$$module$js$nodes(a,"Wind magic")).option(G).option(m,U).option(Ka,Da,U),Ic=(new Condition$$module$js$nodes(a,"Fire magic")).option(G).option(m,U).option(La,Ma,U),Jc=(new Condition$$module$js$nodes(a,"Water magic")).option(G).option(m,U).option(Ea,Na,U),ye=(new Condition$$module$js$nodes(a,"Thunder magic")).option(G).option(m,U).option(Oa,U).option(Pa,U),ze=(new Condition$$module$js$nodes(a,"Flute of lime or glitch")).option(Ra).option(Ab).option(lb,
ub),Ae=(new Condition$$module$js$nodes(a,"Second flute of lime")).option(ub).option(lb,Ra),Kc=(new Condition$$module$js$nodes(a,"Change or glitch")).option(pa).option(Ab),Bb=(new Condition$$module$js$nodes(a,"Pass shooting statues")).option(kc).option(tb,Xb).option(G).option(z),Be=(new Condition$$module$js$nodes(a,"Healed dolphin if required")).option(yc).option(u),Lc=(new Condition$$module$js$nodes(a,"Match sword of wind")).option(M).option(G,I).option(q,I),Mc=(new Condition$$module$js$nodes(a,"Match sword of fire")).option(S).option(G,
I).option(q,I),Nc=(new Condition$$module$js$nodes(a,"Match sword of water")).option(N).option(G,I).option(q,I),Ce=(new Condition$$module$js$nodes(a,"Match sword of thunder")).option(K).option(G,I).option(q,I),Ya=(new Condition$$module$js$nodes(a,"Travel swamp")).option(Sa).option(p).option(G),De=(new Condition$$module$js$nodes(a,"Calmed sea if required")).option(yb).option(l),Ee=e(0,"Vampire 1",I).get(Vb),Fe=e(1,"Insect",Ub,se).get(La),Ge=e(2,"Kelbesque 1",Lc,Hc).get(Ma),He=e(12,"Vampire 2",I).get(Td),
Oc=e(4,"Sabera 1",Mc,Ic).get(cc),Ie=e(5,"Mado 1",Nc,Jc).get(Oa),Je=e(6,"Kelbesque 2",Lc,Hc).get(pb),Ke=e(7,"Sabera 2",Mc,Ic).get(Sb),Le=e(8,"Mado 2",Nc,Jc).get(Jd),Me=e(9,"Karmine",Ce,ye).get(sb),Ne=e(10,"Draygon 1",I).get(Md),Oe=e(null,"Statues",hc,gc),Pe=e(11,"Draygon 2",I,ic),Qe=e(13,"Dyna",mb),O=new Area$$module$js$nodes(a,"Leaf"),ra=new Area$$module$js$nodes(a,"Valley of Wind"),L=new Area$$module$js$nodes(a,"Sealed Cave"),ka=new Area$$module$js$nodes(a,"Cordel Plain"),sa=new Area$$module$js$nodes(a,
"Brynmaer"),ta=new Area$$module$js$nodes(a,"Amazones"),B=new Area$$module$js$nodes(a,"Mt Sabre West"),t=new Area$$module$js$nodes(a,"Mt Sabre North"),Za=new Area$$module$js$nodes(a,"Nadare's"),Ga=new Area$$module$js$nodes(a,"Oak"),aa=new Area$$module$js$nodes(a,"Waterfall Valley"),A=new Area$$module$js$nodes(a,"Portoa"),ba=new Area$$module$js$nodes(a,"Waterfall Cave"),C=new Area$$module$js$nodes(a,"Fog Lamp Cave"),ca=new Area$$module$js$nodes(a,"Kirisa Plant Cave"),la=new Area$$module$js$nodes(a,
"Angry Sea"),da=new Area$$module$js$nodes(a,"Joel"),F=new Area$$module$js$nodes(a,"Evil Spirit Island"),ea=new Area$$module$js$nodes(a,"Sabera's Castle"),P=new Area$$module$js$nodes(a,"Swan"),x=new Area$$module$js$nodes(a,"Mt Hydra"),fa=new Area$$module$js$nodes(a,"Shyron"),ma=new Area$$module$js$nodes(a,"Styx"),na=new Area$$module$js$nodes(a,"Goa"),$a=new Area$$module$js$nodes(a,"Draygonia Fortress 1"),Q=new Area$$module$js$nodes(a,"Draygonia Fortress 2"),ua=new Area$$module$js$nodes(a,"Draygonia Fortress 3"),
J=new Area$$module$js$nodes(a,"Draygonia Fortress 4"),E=new Area$$module$js$nodes(a,"Oasis Cave");new Area$$module$js$nodes(a,"Desert");const R=new Area$$module$js$nodes(a,"Sahara"),va=new Area$$module$js$nodes(a,"Pyramid Front"),V=new Area$$module$js$nodes(a,"Pyramid Back"),ha=new Area$$module$js$nodes(a,"Tower"),Cb=(new Location$$module$js$nodes(a,0,O,"Start")).overworld().start(),Re=(new Location$$module$js$nodes(a,0,O,"Mezame Shrine")).overworld().from(Cb),Se=(new Location$$module$js$nodes(a,
1,O,"Outside Start")).overworld().connect(Re),ia=(new Location$$module$js$nodes(a,2,O,"Town")).town().connect(Se),Ha=(new Location$$module$js$nodes(a,3,ra,"Main")).overworld().connect(ia).trigger(oc,Ua),Pc=(new Location$$module$js$nodes(a,3,ra,"Outside Windmill")).overworld(),Qc=(new Location$$module$js$nodes(a,4,L,"Tunnel 1 (entrance)")).cave().from(Ha,Ua),Te=(new Location$$module$js$nodes(a,5,L,"Tunnel 2 (over bridge)")).cave().connect(Qc),Ue=(new Location$$module$js$nodes(a,6,L,"Tunnel 6 (herb dead end)")).cave().chest(X,
15),Rc=(new Location$$module$js$nodes(a,7,L,"Tunnel 4a (ball corridor)")).cave().chest(X,20,80).chest(Ka,21);(new Location$$module$js$nodes(a,7,L,"Tunnel 4b (antidote dead end)")).cave().connect(Rc,v).chest(Qa,19);const Ve=(new Location$$module$js$nodes(a,8,L,"Tunnel 5 (warp boots dead end)")).cave().chest(ob,14),We=(new Location$$module$js$nodes(a,9,L,"Tunnel 3a (branch, front)")).cave().connect(Te).connectTo(Rc).connectTo(Ve),Xe=(new Location$$module$js$nodes(a,9,L,"Tunnel 3b (branch, back)")).cave().connect(We,
v).connectTo(Ue),Ye=(new Location$$module$js$nodes(a,10,L,"Tunnel 7 (boss)")).cave().connect(Xe).boss(Ee),Ze=(new Location$$module$js$nodes(a,12,L,"Tunnel 8a (exit, above wall)")).cave().connect(Ye),$e=(new Location$$module$js$nodes(a,12,L,"Tunnel 8b (exit, below wall")).cave().connect(Ze,v);(new Location$$module$js$nodes(a,14,ra,"Windmill Cave")).cave().connect(Ha).connectTo(Pc).trigger(Ud,qb,vb);(new Location$$module$js$nodes(a,15,ra,"Windmill")).misc().connect(Pc).trigger(Ua,Yb);const af=(new Location$$module$js$nodes(a,
16,ra,"Zebu's Cave Front")).cave().connect(Ha).trigger(vb,lc,mc).trigger(oc,Ua,vb),bf=(new Location$$module$js$nodes(a,16,ra,"Zebu's Cave Back")).cave().trigger(rc).connect(af,w),cf=(new Location$$module$js$nodes(a,17,B,"Tunnel 1 (to Zebu)")).cave().connect(bf),wa=(new Location$$module$js$nodes(a,20,ka,"West")).overworld().connect($e),df=(new Location$$module$js$nodes(a,20,ka,"South")).overworld().connect(wa,r),Sc=(new Location$$module$js$nodes(a,21,ka,"East")).overworld().connect(wa).chest(Tb,24),
Ia=(new Location$$module$js$nodes(a,24,sa,"Town")).town().connect(wa).trigger(Vd,Tb),ef=(new Location$$module$js$nodes(a,25,ka,"Outside Stom's House")).town().connect(wa),Db=(new Location$$module$js$nodes(a,26,ka,"Swamp")).overworld().connect(Sc,Ya).trigger(wb,qc,Ya);(new Location$$module$js$nodes(a,26,ka,"Swamp Insect Area")).overworld().connect(Db,Ya).boss(Fe);const ab=(new Location$$module$js$nodes(a,27,ta,"Town")).town().connect(df),bb=(new Location$$module$js$nodes(a,28,Ga,"Town")).town().connect(Db,
Ya).trigger(pc);(new Location$$module$js$nodes(a,30,ka,"Stom's House")).house().connect(ef).trigger(Wd,pc);const xa=(new Location$$module$js$nodes(a,32,B,"Entrance")).overworld().connect(wa).connect(cf),ff=(new Location$$module$js$nodes(a,32,B,"Up Slope")).overworld().from(xa,Fc).to(xa),gf=(new Location$$module$js$nodes(a,32,B,"Dead End (warp boots)")).overworld().chest(ob,24,106),Tc=(new Location$$module$js$nodes(a,33,B,"Upper")).overworld().from(xa,D).to(xa),hf=(new Location$$module$js$nodes(a,
33,B,"Tornel Dead End")).overworld().trigger(Zd,Da).chest(Y,23,105),Uc=(new Location$$module$js$nodes(a,34,B,"Tunnel 2a (fork at start)")).cave().connect(xa);(new Location$$module$js$nodes(a,34,B,"Tunnel 2b (left branch to dead end)")).cave().connect(Uc,w).connectTo(gf);const jf=(new Location$$module$js$nodes(a,34,B,"Tunnel 2c (right branch to upper)")).cave().connect(Uc,w),kf=(new Location$$module$js$nodes(a,35,B,"Tunnel 3a (tunnel to upper, with herb chest)")).cave().connect(jf).chest(X,23,82),
lf=(new Location$$module$js$nodes(a,35,B,"Tunnel 3b (tunnel to upper, branch below ice)")).cave().connect(kf,w).connectTo(Tc),Vc=(new Location$$module$js$nodes(a,36,B,"Tunnel 4a (branch to upper or Tornel)")).cave().connect(lf);(new Location$$module$js$nodes(a,36,B,"Tunnel 4b (out to upper)")).cave().connect(Vc,w).connectTo(Tc);const mf=(new Location$$module$js$nodes(a,37,B,"Tunnel 5 (tiny connector)")).cave().connect(Vc),nf=(new Location$$module$js$nodes(a,38,B,"Tunnel 6a (exit to Tornel, above ice)")).cave().connect(mf);
...
Slot$$module$js$nodes&&a.write(b);if(c){c.items=[];c.route=[];for(const a of g){const [b,d]=a;c.route.push(d);null!=b.slotName&&(y=b.slotName,0>y.indexOf(b.orig)&&(y+=` (normally ${b.orig})`),c.items.push({slotIndex:b.slotIndex,itemIndex:b.itemIndex,origName:b.vanillaItemName,slotName:b.slotName,itemName:b.item.name,text:`${b.item.name}: ${y}`}))}}}),shuffle2$$module$js$depgraph=(b,a,c,d,e)=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){e&&e.addTasks(100);for(let f=0;100>f;f++){try{yield shuffle3$$module$js$depgraph(b,
a,c,d,e)}catch(g){e&&(e.addCompleted(1),0===f%5&&(yield new Promise(requestAnimationFrame)));continue}console.log(`success after ${f} attempts`);return}throw Error("failed");}),shuffle3$$module$js$depgraph=(b,a,c,d,e)=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){const e=generate$$module$js$depgraph(d);var g=e.integrate(),h=e.nodes.filter((a)=>a instanceof Slot$$module$js$nodes&&a.slots&&a.slotName),k=new Map(a.shuffle(h.map((a)=>[a,[a.item,a.itemIndex]])));const n=new Set(a.shuffle(h)),
m=new Map,q={};d||(d=new FlagSet$$module$js$flagset("Sbkm Sct"));d.flags.S.forEach((a,b)=>{for(const c of a)q[c]=b});for(var p of n)m.set(p.item,p);const u=(a)=>13<=a&&36>=a||38===a||40===a||49===a||72<a&&91!=a,l=(a,b)=>q[a.slotType?a.slotType[0]:"c"]==q[b.slotType?b.slotType[0]:"c"]&&(112!==b.itemIndex||a instanceof Chest$$module$js$nodes&&!a.isInvisible)&&(!u(b.itemIndex)||(a instanceof Chest$$module$js$nodes||a instanceof BossDrop$$module$js$nodes)&&9!==a.origIndex)?!0:!1;p=g.assumedFill(a,(a,
b)=>l(a,m.get(b)));if(null==p)throw Error("Could not fill!");const y=new Map;for(let a=0;a<p.length;a++){var z=p[a];if(null!=z){var ja=m.get(g.item(z));z=k.get(ja);k.delete(ja);ja=g.location(a);y.set(ja,z);n.delete(ja)}}g=(a,b)=>{for(const c of n)if(l(c,a)){y.set(c,b);n.delete(c);return}throw Error("Could not fill extra items");};for(const a of k){const [b,c]=a;112===b.itemIndex&&(g(b,c),k.delete(b))}for(const a of k){const [b,c]=a;u(b.itemIndex)&&(g(b,c),k.delete(b))}for(const a of k){const [b,c]=
a;g(b,c)}for(const a of y){const [b,c]=a;b.set(...c)}const {win:kb,path:Ba}=e.traverse({wanted:h,dfs:!1});if(!kb)throw Error("FAIL");for(const a of e.nodes)a instanceof Slot$$module$js$nodes&&a.write(b);if(c){c.items=[];c.route=[];for(const a of Ba){const [b,d]=a;c.route.push(d);h=e.nodes[b];null!=h.slotName&&(k=h.slotName,0>k.indexOf(h.orig)&&(k+=` (normally ${h.vanillaItemName})`),c.items.push({slotIndex:h.slotIndex,itemIndex:h.itemIndex,origName:h.vanillaItemName,slotName:h.slotName,itemName:h.item.name,
text:`${h.item.name}: ${k}`}))}}});var module$js$depgraph={};module$js$depgraph.generate=generate$$module$js$depgraph;module$js$depgraph.shuffle=shuffle$$module$js$depgraph;module$js$depgraph.shuffle2=shuffle2$$module$js$depgraph;module$js$depgraph.shuffle3=shuffle3$$module$js$depgraph;class Deque$$module$js$util{constructor(){this.buffer=Array(16);this.mask=15;this.size=this.end=this.start=0}get length(){return this.size}upsize(b){for(;this.mask<b;)this.end<=this.start&&(this.start+=this.mask+1),this.mask=this.mask<<1|1,this.buffer=this.buffer.concat(this.buffer);this.size=b}push(...b){this.upsize(this.size+b.length);for(const a of b)this.buffer[this.end]=a,this.end=this.end+1&this.mask}pop(){if(this.size)return this.end=this.end-1&this.mask,this.size--,this.buffer[this.end]}peek(){if(this.size)return this.buffer[this.end-
...
class Trigger$$module$js$nodes extends Node$$module$js$graph{constructor(b,a){super(b,a);this.slot=null;this.reqs=[]}get nodeType(){return"Trigger"}edges(){const b=[...this.reqs];this.slot&&b.push(module$js$graph.Edge.of(this.slot,this));return b}get(b){if(this.slot)throw Error("already have a slot");this.slot=b;return this}}
class Condition$$module$js$nodes extends Node$$module$js$graph{constructor(b,a){super(b,a);this.options=[]}get nodeType(){return"Condition"}edges(){return this.options.map((b)=>module$js$graph.Edge.of(this,...b))}option(...b){this.options.push(b.map((a)=>a instanceof Slot$$module$js$nodes?a.item:a));return this}}
...
class Location$$module$js$nodes extends Node$$module$js$graph{constructor(b,a,c,d){super(b,c.name+": "+d);this.simpleName=d;this.id=a;this.area=c;this.connections=[];this.chests=[];this.type=this.bossNode=null;this.isStart=!1}get nodeType(){return"Location"}toString(){return`Location ${this.id.toString(16).padStart(2,0)} ${this.name}`}edges(){const b=[];for(const a of this.connections)b.push(module$js$graph.Edge.of(a.to,a.from,...a.deps,...a.from.bossNode?[a.from.bossNode]:[])),a.bidi&&b.push(module$js$graph.Edge.of(a.from,
a.to,...a.deps,...a.to.bossNode?[a.to.bossNode]:[]));for(const a of this.chests)b.push(module$js$graph.Edge.of(a,this));this.isStart&&b.push(module$js$graph.Edge.of(this));return b}addConnection(b){b.from.connections.push(b);b.to.connections.push(b);return this}from(b,...a){return this.addConnection(new Connection$$module$js$nodes(b,this,!1,a))}to(b,...a){return this.addConnection(new Connection$$module$js$nodes(this,b,!1,a))}connect(b,...a){return this.addConnection(new Connection$$module$js$nodes(b,
...
class WorldGraph$$module$js$nodes extends Graph$$module$js$graph{integrate(b={}){const {removeConditions:a=!0,removeTriggers:c=!0,removeOptions:d=!0,removeTrackers:e=!0}=b,f=new SparseDependencyGraph$$module$js$graph(this.nodes.length),g=[],h=new Set,k=new Map;var n=[],m=[];const q=[];var p=[];const u=[];b=new Set;for(var l of this.nodes)if(l instanceof Location$$module$js$nodes){if(l.isStart){const [a]=f.addRoute([l.uid]);k.set(a.label,a)}g[l.uid]=[];for(var y of l.connections)h.has(y)||(h.add(y),
y.bidi&&h.add(y.reverse()));for(const a of l.chests)f.addRoute([a.uid,l.uid])}else if(l instanceof Slot$$module$js$nodes&&"Alarm Flute"===l.item.name)f.addRoute([l.item.uid]),f.finalize(l.item.uid);else if(!(l instanceof Slot$$module$js$nodes&&null==l.slotName))if(l instanceof ItemGet$$module$js$nodes&&"Medical Herb"===l.name&&(f.addRoute([l.uid]),f.finalize(l.uid)),l instanceof Option$$module$js$nodes)n.push(l);else if(l instanceof TrackerNode$$module$js$nodes)m.push(l);else if(l instanceof Condition$$module$js$nodes)q.push(l);
else if(l instanceof Trigger$$module$js$nodes)p.push(l);else if(l instanceof ItemGet$$module$js$nodes)u.push(l);else if(l instanceof Slot$$module$js$nodes)b.add(l);else if(!(l instanceof Area$$module$js$nodes))throw Error(`Unknown node type: ${l.nodeType}`);for(const a of h)g[a.from.uid].push(a);l=(a)=>{for(const b of a){for(const a of b.edges())f.addRoute(a);f.finalize(b.uid)}};d&&l(n);e&&l(m);c&&l(p);for(n=k.values();!(m=n.next()).done;){m=m.value;p=m.target;for(var z of g[p]){p=[z.to.uid,...m.deps];
for(y=z.deps.length-1;0<=y;y--)p.push(z.deps[y].uid);z.from.bossNode&&p.push(z.from.bossNode.uid);for(const a of f.addRoute(p))k.has(a.label)||k.set(a.label,a)}}a&&l(q);for(z=0;z<this.nodes.length;z++)this.nodes[z]instanceof ItemGet$$module$js$nodes||f.finalize(z);z=new LocationList$$module$js$nodes(this);for(const a of b)for(const b of f.nodes[a.uid].values())z.addRoute([a.uid,...b]);return z}}
...
return Math.min(AM$$module$js$random*this.iy,RNMX$$module$js$random)}nextInt(b){return Math.floor(this.next()*b)}shuffle(b){for(let a=b.length;a;){const c=this.nextInt(a--);[b[a],b[c]]=[b[c],b[a]]}return b}}var module$js$random={};module$js$random.Random=Random$$module$js$random;const STATUS$$module$js$version="unstable",VERSION$$module$js$version="assumed",LABEL$$module$js$version="assumed 1c33399",HASH$$module$js$version="1c3339970b1c14a1919a38fe2f2fd41bfafd0d59",DATE$$module$js$version=new Date(1551181073E3);var module$js$version={};module$js$version.STATUS=STATUS$$module$js$version;module$js$version.VERSION=VERSION$$module$js$version;module$js$version.LABEL=LABEL$$module$js$version;module$js$version.HASH=HASH$$module$js$version;module$js$version.DATE=DATE$$module$js$version;var $jscompDefaultExport$$module$js$patch={apply(b,a){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){let c;a.seed||(a.seed=parseSeed$$module$js$patch("").toString(16),window.location.hash+="&seed="+a.seed);c=a.flags?new FlagSet$$module$js$flagset(a.flags):new FlagSet$$module$js$flagset("Em Gt Mr Rlpt Sbk Sct Sm Tasd");for(const b in a)"false"===a[b]&&(a[b]=!1);yield shuffle$$module$js$patch(b,parseSeed$$module$js$patch(a.seed),c)})}};
