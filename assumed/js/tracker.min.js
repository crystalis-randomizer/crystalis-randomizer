...
ke=(new Trigger$$module$js$nodes(a,"Talked to Deo")).get(Rd),le=(new Trigger$$module$js$nodes(a,"Talked to Akahana's Friend")).get(Pd),me=(new Trigger$$module$js$nodes(a,"Get Bow of Truth")).get(ic),ne=(new Trigger$$module$js$nodes(a,"Forged Crystalis")).get(mb),oe=new Trigger$$module$js$nodes(a,"Win"),Va=(new Condition$$module$js$nodes(a,"Early flight")).option(kb).option(lb),Wa=(new Condition$$module$js$nodes(a,"Sword charge glitch")).option(Id).option(Ca),pe=(new Condition$$module$js$nodes(a,"Rabbit skip")).option(n).option(Ca),
...
return Math.min(AM$$module$js$random*this.iy,RNMX$$module$js$random)}nextInt(b){return Math.floor(this.next()*b)}shuffle(b){for(let a=b.length;a;){const c=this.nextInt(a--);[b[a],b[c]]=[b[c],b[a]]}return b}}var module$js$random={};module$js$random.Random=Random$$module$js$random;const STATUS$$module$js$version="unstable",VERSION$$module$js$version="assumed",LABEL$$module$js$version="assumed 0d3f3ab",HASH$$module$js$version="0d3f3ab4aa0d8e13e2bb61ba994052dd1ed6186f",DATE$$module$js$version=new Date(1551209761E3);var module$js$version={};module$js$version.STATUS=STATUS$$module$js$version;module$js$version.VERSION=VERSION$$module$js$version;module$js$version.LABEL=LABEL$$module$js$version;module$js$version.HASH=HASH$$module$js$version;module$js$version.DATE=DATE$$module$js$version;var $jscompDefaultExport$$module$js$patch={apply(b,a){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){let c;a.seed||(a.seed=parseSeed$$module$js$patch("").toString(16),window.location.hash+="&seed="+a.seed);c=a.flags?new FlagSet$$module$js$flagset(a.flags):new FlagSet$$module$js$flagset("Em Gt Mr Rlpt Sbk Sct Sm Tasd");for(const b in a)"false"===a[b]&&(a[b]=!1);yield shuffle$$module$js$patch(b,parseSeed$$module$js$patch(a.seed),c)})}};
