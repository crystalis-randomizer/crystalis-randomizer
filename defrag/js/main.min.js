...
"Forged Crystalis")).get(pb),$a=(new Condition$$module$nodes(b,"Sword charge glitch")).option(C,U),Fe=(new Condition$$module$nodes(b,"Rabbit skip")).option(p,D),Ge=(new Condition$$module$nodes(b,"Teleport skip")).option(q,m),ab=(new Condition$$module$nodes(b,"Statue glitch")).option(l,r),He=(new Condition$$module$nodes(b,"Wild warp")).option(va,oa),bb=(new Condition$$module$nodes(b,"Any level 2 sword")).option(P,Ra).option(P,Ia).option(Q,Sa).option(Q,Ta).option(L,Ua).option(L,Va).option(J,Wa).option(J,
...
Ie=(new Condition$$module$nodes(b,"Match insect sword (fire/water/thunder)")).option(Q).option(L).option(J).option(Ya,H.value?H:g,P),Je=(new Condition$$module$nodes(b,"Match vampire 2 sword (wind/water/thunder)")).option(P).option(L).option(J).option(H.value?H:g,Q),Oc=(new Condition$$module$nodes(b,"Speed boots")).option(fc,k),Pc=(new Condition$$module$nodes(b,"Climb slopes")).option(vb).option(z).option(Oc),Ke=(new Condition$$module$nodes(b,"Enter Mt Sabre North (rabbit)")).option(Ac).option(Fe),
...
(new Location$$module$nodes(b,57,u,"Tunnel 7 (to upper)")).cave().connect(Sf).connectTo(ib);const Yf=(new Location$$module$nodes(b,60,db,"Inn")).shop(),Zf=(new Location$$module$nodes(b,61,db,"Tool Shop")).shop(E,K,M,R),$f=(new Location$$module$nodes(b,62,db,"Back Room")).house(),Kb=(new Location$$module$nodes(b,64,ha,"Summit")).overworld().connect(Xf,nf),jb=(new Location$$module$nodes(b,64,ha,"Northwest")).overworld().from(Kb).to(Kb,z),ag=(new Location$$module$nodes(b,64,ha,"Northeast")).overworld().connect(jb,
...
215,y,"Palace Entrance")).house().connect(ua);(new Location$$module$nodes(b,216,y,"Fortune Teller Front")).house().connect(ua).trigger(Cc,Bc);(new Location$$module$nodes(b,216,y,"Fortune Teller Back")).house().connect(Eg).connect(wd);(new Location$$module$nodes(b,217,y,"Pawn Shop")).shop().connect(ua);(new Location$$module$nodes(b,218,y,"Armor Shop")).shop(be,bc,Zb).connect(ua);(new Location$$module$nodes(b,220,y,"Inn")).shop().connect(ua);(new Location$$module$nodes(b,221,y,"Tool Shop")).shop(E,
...
$jscomp.asyncExecutePromiseGeneratorFunction(function*(){const f=generate$$module$depgraph(d);d.shuffleShops()&&f.shuffleShops(b);const g=f.nodes.filter((a)=>a instanceof Slot$$module$nodes&&a.slots&&a.slotName);d||(d=new FlagSet$$module$flagset("Sbkm Sct"));var h={};for(var k of g)if(k instanceof Slot$$module$nodes&&k.slots){var n=k.slotType[0];(h[n]=h[n]||[]).push(k)}k=[];n=[];var l={};for(var r of d.flags.S||[]){var q=[];for(var m of r){l[m]=!0;for(var p of h[m])q.push(p),n.push([p,k.length])}k.push(q)}for(var D of"bc")if(l[D])for(r=
h[D],m=r.map((a)=>[a.item,a.itemIndex]),b.shuffle(m),p=0;p<r.length;p++)r[p].set(...m[p]);q=(a)=>g.find((b)=>b.vanillaItemName===a);h=q("Sword of Wind");D=q("Sword of Fire");r=q("Ball of Wind");m=q("Ball of Fire");p=q("Statue of Onyx");q=q("Gas Mask");l.k&&b.nextInt(2)&&(D.swap(h),m.swap(r));l.k&&!b.nextInt(3)&&p.swap(q);l=[1,1,1,1,2,2,2,3,3,3,5,5,8,13];h=[];e&&e.addTasks(10);for(D=0;1E3>D;D++){e&&0==D%100&&(yield new Promise(requestAnimationFrame),e.addCompleted(1));b.shuffle(n);for(var C of k)b.shuffle(C);
r=k.map(()=>-1);m=l[b.nextInt(l.length)];p=[];q=(a,b)=>a.itemIndex===b.itemIndex||a.isMimic()&&!b.canHoldMimic()||b.isMimic()&&!a.canHoldMimic()||a.needsChest()&&!b.isChest()||b.needsChest()&&!a.isChest()?!1:!0;for(let a=0;a<m;a++){const [b,c]=n[a],d=k[c][++r[c]];q(b,d)&&(b.swap(d),p.push([b,d]))}if(!p.length)continue;const {win:a,path:c}=f.traverse({wanted:g,dfs:!1});if(a)h=c;else for(;p.length;){const [a,b]=p.pop();a.swap(b)}}a&&f.write(a);if(c){c.items=[];c.route=[];for(const a of h){const [b,
d]=a;c.route.push(d);C=f.nodes[b];null!=C.slotName&&(k=C.slotName,0>k.indexOf(C.orig)&&(k+=` (normally ${C.vanillaItemName})`),c.items.push({slotIndex:C.slotIndex,itemIndex:C.itemIndex,origName:C.vanillaItemName,slotName:C.slotName,itemName:C.item.name,text:`${C.item.name}: ${k}`}))}}}),shuffle2$$module$depgraph=(a,b,c,d,e)=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){const f=generate$$module$depgraph(d);d.shuffleShops()&&f.shuffleShops(b);const g=f.integrate();e&&e.addTasks(1E3);for(let h=
0;1E3>h;h++){if(yield shuffle3$$module$depgraph(f,g,a,b,c,d,e)){console.log(`success after ${h} attempts`);return}e&&(e.addCompleted(1),0===h%50&&(yield new Promise(requestAnimationFrame)))}return shuffle$$module$depgraph(a,b,c,d,e)}),shuffle3$$module$depgraph=(a,b,c,d,e,f,g)=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){const g=e&&e.stats;var k=a.nodes.filter((a)=>a instanceof Slot$$module$nodes&&a.slots&&a.slotName);const n=new Map(d.shuffle(k.map((a)=>[a,[a.item,a.itemIndex]]))),l=
new Set(d.shuffle(k)),r=new Map,q={};g&&g.seeds++;f||(f=new FlagSet$$module$flagset("Sbkm Sct"));(f.flags.S||[]).forEach((a,b)=>{for(const c of a)q[c]=b});for(var m of l)r.set(m.item,m);let p=new Set;const D=(a,b)=>{if(null==q[a.slotType?a.slotType[0]:"c"])return a==b;if(b.isMimic()&&!a.canHoldMimic()||"unique"!=b.item.inventoryRow&&a.requiresUnique||"armor"==b.item.inventoryRow&&!a.isChest())return!1;if(f.guaranteeSword()){if(0===a.slotIndex)return 4>b.item.id;if(4>b.item.id&&(p.add(b),4===p.size))return 0===
a.slotIndex}return q[a.slotType?a.slotType[0]:"c"]==q[b.slotType?b.slotType[0]:"c"]};m=b.assumedFill(d,(a,b)=>D(a,r.get(b)));if(null==m){g&&g.endState.key++;return!1;throw Error("Could not fill!");}const C=new Map;for(var U=0;U<m.length;U++){var va=m[U];if(null!=va){var oa=r.get(b.item(va));va=n.get(oa);n.delete(oa);oa=b.location(U);C.set(oa,va);l.delete(oa)}}U=(a,b,c)=>{c=void 0===c?"":c;for(const c of l)if(D(c,a))return C.set(c,b),l.delete(c),n.delete(a),!0;g&&("traps"==c?g.endState.traps++:"chest"==
c?g.endState.chest++:g.endState.misc++);return!1};for(var F of n){const [a,b]=F;if(a.isMimic()&&!U(a,b,"traps"))return!1}for(const a of n){const [b,c]=a;if(b.needsChest()&&!U(b,c,"chest"))return!1}for(const a of n){const [b,c]=a;if(!U(b,c))return!1}for(const a of C){const [b,c]=a;b.set(...c)}const {win:H,path:Pa}=a.traverse({wanted:k,dfs:!1});if(!H)return g&&g.endState.no_win++,!1;c&&a.write(c);if(!e)return!0;g&&(g.endState.success++,g.analyze(a,b,m));e.items=[];e.route=[];for(const b of Pa){const [c,
...
4<=e.sells.length?g.push(d):e.sells.push(d));for(const {shops:a}of[b,c])for(const b of a)b.sells.sort((a,b)=>a.id-b.id)}integrate(a={}){const {tracker:b=!1}=a;var c=!b;const d=new SparseDependencyGraph$$module$graph(this.nodes.length),e=[],f=[],g=new Set,h=new Map;var k=[],n=[];const l=[];var r=[];const q=[];a=new Set;for(var m of this.nodes)if(m instanceof Location$$module$nodes){if(m.isStart){const [a]=d.addRoute([m.uid]);h.set(a.label,a)}e[m.uid]=[];f[m.uid]=[];for(const a of m.connections)g.has(a)||
(g.add(a),a.bidi&&g.add(a.reverse()));for(const a of m.chests)d.addRoute([a.uid,m.uid])}else if(m instanceof ItemGet$$module$nodes&&"Medical Herb"===m.name&&(d.addRoute([m.uid]),d.finalize(m.uid)),m instanceof Option$$module$nodes)k.push(m);else if(m instanceof TrackerNode$$module$nodes)n.push(m);else if(m instanceof Condition$$module$nodes)l.push(m);else if(m instanceof Trigger$$module$nodes)r.push(m);else if(m instanceof ItemGet$$module$nodes)q.push(m);else if(m instanceof Slot$$module$nodes)a.add(m);
else if(!(m instanceof Area$$module$nodes))throw Error(`Unknown node type: ${m.nodeType}`);for(const a of g)e[a.from.uid].push(a),f[a.to.uid].push(a);m=(a)=>{for(const c of a){for(const a of c.edges({tracker:b}))d.addRoute(a);d.finalize(c.uid)}};m(k);c&&m(n);m(r);for(c=h.values();!(k=c.next()).done;){k=k.value;n=k.target;for(var p of e[n]){n=[p.to.uid,...k.deps];for(r=p.deps.length-1;0<=r;r--)n.push(p.deps[r].uid);p.from.bossNode&&n.push(p.from.bossNode.uid);for(const a of d.addRoute(n))h.has(a.label)||
h.set(a.label,a)}}m(l);for(p=0;p<this.nodes.length;p++)this.nodes[p]instanceof ItemGet$$module$nodes||this.nodes[p]instanceof TrackerNode$$module$nodes||d.finalize(p);p=new LocationList$$module$nodes(this);for(const b of a)for(const a of d.nodes[b.uid].values())p.addRoute([b.uid,...a]);return p}}
...
f[a]);c.shuffleSlots(h,k,a);let n=!1;for(const a of k)if(null==f[a]&&a!=this.win&&b(this.worldGraph.nodes[this.locationToUid[a]],h)){if(100<a)throw Error("WTF");f[a]=g;n=!0;break}if(!n)return null}return f}}class FillStrategy$$module$nodes{shuffleSlots(a,b,c){}static shuffleSlots(a,b,c){c.shuffle(b)}shuffleItems(a,b){}static shuffleItems(a,b){const c=[];for(let b=0;b<a.length;b++){const {shufflePriority:d=1}=a[b];for(let a=0;a<d;a++)c.push(b)}b.shuffle(c);return c}}
...
g=concatIterables$$module$rom$util(b.entrances),h=[...concatIterables$$module$rom$util(b.exits),255],k=[...concatIterables$$module$rom$util(b.flags),255],n=concatIterables$$module$rom$util(b.pits),[l,r,q,m,p,D]=yield Promise.all([d(e,"Layout"),d(f,"Graphics"),d(g,"Entrances"),d(h,"Exits"),d(k,"Flags"),...n.length?[d(n,"Pits")]:[]]);e=[l&255,(l>>>8)-192,r&255,(r>>>8)-192,q&255,(q>>>8)-192,m&255,(m>>>8)-192,p&255,(p>>>8)-192,...D?[D&255,(D>>8)-192]:[]];d=yield d(e,"MapData");writeLittleEndian$$module$rom$util(a.rom,
...
hex$$module$rom$util).join(" ")}`);a.reject(Error(`Could not find sufficient chunk in ${hex$$module$rom$util(a.startPage)}..${hex$$module$rom$util(a.endPage)} to write ${a.name}: ${a.data}`))}}var module$rom$writer={};module$rom$writer.Writer=Writer$$module$rom$writer;const watchArray$$module$rom=(a,b)=>new Proxy(a,{get:function(a,d){let c=a[d];if("subarray"===d)return(c,d)=>{const e=a.subarray(c,d);return c<=b&&b<d?watchArray$$module$rom(e,b-c):e};if("set"===d)return(c)=>{console.log(`Setting overlapping array ${b}`);a.set(c)};"function"===typeof c&&(c=c.bind(a));return c},set:function(a,d,e,f){if(d==b){console.log(`Writing ${b.toString(16)}`);debugger}a[d]=e;return!0}});
...
(a[c]|=128>>>d,a.push(this.objectData[8*b+d]))}return Uint8Array.from(a)}write(a){const b=this;return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){const c=yield a.write(b.serialize(),106496,114687,`Object ${hex$$module$rom$util(b.id)}`);a.rom[b.objectDataPointer]=c&255;a.rom[b.objectDataPointer+1]=c>>>8})}get(a){return this.objectData[a-768>>>5]}static setupProps(){const a=(...a)=>({get(){let b=0;for(const c of a){const [a,d=255,g=0]=c;b|=(this.objectData[a-768>>>5]&d)>>>(0>g?0:g)<<(0>
g?-g:0)}return b},set(b){for(const c of a){const [a,d=255,g=0]=c;{const c=a-768>>>5;this.objectData[c]=this.objectData[c]&~d|b>>>(0>g?-g:0)<<(0>g?0:g)&d}}}});Object.defineProperties(this.prototype,{metasprite:a([768]),collisionPlane:a([928,240,4]),hitbox:a([1056,64,2],[928,15]),hp:a([960]),atk:a([992]),def:a([1024]),level:a([1056,31]),child:a([1088]),terrainSusceptibility:a([1120]),immobile:a([1184,128,7]),action:a([1184,127]),replacement:a([1216]),goldDrop:a([1280,240,4]),elements:a([1280,15]),expReward:a([1312]),
attackType:a([1344])})}parents(){return this.rom.monsters.filter((a)=>a.child&&this.rom.adHocSpawns[a.child].object==this.id)}locations(){return this.rom.locations.filter((a)=>a.valid&&a.spawns.some((a)=>a.isMonster()&&a.monsterId===this.id))}palettes(a){if(34==this.action)return[3];let b=this.objectData[0];42==this.action&&(b=this.objectData[31]|1);41==this.action&&(b=107);38==this.action&&(b=156);a=(void 0===a?0:a)&&this.child?this.rom.metasprites[this.rom.objects[this.rom.adHocSpawns[this.child].objectId].objectData[0]]:
null;return[...new Set([...this.rom.metasprites[b].palettes(),...a?a.palettes():[]])]}}ObjectData$$module$rom.setupProps();
...
class DataTable$$module$rom extends Array{constructor(a,b,c,d,e){e=void 0===e?1<d?(...a)=>a:(a)=>a:e;super(c);this.rom=a;this.base=b;this.count=c;this.width=d;for(let f=0;f<c;f++)this[f]=e(...slice$$module$rom$util(a.prg,b+f*d,d))}}
...
class Message$$module$rom{constructor(a,b,c,d){this.messages=a;this.part=b;this.id=c;this.addr=d;b=a.rom.prg;c=[];for(this.bytes=[];b[d];d++){let e=b[d];this.bytes.push(e);if(1!=e)if(2==e)c.push("\n");else if(3==e)c.push("\u25bc\n");else if(4==e)c.push("SIMEA");else if(8==e)c.push("ITEM");else if(5<=e&&9>=e){const f=b[++d];9==e?c.push(" ".repeat(f)):(c.push(a.extraWords[e][f]),PUNCTUATION$$module$rom[String.fromCharCode(b[d+1])]||c.push(" "))}else if(128<=e)c.push(a.basicWords[e-128]),PUNCTUATION$$module$rom[String.fromCharCode(b[d+
...
(a)=>new AdHocSpawn$$module$rom$adhocspawn(this,a));this.metasprites=seq$$module$rom$util(256,(a)=>new Metasprite$$module$rom(this,a));this.messages=new Messages$$module$rom(this);this.itemGets=seq$$module$rom$util(113,(a)=>new ItemGet$$module$rom$itemget(this,a));this.npcs=seq$$module$rom$util(205,(a)=>new Npc$$module$rom$npc(this,a))}get monsters(){let a=new Set;for(const b of this.locations)if(b.valid&&b.hasSpawns)for(const c of b.spawns)0==(c[2]&7)&&a.add(this.objects[c[3]+80&255]);a=[...a];a.sort((a,
c)=>a.id-c.id);return a}get projectiles(){let a=new Set;for(const b of this.monsters)b.child&&a.add(this.objects[this.adHocSpawns[b.child].object]);a=[...a];a.sort((a,c)=>a.id-c.id);return a}get monsterGraphics(){const a={};for(const b of this.locations)if(b.valid&&b.hasSpawns)for(const c of b.spawns)if(!(c[2]&7)){const d=c[2]&128?1:0,e=(c[3]+80).toString(16).padStart(2,0);(a[e]=a[e]||{})[`${d}:${b.spritePatterns[d].toString(16)}:${b.spritePalettes[d].toString(16)}`]={slot:d,pat:b.spritePatterns[d],
...
console.log(`Tile ${a.toString(16).padStart(2,0)}: ${e}`)}swapMetatiles(a,...b){var c=new Map,d=seq$$module$rom$util(256);const e=new Map,f=(a)=>Array.isArray(a)?a[0]:0>a?~a:a;for(var g of b){for(var h=0;h<g.length-1;h++)Array.isArray(g[h])&&(e.set(g[h][0],g[h][1]),g[h]=g[h][0]);for(h=0;h<g.length-1;h++){let a=g[h],b=g[h+1];0>a||0>b||(c.set(b,a),d[b]=a)}}g=new Set;c=new Set;a=new Set(a);for(var k of this.locations)if(k.valid&&a.has(k.tileset)){c.add(k.tileEffects);for(const a of k.allScreens())g.add(a)}for(var n of g)for(var l of n.tiles)for(k=
...
RNMX$$module$random)}nextInt(a){return Math.floor(this.next()*a)}shuffle(a){for(let b=a.length;b;){const c=this.nextInt(b--);[a[b],a[c]]=[a[c],a[b]]}return a}}var module$random={};module$random.Random=Random$$module$random;const STATUS$$module$version="unstable",VERSION$$module$version="defrag",LABEL$$module$version="defrag 8288e1c",HASH$$module$version="8288e1c2c5027b73562ce3bf71d3585d17388a37",DATE$$module$version=new Date(1556348439E3);var module$version={};module$version.STATUS=STATUS$$module$version;module$version.VERSION=VERSION$$module$version;module$version.LABEL=LABEL$$module$version;module$version.HASH=HASH$$module$version;module$version.DATE=DATE$$module$version;var $jscompDefaultExport$$module$patch={apply(a,b,c){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){let d;b.seed||(b.seed=parseSeed$$module$patch("").toString(16),window.location.hash+="&seed="+b.seed);d=b.flags?new FlagSet$$module$flagset(b.flags):new FlagSet$$module$flagset("Em Gt Mr Rlpt Sbk Sct Sm Tasd");for(const a in b)"false"===b[a]&&(b[a]=!1);yield shuffle$$module$patch(a,parseSeed$$module$patch(b.seed),d,new FetchReader$$module$fetchreader(c))})}};
...
_TELEPORT_ON_THUNDER_SWORD:c.teleportOnThunderSword()},k=new Assembler$$module$6502,n=(b)=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){k.assemble(yield d.read(b),b);k.patchRom(a)});var l=Object.keys(h).filter((a)=>h[a]).map((a)=>`define ${a} 1\n`).join("");k.assemble(l,"flags.s");yield n("preshuffle.s");g=new Random$$module$random(g);yield shuffle2$$module$depgraph(a,g,e,c,f);rescaleShops$$module$patch(a,k,c.bargainHunting()?g:null);l=new Rom$$module$rom(a);rescaleMonsters$$module$patch(a,
l);c.shuffleMonsters()&&shuffleMonsters$$module$patch(a,l,g);identifyKeyItemsForDifficultyBuffs$$module$patch(l);c.doubleBuffMedicalHerb?(a[115996]*=2,a[115962]*=3):c.singleBuffMedicalHerb()&&(a[115996]+=16,a[115962]*=2);c.connectLimeTreeToLeaf()&&connectLimeTreeToLeaf$$module$patch(l);addCordelWestTriggers$$module$patch(l,c);c.disableRabbitSkip()&&fixRabbitSkip$$module$patch(l);c.storyMode()&&storyMode$$module$patch(l);c.chargeShotsOnly()&&disableStabs$$module$patch(l);c.orbsOptional()&&orbsOptional$$module$patch(l);
closeCaveEntrances$$module$patch(l,c);misc$$module$patch(l,c);yield l.writeData();return yield postParsedShuffle$$module$patch(a,g,b,c,k,n)}),postParsedShuffle$$module$patch=(a,b,c,d,e,f)=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){yield f("postshuffle.s");updateDifficultyScalingTables$$module$patch(a,d,e);updateCoinDrops$$module$patch(a,d);shuffleRandomNumbers$$module$patch(a,b);return stampVersionSeedAndHash$$module$patch(a,c,d)}),misc$$module$patch=(a,b)=>{},closeCaveEntrances$$module$patch=
...
{sdef:f,swrd:g,hits:h,satk:k,dgld:n,sexp:l}]=d;a=b.objects[e].objectData;c=[87,94,104,125,136,151,155,158].includes(e)?1:0;a[2]|=128;a[6]=h;a[7]=k;a[8]=f|g<<4;a[9]=a[9]&224|c;a[16]=a[16]&15|n<<4;a[17]=l}b.objects[125].objectData[16]|=8},shuffleMonsters$$module$patch=(a,b,c,d)=>{a=new MonsterPool$$module$patch({});for(const c of b.locations)c.valid&&a.populate(c);a.shuffle(c)},identifyKeyItemsForDifficultyBuffs$$module$patch=(a)=>{for(const b of a.itemGets)(a=ITEMS$$module$patch.get(b.item))&&a.key&&
(b.key=!0)},SCALED_MONSTERS$$module$patch=new Map([[63,"p","Sorceror shot",,,,19,,,],[75,"m","wraith??",2,,2,22,4,61],[79,"m","wraith",1,,2,20,4,61],[80,"m","Blue Slime",,,1,16,2,32],[81,"m","Weretiger",,,1,21,4,40],[82,"m","Green Jelly",4,,3,16,4,36],[83,"m","Red Slime",6,,4,16,4,48],[84,"m","Rock Golem",6,,11,24,6,85],[85,"m","Blue Bat",,,,4,,32],[86,"m","Green Wyvern",4,,4,24,6,52],[87,"b","Vampire",3,,12,18,,,],[88,"m","Orc",3,,4,21,4,57],[89,"m","Red Flying Swamp Insect",3,,1,21,4,57],[90,"m",
"Blue Mushroom",2,,1,21,4,44],[91,"m","Swamp Tomato",3,,2,35,4,52],[92,"m","Flying Meadow Insect",3,,3,23,4,81],[93,"m","Swamp Plant",,,,,,36],[94,"b","Insect",,1,8,6,,,],[95,"m","Large Blue Slime",5,,3,20,4,52],[96,"m","Ice Zombie",5,,7,14,4,57],[97,"m","Green Living Rock",,,1,9,4,28],[98,"m","Green Spider",4,,4,22,4,44],[99,"m","Red/Purple Wyvern",3,,4,30,4,65],[100,"m","Draygonia Soldier",6,,11,36,4,89],[101,"m","Ice Entity",3,,2,24,4,52],[102,"m","Red Living Rock",,,1,13,4,40],[103,"m","Ice Golem",
7,2,11,28,4,81],[104,"b","Kelbesque",4,6,12,29,,,],[105,"m","Giant Red Slime",7,,40,90,4,102],[106,"m","Troll",2,,3,24,4,65],[107,"m","Red Jelly",2,,2,14,4,44],[108,"m","Medusa",3,,4,36,8,77],[109,"m","Red Crab",2,,1,21,4,44],[110,"m","Medusa Head",,,1,29,4,36],[111,"m","Evil Bird",,,2,30,6,65],[113,"m","Red/Purple Mushroom",3,,5,19,6,69],[114,"m","Violet Earth Entity",3,,3,18,6,61],[115,"m","Mimic",,,3,26,15,73],[116,"m","Red Spider",3,,4,22,6,48],[117,"m","Fishman",4,,6,19,5,61],[118,"m","Jellyfish",
,,3,14,3,48],[119,"m","Kraken",5,,11,25,7,73],[120,"m","Dark Green Wyvern",4,,5,21,5,61],[121,"m","Sand Monster",5,,8,6,4,57],[123,"m","Wraith Shadow 1",,,,9,7,44],[124,"m","Killer Moth",,,2,35,,77],[125,"b","Sabera",3,7,13,24,,,],[128,"m","Draygonia Archer",1,,3,20,6,61],[129,"m","Evil Bomber Bird",,,1,19,4,65],[130,"m","Lavaman/blob",3,,3,24,6,85],[132,"m","Lizardman (w/ flail(",2,,3,30,6,81],[133,"m","Giant Eye",3,,5,33,4,81],[134,"m","Salamander",2,,4,29,8,77],[135,"m","Sorceror",2,,5,31,6,65],
[136,"b","Mado",4,8,10,30,,,],[137,"m","Draygonia Knight",2,,3,24,4,77],[138,"m","Devil",,,1,18,4,52],[139,"b","Kelbesque 2",4,6,11,27,,,],[140,"m","Wraith Shadow 2",,,,17,4,48],[144,"b","Sabera 2",5,7,21,27,,,],[145,"m","Tarantula",3,,3,21,6,73],[146,"m","Skeleton",,,4,30,6,69],[147,"b","Mado 2",4,8,11,25,,,],[148,"m","Purple Giant Eye",4,,10,23,6,102],[149,"m","Black Knight (w/ flail)",3,,7,26,6,89],[150,"m","Scorpion",3,,5,29,2,73],[151,"b","Karmine",4,,14,26,,,],[152,"m","Sandman/blob",3,,5,36,
6,98],[153,"m","Mummy",5,,19,36,6,110],[154,"m","Tomb Guardian",7,,60,37,6,106],[155,"b","Draygon",5,6,16,41,,,],[158,"b","Draygon 2",7,6,28,40,,,],[160,"m","Ground Sentry (1)",4,,12,26,,],[161,"m","Tower Defense Mech (2)",5,,16,36,,],[162,"m","Tower Sentinel",,,2,,,],[163,"m","Air Sentry",3,,4,26,,],[164,"b","Dyna",6,5,16,,,,],[165,"b","Vampire 2",3,,12,27,,,],[180,"b","dyna pod",15,,255,26,,,],[184,"p","dyna counter",,,,26,,,],[185,"p","dyna laser",,,,26,,,],[186,"p","dyna bubble",,,,36,,,],[188,
...
"p","mado shuriken",,,,36,,,],[253,"p","guardian statue missile",,,,23,,,],[254,"p","demon wall fire",,,,23,,,]].map((a)=>{var [b,c,d,e=0,f=0,g=0,h=0,k=0,n=0]=a;return[b,{id:b,type:c,name:d,sdef:e,swrd:f,hits:g,satk:h,dgld:k,sexp:n}]}));
...
d.id.toString(16).padStart(2,0)]=[];const {maxFlyers:g,nonFlyers:h={},fixedSlots:k={}}=MONSTER_ADJUSTMENTS$$module$patch[d.id]||{};let n=k.pat0||null,l=k.pat1||null,r=k.pal2||null,q=k.pal3||null,m=g,p=!1;for(const a of d.spawns)if(a.isChest()&&(p=!0),a.isMonster()){var b=a.monsterId;126==b||127==b||159==b?l=98:143==b&&(n=97)}if(21==d.id||71==d.id)p=!1;f.push(`Initial pass: ${[p,n,l,r,q].join(", ")}`);b=(a)=>{var b=FLYERS$$module$patch.has(a.id),c=MOTHS_AND_BATS$$module$patch.has(a.id);if(b){if(!m)return!1;
--m}if(null!=r&&null!=a.pal2&&r!=a.pal2||null!=q&&null!=a.pal3&&q!=a.pal3)return!1;var g=!p||TREASURE_CHEST_BANKS$$module$patch.has(a.pat);let k;if(d.rom.objects[a.id].child||RETAIN_SLOTS$$module$patch.has(a.id)){const b=(k=a.patBank)?l:n;if(null!=b&&b!=a.pat)return!1;if(k)l=a.pat;else if(g)n=a.pat;else return!1;f.push(`  Adding ${a.id.toString(16)}: pat(${k}) <-  ${a.pat.toString(16)}`)}else if(null==n&&g||n==a.pat)n=a.pat,k=0,f.push(`  Adding ${a.id.toString(16)}: pat0 <-  ${a.pat.toString(16)}`);
