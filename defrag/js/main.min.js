...
"Forged Crystalis")).get(pb),$a=(new Condition$$module$nodes(b,"Sword charge glitch")).option(D,U),Fe=(new Condition$$module$nodes(b,"Rabbit skip")).option(p,y),Ge=(new Condition$$module$nodes(b,"Teleport skip")).option(n,r),ab=(new Condition$$module$nodes(b,"Statue glitch")).option(m,q),He=(new Condition$$module$nodes(b,"Wild warp")).option(va,oa),bb=(new Condition$$module$nodes(b,"Any level 2 sword")).option(P,Ra).option(P,Ia).option(Q,Sa).option(Q,Ta).option(L,Ua).option(L,Va).option(J,Wa).option(J,
...
$jscomp.asyncExecutePromiseGeneratorFunction(function*(){const f=generate$$module$depgraph(d);d.shuffleShops()&&f.shuffleShops(b);const g=f.nodes.filter((a)=>a instanceof Slot$$module$nodes&&a.slots&&a.slotName);d||(d=new FlagSet$$module$flagset("Sbkm Sct"));var h={};for(var k of g)if(k instanceof Slot$$module$nodes&&k.slots){var l=k.slotType[0];(h[l]=h[l]||[]).push(k)}k=[];l=[];var m={};for(var q of d.flags.S||[]){var n=[];for(var r of q){m[r]=!0;for(var p of h[r])n.push(p),l.push([p,k.length])}k.push(n)}for(var y of"bc")if(m[y])for(q=
h[y],r=q.map((a)=>[a.item,a.itemIndex]),b.shuffle(r),p=0;p<q.length;p++)q[p].set(...r[p]);n=(a)=>g.find((b)=>b.vanillaItemName===a);h=n("Sword of Wind");y=n("Sword of Fire");q=n("Ball of Wind");r=n("Ball of Fire");p=n("Statue of Onyx");n=n("Gas Mask");m.k&&b.nextInt(2)&&(y.swap(h),r.swap(q));m.k&&!b.nextInt(3)&&p.swap(n);m=[1,1,1,1,2,2,2,3,3,3,5,5,8,13];h=[];e&&e.addTasks(10);for(y=0;1E3>y;y++){e&&0==y%100&&(yield new Promise(requestAnimationFrame),e.addCompleted(1));b.shuffle(l);for(var D of k)b.shuffle(D);
q=k.map(()=>-1);r=m[b.nextInt(m.length)];p=[];n=(a,b)=>a.itemIndex===b.itemIndex||a.isMimic()&&!b.canHoldMimic()||b.isMimic()&&!a.canHoldMimic()||a.needsChest()&&!b.isChest()||b.needsChest()&&!a.isChest()?!1:!0;for(let a=0;a<r;a++){const [b,c]=l[a],d=k[c][++q[c]];n(b,d)&&(b.swap(d),p.push([b,d]))}if(!p.length)continue;const {win:a,path:c}=f.traverse({wanted:g,dfs:!1});if(a)h=c;else for(;p.length;){const [a,b]=p.pop();a.swap(b)}}a&&f.write(a);if(c){c.items=[];c.route=[];for(const a of h){const [b,
...
a;this.itemIndex=b}write(a){if(this.slots)for(const b of this.slots)b(a,this)}swap(a){const b=this.item,c=this.itemIndex;this.set(a.item,a.itemIndex);a.set(b,c)}key(){this.slotType="key";return this}bonus(){this.slotType="bonus";return this}direct(a){this.slots.push((b,c)=>{write$$module$nodes(b,a,c.itemIndex)});return this}npcSpawn(a,b=-1,c=0){this.slots.push((d,e)=>{let f=addr$$module$nodes(d,116192,81920,a);for(;-1!==b&&d[f]!==b;){for(f++;!(d[f]&128);)f+=2,checkBounds$$module$nodes(f,d,this.name2,
b);f+=2;checkBounds$$module$nodes(f,d,this.name2,b)}f+=2*c+1;d[f]&=-2;d[f]|=2;write$$module$nodes(d,f+1,e.itemIndex)});return this}dialog(a,b=-1,c=0,d=-1){this.slots.push((e,f)=>{let g=addr$$module$nodes(e,117085,81920,a);for(;!(e[g]&128);)g+=4,checkBounds$$module$nodes(g,e,this.name2);g+=4;let h=0;for(;255!==e[g];)-1!==b&&e[g]===b&&(h=e[g+1]),g+=2,checkBounds$$module$nodes(g,e,b);for(g+=h+1;c;){if(e[g]&64){for(g+=5;!(e[g]&64);)g+=2,checkBounds$$module$nodes(g,e,this.name2);g+=2}else g+=5;--c}if(-1!==
d)for(g+=5;d;)g+=2,--d;e[g]&=-2;e[g]|=2;write$$module$nodes(e,g+1,f.itemIndex)});return this}trigger(a,b=0,c=-1){this.slots.push((d,e)=>{let f=addr$$module$nodes(d,123258,81920,a&127);if(-1===c)f+=2*b;else{for(;!(d[f]&128);)f+=2,checkBounds$$module$nodes(f,d,this.name2);f=f+4+2*c}d[f]&=-2;d[f]|=2;write$$module$nodes(d,f+1,e.itemIndex)});return this}}class FixedSlot$$module$nodes extends Slot$$module$nodes{isFixed(){return!0}}
...
a;)d=g.pop(),e=f[a++%f.length],d&&(0<=e.sells.indexOf(d)||4<=e.sells.length?g.push(d):e.sells.push(d));for(const {shops:a}of[b,c])for(const b of a)b.sells.sort((a,b)=>a.id-b.id)}integrate({tracker:a=!1}={}){var b=!a;const c=new SparseDependencyGraph$$module$graph(this.nodes.length);var d=[],e=[];const f=new Set,g=new Map;var h=[];const k=[],l=[],m=[],q=[],n=new Set;for(const a of this.nodes)if(a instanceof Location$$module$nodes){if(a.isStart){const [b]=c.addRoute([a.uid]);g.set(b.label,b)}d[a.uid]=
[];e[a.uid]=[];for(const b of a.connections)f.has(b)||(f.add(b),b.bidi&&f.add(b.reverse()));for(const b of a.chests)c.addRoute([b.uid,a.uid])}else if(a instanceof ItemGet$$module$nodes&&"Medical Herb"===a.name&&(c.addRoute([a.uid]),c.finalize(a.uid)),a instanceof Option$$module$nodes)h.push(a);else if(a instanceof TrackerNode$$module$nodes)k.push(a);else if(a instanceof Condition$$module$nodes)l.push(a);else if(a instanceof Trigger$$module$nodes)m.push(a);else if(a instanceof ItemGet$$module$nodes)q.push(a);
...
module$nodes.LocationList=LocationList$$module$nodes;module$nodes.FillStrategy=FillStrategy$$module$nodes;function seq$$module$rom$util(a,b=(a)=>a){return Array(a).fill(0).map((a,d)=>b(d))}function slice$$module$rom$util(a,b,c){return a.slice(b,b+c)}function tuple$$module$rom$util(a,b,c){return Array.from(a.slice(b,b+c))}function signed$$module$rom$util(a){return 128>a?a:a-256}function unsigned$$module$rom$util(a){return 0>a?a+256:a}function varSlice$$module$rom$util(a,b,c,d,e=Infinity,f){f||(f=(a)=>a);const g=[];for(;b+c<=e&&a[b]!==d;)g.push(f(a.slice(b,b+c))),b+=c;return g}
...
g=concatIterables$$module$rom$util(b.entrances),h=[...concatIterables$$module$rom$util(b.exits),255],k=[...concatIterables$$module$rom$util(b.flags),255],l=concatIterables$$module$rom$util(b.pits),[m,q,n,r,p,y]=yield Promise.all([d(e,"Layout"),d(f,"Graphics"),d(g,"Entrances"),d(h,"Exits"),d(k,"Flags"),...l.length?[d(l,"Pits")]:[]]);e=[m&255,(m>>>8)-192,q&255,(q>>>8)-192,n&255,(n>>>8)-192,r&255,(r>>>8)-192,p&255,(p>>>8)-192,...y?[y&255,(y>>8)-192]:[]];d=yield d(e,"MapData");writeLittleEndian$$module$rom$util(a.rom,
...
console.log(`Tile ${hex$$module$rom$util(a)}: ${e}`)}swapMetatiles(a,...b){var c=new Map,d=seq$$module$rom$util(256);const e=new Map,f=(a)=>Array.isArray(a)?a[0]:0>a?~a:a;for(var g of b){for(var h=0;h<g.length-1;h++)if(Array.isArray(g[h])){var k=g[h];e.set(k[0],k[1]);g[h]=k[0]}for(h=0;h<g.length-1;h++){k=g[h];const a=g[h+1];0>k||0>a||(c.set(a,k),d[a]=k)}}g=new Set;c=new Set;a=new Set(a);for(var l of this.locations)if(l.used&&a.has(l.tileset)){c.add(l.tileEffects);for(var m of l.allScreens())g.add(m)}for(var q of g)for(var n of q.tiles)for(l=
0;l<n.length;l++)n[l]=d[n[l]];for(var r of a){d=this.tilesets[(r&127)>>>2];for(var p of b)for(q=0;q<p.length-1;q++){n=f(p[q]);l=f(p[q+1]);for(m=0;4>m;m++)d.tiles[m][n]=d.tiles[m][l];d.attrs[n]=d.attrs[l];if(32>l&&d.alternates[l]!==l){if(32<=n)throw Error(`Cannot unflag: ${r} ${n} ${l} ${d.alternates[l]}`);d.alternates[n]=d.alternates[l]}}for(var y of e){const [a,b]=y;d.alternates[a]=b}}for(const a of c){r=this.tileEffects[a-179];for(const a of b)for(p=0;p<a.length-1;p++)y=f(a[p]),d=f(a[p+1]),r.effects[y]=
r.effects[d];for(const a of e.keys())r.effects[a]|=8}}}
...
return Math.min(AM$$module$random*this.iy,RNMX$$module$random)}nextInt(a){return Math.floor(this.next()*a)}shuffle(a){for(let b=a.length;b;){const c=this.nextInt(b--);[a[b],a[c]]=[a[c],a[b]]}return a}}var module$random={};module$random.Random=Random$$module$random;const STATUS$$module$version="unstable",VERSION$$module$version="defrag",LABEL$$module$version="defrag d6f1559",HASH$$module$version="d6f15598c78221222c908c2abab066c6db5654af",DATE$$module$version=new Date(1556611329E3);var module$version={};module$version.STATUS=STATUS$$module$version;module$version.VERSION=VERSION$$module$version;module$version.LABEL=LABEL$$module$version;module$version.HASH=HASH$$module$version;module$version.DATE=DATE$$module$version;var $jscompDefaultExport$$module$patch={apply(a,b,c){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){let d;b.seed||(b.seed=parseSeed$$module$patch("").toString(16),window.location.hash+="&seed="+b.seed);d=b.flags?new FlagSet$$module$flagset(b.flags):new FlagSet$$module$flagset("Em Gt Mr Rlpt Sbk Sct Sm Tasd");for(const a in b)"false"===b[a]&&(b[a]=!1);yield shuffle$$module$patch(a,parseSeed$$module$patch(b.seed),d,new FetchReader$$module$fetchreader(c))})}};
