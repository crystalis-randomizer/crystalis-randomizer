...
a.has(l.tileset)){c.add(l.tileEffects);for(var m of l.allScreens())g.add(m)}for(var r of g)for(var n of r.tiles)for(l=0;l<n.length;l++)n[l]=d[n[l]];for(var q of a){d=this.tilesets[(q&127)>>>2];for(var p of b)for(r=0;r<p.length-1;r++){n=f(p[r]);l=f(p[r+1]);for(m=0;4>m;m++)d.tiles[m][n]=d.tiles[m][l];d.attrs[n]=d.attrs[l];if(32>l&&d.alternates[l]!==l){if(32<=n)throw Error(`Cannot unflag: ${q} ${n} ${l} ${d.alternates[l]}`);d.alternates[n]=d.alternates[l]}}for(var y of e){const [a,b]=y;d.alternates[a]=
b}}for(const a of c){q=this.tileEffects[a-179];for(const a of b)for(p=0;p<a.length-1;p++)y=f(a[p]),d=f(a[p+1]),q.effects[y]=q.effects[d];for(const a of e.keys())q.effects[a]|=8}}}
...
return Math.min(AM$$module$random*this.iy,RNMX$$module$random)}nextInt(a){return Math.floor(this.next()*a)}shuffle(a){for(let b=a.length;b;){const c=this.nextInt(b--);[a[b],a[c]]=[a[c],a[b]]}return a}}var module$random={};module$random.Random=Random$$module$random;const STATUS$$module$version="unstable",VERSION$$module$version="defrag",LABEL$$module$version="defrag b57ceec",HASH$$module$version="b57ceec864c72353eba15e0140e73cfdaf44f92f",DATE$$module$version=new Date(155687701E4);var module$version={};module$version.STATUS=STATUS$$module$version;module$version.VERSION=VERSION$$module$version;module$version.LABEL=LABEL$$module$version;module$version.HASH=HASH$$module$version;module$version.DATE=DATE$$module$version;var $jscompDefaultExport$$module$patch={apply(a,b,c){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){let d;b.seed||(b.seed=parseSeed$$module$patch("").toString(16),window.location.hash+="&seed="+b.seed);d=b.flags?new FlagSet$$module$flagset(b.flags):new FlagSet$$module$flagset("Em Gt Mr Rlpt Sbk Sct Sm Tasd");for(const a in b)"false"===b[a]&&(b[a]=!1);yield shuffle$$module$patch(a,parseSeed$$module$patch(b.seed),d,new FetchReader$$module$fetchreader(c))})}};
