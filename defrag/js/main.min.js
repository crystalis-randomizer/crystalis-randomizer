...
d,e)=>{d=void 0===d?new FlagSet$$module$flagset("Sbkm Sct"):d;return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){const f=generate$$module$depgraph(d);d.shuffleShops()&&f.shuffleShops(b);const g=f.nodes.filter((a)=>a instanceof Slot$$module$nodes&&a.slots&&a.slotName);var h={};for(var k of g)if(k instanceof Slot$$module$nodes&&k.slots){var l=k.slotType[0];(h[l]=h[l]||[]).push(k)}k=[];l=[];var m={};for(var r of d.get("S")){var n=[];for(var q of r){m[q]=!0;for(var p of h[q])n.push(p),l.push([p,
k.length])}k.push(n)}for(var y of"bc")if(m[y])for(r=h[y],q=r.map((a)=>[a.item,a.itemIndex]),b.shuffle(q),p=0;p<r.length;p++)r[p].set(q[p][0],q[p][1]);n=(a)=>g.find((b)=>b.vanillaItemName===a);h=n("Sword of Wind");y=n("Sword of Fire");r=n("Ball of Wind");q=n("Ball of Fire");p=n("Statue of Onyx");n=n("Gas Mask");m.k&&b.nextInt(2)&&(y.swap(h),q.swap(r));m.k&&!b.nextInt(3)&&p.swap(n);m=[1,1,1,1,2,2,2,3,3,3,5,5,8,13];h=[];e&&e.addTasks(10);for(y=0;1E3>y;y++){e&&0===y%100&&(yield new Promise(requestAnimationFrame),
e.addCompleted(1));b.shuffle(l);for(var z of k)b.shuffle(z);r=k.map(()=>-1);q=m[b.nextInt(m.length)];p=[];n=(a,b)=>a.itemIndex===b.itemIndex||a.isMimic()&&!b.canHoldMimic()||b.isMimic()&&!a.canHoldMimic()||a.needsChest()&&!b.isChest()||b.needsChest()&&!a.isChest()?!1:!0;for(let a=0;a<q;a++){const [b,c]=l[a],d=k[c][++r[c]];n(b,d)&&(b.swap(d),p.push([b,d]))}if(!p.length)continue;const {win:a,path:c}=f.traverse({wanted:g,dfs:!1});if(a)h=c;else for(;p.length;){const [a,b]=p.pop();a.swap(b)}}a&&f.write(a);
...
if(f.guaranteeSword()){if(0===a.slotIndex)return 4>b.item.id;if(4>b.item.id&&(p.add(b),4===p.size))return 0===a.slotIndex}return n[a.slotType?a.slotType[0]:"c"]===n[b.slotType?b.slotType[0]:"c"]};q=b.assumedFill(d,(a,b)=>y(a,r.get(b)));if(null==q){g&&g.endState.key++;return!1;throw Error("Could not fill!");}const z=new Map;for(var V=0;V<q.length;V++){var ua=q[V];if(null!=ua){var G=r.get(b.item(ua));ua=l.get(G);l.delete(G);G=b.location(V);z.set(G,ua);m.delete(G)}}V=(a,b,c)=>{c=void 0===c?"":c;for(const c of m)if(y(c,
...
g=concatIterables$$module$rom$util(b.entrances),h=[...concatIterables$$module$rom$util(b.exits),255],k=[...concatIterables$$module$rom$util(b.flags),255],l=concatIterables$$module$rom$util(b.pits),[m,r,n,q,p,y]=yield Promise.all([d(e,"Layout"),d(f,"Graphics"),d(g,"Entrances"),d(h,"Exits"),d(k,"Flags"),...l.length?[d(l,"Pits")]:[]]);e=[m&255,(m>>>8)-192,r&255,(r>>>8)-192,n&255,(n>>>8)-192,q&255,(q>>>8)-192,p&255,(p>>>8)-192,...y?[y&255,(y>>8)-192]:[]];d=yield d(e,"MapData");writeLittleEndian$$module$rom$util(a.rom,
...
0;l<n.length;l++)n[l]=d[n[l]];for(var q of a){d=this.tilesets[(q&127)>>>2];for(var p of b)for(r=0;r<p.length-1;r++){n=f(p[r]);l=f(p[r+1]);for(m=0;4>m;m++)d.tiles[m][n]=d.tiles[m][l];d.attrs[n]=d.attrs[l];if(32>l&&d.alternates[l]!==l){if(32<=n)throw Error(`Cannot unflag: ${q} ${n} ${l} ${d.alternates[l]}`);d.alternates[n]=d.alternates[l]}}for(var y of e){const [a,b]=y;d.alternates[a]=b}}for(const a of c){q=this.tileEffects[a-179];for(const a of b)for(p=0;p<a.length-1;p++)y=f(a[p]),d=f(a[p+1]),q.effects[y]=
q.effects[d];for(const a of e.keys())q.effects[a]|=8}}}
...
return Math.min(AM$$module$random*this.iy,RNMX$$module$random)}nextInt(a){return Math.floor(this.next()*a)}shuffle(a){for(let b=a.length;b;){const c=this.nextInt(b--);[a[b],a[c]]=[a[c],a[b]]}return a}}var module$random={};module$random.Random=Random$$module$random;const STATUS$$module$version="unstable",VERSION$$module$version="defrag",LABEL$$module$version="defrag 824ef39",HASH$$module$version="824ef393174046a0ed4de09b34fb93883dbdfac2",DATE$$module$version=new Date(1556780434E3);var module$version={};module$version.STATUS=STATUS$$module$version;module$version.VERSION=VERSION$$module$version;module$version.LABEL=LABEL$$module$version;module$version.HASH=HASH$$module$version;module$version.DATE=DATE$$module$version;var $jscompDefaultExport$$module$patch={apply(a,b,c){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){let d;b.seed||(b.seed=parseSeed$$module$patch("").toString(16),window.location.hash+="&seed="+b.seed);d=b.flags?new FlagSet$$module$flagset(b.flags):new FlagSet$$module$flagset("Em Gt Mr Rlpt Sbk Sct Sm Tasd");for(const a in b)"false"===b[a]&&(b[a]=!1);yield shuffle$$module$patch(a,parseSeed$$module$patch(b.seed),d,new FetchReader$$module$fetchreader(c))})}};
...
31:45,32:40,33:36,34:200,35:150,36:80,38:300},rescaleMonsters$$module$patch=(a,b)=>{for(var c of SCALED_MONSTERS$$module$patch.keys())for(const a in b.objects){if(SCALED_MONSTERS$$module$patch.has(a))return;b.objects[c].objectDataBase==b.objects[a].objectDataBase&&(SCALED_MONSTERS$$module$patch[a]=SCALED_MONSTERS$$module$patch[c])}for(const d of SCALED_MONSTERS$$module$patch){const [e,{sdef:f,swrd:g,hits:h,satk:k,dgld:l,sexp:m}]=d;a=b.objects[e].data;c=[87,94,104,125,136,151,155,158].includes(e)?
1:0;a[2]|=128;a[6]=h;a[7]=k;a[8]=f|g<<4;a[9]=a[9]&224|c;a[16]=a[16]&15|l<<4;a[17]=m}b.objects[125].elements|=8},shuffleMonsters$$module$patch=(a,b,c,d)=>{a=new MonsterPool$$module$patch({});for(const c of b.locations)c.used&&a.populate(c);a.shuffle(c)},identifyKeyItemsForDifficultyBuffs$$module$patch=(a)=>{for(const b of a.itemGets)(a=ITEMS$$module$patch.get(b.item))&&a.key&&(b.key=!0)},SCALED_MONSTERS$$module$patch=new Map([[63,"p","Sorceror shot",,,,19,,,],[75,"m","wraith??",2,,2,22,4,61],[79,"m",
...
{var [b,c,d,e=0,f=0,g=0,h=0,k=0,l=0]=a;return[b,{id:b,type:c,name:d,sdef:e,swrd:f,hits:g,satk:h,dgld:k,sexp:l}]}));
