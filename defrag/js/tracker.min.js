...
0)]={};for(const b of a.objects)if(!(b[2]&7)){const a=b[2]&128?1:0,d=b[3]+80;c[`${a}:${d.toString(16)}`]=(c[`${a}:${d.toString(16)}`]||0)+1}}return b}static load(b){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){const a=yield pickFile$$module$js$rom();b&&(yield b(a));return new Rom$$module$js$rom(a)})}writeObjectData(){let b=110080;const a={};for(const c of this.objects){const d=c.serialize(),e=d.join(" ");e in a?c.objectDataBase=a[e]:(c.objectDataBase=b,a[e]=b,b+=d.length);c.write()}return b}writeData(){this.prg[246387]=
this.prg[246392]=190;this.prg[246399]=this.prg[246404]=191;const b=new Writer$$module$js$rom(this.prg,83192,97792),a=new Writer$$module$js$rom(this.prg,103415,113408),c=[];for(const d of this.locations)d&&(c.push(d.writeMapData(b)),c.push(d.writeNpcData(a)));for(const b of this.objects)b.write(a,114176);b.commit();a.commit();return Promise.all(c).then(()=>{})}}
...
return Math.min(AM$$module$js$random*this.iy,RNMX$$module$js$random)}nextInt(b){return Math.floor(this.next()*b)}shuffle(b){for(let a=b.length;a;){const c=this.nextInt(a--);[b[a],b[c]]=[b[c],b[a]]}return b}}var module$js$random={};module$js$random.Random=Random$$module$js$random;const STATUS$$module$js$version="unstable",VERSION$$module$js$version="defrag",LABEL$$module$js$version="defrag e081813",HASH$$module$js$version="e081813c926b9cd0acbb4189084d4545d89520d8",DATE$$module$js$version=new Date(1554200589E3);var module$js$version={};module$js$version.STATUS=STATUS$$module$js$version;module$js$version.VERSION=VERSION$$module$js$version;module$js$version.LABEL=LABEL$$module$js$version;module$js$version.HASH=HASH$$module$js$version;module$js$version.DATE=DATE$$module$js$version;var $jscompDefaultExport$$module$js$patch={apply(b,a){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){a.seed||(a.seed=parseSeed$$module$js$patch("").toString(16),window.location.hash+="&seed="+a.seed);a.flags?new FlagSet$$module$js$flagset(a.flags):new FlagSet$$module$js$flagset("Em Gt Mr Rlpt Sbk Sct Sm Tasd");for(const b in a)"false"===a[b]&&(a[b]=!1)})}};
...
h,g);identifyKeyItemsForDifficultyBuffs$$module$js$patch(h);c.check("Em")?(b[115996]*=2,b[115962]*=3):c.check("Hm")||(b[115996]+=16,b[115962]*=2);c.check("Rp")&&connectLimeTreeToLeaf$$module$js$patch(h);yield p("postshuffle.s");updateDifficultyScalingTables$$module$js$patch(b,c,m);updateCoinDrops$$module$js$patch(b,c);shuffleRandomNumbers$$module$js$patch(b,g);yield h.writeData();return stampVersionSeedAndHash$$module$js$patch(b,a,c)}),connectLimeTreeToLeaf$$module$js$patch=(b)=>{const a=b.locations[3];
b=b.locations[66];a.screens[5][4]=16;b.screens[1][0]=26;b.screens[2][0]=12;const c=a.entrances.push([239,4,120,5])-1,d=b.entrances.push([16,0,192,1])-1;a.exits.push([79,86,66,d],[79,87,66,d]);b.exits.push([0,27,3,c],[0,28,3,c])},stampVersionSeedAndHash$$module$js$patch=(b,a,c)=>{const d=crc32$$module$js$crc32(b),e=d.toString(16).padStart(8,0).toUpperCase(),f="unstable"==STATUS$$module$js$version?HASH$$module$js$version.substring(0,7).padStart(7,0).toUpperCase()+"     ":VERSION$$module$js$version.substring(0,
12).padEnd(12," ");a=a.toString(16).padStart(8,0).toUpperCase();const g=(a,c)=>{for(let d=0;d<c.length;d++)b[a+16+d]=c.charCodeAt(d)},h=(a,b)=>{const c=[];for(let d=0;d<a.length||d<b.length;d++)c.push(a[d]||" "),c.push(b[d]||" ");return c.join("")};g(161743,h("  VERSION     SEED      ",`  ${f}${a}`));a=String(c);let k;if(46<a.length){if(92<a.length)throw Error("Flag string way too long!");k=a.substring(46,92).padEnd(46," ");a=a.substring(0,46)}a=a.padEnd(46," ");g(161791,h(a.substring(0,23),a.substring(23)));
...
48],[84,"m","Rock Golem",6,,11,24,6,85],[85,"m","Blue Bat",,,,4,,32],[86,"m","Green Wyvern",4,,4,24,6,52],[87,"b","Vampire",3,,13,18,,,],[88,"m","Orc",3,,4,21,4,57],[89,"m","Red Flying Swamp Insect",3,,1,21,4,57],[90,"m","Blue Mushroom",2,,1,21,4,44],[91,"m","Swamp Tomato",3,,2,35,4,52],[92,"m","Flying Meadow Insect",3,,3,23,4,81],[93,"m","Swamp Plant",,,,,,36],[94,"b","Insect",,1,8,6,,,],[95,"m","Large Blue Slime",5,,3,20,4,52],[96,"m","Ice Zombie",5,,7,14,4,57],[97,"m","Green Living Rock",,,1,9,
