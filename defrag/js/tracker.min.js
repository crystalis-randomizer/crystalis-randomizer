...
class Npc$$module$js$rom extends Entity$$module$js$rom{constructor(b,a){super(b,a);this.used=!UNUSED_NPCS$$module$js$rom.has(a)&&(143>a||192<=a);this.spawnPointer=116192+(a<<1);this.spawnBase=addr$$module$js$rom(b.prg,this.spawnPointer,81920);this.spawnConditions={};this.dataBase=33008|(a&252)<<6|(a&3)<<2;this.data=slice$$module$js$rom(b.prg,this.dataBase,4);a=this.spawnBase;let c;for(;this.used&&255!=(c=b.prg[a++]);){this.spawnConditions[c]=[];let d;do{d=b.prg[a]<<8|b.prg[a+1];const e=d&4095;this.spawnConditions[c].push(d&
...
b.rom[c+2*d.id]=a&255;b.rom[c+2*d.id+1]=(a>>>8)-64})}}class AdHocSpawn$$module$js$rom extends Entity$$module$js$rom{constructor(b,a){super(b,a);this.base=(a<<2)+171008;this.lowerSlot=b.prg[this.base];this.upperSlot=b.prg[this.base+1];this.object=b.prg[this.base+2];this.count=b.prg[this.base+3]}}
...
b.prg[246392]=190;b.prg[246399]=b.prg[246404]=191;var a=new Writer$$module$js$rom(b.prg,83192,97792);const c=new Writer$$module$js$rom(b.prg,103417,113408),d=[];for(const e of b.locations)e&&(d.push(e.writeMapData(a)),d.push(e.writeNpcData(c)));yield a.commit();a=yield c.commit();a=new Writer$$module$js$rom(b.prg,Math.max(106496,a),113408);for(var e of b.objects)e.write(a,114176);yield a.commit();e=new Writer$$module$js$rom(b.prg,123392,123888);for(var f of b.triggers)f.used&&d.push(f.write(e));yield e.commit();
f=new Writer$$module$js$rom(b.prg,116602,117085);for(const a of b.npcs)a.used&&d.push(a.write(f));yield f.commit();for(const a of b.tilesets)a.write(b.prg);for(const a of b.tileEffects)a.write(b.prg);for(const a of b.screens)a.write(b.prg);return Promise.all(d).then(()=>{})})}analyzeTiles(){}disjointTilesets(){var b=[];for(var a of this.locations)if(a){var c=a.tileset,d=a.extended?256:0;for(const e of a.screens)for(const a of e)(b[a+d]||(b[a+d]=new Set)).add(c)}a=Array(256).fill(0).map(()=>new UnionFind$$module$js$unionfind);
...
return Math.min(AM$$module$js$random*this.iy,RNMX$$module$js$random)}nextInt(b){return Math.floor(this.next()*b)}shuffle(b){for(let a=b.length;a;){const c=this.nextInt(a--);[b[a],b[c]]=[b[c],b[a]]}return b}}var module$js$random={};module$js$random.Random=Random$$module$js$random;const STATUS$$module$js$version="unstable",VERSION$$module$js$version="defrag",LABEL$$module$js$version="defrag a673996",HASH$$module$js$version="a67399609a4770521e897b47b91912f340de0219",DATE$$module$js$version=new Date(1555578809E3);var module$js$version={};module$js$version.STATUS=STATUS$$module$js$version;module$js$version.VERSION=VERSION$$module$js$version;module$js$version.LABEL=LABEL$$module$js$version;module$js$version.HASH=HASH$$module$js$version;module$js$version.DATE=DATE$$module$js$version;var $jscompDefaultExport$$module$js$patch={apply(b,a,c){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){let d;a.seed||(a.seed=parseSeed$$module$js$patch("").toString(16),window.location.hash+="&seed="+a.seed);d=a.flags?new FlagSet$$module$js$flagset(a.flags):new FlagSet$$module$js$flagset("Em Gt Mr Rlpt Sbk Sct Sm Tasd");for(const b in a)"false"===a[b]&&(a[b]=!1);yield shuffle$$module$js$patch(b,parseSeed$$module$js$patch(a.seed),d,new FetchReader$$module$js$fetchreader(c))})}};
