...
class FlagSet$$module$js$flagset{constructor(b="RtGftTab"){b.startsWith("@")&&(b=PRESETS_BY_KEY$$module$js$flagset[b.toLowerCase()]);this.flags={};b=b.replace(/[^A-Za-z0-9!]/g,"");const a=/([A-Z])([a-z0-9!]+)/g;let c;for(;c=a.exec(b);){let [,a,b]=c;REPEATABLE_FLAGS$$module$js$flagset.has(a)&&(b=[b]);for(const c of b)this.set(a+c,!0)}}set(b,a){const c=b[0],d=b.substring(1);a?(this.removeConflicts(b),b=(this.flags[c]||[]).filter((a)=>a!==d),b.push(d),b.sort(),this.flags[c]=b):(b=(this.flags[c]||[]).filter((a)=>
...
Ce=(new Trigger$$module$js$nodes(a,"Talked to Akahana's Friend")).get(ge),De=(new Trigger$$module$js$nodes(a,"Get Bow of Truth")).get(rc),Ee=(new Trigger$$module$js$nodes(a,"Forged Crystalis")).get(pb),$a=(new Condition$$module$js$nodes(a,"Sword charge glitch")).option(C,U),Fe=(new Condition$$module$js$nodes(a,"Rabbit skip")).option(r,I),Ge=(new Condition$$module$js$nodes(a,"Teleport skip")).option(p,m),ab=(new Condition$$module$js$nodes(a,"Statue glitch")).option(l,q),He=(new Condition$$module$js$nodes(a,
...
"Ride dolphin")).option(yb,Ic),Ne=(new Condition$$module$js$nodes(a,"Ghetto flight")).option(n,Rc,vb),ba=(new Condition$$module$js$nodes(a,"Cross sea")).option(Rc).option(z),Oe=(new Condition$$module$js$nodes(a,"Cross whirlpool")).option(Db).option(z).option(Ne),S=(new Condition$$module$js$nodes(a,"Refresh if needed")).option(Xb.value?Xb:g).option(Za),Sc=(new Condition$$module$js$nodes(a,"Wind magic")).option(E.value?E:g,S).option(Ra,Ia,S),Tc=(new Condition$$module$js$nodes(a,"Fire magic")).option(E.value?
E:g,S).option(Sa,Ta,S),Uc=(new Condition$$module$js$nodes(a,"Water magic")).option(E.value?E:g,S).option(E,S).option(Ua,Va,S),Pe=(new Condition$$module$js$nodes(a,"Thunder magic")).option(E.value?E:g,S).option(E,S).option(Wa,S).option(Xa,S),Qe=(new Condition$$module$js$nodes(a,"Flute of lime or glitch")).option(ub).option(ab),Re=(new Condition$$module$js$nodes(a,"Change or glitch")).option(pa).option(ab),Se=(new Condition$$module$js$nodes(a,"Change or glitch or paralysis")).option(pa).option(ab).option(Ja),
Fb=(new Condition$$module$js$nodes(a,"Pass shooting statues")).option(tc).option(aa,Wb.value?Wb:g).option(Za,gc),Te=(new Condition$$module$js$nodes(a,"Healed dolphin if required")).option(Hc).option(Pd),Vc=(new Condition$$module$js$nodes(a,"Match sword of wind")).option(P).option(G.value?G:g,aa),Wc=(new Condition$$module$js$nodes(a,"Match sword of fire")).option(Q).option(G.value?G:g,aa),Xc=(new Condition$$module$js$nodes(a,"Match sword of water")).option(L).option(G.value?G:g,aa),Ue=(new Condition$$module$js$nodes(a,
"Match sword of thunder")).option(J).option(G.value?G:g,aa),Ve=(new Condition$$module$js$nodes(a,"Swamp run possible")).option(aa).option(Oc).option(Wd).option(Za),We=(new Condition$$module$js$nodes(a,"Insect possible to kill")).option(Ya),cb=(new Condition$$module$js$nodes(a,"Travel swamp")).option(Ya).option(Pa.value?Pa:g,Ve),Xe=(new Condition$$module$js$nodes(a,"Calmed sea if required")).option(Db).option(Qd),Ye=(new Condition$$module$js$nodes(a,"Started windmill if required")).option(Ka).option(Sd),
Ze=(new Condition$$module$js$nodes(a,"Telepathy if required for Deo")).option(fa).option(Ud),$e=e(0,"Vampire 1",aa).get(vb),af=e(1,"Insect",ec,Ie,We).get(Sa),bf=e(2,"Kelbesque 1",Vc,Sc).get(Ta),cf=e(12,"Vampire 2",Je).get(ke),Yc=e(4,"Sabera 1",Wc,Tc).get(lc),df=e(5,"Mado 1",Xc,Uc).get(Wa),ef=e(6,"Kelbesque 2",Vc,Sc).get(tb),ff=e(7,"Sabera 2",Wc,Tc).get(sb),gf=e(8,"Mado 2",Xc,Uc).get(qb),hf=e(9,"Karmine",Ue,Pe).get(zb),jf=e(10,"Draygon 1",aa).get(ee),kf=e(null,"Statues",qc,pc),lf=e(11,"Draygon 2",
aa,rc),mf=e(13,"Dyna",pb),nf=(new Condition$$module$js$nodes(a,"Opened prison if required")).option(xb).option(Rd),W=new Area$$module$js$nodes(a,"Leaf"),ya=new Area$$module$js$nodes(a,"Valley of Wind"),T=new Area$$module$js$nodes(a,"Sealed Cave"),qa=new Area$$module$js$nodes(a,"Cordel Plain"),za=new Area$$module$js$nodes(a,"Brynmaer"),Aa=new Area$$module$js$nodes(a,"Amazones"),A=new Area$$module$js$nodes(a,"Mt Sabre West"),u=new Area$$module$js$nodes(a,"Mt Sabre North"),db=new Area$$module$js$nodes(a,
"Nadare's"),La=new Area$$module$js$nodes(a,"Oak"),ha=new Area$$module$js$nodes(a,"Waterfall Valley"),y=new Area$$module$js$nodes(a,"Portoa"),ia=new Area$$module$js$nodes(a,"Waterfall Cave"),B=new Area$$module$js$nodes(a,"Fog Lamp Cave"),ja=new Area$$module$js$nodes(a,"Kirisa Plant Cave"),ra=new Area$$module$js$nodes(a,"Angry Sea"),ka=new Area$$module$js$nodes(a,"Joel"),H=new Area$$module$js$nodes(a,"Evil Spirit Island"),la=new Area$$module$js$nodes(a,"Sabera's Castle"),X=new Area$$module$js$nodes(a,
"Swan"),x=new Area$$module$js$nodes(a,"Mt Hydra"),ma=new Area$$module$js$nodes(a,"Shyron"),sa=new Area$$module$js$nodes(a,"Styx"),ta=new Area$$module$js$nodes(a,"Goa"),eb=new Area$$module$js$nodes(a,"Draygonia Fortress 1"),Y=new Area$$module$js$nodes(a,"Draygonia Fortress 2"),Ba=new Area$$module$js$nodes(a,"Draygonia Fortress 3"),O=new Area$$module$js$nodes(a,"Draygonia Fortress 4"),F=new Area$$module$js$nodes(a,"Oasis Cave");new Area$$module$js$nodes(a,"Desert");const Z=new Area$$module$js$nodes(a,
"Sahara"),Ca=new Area$$module$js$nodes(a,"Pyramid Front"),ca=new Area$$module$js$nodes(a,"Pyramid Back"),da=new Area$$module$js$nodes(a,"Tower"),Gb=(new Location$$module$js$nodes(a,0,W,"Start")).overworld().start(),of=(new Location$$module$js$nodes(a,0,W,"Mezame Shrine")).overworld().from(Gb),pf=(new Location$$module$js$nodes(a,1,W,"Outside Start")).overworld().connect(of),na=(new Location$$module$js$nodes(a,2,W,"Town")).town().connect(pf),Ma=(new Location$$module$js$nodes(a,3,ya,"Main")).overworld().connect(na).trigger(wc,
Ka),Zc=(new Location$$module$js$nodes(a,3,ya,"Outside Windmill")).overworld(),$c=(new Location$$module$js$nodes(a,4,T,"Tunnel 1 (entrance)")).cave().from(Ma,Ka),qf=(new Location$$module$js$nodes(a,5,T,"Tunnel 2 (over bridge)")).cave().connect($c),rf=(new Location$$module$js$nodes(a,6,T,"Tunnel 6 (herb dead end)")).cave().chest(D,15),ad=(new Location$$module$js$nodes(a,7,T,"Tunnel 4a (ball corridor)")).cave().chest(D,20,80).chest(Ra,21);(new Location$$module$js$nodes(a,7,T,"Tunnel 4b (antidote dead end)")).cave().connect(ad,
v).chest(K,19);const sf=(new Location$$module$js$nodes(a,8,T,"Tunnel 5 (warp boots dead end)")).cave().chest(R,14),tf=(new Location$$module$js$nodes(a,9,T,"Tunnel 3a (branch, front)")).cave().connect(qf).connectTo(ad).connectTo(sf),uf=(new Location$$module$js$nodes(a,9,T,"Tunnel 3b (branch, back)")).cave().connect(tf,v).connectTo(rf),vf=(new Location$$module$js$nodes(a,10,T,"Tunnel 7 (boss)")).cave().connect(uf).boss($e),wf=(new Location$$module$js$nodes(a,12,T,"Tunnel 8a (exit, above wall)")).cave().connect(vf),
xf=(new Location$$module$js$nodes(a,12,T,"Tunnel 8b (exit, below wall")).cave().connect(wf,v);(new Location$$module$js$nodes(a,14,ya,"Windmill Cave")).cave().connect(Ma).connectTo(Zc).trigger(le,wb,Ab);(new Location$$module$js$nodes(a,15,ya,"Windmill")).misc().connect(Zc).trigger(Ka,hc);const yf=(new Location$$module$js$nodes(a,16,ya,"Zebu's Cave Front")).cave().connect(Ma).trigger(Ab,uc,vc).trigger(wc,Ka,Ab),zf=(new Location$$module$js$nodes(a,16,ya,"Zebu's Cave Back")).cave().trigger(zc).connect(yf,
...
Df=(new Location$$module$js$nodes(a,32,A,"Up Slope")).overworld().from(Ea,Pc).to(Ea),Ef=(new Location$$module$js$nodes(a,32,A,"Dead End (warp boots)")).overworld().chest(R,24,106),cd=(new Location$$module$js$nodes(a,33,A,"Upper")).overworld().from(Ea,z).to(Ea),Ff=(new Location$$module$js$nodes(a,33,A,"Tornel Dead End")).overworld().trigger(qe,Ia).chest(V,23,105),dd=(new Location$$module$js$nodes(a,34,A,"Tunnel 2a (fork at start)")).cave().connect(Ea);(new Location$$module$js$nodes(a,34,A,"Tunnel 2b (left branch to dead end)")).cave().connect(dd,
...
112,ka,"Secret Passage")).cave().connectTo(vd),lb=(new Location$$module$js$nodes(a,113,ka,"Town")).town().connect(Bg),ea=(new Location$$module$js$nodes(a,114,X,"Town")).town().connect(Dg),Qg=(new Location$$module$js$nodes(a,115,X,"Inside Gate")).overworld().connect(ea),Rg=(new Location$$module$js$nodes(a,115,X,"Outside Gate")).overworld().from(Qg,pa),mb=(new Location$$module$js$nodes(a,120,ta,"Valley")).overworld().connect(Rg),Pb=(new Location$$module$js$nodes(a,124,x,"Entrance")).overworld().connect(mb),
Sg=(new Location$$module$js$nodes(a,124,x,"Over first river toward Shyron")).overworld().connect(Pb,t),Bd=(new Location$$module$js$nodes(a,124,x,"After first tunnel")).overworld(),Cd=(new Location$$module$js$nodes(a,124,x,"Door to Styx")).overworld().connect(Bd,t),Tg=(new Location$$module$js$nodes(a,124,x,"Dead end (no item)")).overworld(),Ug=(new Location$$module$js$nodes(a,124,x,"Dead end (fruit of lime)")).overworld().chest(rb,26),Vg=(new Location$$module$js$nodes(a,124,x,"Dead end (magic ring)")).overworld().chest(V,
...
t);(new Location$$module$js$nodes(a,145,F,"Area 11 (center)")).cave().connect(mh,t).connectTo(Hd);(new Location$$module$js$nodes(a,145,F,"Area 12 (top center islands)")).cave().connect(nb,z).chest(de,28);const nh=(new Location$$module$js$nodes(a,146,Z,"Desert Cave 1")).cave().connect(Qb,z),Ha=(new Location$$module$js$nodes(a,147,Z,"Town")).town(),oh=(new Location$$module$js$nodes(a,148,Z,"Outside Cave")).overworld().connect(Ha),ph=(new Location$$module$js$nodes(a,149,Z,"Desert Cave 2")).cave().connect(oh);
(new Location$$module$js$nodes(a,150,Z,"Meadow")).overworld().connect(nh).connectTo(Ha).trigger(Be,pa,Ze);const Rb=(new Location$$module$js$nodes(a,152,Z,"Desert 2")).overworld().connect(ph),qh=(new Location$$module$js$nodes(a,156,Ca,"Entrance")).fortress().connect(Rb,z),rh=(new Location$$module$js$nodes(a,156,Ca,"Azteca")).fortress().trigger(De),sh=(new Location$$module$js$nodes(a,157,Ca,"Fork")).fortress().connect(qh),Jd=(new Location$$module$js$nodes(a,158,Ca,"Main")).fortress().connect(sh);(new Location$$module$js$nodes(a,
158,Ca,"Treasure Chest (magic ring)")).fortress().connect(Jd).chest(V,27,108);(new Location$$module$js$nodes(a,159,Ca,"Draygon")).fortress().connect(Jd).to(rh).boss(jf);const th=(new Location$$module$js$nodes(a,160,ca,"Entrance")).fortress().connect(Rb,z),uh=(new Location$$module$js$nodes(a,160,ca,"Statues")).fortress().connect(th).boss(kf),vh=(new Location$$module$js$nodes(a,161,ca,"Hall 1")).fortress().connect(uh),Sb=(new Location$$module$js$nodes(a,162,ca,"Branch")).fortress().connect(vh);(new Location$$module$js$nodes(a,
...
Fb).trigger(Nc,J),yh=(new Location$$module$js$nodes(a,169,eb,"Main")).fortress().from(Kd,xa),zh=(new Location$$module$js$nodes(a,169,eb,"Boss")).fortress().connect(yh).boss(ef),Ah=(new Location$$module$js$nodes(a,170,eb,"Zebu")).fortress().connect(zh),ob=(new Location$$module$js$nodes(a,171,Y,"Entrance")).fortress().connect(Ah);(new Location$$module$js$nodes(a,171,Y,"Dead End Behind Iron (fruit of power)")).fortress().connect(ob,xa).chest(M,28,98);(new Location$$module$js$nodes(a,171,Y,"Dead End Loop Across Closer Bridges")).fortress().connect(ob,
t).connect(ob,t);const Bh=(new Location$$module$js$nodes(a,171,Y,"Across First Bridge (fruit of repun)")).fortress().connect(ob,t).chest(sb,30,102),Tb=(new Location$$module$js$nodes(a,171,Y,"Across Second Bridge")).fortress().connect(Bh,t);(new Location$$module$js$nodes(a,171,Y,"Dead End Across Two Bridges ()")).fortress().connect(Tb,t).connect(Tb,t).chest(wa,29,93);const Ch=(new Location$$module$js$nodes(a,171,Y,"Across Third Bridge")).fortress().connect(Tb,t),Dh=(new Location$$module$js$nodes(a,
171,Y,"Exit Behind Iron Door")).fortress().connect(Ch,xa),Eh=(new Location$$module$js$nodes(a,172,Y,"Boss")).fortress().connect(Dh).boss(ff),Fh=(new Location$$module$js$nodes(a,172,Y,"Tornel")).fortress().connect(Eh),Ld=(new Location$$module$js$nodes(a,173,Ba,"Lower")).fortress().connect(Fh).chest(tb,26,99).chest(V,27,111),Gh=(new Location$$module$js$nodes(a,174,Ba,"Upper Loop")).fortress().connect(Ld).chest(K,22,96);(new Location$$module$js$nodes(a,174,Ba,"Upper Loop Behind Wall (magic ring)")).fortress().connect(Gh,
xa).chest(V,23,107);const Hh=(new Location$$module$js$nodes(a,175,Ba,"Upper Passage (toward Mado)")).fortress().connect(Ld).chest(V,27,84),Ub=(new Location$$module$js$nodes(a,176,O,"Initial Fork")).fortress(),Ih=(new Location$$module$js$nodes(a,177,O,"Left Branch")).fortress().connect(Ub),Md=(new Location$$module$js$nodes(a,178,O,"Main Area (right branch, over bridges)")).fortress().connect(Ub),Jh=(new Location$$module$js$nodes(a,179,O,"U-shaped Passage (between floors)")).fortress().connect(Md),
Kh=(new Location$$module$js$nodes(a,180,O,"Main Area Lower (under bridge)")).fortress().connect(Ih).connect(Md).connect(Jh),Nd=(new Location$$module$js$nodes(a,180,O,"Behind Iron Wall")).fortress().connect(Kh,xa),Lh=(new Location$$module$js$nodes(a,181,O,"Lower")).fortress().connect(Nd).chest(N,13,121).chest(N,14,122).chest(N,15,123).chest(V,23,88).chest(R,24,110),Mh=(new Location$$module$js$nodes(a,182,O,"Boss Corridor")).fortress().connect(Lh),Nh=(new Location$$module$js$nodes(a,182,O,"Boss")).fortress().connect(Mh,
...
192,W,"Elder House")).house().connect(na).trigger(uc);(new Location$$module$js$nodes(a,193,W,"Rabbit Hut")).house().connect(na).trigger(Ac,zc,fa);(new Location$$module$js$nodes(a,194,W,"Inn")).shop().connect(na);(new Location$$module$js$nodes(a,195,W,"Tool Shop")).shop(D,K,R,M).connect(na);(new Location$$module$js$nodes(a,196,W,"Armor Shop")).shop($d,Yb).connect(na);(new Location$$module$js$nodes(a,197,W,"Student House")).house().connect(na).trigger(vc);(new Location$$module$js$nodes(a,198,za,"Tavern")).house().connect(Na);
...
(new Location$$module$js$nodes(a,233,H,"Zombie Town Basement")).house().connect(Rh).trigger(ue,Yc);(new Location$$module$js$nodes(a,235,X,"Tool Shop")).shop(D,K,M,R).connect(ea);(new Location$$module$js$nodes(a,236,X,"Stom's Hut")).house().connect(ea).trigger(Kc);(new Location$$module$js$nodes(a,237,X,"Inn")).shop().connect(ea);(new Location$$module$js$nodes(a,238,X,"Armor Shop")).shop(ce,cc,Yd,ac).connect(ea);(new Location$$module$js$nodes(a,239,X,"Tavern")).house().connect(ea).trigger(Lc,Kc,Ja);
(new Location$$module$js$nodes(a,240,X,"Pawn Shop")).shop().connect(ea);(new Location$$module$js$nodes(a,241,X,"Dance Hall")).house().connect(ea).trigger(Mc,Lc,Ja).trigger(xe,Mc,nc);const Sh=(new Location$$module$js$nodes(a,242,ma,"Temple (pre-massacre)")).fortress().connect(Ga).from(Gb,J,Td).trigger(ze);(new Location$$module$js$nodes(a,242,ma,"Temple (post-massacre)")).fortress().from(Sh,Nc).boss(df);(new Location$$module$js$nodes(a,243,ma,"Training Hall")).house().connect(Ga);(new Location$$module$js$nodes(a,
244,ma,"Hospital")).house().connect(Ga);(new Location$$module$js$nodes(a,245,ma,"Armor Shop")).shop(cc,qb,ac).connect(Ga);(new Location$$module$js$nodes(a,246,ma,"Tool Shop")).shop(D,K,rb,V).connect(Ga);(new Location$$module$js$nodes(a,247,ma,"Inn")).shop().connect(Ga);(new Location$$module$js$nodes(a,248,Z,"Inn")).shop().connect(Ha);(new Location$$module$js$nodes(a,249,Z,"Tool Shop")).shop(K,R,sb,V).connect(Ha);(new Location$$module$js$nodes(a,250,Z,"Elder's House")).house().connect(Ha);(new Location$$module$js$nodes(a,
251,Z,"Pawn Shop")).house().connect(Ha);const Th=[na,Ma,$c,Da,Hb,Ea,Kb,ld,Mb,ea,mb,Pb,Qb,Kd,Rb];for(const a of Th)a.from(Gb,He);return a},shuffle$$module$js$depgraph=(b,a,c,d,e)=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){const f=generate$$module$js$depgraph(d);d.shuffleShops()&&f.shuffleShops(a);const g=f.nodes.filter((a)=>a instanceof Slot$$module$js$nodes&&a.slots&&a.slotName);d||(d=new FlagSet$$module$js$flagset("Sbkm Sct"));var k={};for(var h of g)if(h instanceof Slot$$module$js$nodes&&
...
b;if(b.isMimic()&&!a.canHoldMimic()||"unique"!=b.item.inventoryRow&&a.requiresUnique||"armor"==b.item.inventoryRow&&!a.isChest())return!1;if(f.guaranteeSword()){if(0===a.slotIndex)return 4>b.item.id;if(4>b.item.id&&(r.add(b),4===r.size))return 0===a.slotIndex}return p[a.slotType?a.slotType[0]:"c"]==p[b.slotType?b.slotType[0]:"c"]};m=a.assumedFill(d,(a,b)=>I(a,q.get(b)));if(null==m){g&&g.endState.key++;return!1;throw Error("Could not fill!");}const C=new Map;for(var U=0;U<m.length;U++){var va=m[U];
if(null!=va){var oa=q.get(a.item(va));va=n.get(oa);n.delete(oa);oa=a.location(U);C.set(oa,va);l.delete(oa)}}U=(a,b,c)=>{c=void 0===c?"":c;for(const c of l)if(I(c,a))return C.set(c,b),l.delete(c),n.delete(a),!0;g&&("traps"==c?g.endState.traps++:"chest"==c?g.endState.chest++:g.endState.misc++);return!1};for(var E of n){const [a,b]=E;if(a.isMimic()&&!U(a,b,"traps"))return!1}for(const a of n){const [b,c]=a;if(b.needsChest()&&!U(b,c,"chest"))return!1}for(const a of n){const [b,c]=a;if(!U(b,c))return!1}for(const a of C){const [b,
...
b))}}sets(){const b=new Map;for(const a of this.data.keys()){const c=this.find(a);b.has(c)||b.set(c,new Set);b.get(c).add(a)}return[...b.values()]}}var module$js$unionfind={};module$js$unionfind.UnionFind=UnionFind$$module$js$unionfind;var __awaiter$$module$js$rom$writer=this&&this.__awaiter||function(b,a,c,d){return new (c||(c=Promise))(function(e,f){function g(a){try{h(d.next(a))}catch(l){f(l)}}function k(a){try{h(d["throw"](a))}catch(l){f(l)}}function h(a){a.done?e(a.value):(new c(function(b){b(a.value)})).then(g,k)}h((d=d.apply(b,a||[])).next())})};function page$$module$js$rom$writer(b){return b>>>13}
class Chunk$$module$js$rom$writer{constructor(b,a){this.start=b;this.end=a;this.page=page$$module$js$rom$writer(b);if(page$$module$js$rom$writer(a)!==this.page)throw Error("Chunk spans pages");this.pos=b}free(){return this.end-this.pos}find(b,a){for(let c=this.start;c<=this.pos-b.length;c++){let d=!0;for(let e=0;e<b.length;e++)if(a[c+e]!==b[e]){d=!1;break}if(d)return c}return-1}}
class Writer$$module$js$rom$writer{constructor(b){this.rom=b;this.chunks=[];this.writes=[];this.promises=[]}alloc(b,a){for(;page$$module$js$rom$writer(a)>page$$module$js$rom$writer(b);){const a=page$$module$js$rom$writer(b)+1<<13;this.chunks.push(new Chunk$$module$js$rom$writer(b,a-1));b=a}this.chunks.push(new Chunk$$module$js$rom$writer(b,a))}write(b,a,c){const d=page$$module$js$rom$writer(a),e=page$$module$js$rom$writer(c);a=new Promise((a,c)=>{this.writes.push({data:b,resolve:a,startPage:d,endPage:e})});
this.promises.push(a);return a}commit(){return __awaiter$$module$js$rom$writer(this,void 0,void 0,function*(){for(;this.writes.length;){var b=this.writes.sort((a,b)=>a.endPage-a.startPage-(b.endPage-b.startPage)||b.data.length-a.data.length);const a=this.promises;this.writes=[];this.promises=[];for(const a of b)b=this.find(a),0<=b?a.resolve(b):this.writeOne(a);yield Promise.all(a)}})}find({data:b,startPage:a,endPage:c}){for(const d of this.chunks){if(d.page<a||d.page>c)continue;const e=d.find(b,this.rom);
if(0<=e)return e}return-1}writeOne(b){for(const a of this.chunks)if(!(a.page<b.startPage||a.page>b.endPage||a.free()<b.data.length)){this.rom.subarray(a.pos,a.pos+b.data.length).set(b.data);b.resolve(a.pos);a.pos+=b.data.length;return}throw Error("Could not find sufficient chunk to write");}}var module$js$rom$writer={};module$js$rom$writer.Writer=Writer$$module$js$rom$writer;const seq$$module$js$rom=(b,a)=>{a=void 0===a?(a)=>a:a;return Array(b).fill(0).map((b,d)=>a(d))},slice$$module$js$rom=(b,a,c)=>b.slice(a,a+c),signed$$module$js$rom=(b)=>128>b?b:b-256,varSlice$$module$js$rom=(b,a,c,d,e)=>{e=void 0===e?Infinity:e;const f=[];for(;a+c<=e&&b[a]!=d;)f.push(b.slice(a,a+c)),a+=c;return f},addr$$module$js$rom=(b,a,c)=>(b[a]|b[a+1]<<8)+(void 0===c?0:c),group$$module$js$rom=(b,a)=>seq$$module$js$rom(Math.max(0,Math.floor(a.length/b)),(c)=>slice$$module$js$rom(a,c*b,b)),reverseBits$$module$js$rom=
...
114688,122879);b.rom[c+2*d.id]=a&255;b.rom[c+2*d.id+1]=(a>>>8)-64})}}class AdHocSpawn$$module$js$rom extends Entity$$module$js$rom{constructor(b,a){super(b,a);this.base=(a<<2)+171008;this.lowerSlot=b.prg[this.base];this.upperSlot=b.prg[this.base+1];this.object=b.prg[this.base+2];this.count=b.prg[this.base+3]}}
...
a));this.palettes=seq$$module$js$rom(256,(a)=>new Palette$$module$js$rom(this,a));this.locations=seq$$module$js$rom(256,(a)=>LOCATION_NAMES$$module$js$rom[a]?new Location$$module$js$rom(this,a):null);this.tileAnimations=seq$$module$js$rom(4,(a)=>new TileAnimation$$module$js$rom(this,a));this.hitboxes=seq$$module$js$rom(24,(a)=>new Hitbox$$module$js$rom(this,a));this.objects=seq$$module$js$rom(256,(a)=>new ObjectData$$module$js$rom(this,a));this.adHocSpawns=seq$$module$js$rom(96,(a)=>new AdHocSpawn$$module$js$rom(this,
a));this.metasprites=seq$$module$js$rom(256,(a)=>new Metasprite$$module$js$rom(this,a));this.messages=new Messages$$module$js$rom(this);this.itemGets=seq$$module$js$rom(113,(a)=>new ItemGet$$module$js$rom(this,a));this.npcs=seq$$module$js$rom(205,(a)=>new Npc$$module$js$rom(this,a))}get monsters(){let b=new Set;for(const a of this.locations)if(a&&a.objects)for(const c of a.objects)0==(c[2]&7)&&b.add(this.objects[c[3]+80&255]);b=[...b];b.sort((a,b)=>a.id-b.id);return b}get projectiles(){let b=new Set;
for(const a of this.monsters)a.child&&b.add(this.objects[this.adHocSpawns[a.child].object]);b=[...b];b.sort((a,b)=>a.id-b.id);return b}get monsterGraphics(){const b={};for(const a of this.locations)if(a&&a.objects)for(const c of a.objects)if(!(c[2]&7)){const d=c[2]&128?1:0,e=(c[3]+80).toString(16).padStart(2,0);(b[e]=b[e]||{})[`${d}:${a.spritePatterns[d].toString(16)}:${a.spritePalettes[d].toString(16)}`]={slot:d,pat:a.spritePatterns[d],pal:a.spritePalettes[d]}}return b}get locationMonsters(){const b=
{};for(const a of this.locations){if(!a||!a.objects)continue;const c=b["$"+a.id.toString(16).padStart(2,0)]={};for(const b of a.objects)if(!(b[2]&7)){const a=b[2]&128?1:0,d=b[3]+80;c[`${a}:${d.toString(16)}`]=(c[`${a}:${d.toString(16)}`]||0)+1}}return b}static load(b){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){const a=yield pickFile$$module$js$rom();b&&(yield b(a));return new Rom$$module$js$rom(a)})}writeData(){const b=this;return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){const a=
new Writer$$module$js$rom$writer(b.prg);a.alloc(83192,97792);a.alloc(103417,113408);a.alloc(123392,123888);a.alloc(116602,117085);b.prg[246387]=b.prg[246392]=190;b.prg[246399]=b.prg[246404]=191;const c=[];for(const d of b.locations)d&&(c.push(d.writeMapData(a)),c.push(d.writeNpcData(a)));for(const c of b.objects)c.write(a,114176);for(const d of b.triggers)d.used&&c.push(d.write(a));for(const d of b.npcs)d.used&&c.push(d.write(a));for(const a of b.tilesets)a.write(b.prg);for(const a of b.tileEffects)a.write(b.prg);
for(const a of b.screens)a.write(b.prg);c.push(a.commit());return Promise.all(c).then(()=>{})})}analyzeTiles(){}disjointTilesets(){var b=[];for(var a of this.locations)if(a){var c=a.tileset,d=a.extended?256:0;for(const e of a.screens)for(const a of e)(b[a+d]||(b[a+d]=new Set)).add(c)}a=Array(256).fill(0).map(()=>new UnionFind$$module$js$unionfind);for(c=0;c<b.length;c++)if(b[c]){d=new Set;for(var e of this.screens[c].tiles)for(const a of e)d.add(a);for(const e of d)a[e].union([...b[c]])}for(b=0;b<
a.length;b++)e=a[b].sets().map((a)=>[...a].map((a)=>a.toString(16)).join(" ")).join(" | "),console.log(`Tile ${b.toString(16).padStart(2,0)}: ${e}`)}swapMetatiles(b,...a){var c=new Map,d=seq$$module$js$rom(256);const e=new Map,f=(a)=>Array.isArray(a)?a[0]:0>a?~a:a;for(var g of a){for(var k=0;k<g.length-1;k++)Array.isArray(g[k])&&(e.set(g[k][0],g[k][1]),g[k]=g[k][0]);for(k=0;k<g.length-1;k++){let a=g[k],b=g[k+1];0>a||0>b||(c.set(b,a),d[b]=a)}}g=new Set;c=new Set;b=new Set(b);for(var h of this.locations)if(h&&
b.has(h.tileset)){c.add(h.tileEffects);for(const a of h.allScreens())g.add(a)}for(var n of g)for(var l of n.tiles)for(h=0;h<l.length;h++)l[h]=d[l[h]];for(var q of b){b=this.tilesets[(q&127)>>>2];for(var p of a)for(d=0;d<p.length-1;d++){n=f(p[d]);l=f(p[d+1]);for(h=0;4>h;h++)b.tiles[h][n]=b.tiles[h][l];b.attrs[n]=b.attrs[l];if(32>l&&b.alternates[l]!=l){if(32<=n)throw Error(`Cannot unflag: ${q} ${n} ${l} ${b.alternates[l]}`);b.alternates[n]=b.alternates[l]}}for(var m of e){const [a,c]=m;b.alternates[a]=
c}}for(const d of c){q=this.tileEffects[d-179];for(const c of a)for(p=0;p<c.length-1;p++)m=f(c[p]),b=f(c[p+1]),q.effects[m]=q.effects[b];for(const a of e.keys())q.effects[a]|=8}}}
...
return Math.min(AM$$module$js$random*this.iy,RNMX$$module$js$random)}nextInt(b){return Math.floor(this.next()*b)}shuffle(b){for(let a=b.length;a;){const c=this.nextInt(a--);[b[a],b[c]]=[b[c],b[a]]}return b}}var module$js$random={};module$js$random.Random=Random$$module$js$random;const STATUS$$module$js$version="unstable",VERSION$$module$js$version="defrag",LABEL$$module$js$version="defrag aceb562",HASH$$module$js$version="aceb562bce3c254f55817871e266c98cfcc09a42",DATE$$module$js$version=new Date(155566723E4);var module$js$version={};module$js$version.STATUS=STATUS$$module$js$version;module$js$version.VERSION=VERSION$$module$js$version;module$js$version.LABEL=LABEL$$module$js$version;module$js$version.HASH=HASH$$module$js$version;module$js$version.DATE=DATE$$module$js$version;var $jscompDefaultExport$$module$js$patch={apply(b,a,c){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){let d;a.seed||(a.seed=parseSeed$$module$js$patch("").toString(16),window.location.hash+="&seed="+a.seed);d=a.flags?new FlagSet$$module$js$flagset(a.flags):new FlagSet$$module$js$flagset("Em Gt Mr Rlpt Sbk Sct Sm Tasd");for(const b in a)"false"===a[b]&&(a[b]=!1);yield shuffle$$module$js$patch(b,parseSeed$$module$js$patch(a.seed),d,new FetchReader$$module$js$fetchreader(c))})}};
...
h,c.bargainHunting()?g:null);l=new Rom$$module$js$rom(b);rescaleMonsters$$module$js$patch(b,l);c.shuffleMonsters()&&shuffleMonsters$$module$js$patch(b,l,g);identifyKeyItemsForDifficultyBuffs$$module$js$patch(l);c.doubleBuffMedicalHerb?(b[115996]*=2,b[115962]*=3):c.singleBuffMedicalHerb()&&(b[115996]+=16,b[115962]*=2);c.connectLimeTreeToLeaf()&&connectLimeTreeToLeaf$$module$js$patch(l);addCordelWestTriggers$$module$js$patch(l,c);c.disableRabbitSkip()&&fixRabbitSkip$$module$js$patch(l);c.storyMode()&&
storyMode$$module$js$patch(l);c.chargeShotsOnly()&&disableStabs$$module$js$patch(l);c.orbsOptional()&&orbsOptional$$module$js$patch(l);closeCaveEntrances$$module$js$patch(l,c);misc$$module$js$patch(l,c);yield n("postshuffle.s");updateDifficultyScalingTables$$module$js$patch(b,c,h);updateCoinDrops$$module$js$patch(b,c);shuffleRandomNumbers$$module$js$patch(b,g);yield l.writeData();return stampVersionSeedAndHash$$module$js$patch(b,a,c)}),misc$$module$js$patch=(b,a)=>{},closeCaveEntrances$$module$js$patch=
(b,a)=>{b.swapMetatiles([144],[7,[1,0],-194],[14,[2,0],-194],[32,[3,10],-216],[33,[4,10],-216]);b.swapMetatiles([148,156],[104,[1,0],-194],[131,[2,0],-194],[136,[3,10],-216],[137,[4,10],-216]);b.screens[10].tiles[3][8]=1;b.screens[10].tiles[3][9]=2;b.screens[10].tiles[4][8]=3;b.screens[10].tiles[4][9]=4;b.screens[21].tiles[7][9]=1;b.screens[21].tiles[7][10]=2;b.screens[21].tiles[8][9]=3;b.screens[21].tiles[8][10]=4;b.screens[25].tiles[4][8]=1;b.screens[25].tiles[4][9]=2;b.screens[25].tiles[5][8]=
3;b.screens[25].tiles[5][9]=4;b.screens[62].tiles[5][6]=1;b.screens[62].tiles[5][7]=2;b.screens[62].tiles[6][6]=3;b.screens[62].tiles[6][7]=4;var c=[[3,48],[20,48],[21,48],[64,0],[64,20],[65,116],[71,16],[148,0],[152,65]];for(const a of c){const [c,d]=a;b.locations[c].flags.push([239,d])}c=(a,c,f)=>{for(const d of b.locations[a].flags)if(d[1]==c){d[0]=f;return}throw Error(`Could not find flag to replace at ${a}:${c}`);};a.paralysisRequiresPrisonKey()&&(c(20,48,238),c(21,48,238),c(64,0,216),b.locations[64].objects.splice(1,
0,[6,6,4,44]),b.locations[64].objects.push([7,7,2,173]))},eastCave$$module$js$patch=(b)=>{},addCordelWestTriggers$$module$js$patch=(b,a)=>{for(const c of b.locations[21].objects)2===c[2]&&(128>c[3]||a.disableTeleportSkip())&&b.locations[20].objects.push([...c])},fixRabbitSkip$$module$js$patch=(b)=>{for(const a of b.locations[40].objects)2===a[2]&&134===a[3]&&116===a[1]&&(a[0]++,a[1]++)},storyMode$$module$js$patch=(b)=>{const a=b.npcs[203].spawnConditions[166];a.push(~b.npcs[194].spawnConditions[40][0]);
a.push(~b.npcs[132].spawnConditions[110][0]);a.push(~b.triggers[26].conditions[1]);a.push(~b.npcs[197].spawnConditions[169][0]);a.push(~b.npcs[198].spawnConditions[172][0]);a.push(~b.npcs[199].spawnConditions[185][0]);a.push(~b.npcs[200].spawnConditions[182][0]);a.push(~b.npcs[203].spawnConditions[159][0]);a.push(512);a.push(513);a.push(514);a.push(515)},disableStabs$$module$js$patch=(b)=>{for(const a of[8,9,39])b.objects[a].collisionPlane=0},orbsOptional$$module$js$patch=(b)=>{for(const a of[16,
20,24,29])b.objects[a].terrainSusceptibility&=-5,b.objects[a].level=2},connectLimeTreeToLeaf$$module$js$patch=(b)=>{const a=b.locations[3];b=b.locations[66];a.screens[5][4]=16;b.screens[1][0]=26;b.screens[2][0]=12;const c=a.entrances.push([239,4,120,5])-1,d=b.entrances.push([16,0,192,1])-1;a.exits.push([79,86,66,d],[79,87,66,d]);b.exits.push([0,27,3,c],[0,28,3,c])},stampVersionSeedAndHash$$module$js$patch=(b,a,c)=>{const d=crc32$$module$js$crc32(b),e=d.toString(16).padStart(8,0).toUpperCase(),f="unstable"==
STATUS$$module$js$version?HASH$$module$js$version.substring(0,7).padStart(7,0).toUpperCase()+"     ":VERSION$$module$js$version.substring(0,12).padEnd(12," ");a=a.toString(16).padStart(8,0).toUpperCase();const g=(a,c)=>{for(let d=0;d<c.length;d++)b[a+16+d]=c.charCodeAt(d)},k=(a,b)=>{const c=[];for(let d=0;d<a.length||d<b.length;d++)c.push(a[d]||" "),c.push(b[d]||" ");return c.join("")};g(161743,k("  VERSION     SEED      ",`  ${f}${a}`));a=String(c);let h;if(46<a.length){if(92<a.length)throw Error("Flag string way too long!");
...
"p","demon wall fire",,,,23,,,]].map((b)=>{var [a,c,d,e=0,f=0,g=0,k=0,h=0,n=0]=b;return[a,{id:a,type:c,name:d,sdef:e,swrd:f,hits:g,satk:k,dgld:h,sexp:n}]}));
