...
f;document.getElementById("checksum").textContent=read(a,161925,4)+read(a,161926,4);a=`flags=${f.replace(/ /g,"")}&seed=${e.toLowerCase()}`;document.getElementById("query").textContent=a;c=`https://crystalisrandomizer.com/${/\./.test(c)?c:`sha/${c.toLowerCase()}`}/#${a}`;a=document.createElement("a");a.href=c;a.textContent=c;document.getElementById("permalink").appendChild(a)});d.readAsArrayBuffer(b)});
