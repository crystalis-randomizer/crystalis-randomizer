...
class Context$$module$6502{constructor(a){this.labels=a;this.pc=0;this.cpuToPrg=[];this.prgToCpu=[]}updateBank(a,b,c){for(var d=0;d<c;d++){var e=b+d,f=this.cpuToPrg[e];null!=f&&(this.prgToCpu[f]=null,this.cpuToPrg[e]=null)}for(d=0;d<c;d++)e=b+d,f=a+d,this.prgToCpu[f]=e,this.cpuToPrg[e]=f}mapLabel(a,b){var c=/([^-+]+)([-+])(.*)/.exec(a);if(c)return a=this.map(parseNumber$$module$6502(c[1],!0),b),b=this.map(parseNumber$$module$6502(c[3],!0),b),"-"===c[2]?a-b:a+b;if(c=/([<>])(.*)/.exec(a))return b=this.map(parseNumber$$module$6502(c[2],
!0),b),"<"===c[1]?b&255:b>>>8&255;c=this.labels[a];if(!c)throw Error(`Label not found: ${a}`);if(null==b){if(1<c.length)throw Error(`Ambiguous label: ${a}`);return c[0]}b=~(b+2);const d=find$$module$6502(c,b);if(0<=d)return c[d];if(-1===d)return c[0];if(d===~c.length)return c[c.length-1];c=c.slice(~d-1,~d+1);return a.startsWith("-")?c[1]:a.startsWith("+")?c[0]:b<(c[0]+c[1])/2?c[0]:c[1]}mapPrgToCpu(a){const b=this.prgToCpu[a];if(null==b)throw Error(`PRG address unmapped: $${a.toString(16)}`);return b}map(a,
b){if(null==a)return a;"string"===typeof a&&(a=this.mapLabel(a,b));0>a&&(a=this.mapPrgToCpu(~a));return a}}class Chunk$$module$6502 extends Uint8Array{constructor(a,b){super(b.length);this.start=a;this.set(b)}apply(a){a.subarray(this.start,this.start+this.length).set(this)}shift(a){return new Chunk$$module$6502(this.start+a,this)}}
...
a))}}sets(){const a=new Map;for(const b of this.data.keys()){const c=this.find(b);a.has(c)||a.set(c,new Set);a.get(c).add(b)}return[...a.values()]}}var module$unionfind={};module$unionfind.UnionFind=UnionFind$$module$unionfind;class Rom$$module$rom{constructor(a){this.omitLocalDialogSuffix=this.omitItemGetDataSuffix=!1;this.prg=a.subarray(16,262160);this.chr=a.subarray(262160);for(const a of ADJUSTMENTS$$module$rom){const [b,d]=a;this.prg[b]=d}this.screens=seq$$module$rom$util(259,(a)=>new Screen$$module$rom$screen(this,a));this.tilesets=seq$$module$rom$util(12,(a)=>new Tileset$$module$rom$tileset(this,a<<2|128));this.tileEffects=seq$$module$rom$util(11,(a)=>new TileEffects$$module$rom$tileeffects(this,a+179));this.triggers=
seq$$module$rom$util(67,(a)=>new Trigger$$module$rom$trigger(this,128|a));this.patterns=seq$$module$rom$util(this.chr.length>>4,(a)=>new Pattern$$module$rom$pattern(this,a));this.palettes=seq$$module$rom$util(256,(a)=>new Palette$$module$rom$palette(this,a));this.locations=seq$$module$rom$util(256,(a)=>new Location$$module$rom$location(this,a));this.tileAnimations=seq$$module$rom$util(4,(a)=>new TileAnimation$$module$rom$tileanimation(this,a));this.hitboxes=seq$$module$rom$util(24,(a)=>new Hitbox$$module$rom$hitbox(this,
a));this.objects=seq$$module$rom$util(256,(a)=>new ObjectData$$module$rom$objectdata(this,a));this.adHocSpawns=seq$$module$rom$util(96,(a)=>new AdHocSpawn$$module$rom$adhocspawn(this,a));this.metasprites=seq$$module$rom$util(256,(a)=>new Metasprite$$module$rom$metasprite(this,a));this.messages=new Messages$$module$rom$messages(this);this.itemGets=seq$$module$rom$util(113,(a)=>new ItemGet$$module$rom$itemget(this,a));this.npcs=seq$$module$rom$util(205,(a)=>new Npc$$module$rom$npc(this,a))}get monsters(){const a=
new Set;for(const b of this.locations)if(b.used&&b.hasSpawns)for(const c of b.spawns)c.isMonster()&&a.add(this.objects[c.monsterId]);return[...a].sort((a,c)=>a.id-c.id)}get projectiles(){const a=new Set;for(const b of this.monsters)b.child&&a.add(this.objects[this.adHocSpawns[b.child].objectId]);return[...a].sort((a,c)=>a.id-c.id)}get monsterGraphics(){const a={};for(const b of this.locations)if(b.used&&b.hasSpawns)for(const c of b.spawns)if(!(c.data[2]&7)){const d=c.data[2]&128?1:0,e=hex$$module$rom$util(c.data[3]+
80);(a[e]=a[e]||{})[`${d}:${b.spritePatterns[d].toString(16)}:${b.spritePalettes[d].toString(16)}`]={pal:b.spritePalettes[d],pat:b.spritePatterns[d],slot:d}}return a}get locationMonsters(){const a={};for(const b of this.locations){if(!b.used||!b.hasSpawns)continue;const c=a["$"+hex$$module$rom$util(b.id)]={};for(const a of b.spawns)if(!(a.data[2]&7)){const b=a.data[2]&128?1:0,d=a.data[3]+80;c[`${b}:${d.toString(16)}`]=(c[`${b}:${d.toString(16)}`]||0)+1}}return a}static load(a){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){const b=
yield pickFile$$module$rom();a&&(yield a(b));return new Rom$$module$rom(b)})}writeData(){const a=this;return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){const b=new Writer$$module$rom$writer(a.prg);b.alloc(83192,97792);b.alloc(103417,109568);b.alloc(110080,113920);b.alloc(116602,117085);b.alloc(117477,121076);b.alloc(122342,122981);b.alloc(123392,123888);const c=[],d=(a)=>{for(const d of a)c.push(d.write(b))};d(a.locations);d(a.objects);d(a.hitboxes);d(a.triggers);d(a.npcs);d(a.tilesets);
d(a.tileEffects);d(a.screens);d(a.adHocSpawns);d(a.itemGets);c.push(b.commit());yield Promise.all(c).then(()=>{})})}analyzeTiles(){}disjointTilesets(){var a=[];for(var b of this.locations)if(b.used){var c=b.tileset,d=b.extended?256:0;for(const e of b.screens)for(const b of e)(a[b+d]||(a[b+d]=new Set)).add(c)}b=seq$$module$rom$util(256,()=>new UnionFind$$module$unionfind);for(c=0;c<a.length;c++)if(a[c]){d=new Set;for(var e of this.screens[c].tiles)for(const a of e)d.add(a);for(const e of d)b[e].union([...a[c]])}for(a=
0;a<b.length;a++)e=b[a].sets().map((a)=>[...a].map(hex$$module$rom$util).join(" ")).join(" | "),console.log(`Tile ${hex$$module$rom$util(a)}: ${e}`)}swapMetatiles(a,...b){var c=new Map,d=seq$$module$rom$util(256);const e=new Map,f=(a)=>Array.isArray(a)?a[0]:0>a?~a:a;for(var g of b){for(var h=0;h<g.length-1;h++)if(Array.isArray(g[h])){var k=g[h];e.set(k[0],k[1]);g[h]=k[0]}for(h=0;h<g.length-1;h++){k=g[h];const a=g[h+1];0>k||0>a||(c.set(a,k),d[a]=k)}}g=new Set;c=new Set;a=new Set(a);for(var l of this.locations)if(l.used&&
a.has(l.tileset)){c.add(l.tileEffects);for(var m of l.allScreens())g.add(m)}for(var r of g)for(var n of r.tiles)for(l=0;l<n.length;l++)n[l]=d[n[l]];for(var p of a){d=this.tilesets[(p&127)>>>2];for(var q of b)for(r=0;r<q.length-1;r++){n=f(q[r]);l=f(q[r+1]);for(m=0;4>m;m++)d.tiles[m][n]=d.tiles[m][l];d.attrs[n]=d.attrs[l];if(32>l&&d.alternates[l]!==l){if(32<=n)throw Error(`Cannot unflag: ${p} ${n} ${l} ${d.alternates[l]}`);d.alternates[n]=d.alternates[l]}}for(var y of e){const [a,b]=y;d.alternates[a]=
b}}for(const a of c){p=this.tileEffects[a-179];for(const a of b)for(q=0;q<a.length-1;q++)y=f(a[q]),d=f(a[q+1]),p.effects[y]=p.effects[d];for(const a of e.keys())p.effects[a]|=8}}}
...
return Math.min(AM$$module$random*this.iy,RNMX$$module$random)}nextInt(a){return Math.floor(this.next()*a)}shuffle(a){for(let b=a.length;b;){const c=this.nextInt(b--);[a[b],a[c]]=[a[c],a[b]]}return a}}var module$random={};module$random.Random=Random$$module$random;const STATUS$$module$version="unstable",VERSION$$module$version="experimental",LABEL$$module$version="experimental b795da2",HASH$$module$version="b795da2a418e1a8b073edd4cbfdc176619ecb3c6",DATE$$module$version=new Date(1557293111E3);var module$version={};module$version.STATUS=STATUS$$module$version;module$version.VERSION=VERSION$$module$version;module$version.LABEL=LABEL$$module$version;module$version.HASH=HASH$$module$version;module$version.DATE=DATE$$module$version;var $jscompDefaultExport$$module$patch={apply(a,b,c){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){let d;b.seed||(b.seed=parseSeed$$module$patch("").toString(16),window.location.hash+="&seed="+b.seed);d=b.flags?new FlagSet$$module$flagset(b.flags):new FlagSet$$module$flagset("Em Gt Mr Rlpt Sbk Sct Sm Tasd");for(const a in b)"false"===b[a]&&(b[a]=!1);yield shuffle$$module$patch(a,parseSeed$$module$patch(b.seed),d,new FetchReader$$module$fetchreader(c))})}};
