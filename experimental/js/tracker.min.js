...
var m=Object.keys(k).filter((a)=>k[a]).map((a)=>`define ${a} 1\n`).join("");l.assemble(m,"flags.s");yield g("preshuffle.s");h=new Random$$module$js$random(h);m=new Rom$$module$js$rom(a);fixCoinSprites$$module$js$patch(m);"object"==typeof window&&(window.rom=m);m.spoiler=new Spoiler$$module$js$rom$spoiler(m);e&&(e.spoiler=m.spoiler);fixMimics$$module$js$patch(m);makeBraceletsProgressive$$module$js$patch(m);c.blackoutMode()&&blackoutMode$$module$js$patch(m);closeCaveEntrances$$module$js$patch(m,c);
reversibleSwanGate$$module$js$patch(m);adjustGoaFortressTriggers$$module$js$patch(m);preventNpcDespawns$$module$js$patch(m,c);c.requireHealedDolphinToRide()&&requireHealedDolphin$$module$js$patch(m);c.saharaRabbitsRequireTelepathy()&&requireTelepathyForDeo$$module$js$patch(m);adjustItemNames$$module$js$patch(m,c);yield g("postparse.s");alarmFluteIsKeyItem$$module$js$patch(m);c.teleportOnThunderSword()?teleportOnThunderSword$$module$js$patch(m):noTeleportOnThunderSword$$module$js$patch(m);m.scalingLevels=
...
c,h);unidentifiedItems$$module$js$pass$unidentifieditems(m,c,h);shuffleTrades$$module$js$pass$shuffletrades(m,c,h);c.randomizeMaps()&&shuffleMazes$$module$js$pass$shufflemazes(m,h);const n=World$$module$js$graph$world.build(m,c),q=yield(new AssumedFill$$module$js$graph$shuffle(m,c)).shuffle(n.graph,h,f);if(q)n.traverse(n.graph,q),update$$module$js$rom$slots(m,q.slots);else return-1;rescaleShops$$module$js$patch(m,l,c.bargainHunting()?h:void 0);normalizeSwords$$module$js$patch(m,c,h);c.shuffleMonsters()&&
shuffleMonsters$$module$js$patch(m,c,h);identifyKeyItemsForDifficultyBuffs$$module$js$patch(m);c.doubleBuffMedicalHerb()?(a[115996]*=2,a[115962]*=3):c.buffMedicalHerb()&&(a[115996]+=16,a[115962]*=2);c.storyMode()&&storyMode$$module$js$patch(m);c.chargeShotsOnly()&&disableStabs$$module$js$patch(m);c.orbsOptional()&&orbsOptional$$module$js$patch(m);shuffleMusic$$module$js$patch(m,c,h);shufflePalettes$$module$js$pass$shufflepalettes(m,c,h);misc$$module$js$patch(m,c,h);fixDialog$$module$js$pass$fixdialog(m);
fixReverseWalls$$module$js$pass$fixreversewalls(m);c.buffDyna()&&buffDyna$$module$js$patch(m,c);yield m.writeData();buffDyna$$module$js$patch(m,c);return yield postParsedShuffle$$module$js$patch(a,h,b,c,l,g)})}
