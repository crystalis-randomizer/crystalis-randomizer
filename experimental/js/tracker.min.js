...
179];for(const a of b.spawns)a.isWall()&&f.set(module$js$graph$geometry.ScreenId.from(b,a),a.id&3);for(let g=0,h=b.height;g<h;g++){const h=b.screens[g],k=b.id<<8|g<<4;for(let g=0,l=b.width;g<l;g++){var r=function(a,b){if(!p)throw Error("flag expected");return b?a?t(a,b):module$js$graph$condition.Terrain.meet({enter:module$js$graph$condition.Condition(p.flag)},b):a};const l=a.screens[h[g]|c],m=module$js$graph$geometry.ScreenId(k|g),n=m&255,z=f.get(m),p=null!=z?d.wallCapability(z):b.flags.find((a)=>
a.yx===n),t=memoize2$$module$js$graph$condition((a,b)=>{if(!p)throw Error("flag expected");b=Object.assign({},b,{enter:meet$$module$js$graph$condition(b.enter||[[]],module$js$graph$condition.Condition(p.flag))});return module$js$graph$condition.Terrain.join(a,b)});for(let a=0;240>a;a++){const b=module$js$graph$geometry.TileId(m<<8|a);let f=l.tiles[a];p&&751===p.flag&&32>f&&(f=q.alternates[f]);let g=d.makeTerrain(c?0:u.effects[f]&38,b);if(32>f&&q.alternates[f]!=f&&p&&751!==p.flag){const a=d.makeTerrain(u.effects[q.alternates[f]],
...
k[1]);g[h]=k[0]}for(h=0;h<g.length-1;h++){k=g[h];const a=g[h+1];0>k||0>a||(c.set(a,k),d[a]=k)}}g=new Set;c=new Set;a=new Set(a);for(var l of this.locations)if(l.used&&a.has(l.tileset)){c.add(l.tileEffects);for(var m of l.allScreens())g.add(m)}for(var n of g)for(let a=0,b=n.tiles.length;a<b;a++)n.tiles[a]=d[n.tiles[a]];for(var r of a){d=this.tilesets[(r&127)>>>2];for(var p of b)for(l=0;l<p.length-1;l++){m=f(p[l]);n=f(p[l+1]);for(a=0;4>a;a++)d.tiles[a][m]=d.tiles[a][n];d.attrs[m]=d.attrs[n];if(32>n&&
d.alternates[n]!==n){if(32<=m)throw Error(`Cannot unflag: ${r} ${m} ${n} ${d.alternates[n]}`);d.alternates[m]=d.alternates[n]}}for(var q of e){const [a,b]=q;d.alternates[a]=b}}for(const a of c){r=this.tileEffects[a-179];for(const a of b)for(p=0;p<a.length-1;p++)q=f(a[p]),d=f(a[p+1]),r.effects[q]=r.effects[d];for(const a of e.keys())r.effects[a]|=8}}static load(a,b){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){const c=yield pickFile$$module$js$rom(b);a&&(yield a(c));return new Rom$$module$js$rom(c)})}}
