...
this.omitLocalDialogSuffix=Rom$$module$js$rom.OMIT_LOCAL_DIALOG_SUFFIX.get(a);this.compressedMapData=Rom$$module$js$rom.COMPRESSED_MAPDATA.get(a);for(const a of ADJUSTMENTS$$module$js$rom){const [b,d,e]=a;this.prg[b]===d&&(this.prg[b]=e)}this.screens=seq$$module$js$rom$util(259,(a)=>new Screen$$module$js$rom$screen(this,a));this.tilesets=seq$$module$js$rom$util(12,(a)=>new Tileset$$module$js$rom$tileset(this,a<<2|128));this.tileEffects=seq$$module$js$rom$util(11,(a)=>new TileEffects$$module$js$rom$tileeffects(this,
...
k[1]);g[h]=k[0]}for(h=0;h<g.length-1;h++){k=g[h];const a=g[h+1];0>k||0>a||(c.set(a,k),d[a]=k)}}g=new Set;c=new Set;a=new Set(a);for(var l of this.locations)if(l.used&&a.has(l.tileset)){c.add(l.tileEffects);for(var m of l.allScreens())g.add(m)}for(var n of g)for(let a=0,b=n.tiles.length;a<b;a++)n.tiles[a]=d[n.tiles[a]];for(var q of a){d=this.tilesets[(q&127)>>>2];for(var p of b)for(l=0;l<p.length-1;l++){m=f(p[l]);n=f(p[l+1]);for(a=0;4>a;a++)d.tiles[a][m]=d.tiles[a][n];d.attrs[m]=d.attrs[n];if(32>n&&
d.alternates[n]!==n){if(32<=m)throw Error(`Cannot unflag: ${q} ${m} ${n} ${d.alternates[n]}`);d.alternates[m]=d.alternates[n]}}for(var r of e){const [a,b]=r;d.alternates[a]=b}}for(const a of c){q=this.tileEffects[a-179];for(const a of b)for(p=0;p<a.length-1;p++)r=f(a[p]),d=f(a[p+1]),q.effects[r]=q.effects[d];for(const a of e.keys())q.effects[a]|=8}}static load(a,b){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){const c=yield pickFile$$module$js$rom(b);a&&(yield a(c));return new Rom$$module$js$rom(c)})}}
