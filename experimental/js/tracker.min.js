...
for(let l=0;l<this.width;l++){const p=this.rom.screens[h[l]],q=c<<4|l,A=this.flags.find(a=>a.screen===q);for(let c=0;240>c;c++){const h=q<<8|c;if(b.has(h))continue;let l=p.tiles[c];var g=e.effects[l];g=a?g&4:g&6;A&&g&&32>l&&d.alternates[l]!=l&&(l=d.alternates[l],g=e.effects[l],g=a?g&4:g&6);g||f.add(h)}}}for(let b of f)a=15===(b&15)?b+241:b+1,f.has(a)&&c.union([b,a]),a=224===(b&240)?b+3872:b+16,f.has(a)&&c.union([b,a]);f=c.map();c=new Set;for(var h of this.entrances)h.used&&c.add(f.get(h.screen<<8|
...
e[b];null!=f&&(0>f&&(f=~f,this.flags.push(Sd.of({screen:a+d<<4|c+b,flag:this.rom.flags.AlwaysTrue.id}))),this.screens[a+d][c+b]=f)}}}connect(a,b,c){var d=0>a?256:0,e=0>c?256:0;a=0>a?~a:a;c=0>c?~c:c;const f=a>>>4,g=a&15,h=c>>>4,l=c&15,q=b.screens[h][l],[p,z]=Ae[d|this.screens[f][g]],[B,A]=Ae[e|q];d=this.entrances.length;e=b.entrances.length;this.entrances.push(Qd.of({y:f<<8|p>>>8,x:g<<8|p&255}));b.entrances.push(Qd.of({y:h<<8|B>>>8,x:l<<8|B&255}));for(const c of z)this.exits.push(Rd.of({screen:a,tile:c,
