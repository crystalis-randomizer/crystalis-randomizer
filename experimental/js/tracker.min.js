...
exclusiveFlags$$module$js$flagset=(b)=>b.startsWith("S")?new RegExp(`S.*[${b.substring(1)}]`):FLAG_CONFLICTS$$module$js$flagset[b],FLAG_CONFLICTS$$module$js$flagset={Hm:/Em/,Hx:/Ex/,Em:/Hm/,Ex:/Hx/,Tw:/Gw/,Gw:/Tw/,Ft:/Gt/,Gt:/Ft/};
...
ma=(new Magic$$module$js$nodes(a,67,"Telepathy",67,null)).direct("Tornel at Stom's house",223220).npcSpawn(95,30,1).trigger(133,1),oc=(new Magic$$module$js$nodes(a,68,"Teleport",68,null)).fromPerson("Tornel on Mt. Sabre",95).dialog(95,33,0),ee=(new Magic$$module$js$nodes(a,69,"Recover",69,null)).direct("Asina in Portoa",250361),pc=(new Magic$$module$js$nodes(a,70,"Barrier",70,null)).direct("Asina on Angry sea",251609).requireUnique().trigger(132,0),ua=(new Magic$$module$js$nodes(a,71,"Change",71,
null)).direct("Kensu in Swan",251614).npcSpawn(116,241,1),B=(new Magic$$module$js$nodes(a,72,"Flight",72,null)).weight(15).direct("Kensu in Draygonia Fortress",250255),fe=I.bossDrop("Vampire 2",12,97).npcSpawn(204).key(),J=d(112,"Mimic"),qc=(new Trigger$$module$js$nodes(a,"Talked to Leaf Elder")).get(V),rc=(new Trigger$$module$js$nodes(a,"Talked to Leaf Student")).get(tb),wb=new Trigger$$module$js$nodes(a,"Talked to Zebu in cave"),ge=(new Trigger$$module$js$nodes(a,"Woke up Windmill Guard")).get(cc),
Xa=new Trigger$$module$js$nodes(a,"Started Windmill"),sc=(new Trigger$$module$js$nodes(a,"Learned Refresh")).get(Va),he=(new Trigger$$module$js$nodes(a,"Gave Statue to Akahana")).get(Ua),ie=(new Trigger$$module$js$nodes(a,"Fought Stom")).get(ma),tc=new Trigger$$module$js$nodes(a,"Visited Oak"),uc=new Trigger$$module$js$nodes(a,"Talked to Oak Mother"),xb=new Trigger$$module$js$nodes(a,"Rescued Oak Child"),je=(new Trigger$$module$js$nodes(a,"Talked to Oak Mother Again")).get($b),ke=(new Trigger$$module$js$nodes(a,
"Talked to Oak Elder")).get(W),le=(new Trigger$$module$js$nodes(a,"Talked to Tornel on Mt Sabre")).get(oc),vc=new Trigger$$module$js$nodes(a,"Villagers Abducted"),wc=new Trigger$$module$js$nodes(a,"Talked to Rabbit in Leaf"),me=(new Trigger$$module$js$nodes(a,"Learned Paralysis")).get(Wa),xc=new Trigger$$module$js$nodes(a,"Talked to Portoa Queen"),yc=new Trigger$$module$js$nodes(a,"Talked to Fortune Teller"),zc=new Trigger$$module$js$nodes(a,"Visited Underground Channel"),ne=(new Trigger$$module$js$nodes(a,
"Sent to Waterfall Cave")).get(rb),oe=(new Trigger$$module$js$nodes(a,"Cured Akahana")).get(bc),Ac=(new Trigger$$module$js$nodes(a,"Talked to Rage")).get(Qa),Bc=new Trigger$$module$js$nodes(a,"Mesia recording played"),Cc=(new Trigger$$module$js$nodes(a,"Talked to Asina")).get(ee),Dc=(new Trigger$$module$js$nodes(a,"Healed Dolphin")).get(ub),yb=new Trigger$$module$js$nodes(a,"Returned Fog Lamp"),Ec=new Trigger$$module$js$nodes(a,"Talked to Kensu in Cabin"),Fc=new Trigger$$module$js$nodes(a,"Talked to Joel Elder"),
pe=(new Trigger$$module$js$nodes(a,"Talked to Clark")).get(gc),qe=(new Trigger$$module$js$nodes(a,"Talked to Kensu in Lighthouse")).get(ic),zb=new Trigger$$module$js$nodes(a,"Calmed the Angry Sea"),re=(new Trigger$$module$js$nodes(a,"Learned Barrier")).get(pc),Gc=new Trigger$$module$js$nodes(a,"Talked to Stom in Swan Hut"),Hc=new Trigger$$module$js$nodes(a,"Talked to Kensu in Swan tavern"),Ic=new Trigger$$module$js$nodes(a,"Talked to Kensu at Swan dance"),se=(new Trigger$$module$js$nodes(a,"Returned Kensu's love pendant")).get(ua),
te=(new Trigger$$module$js$nodes(a,"Talked to Amazones Queen")).get(lc),Ab=new Trigger$$module$js$nodes(a,"Entered Shyron"),ue=(new Trigger$$module$js$nodes(a,"Talked to Zebu in Shyron")).get(ec),Jc=new Trigger$$module$js$nodes(a,"Shyron Massacre"),ve=(new Trigger$$module$js$nodes(a,"Saved Kensu")).get(B),we=(new Trigger$$module$js$nodes(a,"Talked to Deo")).get(de),xe=(new Trigger$$module$js$nodes(a,"Talked to Akahana's Friend")).get(be),ye=(new Trigger$$module$js$nodes(a,"Get Bow of Truth")).get(nc),
ze=(new Trigger$$module$js$nodes(a,"Forged Crystalis")).get(mb),Ya=(new Condition$$module$js$nodes(a,"Sword charge glitch")).option(Qd),Ae=(new Condition$$module$js$nodes(a,"Rabbit skip")).option(l),Be=(new Condition$$module$js$nodes(a,"Teleport skip")).option(t),Bb=(new Condition$$module$js$nodes(a,"Statue glitch")).option(r,Pd),Ce=(new Condition$$module$js$nodes(a,"Wild warp")).option(Nd,Od),Za=(new Condition$$module$js$nodes(a,"Any level 2 sword")).option(V,Na).option(V,Ga).option(W,Oa).option(W,
...
De=(new Condition$$module$js$nodes(a,"Match insect sword (fire/water/thunder)")).option(W).option(P).option(L).option(Ua,y.value?y:h,V),Kc=(new Condition$$module$js$nodes(a,"Speed boots")).option(ac,m),Lc=(new Condition$$module$js$nodes(a,"Climb slopes")).option(sb).option(B).option(Kc),Ee=(new Condition$$module$js$nodes(a,"Enter Mt Sabre North (rabbit)")).option(wc).option(Ae),Fe=(new Condition$$module$js$nodes(a,"Enter Mt Sabre North (Teleport)")).option(oc).option(Be,B),Mc=(new Condition$$module$js$nodes(a,
"Asina in her room")).option(Bc),Ge=(new Condition$$module$js$nodes(a,"Paralysis or Ball of Water")).option(Wa).option(Mc).option(Bb),Nc=(new Condition$$module$js$nodes(a,"Ride dolphin")).option(ub,Ec),He=(new Condition$$module$js$nodes(a,"Ghetto flight")).option(p,Nc,sb),Y=(new Condition$$module$js$nodes(a,"Cross sea")).option(Nc).option(B),Ie=(new Condition$$module$js$nodes(a,"Cross whirlpool")).option(zb).option(B).option(He),N=(new Condition$$module$js$nodes(a,"Refresh if needed")).option(Ma.value?
Ma:h).option(Va),Oc=(new Condition$$module$js$nodes(a,"Wind magic")).option(n.value?n:h,N).option(Na,Ga,N),Pc=(new Condition$$module$js$nodes(a,"Fire magic")).option(n.value?n:h,N).option(Oa,Pa,N),Qc=(new Condition$$module$js$nodes(a,"Water magic")).option(n.value?n:h,N).option(n,N).option(Qa,Ra,N),Je=(new Condition$$module$js$nodes(a,"Thunder magic")).option(n.value?n:h,N).option(n,N).option(Sa,N).option(Ta,N),Ke=(new Condition$$module$js$nodes(a,"Flute of lime or glitch")).option(rb).option(Bb),
Rc=(new Condition$$module$js$nodes(a,"Change or glitch")).option(ua).option(Bb),Cb=(new Condition$$module$js$nodes(a,"Pass shooting statues")).option(pc).option(La.value?La:h).option(Va,bc),Le=(new Condition$$module$js$nodes(a,"Healed dolphin if required")).option(Dc).option(sa),Sc=(new Condition$$module$js$nodes(a,"Match sword of wind")).option(V).option(y.value?y:h,X),Tc=(new Condition$$module$js$nodes(a,"Match sword of fire")).option(W).option(y.value?y:h,X),Uc=(new Condition$$module$js$nodes(a,
"Match sword of water")).option(P).option(y.value?y:h,X),Me=(new Condition$$module$js$nodes(a,"Match sword of thunder")).option(L).option(y.value?y:h,X),Ne=(new Condition$$module$js$nodes(a,"Swamp run possible")).option(X).option(Kc).option(Rd).option(Va),Oe=(new Condition$$module$js$nodes(a,"Insect possible to kill")).option(Ua),$a=(new Condition$$module$js$nodes(a,"Travel swamp")).option(Ua).option(v.value?v:h,Ne),Pe=(new Condition$$module$js$nodes(a,"Calmed sea if required")).option(zb).option(ka),
Qe=e(0,"Vampire 1",X).get(sb),Re=e(1,"Insect",$b,De,Oe).get(Oa),Se=e(2,"Kelbesque 1",Sc,Oc).get(Pa),Te=e(12,"Vampire 2",X).get(fe),Vc=e(4,"Sabera 1",Tc,Pc).get(hc),Ue=e(5,"Mado 1",Uc,Qc).get(Sa),Ve=e(6,"Kelbesque 2",Sc,Oc).get(qb),We=e(7,"Sabera 2",Tc,Pc).get(pb),Xe=e(8,"Mado 2",Uc,Qc).get(nb),Ye=e(9,"Karmine",Me,Je).get(vb),Ze=e(10,"Draygon 1",X).get($d),$e=e(null,"Statues",mc,lc),af=e(11,"Draygon 2",X,nc),bf=e(13,"Dyna",mb),R=new Area$$module$js$nodes(a,"Leaf"),wa=new Area$$module$js$nodes(a,"Valley of Wind"),
...
T=new Area$$module$js$nodes(a,"Draygonia Fortress 2"),za=new Area$$module$js$nodes(a,"Draygonia Fortress 3"),K=new Area$$module$js$nodes(a,"Draygonia Fortress 4"),F=new Area$$module$js$nodes(a,"Oasis Cave");new Area$$module$js$nodes(a,"Desert");const U=new Area$$module$js$nodes(a,"Sahara"),Aa=new Area$$module$js$nodes(a,"Pyramid Front"),Z=new Area$$module$js$nodes(a,"Pyramid Back"),aa=new Area$$module$js$nodes(a,"Tower"),Db=(new Location$$module$js$nodes(a,0,R,"Start")).overworld().start(),cf=(new Location$$module$js$nodes(a,
0,R,"Mezame Shrine")).overworld().from(Db),df=(new Location$$module$js$nodes(a,1,R,"Outside Start")).overworld().connect(cf),ja=(new Location$$module$js$nodes(a,2,R,"Town")).town().connect(df),Ia=(new Location$$module$js$nodes(a,3,wa,"Main")).overworld().connect(ja).trigger(sc,Xa),Wc=(new Location$$module$js$nodes(a,3,wa,"Outside Windmill")).overworld(),Xc=(new Location$$module$js$nodes(a,4,O,"Tunnel 1 (entrance)")).cave().from(Ia,Xa),ef=(new Location$$module$js$nodes(a,5,O,"Tunnel 2 (over bridge)")).cave().connect(Xc),
ff=(new Location$$module$js$nodes(a,6,O,"Tunnel 6 (herb dead end)")).cave().chest(E,15),Yc=(new Location$$module$js$nodes(a,7,O,"Tunnel 4a (ball corridor)")).cave().chest(E,20,80).chest(Na,21);(new Location$$module$js$nodes(a,7,O,"Tunnel 4b (antidote dead end)")).cave().connect(Yc,w).chest(H,19);const gf=(new Location$$module$js$nodes(a,8,O,"Tunnel 5 (warp boots dead end)")).cave().chest(M,14),hf=(new Location$$module$js$nodes(a,9,O,"Tunnel 3a (branch, front)")).cave().connect(ef).connectTo(Yc).connectTo(gf),
jf=(new Location$$module$js$nodes(a,9,O,"Tunnel 3b (branch, back)")).cave().connect(hf,w).connectTo(ff),kf=(new Location$$module$js$nodes(a,10,O,"Tunnel 7 (boss)")).cave().connect(jf).boss(Qe),lf=(new Location$$module$js$nodes(a,12,O,"Tunnel 8a (exit, above wall)")).cave().connect(kf),mf=(new Location$$module$js$nodes(a,12,O,"Tunnel 8b (exit, below wall")).cave().connect(lf,w);(new Location$$module$js$nodes(a,14,wa,"Windmill Cave")).cave().connect(Ia).connectTo(Wc).trigger(ge,tb,wb);(new Location$$module$js$nodes(a,
15,wa,"Windmill")).misc().connect(Wc).trigger(Xa,cc);const nf=(new Location$$module$js$nodes(a,16,wa,"Zebu's Cave Front")).cave().connect(Ia).trigger(wb,qc,rc).trigger(sc,Xa,wb),of=(new Location$$module$js$nodes(a,16,wa,"Zebu's Cave Back")).cave().trigger(vc).connect(nf,x),pf=(new Location$$module$js$nodes(a,17,C,"Tunnel 1 (to Zebu)")).cave().connect(of),Ba=(new Location$$module$js$nodes(a,20,na,"West")).overworld().connect(mf),qf=(new Location$$module$js$nodes(a,20,na,"South")).overworld().connect(Ba,
q),Zc=(new Location$$module$js$nodes(a,21,na,"East")).overworld().connect(Ba).chest(Zb,24),Ja=(new Location$$module$js$nodes(a,24,xa,"Town")).town().connect(Ba).trigger(he,Zb),rf=(new Location$$module$js$nodes(a,25,na,"Outside Stom's House")).town().connect(Ba),Eb=(new Location$$module$js$nodes(a,26,na,"Swamp")).overworld().connect(Zc,$a).trigger(xb,uc,$a);(new Location$$module$js$nodes(a,26,na,"Swamp Insect Area")).overworld().connect(Eb,$a).boss(Re);const cb=(new Location$$module$js$nodes(a,27,
ya,"Town")).town().connect(qf),db=(new Location$$module$js$nodes(a,28,Ha,"Town")).town().connect(Eb,$a).trigger(tc);(new Location$$module$js$nodes(a,30,na,"Stom's House")).house().connect(rf).trigger(ie,tc);const Ca=(new Location$$module$js$nodes(a,32,C,"Entrance")).overworld().connect(Ba).connect(pf),sf=(new Location$$module$js$nodes(a,32,C,"Up Slope")).overworld().from(Ca,Lc).to(Ca),tf=(new Location$$module$js$nodes(a,32,C,"Dead End (warp boots)")).overworld().chest(M,24,106),$c=(new Location$$module$js$nodes(a,
33,C,"Upper")).overworld().from(Ca,B).to(Ca),uf=(new Location$$module$js$nodes(a,33,C,"Tornel Dead End")).overworld().trigger(le,Ga).chest(Q,23,105),ad=(new Location$$module$js$nodes(a,34,C,"Tunnel 2a (fork at start)")).cave().connect(Ca);(new Location$$module$js$nodes(a,34,C,"Tunnel 2b (left branch to dead end)")).cave().connect(ad,x).connectTo(tf);const vf=(new Location$$module$js$nodes(a,34,C,"Tunnel 2c (right branch to upper)")).cave().connect(ad,x),wf=(new Location$$module$js$nodes(a,35,C,"Tunnel 3a (tunnel to upper, with herb chest)")).cave().connect(vf).chest(E,
23,82),xf=(new Location$$module$js$nodes(a,35,C,"Tunnel 3b (tunnel to upper, branch below ice)")).cave().connect(wf,x).connectTo($c),bd=(new Location$$module$js$nodes(a,36,C,"Tunnel 4a (branch to upper or Tornel)")).cave().connect(xf);(new Location$$module$js$nodes(a,36,C,"Tunnel 4b (out to upper)")).cave().connect(bd,x).connectTo($c);const yf=(new Location$$module$js$nodes(a,37,C,"Tunnel 5 (tiny connector)")).cave().connect(bd),zf=(new Location$$module$js$nodes(a,38,C,"Tunnel 6a (exit to Tornel, above ice)")).cave().connect(yf);
(new Location$$module$js$nodes(a,38,C,"Tunnel 6b (exit to Tornel, below ice)")).cave().connect(zf,x).connectTo(uf);const Af=(new Location$$module$js$nodes(a,39,C,"Tunnel 7a (tornado bracelet, lower)")).cave().connect(sf),Bf=(new Location$$module$js$nodes(a,39,C,"Tunnel 7b (tornado bracelet, middle)")).cave().connect(Af,x);(new Location$$module$js$nodes(a,39,C,"Tunnel 7c (tornado bracelet, upper)")).cave().connect(Bf,x).chest(Ga,25);const eb=(new Location$$module$js$nodes(a,40,u,"Entrance")).overworld().connect(Zc,
Fe,Ee),fb=(new Location$$module$js$nodes(a,40,u,"Upper")).overworld().from(eb,B).to(eb),Fb=(new Location$$module$js$nodes(a,40,u,"Summit (boss)")).overworld().from(fb,B).to(fb).boss(Se),cd=(new Location$$module$js$nodes(a,41,u,"Connector")).overworld(),Gb=(new Location$$module$js$nodes(a,41,u,"Middle Left")).overworld(),Cf=(new Location$$module$js$nodes(a,41,u,"Middle Right")).overworld().from(Gb,Lc).to(Gb),dd=(new Location$$module$js$nodes(a,42,u,"Tunnel 2a (from entrance to connector)")).cave().connectTo(cd);
(new Location$$module$js$nodes(a,42,u,"Tunnel 2b (under bridge, to antidote)")).cave().connect(dd,x).chest(H,23,94);const ed=(new Location$$module$js$nodes(a,43,u,"Tunnel 3a (branch after connector)")).cave().connect(cd),Df=(new Location$$module$js$nodes(a,43,u,"Tunnel 3b (right branch)")).cave().connect(ed,x),Ef=(new Location$$module$js$nodes(a,43,u,"Tunnel 3c (upper branch)")).cave().connect(ed,x);(new Location$$module$js$nodes(a,44,u,"Tunnel 4 (over bridge, to middle)")).cave().connect(Ef).connectTo(Gb);
const fd=(new Location$$module$js$nodes(a,45,u,"Tunnel 5a (E-shaped, from right branch)")).cave().connect(Df).connectTo(Cf);(new Location$$module$js$nodes(a,45,u,"Tunnel 5b (dead-end with herb)")).cave().connect(fd,x).chest(E,22,83);const Ff=(new Location$$module$js$nodes(a,46,u,"Tunne; 6a (S-shaped hall, right)")).cave().connect(fd),Gf=(new Location$$module$js$nodes(a,46,u,"Tunne; 6b (S-shaped hall, middle)")).cave().connect(Ff,x),Hf=(new Location$$module$js$nodes(a,46,u,"Tunnel 6c (S-shaped hall, left)")).cave().connect(Gf,
x),gd=(new Location$$module$js$nodes(a,47,u,"Prison (hallway)")).cave().connect(fb),If=(new Location$$module$js$nodes(a,48,u,"Left Cell (shopkeepers)")).cave().from(gd,x);(new Location$$module$js$nodes(a,49,u,"Left Cell 2 (back, with prison key)")).cave().from(If,x).chest(dc,13);const Jf=(new Location$$module$js$nodes(a,50,u,"Right Cell (villagers)")).cave().from(gd,x),Kf=(new Location$$module$js$nodes(a,51,u,"Tunnel 8 (behind right cell, toward summit)")).cave().from(Jf,x);(new Location$$module$js$nodes(a,
52,u,"Tunnel 9 (connector to summit)")).cave().connect(Kf).connectTo(Fb);const Lf=(new Location$$module$js$nodes(a,53,u,"Tunnel 10a (summit cave, front)")).cave().from(Fb,dc).to(Fb),Mf=(new Location$$module$js$nodes(a,53,u,"Tunnel 10b (summit cave, behind ice)")).cave().connect(Lf,x).trigger(me);(new Location$$module$js$nodes(a,56,u,"Tunnel 1 (leads from main entrance)")).cave().connect(eb).connectTo(dd);(new Location$$module$js$nodes(a,57,u,"Tunnel 7 (to upper)")).cave().connect(Hf).connectTo(fb);
const Nf=(new Location$$module$js$nodes(a,60,ab,"Inn")).shop(),Of=(new Location$$module$js$nodes(a,61,ab,"Tool Shop")).shop(E,H,I,M),Pf=(new Location$$module$js$nodes(a,62,ab,"Back Room")).house(),Hb=(new Location$$module$js$nodes(a,64,ca,"Summit")).overworld().connect(Mf),gb=(new Location$$module$js$nodes(a,64,ca,"Northwest")).overworld().from(Hb).to(Hb,B),Qf=(new Location$$module$js$nodes(a,64,ca,"Northeast")).overworld().connect(gb,q),hd=(new Location$$module$js$nodes(a,65,ca,"Southwest")).overworld().connect(gb),
Rf=(new Location$$module$js$nodes(a,65,ca,"Southeast")).overworld().connect(hd,q),id=(new Location$$module$js$nodes(a,66,ca,"Lime Tree Valley")).overworld().connect(hd).connect(Ia,Md),Sf=(new Location$$module$js$nodes(a,67,ca,"Lime Tree Lake (Rage)")).cave().connect(id).trigger(Ac,P),Tf=(new Location$$module$js$nodes(a,68,ea,"Tunnel 1a (entrance)")).cave().connect(Rf),Uf=(new Location$$module$js$nodes(a,68,ea,"Tunnel 1b (behind wall)")).cave().connect(Tf,w),jd=(new Location$$module$js$nodes(a,69,
ea,"Tunnel 2a (main path, before wall)")).cave().connect(Uf);(new Location$$module$js$nodes(a,69,ea,"Tunnel 2b (dead end, antidote)")).cave().connect(jd,w).chest(H,25,95);const Vf=(new Location$$module$js$nodes(a,69,ea,"Tunnel 2c (main path, after wall)")).cave().connect(jd,w),Wf=(new Location$$module$js$nodes(a,70,ea,"Tunnel 3a (last room, before wall)")).cave().connect(Vf),Xf=(new Location$$module$js$nodes(a,70,ea,"Tunnel 3b (last room, after wall)")).cave().connect(Wf,w);(new Location$$module$js$nodes(a,
71,ea,"Meadow")).overworld().connect(Xf).chest(kc,14);const Yf=(new Location$$module$js$nodes(a,72,D,"Tunnel 1a (entrance)")).cave().connect(Qf),kd=(new Location$$module$js$nodes(a,72,D,"Tunnel 1b (past wall)")).cave().connect(Yf,w);(new Location$$module$js$nodes(a,72,D,"Tunnel 1c (dead end, lysisPlant)")).cave().connect(kd,w).chest(ta,24);const Zf=(new Location$$module$js$nodes(a,73,D,"Tunnel 2 (tiny connector)")).cave().connect(kd),ld=(new Location$$module$js$nodes(a,74,D,"Tunnel 3a (upper branch)")).cave().connect(Zf);
(new Location$$module$js$nodes(a,74,D,"Tunnel 3b (dead end, mimic)")).cave().connect(ld,w).chest(J,21,112);const md=(new Location$$module$js$nodes(a,74,D,"Tunnel 3c (short passage with mimic)")).cave().connect(ld,w).chest(J,22,113),$f=(new Location$$module$js$nodes(a,74,D,"Tunnel 3d (lower branch)")).cave().connect(md,w);(new Location$$module$js$nodes(a,75,D,"Tunnel 4 (dead end loop)")).cave().connect($f);const nd=(new Location$$module$js$nodes(a,76,D,"Tunnel 5a (right branch over bridge)")).cave().connect(md),
ag=(new Location$$module$js$nodes(a,76,D,"Tunnel 5b (past wall over bridge)")).cave().connect(nd,w),bg=(new Location$$module$js$nodes(a,77,D,"Tunnel 6a (from left branch)")).cave().connect(nd),cg=(new Location$$module$js$nodes(a,77,D,"Tunnel 6b (reconvergence)")).cave().connect(bg,w).connect(ag),dg=(new Location$$module$js$nodes(a,77,D,"Tunnel 6c (between walls)")).cave().connect(cg,w),eg=(new Location$$module$js$nodes(a,77,D,"Tunnel 6d (under bridge)")).cave().connect(dg,w),fg=(new Location$$module$js$nodes(a,
78,D,"Tunnel 7a (over second bridge)")).cave().connect(eg),gg=(new Location$$module$js$nodes(a,78,D,"Tunnel 7b (past wall)")).cave().connect(fg,w),hg=(new Location$$module$js$nodes(a,79,D,"Tunnel 8a (under second bridge)")).cave().connect(gg);(new Location$$module$js$nodes(a,79,D,"Tunnel 8b (fog lamp)")).cave().connect(hg,w).chest(fc,19);const ra=(new Location$$module$js$nodes(a,80,A,"Town")).town().connect(gb),od=(new Location$$module$js$nodes(a,81,A,"Fishherman Island")).town().connect(ra);(new Location$$module$js$nodes(a,
82,ca,"Mesia Shrine")).cave().trigger(Bc).connect(Sf,q,Ac);const pd=(new Location$$module$js$nodes(a,84,da,"Tunnel 1a (entrance)")).cave().connect(gb);(new Location$$module$js$nodes(a,84,da,"Tunnel 1b (dead end, mimic)")).cave().connect(pd,x).chest(J,19,114);const ig=(new Location$$module$js$nodes(a,84,da,"Tunnel 1c (past ice)")).cave().connect(pd,x),jg=(new Location$$module$js$nodes(a,85,da,"Tunnel 2 (stoned pair)")).cave().connect(ig),Ib=(new Location$$module$js$nodes(a,86,da,"Tunnel 3 (wide medusa hallways)")).cave().connect(jg,
Ke),kg=(new Location$$module$js$nodes(a,87,da,"Tunnel 4a (left entrance)")).cave().from(Ib,x).chest(Vb,25).trigger(oe,rb);(new Location$$module$js$nodes(a,87,da,"Tunnel 4b (right entrance)")).cave().from(Ib,x).connect(kg,B);(new Location$$module$js$nodes(a,87,da,"Tunnel 4c (sword of water)")).cave().connect(Ib,x).chest(P,24);const qd=(new Location$$module$js$nodes(a,88,aa,"Entrance")).misc(),lg=(new Location$$module$js$nodes(a,89,aa,"Level 1")).misc().from(qd),mg=(new Location$$module$js$nodes(a,
90,aa,"Level 2")).misc().from(lg),ng=(new Location$$module$js$nodes(a,91,aa,"Level 3")).misc().from(mg),rd=(new Location$$module$js$nodes(a,92,aa,"Outside Mesia Room")).misc().from(ng),og=(new Location$$module$js$nodes(a,93,aa,"Outside Dyna Room")).misc().from(rd,mb);(new Location$$module$js$nodes(a,94,aa,"Mesia")).misc().connect(rd).trigger(ze);const pg=(new Location$$module$js$nodes(a,95,aa,"Dyna")).misc().from(og).boss(bf);(new Location$$module$js$nodes(a,95,aa,"Win")).misc().from(pg).end();const Jb=
(new Location$$module$js$nodes(a,96,oa,"Cabin Beach")).sea().from(od,yb),Da=(new Location$$module$js$nodes(a,96,oa,"South")).sea().connect(Jb,Y),qg=(new Location$$module$js$nodes(a,96,oa,"Joel Beach")).sea().connect(Da,Y),sd=(new Location$$module$js$nodes(a,96,fa,"Outside Lighthouse")).sea();(new Location$$module$js$nodes(a,96,oa,"Altar")).sea().connect(Da,Y).trigger(zb,ic,hc);const rg=(new Location$$module$js$nodes(a,96,oa,"North")).sea().to(Da,Y).from(Da,Y,Ie).trigger(re,Pe),sg=(new Location$$module$js$nodes(a,
96,oa,"Swan Beach")).sea().connect(rg,Y);(new Location$$module$js$nodes(a,97,oa,"Cabin")).misc().connect(Jb).trigger(Ec,yb);(new Location$$module$js$nodes(a,98,fa,"Lighthouse")).misc().connect(sd).trigger(qe,tb);const hb=(new Location$$module$js$nodes(a,100,A,"Underground Channel 1 (from throne room)")).sea().trigger(zc),tg=(new Location$$module$js$nodes(a,100,A,"Underground Channel 2 (to fortune teller)")).sea().connect(hb,q),td=(new Location$$module$js$nodes(a,100,A,"Underground Channel 3 (from fortune teller)")).sea().connect(hb,
B),ud=(new Location$$module$js$nodes(a,100,A,"Underground Channel 4 (asina)")).sea().connect(td,q),ug=(new Location$$module$js$nodes(a,100,A,"Underground Channel 5 (dolphin)")).sea().connect(ud,q).trigger(Dc,E,Cc);(new Location$$module$js$nodes(a,100,A,"Underground Channel 6 (water)")).sea().connect(ug,Y).connectTo(Da,Y).chest(jc,17);const Kb=(new Location$$module$js$nodes(a,101,G,"Zombie Town")).town(),vg=(new Location$$module$js$nodes(a,104,G,"Tunnel 1 (entrance)")).sea().connect(Da,Fc,Y),vd=(new Location$$module$js$nodes(a,
105,G,"Tunnel 2a (start)")).cave().connect(vg);(new Location$$module$js$nodes(a,105,G,"Tunnel 2b (dead end to left)")).cave().connect(vd,q);const wg=(new Location$$module$js$nodes(a,105,G,"Tunnel 2c (across first river)")).cave().connect(vd,q),wd=(new Location$$module$js$nodes(a,105,G,"Tunnel 2d (across second river)")).cave().connect(wg,q);(new Location$$module$js$nodes(a,105,G,"Tunnel 2e (dead end, magic ring)")).cave().connect(wd,w).chest(Q,29);const xg=(new Location$$module$js$nodes(a,105,G,"Tunnel 2f (stair down)")).cave().connect(wd,
w),yg=(new Location$$module$js$nodes(a,106,G,"Tunnel 3a (main area)")).cave().connect(xg).connectTo(Kb).chest(ta,25,92),zg=(new Location$$module$js$nodes(a,106,G,"Tunnel 3b (left area toward items)")).cave().connect(yg,w),Ag=(new Location$$module$js$nodes(a,107,G,"Tunnel 4a (right side, mimic)")).cave().connect(zg).chest(J,14,115);(new Location$$module$js$nodes(a,107,G,"Tunnel 4b (left side, iron necklace)")).cave().connect(Ag,q).chest(ce,15);const Lb=(new Location$$module$js$nodes(a,108,ha,"Floor 1")).fortress().connect(Kb),
Bg=(new Location$$module$js$nodes(a,108,ha,"Miniboss")).fortress().boss(Te).connect(Lb);(new Location$$module$js$nodes(a,109,ha,"Floor 2a (left stair)")).fortress().connect(Lb).chest(I,19);const Cg=(new Location$$module$js$nodes(a,109,ha,"Floor 2b (right stair)")).fortress().connect(Bg).chest(E,30,85),Dg=(new Location$$module$js$nodes(a,110,ha,"Floor 3a (toward boss)")).fortress().connect(Cg),xd=(new Location$$module$js$nodes(a,110,ha,"Floor 3b (boss room)")).fortress().connect(Dg);(new Location$$module$js$nodes(a,
110,ha,"Boss")).fortress().boss(Vc).connect(xd);(new Location$$module$js$nodes(a,110,ha,"Floor 3c (back room trap)")).fortress().from(xd).to(Lb);const Eg=(new Location$$module$js$nodes(a,112,fa,"Secret Passage")).cave().connectTo(sd),ib=(new Location$$module$js$nodes(a,113,fa,"Town")).town().connect(qg),ba=(new Location$$module$js$nodes(a,114,S,"Town")).town().connect(sg),Fg=(new Location$$module$js$nodes(a,115,S,"Inside Gate")).overworld().connect(ba),Gg=(new Location$$module$js$nodes(a,115,S,"Outside Gate")).overworld().from(Fg,
ua),jb=(new Location$$module$js$nodes(a,120,qa,"Valley")).overworld().connect(Gg),Mb=(new Location$$module$js$nodes(a,124,z,"Entrance")).overworld().connect(jb),Hg=(new Location$$module$js$nodes(a,124,z,"Over first river toward Shyron")).overworld().connect(Mb,q),yd=(new Location$$module$js$nodes(a,124,z,"After first tunnel")).overworld(),zd=(new Location$$module$js$nodes(a,124,z,"Door to Styx")).overworld().connect(yd,q),Ig=(new Location$$module$js$nodes(a,124,z,"Dead end (no item)")).overworld(),
Jg=(new Location$$module$js$nodes(a,124,z,"Dead end (fruit of lime)")).overworld().chest(ob,26),Kg=(new Location$$module$js$nodes(a,124,z,"Dead end (magic ring)")).overworld().chest(Q,25,101),Ad=(new Location$$module$js$nodes(a,124,z,"Outside tunnel to bow")).overworld();(new Location$$module$js$nodes(a,124,z,"Floating island (bow of sun)")).overworld().connect(Ad,B).chest(mc,24);const Lg=(new Location$$module$js$nodes(a,125,z,"Tunnel 1 (to Shyron)")).cave().connect(Hg),Mg=(new Location$$module$js$nodes(a,
126,z,"Outside Shyron")).overworld().connect(Lg);(new Location$$module$js$nodes(a,127,z,"Tunnel 2 (fork)")).cave().connect(Mb).connectTo(Jg).connectTo(yd);const Bd=(new Location$$module$js$nodes(a,128,z,"Tunnel 3 (caves)")).cave().connect(zd).connect(Ig),Cd=(new Location$$module$js$nodes(a,129,z,"Tunnel 4 (left branch of cave)")).cave().connect(Bd);(new Location$$module$js$nodes(a,130,z,"Tunnel 5 (dead end, medical herb)")).cave().connect(Cd).chest(E,15,86);const Ng=(new Location$$module$js$nodes(a,
131,z,"Tunnel 6a (left-then-right)")).cave().connect(Cd),Og=(new Location$$module$js$nodes(a,131,z,"Tunnel 6b (past wall)")).cave().connect(Ng,w),Pg=(new Location$$module$js$nodes(a,132,z,"Tunnel 7 (wide hall)")).cave().connect(Og);(new Location$$module$js$nodes(a,133,z,"Tunnel 8 (red slimes)")).cave().from(Pg,w).connectTo(Ad).chest(J,23,116);const Qg=(new Location$$module$js$nodes(a,134,z,"Tunnel 9 (right branch, infinite loop)")).cave().connect(Bd),Rg=(new Location$$module$js$nodes(a,135,z,"Tunnel 10a (toward magic ring)")).cave().connect(Qg);
(new Location$$module$js$nodes(a,135,z,"Tunnel 10b (past wall)")).cave().connect(Rg,w).connectTo(Kg);const Dd=(new Location$$module$js$nodes(a,136,pa,"Entrance")).fortress().from(zd,ec,Cb),Sg=(new Location$$module$js$nodes(a,137,pa,"Left branch")).fortress().connect(Dd).chest(J,19,117),Tg=(new Location$$module$js$nodes(a,137,pa,"Left branch, past one bridge")).fortress().connect(Sg,q),Ug=(new Location$$module$js$nodes(a,137,pa,"Left branch, past two bridges")).fortress().connect(Tg,q).chest(E,29,
87),Vg=(new Location$$module$js$nodes(a,137,pa,"Right branch")).fortress().connect(Dd);(new Location$$module$js$nodes(a,137,pa,"Right branch, across water")).fortress().connect(Vg,B).chest(J,20,118).chest(J,21,119).chest(Ud,28);(new Location$$module$js$nodes(a,138,pa,"Upper floor")).fortress().connect(Ug).chest(L,27);const Ea=(new Location$$module$js$nodes(a,140,ia,"Town")).town().connect(Mg,Rc).trigger(Ab),Ka=(new Location$$module$js$nodes(a,142,qa,"Town")).town().connect(jb),Ed=(new Location$$module$js$nodes(a,
143,F,"Draygonia Fortress Basement 1 (front)")).fortress();(new Location$$module$js$nodes(a,143,F,"Draygonia Fortress Basement 2 (power ring)")).fortress().connect(Ed,va).chest(ae,15);const Nb=(new Location$$module$js$nodes(a,144,qa,"Desert 1")).overworld().connect(jb),kb=(new Location$$module$js$nodes(a,145,F,"Area 1 (from entrance)")).cave().chest(ac,26),Fd=(new Location$$module$js$nodes(a,145,F,"Area 2 (across top bridge)")).cave().connect(kb,q);(new Location$$module$js$nodes(a,145,F,"Area 3 (dead-end across top-right bridge)")).cave().connect(Fd,
q);const Wg=(new Location$$module$js$nodes(a,145,F,"Area 4 (left across middle-right bridge)")).cave().connect(Fd,q),Xg=(new Location$$module$js$nodes(a,145,F,"Area 5 (bottom edge)")).cave().connect(Wg,q),Yg=(new Location$$module$js$nodes(a,145,F,"Area 6 (bottom island)")).cave().connect(Xg,q),Zg=(new Location$$module$js$nodes(a,145,F,"Area 7 (bottom inner ring)")).cave().connect(Yg,q),$g=(new Location$$module$js$nodes(a,145,F,"Area 8 (left outer ring)")).cave().connect(Zg,q).connect(kb,q).chest(I,
27,100),ah=(new Location$$module$js$nodes(a,145,F,"Area 9 (top left inner ring)")).cave().connect($g,q),bh=(new Location$$module$js$nodes(a,145,F,"Area 10 (top right inner ring)")).cave().connect(ah,q);(new Location$$module$js$nodes(a,145,F,"Area 11 (center)")).cave().connect(bh,q).connectTo(Ed);(new Location$$module$js$nodes(a,145,F,"Area 12 (top center islands)")).cave().connect(kb,B).chest(Zd,28);const ch=(new Location$$module$js$nodes(a,146,U,"Desert Cave 1")).cave().connect(Nb,B),Fa=(new Location$$module$js$nodes(a,
147,U,"Town")).town(),dh=(new Location$$module$js$nodes(a,148,U,"Outside Cave")).overworld().connect(Fa),eh=(new Location$$module$js$nodes(a,149,U,"Desert Cave 2")).cave().connect(dh);(new Location$$module$js$nodes(a,150,U,"Meadow")).overworld().connect(ch).connectTo(Fa).trigger(we,ua);const Ob=(new Location$$module$js$nodes(a,152,U,"Desert 2")).overworld().connect(eh),fh=(new Location$$module$js$nodes(a,156,Aa,"Entrance")).fortress().connect(Ob,B),gh=(new Location$$module$js$nodes(a,156,Aa,"Azteca")).fortress().trigger(ye),
hh=(new Location$$module$js$nodes(a,157,Aa,"Fork")).fortress().connect(fh),Gd=(new Location$$module$js$nodes(a,158,Aa,"Main")).fortress().connect(hh);(new Location$$module$js$nodes(a,158,Aa,"Treasure Chest (magic ring)")).fortress().connect(Gd).chest(Q,27,108);(new Location$$module$js$nodes(a,159,Aa,"Draygon")).fortress().connect(Gd).to(gh).boss(Ze);const ih=(new Location$$module$js$nodes(a,160,Z,"Entrance")).fortress().connect(Ob,B),jh=(new Location$$module$js$nodes(a,160,Z,"Statues")).fortress().connect(ih).boss($e),
kh=(new Location$$module$js$nodes(a,161,Z,"Hall 1")).fortress().connect(jh),Pb=(new Location$$module$js$nodes(a,162,Z,"Branch")).fortress().connect(kh);(new Location$$module$js$nodes(a,163,Z,"Left Dead End")).fortress().connect(Pb).chest(J,13,120);(new Location$$module$js$nodes(a,164,Z,"Right Dead End")).fortress().connect(Pb);const lh=(new Location$$module$js$nodes(a,165,Z,"Hall 2")).fortress().connect(Pb).chest(qb,26,109),mh=(new Location$$module$js$nodes(a,166,Z,"Draygon 2")).fortress().connect(lh).boss(af);
(new Location$$module$js$nodes(a,167,Z,"Teleporter")).fortress().from(mh).to(qd);const Hd=(new Location$$module$js$nodes(a,168,bb,"Entrance")).fortress().connect(Ka,Cb).trigger(Jc,L),nh=(new Location$$module$js$nodes(a,169,bb,"Main")).fortress().from(Hd,va),oh=(new Location$$module$js$nodes(a,169,bb,"Boss")).fortress().connect(nh).boss(Ve),ph=(new Location$$module$js$nodes(a,170,bb,"Zebu")).fortress().connect(oh),lb=(new Location$$module$js$nodes(a,171,T,"Entrance")).fortress().connect(ph);(new Location$$module$js$nodes(a,
171,T,"Dead End Behind Iron (fruit of power)")).fortress().connect(lb,va).chest(I,28,98);(new Location$$module$js$nodes(a,171,T,"Dead End Loop Across Closer Bridges")).fortress().connect(lb,q).connect(lb,q);const qh=(new Location$$module$js$nodes(a,171,T,"Across First Bridge (fruit of repun)")).fortress().connect(lb,q).chest(pb,30,102),Qb=(new Location$$module$js$nodes(a,171,T,"Across Second Bridge")).fortress().connect(qh,q);(new Location$$module$js$nodes(a,171,T,"Dead End Across Two Bridges ()")).fortress().connect(Qb,
q).connect(Qb,q).chest(ta,29,93);const rh=(new Location$$module$js$nodes(a,171,T,"Across Third Bridge")).fortress().connect(Qb,q),sh=(new Location$$module$js$nodes(a,171,T,"Exit Behind Iron Door")).fortress().connect(rh,va),th=(new Location$$module$js$nodes(a,172,T,"Boss")).fortress().connect(sh).boss(We),uh=(new Location$$module$js$nodes(a,172,T,"Tornel")).fortress().connect(th),Id=(new Location$$module$js$nodes(a,173,za,"Lower")).fortress().connect(uh).chest(qb,26,99).chest(Q,27,111),vh=(new Location$$module$js$nodes(a,
174,za,"Upper Loop")).fortress().connect(Id).chest(H,22,96);(new Location$$module$js$nodes(a,174,za,"Upper Loop Behind Wall (magic ring)")).fortress().connect(vh,va).chest(Q,23,107);const wh=(new Location$$module$js$nodes(a,175,za,"Upper Passage (toward Mado)")).fortress().connect(Id).chest(Q,27,84),Rb=(new Location$$module$js$nodes(a,176,K,"Initial Fork")).fortress(),xh=(new Location$$module$js$nodes(a,177,K,"Left Branch")).fortress().connect(Rb),Jd=(new Location$$module$js$nodes(a,178,K,"Main Area (right branch, over bridges)")).fortress().connect(Rb),
yh=(new Location$$module$js$nodes(a,179,K,"U-shaped Passage (between floors)")).fortress().connect(Jd),zh=(new Location$$module$js$nodes(a,180,K,"Main Area Lower (under bridge)")).fortress().connect(xh).connect(Jd).connect(yh),Kd=(new Location$$module$js$nodes(a,180,K,"Behind Iron Wall")).fortress().connect(zh,va),Ah=(new Location$$module$js$nodes(a,181,K,"Lower")).fortress().connect(Kd).chest(J,13,121).chest(J,14,122).chest(J,15,123).chest(Q,23,88).chest(M,24,110),Bh=(new Location$$module$js$nodes(a,
182,K,"Boss Corridor")).fortress().connect(Ah),Ch=(new Location$$module$js$nodes(a,182,K,"Boss")).fortress().connect(Bh,Cb).boss(Ye);(new Location$$module$js$nodes(a,182,K,"Behind Boss (stormBracelet)")).fortress().connect(Ch).chest(Ta,18);const Ld=(new Location$$module$js$nodes(a,183,K,"Exit Stairs")).fortress(),Dh=(new Location$$module$js$nodes(a,184,F,"Entrance Back (behind river)")).cave().connect(Ld).chest(I,13,90);(new Location$$module$js$nodes(a,184,F,"Entrance Front")).cave().connect(Dh,B).connectTo(Nb).connectTo(kb);
const Eh=(new Location$$module$js$nodes(a,185,za,"Boss")).fortress().connect(wh).boss(Xe);(new Location$$module$js$nodes(a,185,za,"Asina")).fortress().connect(Eh).connectTo(Rb);(new Location$$module$js$nodes(a,186,K,"Kensu")).fortress().connect(Kd).connectTo(Ld).trigger(ve,vb);(new Location$$module$js$nodes(a,187,qa,"House")).house().connect(Ka).trigger(xe,ua,vb);(new Location$$module$js$nodes(a,188,qa,"Inn")).shop().connect(Ka,Ab);(new Location$$module$js$nodes(a,190,qa,"Tool Shop")).shop(E,H,M,
I).connect(Ka,Ab);(new Location$$module$js$nodes(a,191,qa,"Tavern")).shop().connect(Ka);(new Location$$module$js$nodes(a,192,R,"Elder House")).house().connect(ja).trigger(qc);(new Location$$module$js$nodes(a,193,R,"Rabbit Hut")).house().connect(ja).trigger(wc,vc,ma);(new Location$$module$js$nodes(a,194,R,"Inn")).shop().connect(ja);(new Location$$module$js$nodes(a,195,R,"Tool Shop")).shop(E,H,M,I).connect(ja);(new Location$$module$js$nodes(a,196,R,"Armor Shop")).shop(Vd,Tb).connect(ja);(new Location$$module$js$nodes(a,
197,R,"Student House")).house().connect(ja).trigger(rc);(new Location$$module$js$nodes(a,198,xa,"Tavern")).house().connect(Ja);(new Location$$module$js$nodes(a,199,xa,"Pawn Shop")).shop().connect(Ja);(new Location$$module$js$nodes(a,200,xa,"Inn")).shop().connect(Ja);(new Location$$module$js$nodes(a,201,xa,"Armor Shop")).shop(Wd,Tb,Sd).connect(Ja);(new Location$$module$js$nodes(a,203,xa,"Tool Shop")).shop(E,H,M).connect(Ja);(new Location$$module$js$nodes(a,205,Ha,"Elder House")).house().from(db,ma).trigger(ke,
xb);(new Location$$module$js$nodes(a,206,Ha,"Mother's House")).house().from(db,ma).trigger(uc,ma).trigger(je,xb);(new Location$$module$js$nodes(a,207,Ha,"Tool Shop")).shop(E,H,I).from(db,ma);(new Location$$module$js$nodes(a,208,Ha,"Inn")).shop().from(db,ma);(new Location$$module$js$nodes(a,209,ya,"Inn")).shop().connect(cb);(new Location$$module$js$nodes(a,210,ya,"Tool Shop")).shop(M,I,ta).connect(cb);(new Location$$module$js$nodes(a,211,ya,"Armor Shop")).shop(Xb,Ub,Vb,nb).connect(cb);const Fh=(new Location$$module$js$nodes(a,
212,ya,"Queen's House")).house().from(cb,Rc).trigger(te,ua,kc);(new Location$$module$js$nodes(a,213,ab,"Nadare's")).house().connect(eb).connectTo(Nf).connectTo(Of).connectTo(Pf);(new Location$$module$js$nodes(a,214,A,"Fisherman's House")).house().connect(od).trigger(yb,fc,ub,Le);const Sb=(new Location$$module$js$nodes(a,215,A,"Palace Entrance")).house().connect(ra);(new Location$$module$js$nodes(a,216,A,"Fortune Teller Front")).house().connect(ra).trigger(yc,xc);(new Location$$module$js$nodes(a,216,
A,"Fortune Teller Back")).house().connect(tg).connect(td);(new Location$$module$js$nodes(a,217,A,"Pawn Shop")).shop().connect(ra);(new Location$$module$js$nodes(a,218,A,"Armor Shop")).shop(Xd,Xb,Ub).connect(ra);(new Location$$module$js$nodes(a,220,A,"Inn")).shop().connect(ra);(new Location$$module$js$nodes(a,221,A,"Tool Shop")).shop(E,ta,ob,M).connect(ra);(new Location$$module$js$nodes(a,222,A,"Palace Left")).house().connect(Sb);(new Location$$module$js$nodes(a,223,A,"Palace Throne Room")).house().connect(Sb).to(hb,
Ge).from(hb).trigger(xc).trigger(ne,yc,zc);(new Location$$module$js$nodes(a,224,A,"Palace Right")).house().connect(Sb);(new Location$$module$js$nodes(a,225,A,"Asina's Room")).house().connect(ud).trigger(Cc,Mc);(new Location$$module$js$nodes(a,226,ya,"Queen Downstairs")).house().connect(Fh).chest(Ra,13);(new Location$$module$js$nodes(a,227,fa,"Elder's House")).house().connect(ib).trigger(Fc);(new Location$$module$js$nodes(a,228,fa,"Shed")).house().connect(ib).to(Eg,gc);(new Location$$module$js$nodes(a,
229,fa,"Tool Shop")).shop(E,H,I,ta).connect(ib);(new Location$$module$js$nodes(a,231,fa,"Inn")).shop().connect(ib);const Gh=(new Location$$module$js$nodes(a,232,G,"Zombie Town House")).house().connect(Kb);(new Location$$module$js$nodes(a,233,G,"Zombie Town Basement")).house().connect(Gh).trigger(pe,Vc);(new Location$$module$js$nodes(a,235,S,"Tool Shop")).shop(E,H,I,M).connect(ba);(new Location$$module$js$nodes(a,236,S,"Stom's Hut")).house().connect(ba).trigger(Gc);(new Location$$module$js$nodes(a,
237,S,"Inn")).shop().connect(ba);(new Location$$module$js$nodes(a,238,S,"Armor Shop")).shop(Yd,Yb,Td,Wb).connect(ba);(new Location$$module$js$nodes(a,239,S,"Tavern")).house().connect(ba).trigger(Hc,Gc,Wa);(new Location$$module$js$nodes(a,240,S,"Pawn Shop")).shop().connect(ba);(new Location$$module$js$nodes(a,241,S,"Dance Hall")).house().connect(ba).trigger(Ic,Hc,Wa).trigger(se,Ic,jc);const Hh=(new Location$$module$js$nodes(a,242,ia,"Temple (pre-massacre)")).fortress().connect(Ea).from(Db,L,la).trigger(ue);
(new Location$$module$js$nodes(a,242,ia,"Temple (post-massacre)")).fortress().from(Hh,Jc).boss(Ue);(new Location$$module$js$nodes(a,243,ia,"Training Hall")).house().connect(Ea);(new Location$$module$js$nodes(a,244,ia,"Hospital")).house().connect(Ea);(new Location$$module$js$nodes(a,245,ia,"Armor Shop")).shop(Yb,nb,Wb).connect(Ea);(new Location$$module$js$nodes(a,246,ia,"Tool Shop")).shop(E,H,ob,Q).connect(Ea);(new Location$$module$js$nodes(a,247,ia,"Inn")).shop().connect(Ea);(new Location$$module$js$nodes(a,
248,U,"Inn")).shop().connect(Fa);(new Location$$module$js$nodes(a,249,U,"Tool Shop")).shop(H,M,pb,Q).connect(Fa);(new Location$$module$js$nodes(a,250,U,"Elder's House")).house().connect(Fa);(new Location$$module$js$nodes(a,251,U,"Pawn Shop")).house().connect(Fa);const Ih=[ja,Ia,Xc,Ba,Eb,Ca,Hb,id,Jb,ba,jb,Mb,Nb,Hd,Ob];for(const a of Ih)a.from(Db,Ce);return a},shuffle$$module$js$depgraph=(b,a,c,d,e)=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){const f=generate$$module$js$depgraph(d);d.check("Ps")&&
...
return Math.min(AM$$module$js$random*this.iy,RNMX$$module$js$random)}nextInt(b){return Math.floor(this.next()*b)}shuffle(b){for(let a=b.length;a;){const c=this.nextInt(a--);[b[a],b[c]]=[b[c],b[a]]}return b}}var module$js$random={};module$js$random.Random=Random$$module$js$random;const STATUS$$module$js$version="unstable",VERSION$$module$js$version="experimental",LABEL$$module$js$version="experimental 08eb6a3",HASH$$module$js$version="08eb6a39e107298b707c8617cf5d360d11d49e52",DATE$$module$js$version=new Date(1553799009E3);var module$js$version={};module$js$version.STATUS=STATUS$$module$js$version;module$js$version.VERSION=VERSION$$module$js$version;module$js$version.LABEL=LABEL$$module$js$version;module$js$version.HASH=HASH$$module$js$version;module$js$version.DATE=DATE$$module$js$version;var $jscompDefaultExport$$module$js$patch={apply(b,a){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){a.seed||(a.seed=parseSeed$$module$js$patch("").toString(16),window.location.hash+="&seed="+a.seed);a.flags?new FlagSet$$module$js$flagset(a.flags):new FlagSet$$module$js$flagset("Em Gt Mr Rlpt Sbk Sct Sm Tasd");for(const b in a)"false"===a[b]&&(a[b]=!1)})}};
...
(b,a)=>{a.check("Fs")?patchWords$$module$js$patch(b,216030,[0,5,10,15,25,40,65,105,170,275,445,600,700,800,900,1E3]):patchWords$$module$js$patch(b,216030,[0,1,2,4,8,16,30,50,100,200,300,400,500,600,700,800])},updateDifficultyScalingTables$$module$js$patch=(b,a,c)=>{const d=Array(48).fill(0).map((a,b)=>b);patchBytes$$module$js$patch(b,c.expand("DiffAtk"),d.map((a)=>Math.round(40+15*a/4)));patchBytes$$module$js$patch(b,c.expand("DiffDef"),d.map((a)=>8+2*a));patchBytes$$module$js$patch(b,c.expand("DiffHP"),
d.map((a)=>Math.min(255,48+Math.round(11*a/2))));const e=a.check("Hx")?.25:a.check("Ex")?2.5:1;patchBytes$$module$js$patch(b,c.expand("DiffExp"),d.map((a)=>{a=Math.floor(4*Math.pow(2,(16+9*a)/32)*e);return 128>a?a:Math.min(255,128+(a>>4))}));patchBytes$$module$js$patch(b,216E3,[0,1,3,5,7,9,12,10,16,0,1,3,4,6,9,8,12,16])},rescaleShops$$module$js$patch=(b,a,c)=>{const d=a.expand("BasePrices");var e=a.expand("InnPrices");const f={prices:138836,items:138792},g={prices:138704,items:138660},h=Array(48).fill(0).map((a,
...
c(39,20)},BASE_PRICES$$module$js$patch={13:6,14:22,15:325,16:475,17:1150,18:2750,19:2750,21:8,22:14,23:125,24:425,25:1400,26:2900,29:25,30:30,31:45,32:40,33:36,34:200,35:150,36:80,38:300},rescaleMonsters$$module$js$patch=(b,a)=>{for(var c of SCALED_MONSTERS$$module$js$patch.keys())for(const b in a.objects){if(SCALED_MONSTERS$$module$js$patch.has(b))return;a.objects[c].objectDataBase==a.objects[b].objectDataBase&&(SCALED_MONSTERS$$module$js$patch[b]=SCALED_MONSTERS$$module$js$patch[c])}for(const d of SCALED_MONSTERS$$module$js$patch){const [e,
{sdef:f,swrd:g,hits:h,satk:k,dgld:m,sexp:p}]=d;b=a.objects[e].objectData;c=[87,94,104,125,136,151,155,158].includes(e)?1:0;b[2]|=128;b[6]=h;b[7]=k;b[8]=f|g<<4;b[9]=b[9]&224|c;b[16]=b[16]&15|m<<4;b[17]=p}a.objects[125].objectData[16]|=8;a.writeObjectData()},shuffleMonsters$$module$js$patch=(b,a,c,d)=>{b=new MonsterPool$$module$js$patch({});for(const c of a.locations)c&&b.populate(c);b.shuffle(c);a.writeNpcData()},identifyKeyItemsForDifficultyBuffs$$module$js$patch=(b)=>{for(const a of b.itemGets){const c=
ITEMS$$module$js$patch.get(a.item);if(c&&c.key){if(!a.table)throw Error(`No table for ${c.name}`);if(255==a.table[a.table.length-1])a.table[a.table.length-1]=254;else throw Error(`Expected FF at end of ItemGet table: ${a.id.toString(16)}: ${Array.from(a.table).map((a)=>a.toString(16).padStart(2,0)).join(" ")}`);a.write(b)}}},SCALED_MONSTERS$$module$js$patch=new Map([[63,"p","Sorceror shot",,,,19,,,],[75,"m","wraith??",2,,2,22,4,61],[79,"m","wraith",1,,2,20,4,61],[80,"m","Blue Slime",,,1,16,2,32],
[81,"m","Weretiger",,,1,21,4,40],[82,"m","Green Jelly",4,,3,16,4,36],[83,"m","Red Slime",6,,4,16,4,48],[84,"m","Rock Golem",6,,11,24,6,85],[85,"m","Blue Bat",,,,4,,32],[86,"m","Green Wyvern",4,,4,24,6,52],[87,"b","Vampire",3,,13,18,,,],[88,"m","Orc",3,,4,21,4,57],[89,"m","Red Flying Swamp Insect",3,,1,21,4,57],[90,"m","Blue Mushroom",2,,1,21,4,44],[91,"m","Swamp Tomato",3,,2,35,4,52],[92,"m","Flying Meadow Insect",3,,3,23,4,81],[93,"m","Swamp Plant",,,,,,36],[94,"b","Insect",,1,8,6,,,],[95,"m","Large Blue Slime",
...
"Sabera 2",5,7,21,27,,,],[145,"m","Tarantula",3,,3,21,6,73],[146,"m","Skeleton",,,4,30,6,69],[147,"b","Mado 2",4,8,11,25,,,],[148,"m","Purple Giant Eye",4,,10,23,6,102],[149,"m","Black Knight (w/ flail)",3,,7,26,6,89],[150,"m","Scorpion",3,,5,29,2,73],[151,"b","Karmine",4,,14,26,,,],[152,"m","Sandman/blob",3,,5,36,6,98],[153,"m","Mummy",5,,19,36,6,110],[154,"m","Tomb Guardian",7,,60,37,6,106],[155,"b","Draygon",5,6,16,41,,,],[158,"b","Draygon 2",7,6,28,40,,,],[160,"m","Ground Sentry (1)",4,,12,26,
,],[161,"m","Tower Defense Mech (2)",5,,16,36,,],[162,"m","Tower Sentinel",,,2,,,],[163,"m","Air Sentry",3,,4,26,,],[164,"b","Dyna",6,5,16,,,,],[165,"b","Vampire 2",2,,6,27,,,],[180,"b","dyna pod",15,,255,26,,,],[184,"p","dyna counter",,,,26,,,],[185,"p","dyna laser",,,,26,,,],[186,"p","dyna bubble",,,,36,,,],[188,"m","vamp2 bat",,,,16,,40],[191,"p","draygon2 fireball",,,,26,,,],[193,"m","vamp1 bat",,,,16,,40],[195,"p","giant insect spit",,,,35,,,],[196,"m","summoned insect",4,,2,42,,98],[197,"p",
"kelby1 rock",,,,22,,,],[198,"p","sabera1 balls",,,,19,,,],[199,"p","kelby2 fireballs",,,,11,,,],[200,"p","sabera2 fire",,,1,6,,,],[201,"p","sabera2 balls",,,,17,,,],[202,"p","karmine balls",,,,25,,,],[203,"p","sun/moon statue fireballs",,,,39,,,],[204,"p","draygon1 lightning",,,,37,,,],[205,"p","draygon2 laser",,,,36,,,],[206,"p","draygon2 breath",,,,36,,,],[224,"p","evil bomber bird bomb",,,,2,,,],[226,"p","summoned insect bomb",,,,47,,,],[227,"p","paralysis beam",,,,23,,,],[228,"p","stone gaze",
