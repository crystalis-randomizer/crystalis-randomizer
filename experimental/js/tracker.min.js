...
h=crc32$$module$js$crc32(b.toString(16).padStart(8,"0")+String(c.filterOptional()))>>>0;h=new Random$$module$js$random(h);c=c.filterRandom(h);deterministicPreParse$$module$js$pass$deterministic(a.subarray(16));const k=new Rom$$module$js$rom(a);k.flags.defrag();"object"==typeof window&&(window.rom=k);k.spoiler=new Spoiler$$module$js$rom$spoiler(k);e&&(e.spoiler=k.spoiler);deterministic$$module$js$pass$deterministic(k,c);module$js$pass$standardmapedits.standardMapEdits(k,module$js$pass$standardmapedits.standardMapEdits.generateOptions(c,
h));toggleMaps$$module$js$pass$togglemaps(k,c,h);k.scalingLevels=48;c.shuffleShops()&&shuffleShops$$module$js$patch(k,c,h);shuffleGoa$$module$js$pass$shufflegoa(k,h);randomizeWalls$$module$js$patch(k,c,h);crumblingPlatforms$$module$js$pass$crumblingplatforms(k,h);c.nerfWildWarp()&&k.wildWarp.locations.fill(0);c.randomizeWildWarp()&&shuffleWildWarp$$module$js$patch(k,c,h);c.randomizeThunderTeleport()&&randomizeThunderWarp$$module$js$pass$randomizethunderwarp(k,h);rescaleMonsters$$module$js$pass$rescalemonsters(k,
c,h);unidentifiedItems$$module$js$pass$unidentifieditems(k,c,h);shuffleTrades$$module$js$pass$shuffletrades(k,c,h);c.randomizeMaps()&&shuffleMazes$$module$js$pass$shufflemazes(k,c,h);c.shuffleMimics()&&shuffleMimics$$module$js$pass$shufflemimics(k,c,h);c.shuffleMonsters()&&shuffleMonsters$$module$js$pass$shufflemonsters(k,c,h);var m=new World$$module$js$logic$world(k,c);if(m=yield(new Graph$$module$js$logic$graph([m.getLocationList()])).shuffle(c,h,void 0,f,k.spoiler))for(var l of m){const [a,b]=
l;k.slots[a&255]=b&255}else return[a,-1];c.shuffleShops()&&rescaleShops$$module$js$patch(k,c.bargainHunting()?h:void 0);c.buffMedicalHerb()&&(k.items.MedicalHerb.value=80,k.items.FruitOfPower.value=56);c.storyMode()&&storyMode$$module$js$patch(k);c.blackoutMode()&&blackoutMode$$module$js$patch(k);misc$$module$js$patch(k,c,h);fixDialog$$module$js$pass$fixdialog(k);c.buffDyna()&&buffDyna$$module$js$patch(k,c);c.trainer()&&(k.wildWarp.locations=[10,26,53,72,109,110,140,170,172,176,182,159,166,88,92,
0]);c.randomizeMusic("early")&&shuffleMusic$$module$js$patch(k,c,h);c.shuffleTilePalettes("early")&&shufflePalettes$$module$js$pass$shufflepalettes(k,c,h);updateTablesPreCommit$$module$js$patch(k,c);h.shuffle(k.randomNumbers.values);k.messages.compress();l=a.slice(16);k.modules.push(yield g("early"));k.writeData(l);k.modules.pop();k.modules.push(yield g("late"));l=stampVersionSeedAndHash$$module$js$patch(a,b,c,l);c.randomizeMusic("late")&&shuffleMusic$$module$js$patch(k,c,h);c.noMusic("late")&&noMusic$$module$js$patch(k);
c.shuffleTilePalettes("late")&&shufflePalettes$$module$js$pass$shufflepalettes(k,c,h);k.writeData();EXPAND_PRG$$module$js$patch&&(h=a.subarray(16),h.subarray(507904,524288).set(h.subarray(245760,262144)));return[a,l]})}
