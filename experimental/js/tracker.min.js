...
var $jscompDefaultExport$$module$js$patch={apply(a,b,c){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){b.seed||(b.seed=parseSeed$$module$js$patch("").toString(16),window.location.hash+="&seed="+b.seed);var d=b.flags?new FlagSet$$module$js$flagset(String(b.flags)):new FlagSet$$module$js$flagset("Em Gt Mr Rlpt Sbk Sct Sm Tasd");for(const a in b)"false"===b[a]&&(b[a]=!1);[d]=yield shuffle$$module$js$patch(a,parseSeed$$module$js$patch(String(b.seed)),d,new FetchReader$$module$js$fetchreader(c));
return d})}};function parseSeed$$module$js$patch(a){return a?/^[0-9a-f]{1,8}$/i.test(a)?Number.parseInt(a,16):crc32$$module$js$crc32(a):Random$$module$js$random.newSeed()}
...
_DISABLE_TRIGGER_SKIP:!0,_DISABLE_WILD_WARP:!1,_DISPLAY_DIFFICULTY:!0,_EXTRA_PITY_MP:!0,_FIX_COIN_SPRITES:!0,_FIX_OPEL_STATUE:!0,_FIX_SHAKING:!0,_FIX_VAMPIRE:!0,_HARDCORE_MODE:c.hardcoreMode(),_LEATHER_BOOTS_GIVE_SPEED:c.leatherBootsGiveSpeed(),_NERF_FLIGHT:!0,_NERF_WILD_WARP:c.nerfWildWarp(),_NEVER_DIE:c.neverDie(),_NORMALIZE_SHOP_PRICES:!0,_PITY_HP_AND_MP:!0,_PROGRESSIVE_BRACELET:!0,_RABBIT_BOOTS_CHARGE_WHILE_WALKING:c.rabbitBootsChargeWhileWalking(),_REQUIRE_HEALED_DOLPHIN_TO_RIDE:c.requireHealedDolphinToRide(),
_REVERSIBLE_SWAN_GATE:!0,_SAHARA_RABBITS_REQUIRE_TELEPATHY:c.saharaRabbitsRequireTelepathy(),_SIMPLIFY_INVISIBLE_CHESTS:!0,_TELEPORT_ON_THUNDER_SWORD:c.teleportOnThunderSword(),_UNIDENTIFIED_ITEMS:c.unidentifiedItems()},l=new Assembler$$module$js$6502;var m=Object.keys(k).filter((a)=>k[a]).map((a)=>`define ${a} 1\n`).join("");l.assemble(m,"flags.s");yield g("preshuffle.s");h=new Random$$module$js$random(h);m=new Rom$$module$js$rom(a);"object"==typeof window&&(window.rom=m);m.spoiler=new Spoiler$$module$js$rom$spoiler(m);
e&&(e.spoiler=m.spoiler);deterministic$$module$js$pass$deterministic(m,c);yield g("postparse.s");m.scalingLevels=48;m.uniqueItemTableAddress=l.expand("KeyItemData");c.shuffleShops()&&shuffleShops$$module$js$patch(m,c,h);randomizeWalls$$module$js$patch(m,c,h);c.randomizeWildWarp()&&shuffleWildWarp$$module$js$patch(m,c,h);rescaleMonsters$$module$js$patch(m,c,h);unidentifiedItems$$module$js$pass$unidentifieditems(m,c,h);shuffleTrades$$module$js$pass$shuffletrades(m,c,h);c.randomizeMaps()&&shuffleMazes$$module$js$pass$shufflemazes(m,
h);const n=World$$module$js$graph$world.build(m,c),p=yield(new AssumedFill$$module$js$graph$shuffle(m,c)).shuffle(n.graph,h,f);if(p)n.traverse(n.graph,p),update$$module$js$rom$slots(m,p.slots);else return[a,-1];rescaleShops$$module$js$patch(m,l,c.bargainHunting()?h:void 0);normalizeSwords$$module$js$patch(m,c,h);c.shuffleMonsters()&&shuffleMonsters$$module$js$patch(m,c,h);identifyKeyItemsForDifficultyBuffs$$module$js$patch(m);c.doubleBuffMedicalHerb()?(a[115996]*=2,a[115962]*=3):c.buffMedicalHerb()&&
(a[115996]+=16,a[115962]*=2);c.storyMode()&&storyMode$$module$js$patch(m);shuffleMusic$$module$js$patch(m,c,h);shufflePalettes$$module$js$pass$shufflepalettes(m,c,h);c.blackoutMode()&&blackoutMode$$module$js$patch(m);misc$$module$js$patch(m,c,h);fixDialog$$module$js$pass$fixdialog(m);c.buffDyna()&&buffDyna$$module$js$patch(m,c);yield m.writeData();buffDyna$$module$js$patch(m,c);h=yield postParsedShuffle$$module$js$patch(a,h,b,c,l,g);EXPAND_PRG$$module$js$patch&&(m=a.subarray(16),m.subarray(507904,
524288).set(m.subarray(245760,262144)));return[a,h]})}function postParsedShuffle$$module$js$patch(a,b,c,d,e,f){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){yield f("postshuffle.s");updateDifficultyScalingTables$$module$js$patch(a,d,e);updateCoinDrops$$module$js$patch(a,d);shuffleRandomNumbers$$module$js$patch(a,b);return stampVersionSeedAndHash$$module$js$patch(a,c,d)})}
