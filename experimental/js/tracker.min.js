...
a);k.patchRom(b)});var p=Object.keys(h).filter((a)=>h[a]).map((a)=>`define ${a} 1\n`).join("");k.assemble(p,"flags.s");yield n("preshuffle.s");g=new Random$$module$js$random(g);yield shuffle2$$module$js$depgraph(b,g,e,c,f);rescaleShops$$module$js$patch(b,k,g);p=new Rom$$module$js$rom(b);rescaleMonsters$$module$js$patch(b,p);c.check("Mr")&&shuffleMonsters$$module$js$patch(b,p,g);identifyKeyItemsForDifficultyBuffs$$module$js$patch(p);c.check("Em")?(b[115996]*=2,b[115962]*=3):c.check("Hm")||(b[115996]+=
8,b[115962]*=2);yield n("postshuffle.s");updateDifficultyScalingTables$$module$js$patch(b,c,k);updateCoinDrops$$module$js$patch(b,c);return stampVersionSeedAndHash$$module$js$patch(b,a,c)}),stampVersionSeedAndHash$$module$js$patch=(b,a,c)=>{const d=crc32$$module$js$crc32(b),e=d.toString(16).padStart(8,0).toUpperCase(),f="unstable"==STATUS$$module$js$version?HASH$$module$js$version.substring(0,7).padStart(7,0).toUpperCase()+"     ":VERSION$$module$js$version.substring(0,12).padEnd(12," ");a=a.toString(16).padStart(8,
