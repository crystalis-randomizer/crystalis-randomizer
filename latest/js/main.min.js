...
RNMX$$module$random)}nextInt(b){return Math.floor(this.next()*b)}shuffle(b){for(let a=b.length;a;){const c=this.nextInt(a--);[b[a],b[c]]=[b[c],b[a]]}}}var module$random={};module$random.Random=Random$$module$random;const STATUS$$module$version="unstable",VERSION$$module$version="latest",LABEL$$module$version="latest ()",HASH$$module$version="5ab9dcbdf42a39fb5a189447ad43057401ffeb24",DATE$$module$version=new Date(1551124048E3);var module$version={};module$version.STATUS=STATUS$$module$version;module$version.VERSION=VERSION$$module$version;module$version.LABEL=LABEL$$module$version;module$version.HASH=HASH$$module$version;module$version.DATE=DATE$$module$version;var $jscompDefaultExport$$module$patch={apply(b,a){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){let c;a.seed||(a.seed=parseSeed$$module$patch("").toString(16),window.location.hash+="&seed="+a.seed);c=a.flags?new FlagSet$$module$flagset(a.flags):new FlagSet$$module$flagset("Em Gt Mr Rlpt Sbk Sct Sm Tasd");for(const b in a)"false"===a[b]&&(a[b]=!1);yield shuffle$$module$patch(b,parseSeed$$module$patch(a.seed),c)})}};
...
initVersion$$module$main()}},initVersion$$module$main=()=>{"latest"!==HASH$$module$version&&(document.getElementById("version").textContent=`Current version: ${LABEL$$module$version} (${DATE$$module$version.toDateString()})`);document.body.classList.add("js-works");document.body.classList.remove("js-broken");"rc"==STATUS$$module$version?document.body.classList.add("release-candidate"):"unstable"!=STATUS$$module$version&&document.body.classList.add("versioned")},addSeedListeners$$module$main=()=>{const b=
document.getElementById("seed"),a=()=>{seed$$module$main=b.value;updateDom$$module$main()};b.addEventListener("keyup",a);b.addEventListener("change",a)},initializeStateFromHash$$module$main=(b)=>{for(const a of location.hash.substring(1).split("&")){let [c,d]=a.split("=");d=decodeURIComponent(d);"flags"===c&&(flags$$module$main=new FlagSet$$module$flagset(d),b&&(document.getElementById("preset").value="--",document.getElementById("flagstring").value=d));"seed"===c&&(seed$$module$main=decodeURIComponent(d));
"race"===c&&document.body.classList.add("race")}},click$$module$main=(b)=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){let a=b.target;for(;a;){if("H1"===a.tagName&&a.parentElement.classList.contains("expandable")){a.parentElement.classList.toggle("expanded");break}else if("preset-apply"===a.id){flags$$module$main=new FlagSet$$module$flagset(document.getElementById("flagstring").value);updateDom$$module$main();break}else if("new-seed"===a.id)seed$$module$main=Math.floor(4294967296*Math.random()).toString(16),
...
flags$$module$main.check(b.dataset.flag);b=String(flags$$module$main).replace(/ /g,"");document.getElementById("seed").value=seed$$module$main||"";var a=["#flags=",b];seed$$module$main&&a.push("&seed=",encodeURIComponent(seed$$module$main));history.replaceState({flags:flags$$module$main.flags,seed:seed$$module$main},"",String(window.location).replace(/#.*/,"")+a.join(""));if("unstable"!=STATUS$$module$version){a=seed$$module$main||Math.floor(4294967296*Math.random()).toString(16);const c=VERSION$$module$version;
document.getElementById("race").href=`/${c}/race#flags=${b}&seed=${a}`}updateRaceDom$$module$main()},updateRaceDom$$module$main=()=>{const b=String(flags$$module$main);document.body.classList.toggle("spoiled",flags$$module$main.check("Ds"));document.body.classList.toggle("debug-mode",/D/.test(b));document.getElementById("flagstring-out").textContent=b;document.getElementById("track-url").href=`track#flags=${b.replace(/ /g,"")}`},loadRomFromStorage$$module$main=()=>{const b=window.localStorage.getItem("name"),
a=window.localStorage.getItem("rom"),c=document.getElementById("pick-file"),d=()=>{document.body.classList.add("rom-uploaded");document.body.classList.toggle("rom-broken",crc32$$module$crc32(rom$$module$main)!=EXPECTED_CRC32$$module$rom)};b&&a&&(romName$$module$main=b,rom$$module$main=Uint8Array.from(Array(a.length/2).fill(0).map((b,c)=>Number.parseInt(a[2*c]+a[2*c+1],16))),d());c.addEventListener("change",()=>{const a=c.files[0],b=new FileReader;b.addEventListener("loadend",()=>{const c=new Uint8Array(b.result),
e=Array.from(c,(a)=>a.toString(16).padStart(2,0)).join("");window.localStorage.setItem("rom",e);window.localStorage.setItem("name",a.name);rom$$module$main=c.slice(0,393232);d();romName$$module$main=a.name});b.readAsArrayBuffer(a)})},download$$module$main=(b,a)=>{const c=document.createElement("a");document.body.appendChild(c);c.style="display: none";b=new Blob([b],{type:"octet/stream"});b=window.URL.createObjectURL(b);c.href=b;c.download=a;c.click();window.URL.revokeObjectURL(b);c.remove()};main$$module$main();
var module$main={};
}).call(this)
