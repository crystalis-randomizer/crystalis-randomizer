...
class Graph$$module$graph{constructor(a){this.rom=a;this.nodes=[]}traverse({wanted:a,dfs:b=!1}={}){const c=new Deque$$module$util,d=new Map,e=new Map;for(var f of this.nodes)for(var g of f.edges()){var h=g.join(" ");for(let a=1;a<g.length;a++){const c=g[a];e.has(c)||e.set(c,new Map);e.get(c).set(h,g)}1===g.length&&(h=g[0],d.has(h)||(c.push(h),d.set(h,g)))}a=new Set((a||this.nodes).map((a)=>a instanceof Node$$module$graph?a.uid:a));for(f=new Map;a.size&&c.length;){g=b?c.pop():c.shift();a.delete(g);
a:for(const a of(e.get(g)||f).values())if(g=a[0],!d.has(g)){for(h=1;h<a.length;h++)if(!d.has(a[h]))continue a;d.set(g,a);c.push(g)}}return{path:[...d.values()].map(([a,...c])=>{const b=(a)=>[this.nodes[a]];return[a,[...b(a)," (",c.map((a)=>b(a).join("").replace(/\s+\(.*\)/,"")).join(", "),")"].join("")]}),seen:d,win:!a.size}}}
...
m=h(d("Assume statue glitch",a.assumeStatueGlitch())),n=d("Allow statue glitch",!a.disableStatueGlitch()),p=h(d("Assume teleport skip",a.assumeTeleportSkip())),r=d("Allow teleport skip",!a.disableTeleportSkip()),q=h(d("Assume rabbit skip",a.assumeRabbitSkip())),x=h(d("Allow rabbit skip",!a.disableRabbitSkip())),z=h(d("Assume sword charge glitch",a.assumeSwordChargeGlitch())),V=d("Allow sword charge glitch",!a.disableSwordChargeGlitch()),va=h(d("Assume wild warp",a.assumeWildWarp())),oa=d("Allow wild warp",
...
"Talked to Zebu in Shyron")).get(ic),Nc=new Trigger$$module$nodes(c,"Shyron Massacre"),Ae=(new Trigger$$module$nodes(c,"Saved Kensu")).get(B),Be=(new Trigger$$module$nodes(c,"Talked to Deo")).get(ie),Ce=(new Trigger$$module$nodes(c,"Talked to Akahana's Friend")).get(ge),De=(new Trigger$$module$nodes(c,"Get Bow of Truth")).get(rc),Ee=(new Trigger$$module$nodes(c,"Forged Crystalis")).get(pb),$a=(new Condition$$module$nodes(c,"Sword charge glitch")).option(z,V),Fe=(new Condition$$module$nodes(c,"Rabbit skip")).option(q,
x),Ge=(new Condition$$module$nodes(c,"Teleport skip")).option(p,r),ab=(new Condition$$module$nodes(c,"Statue glitch")).option(m,n),He=(new Condition$$module$nodes(c,"Wild warp")).option(va,oa),bb=(new Condition$$module$nodes(c,"Any level 2 sword")).option(P,Ia).option(P,Ja).option(Q,Sa).option(Q,Ta).option(L,Ua).option(L,Va).option(J,Wa).option(J,Xa),v=(new Condition$$module$nodes(c,"Destroy stone")).option(P,Ra).option(P,Ia).option(P,Ja).option($a,P,bb),w=(new Condition$$module$nodes(c,"Destroy ice")).option(Q,
...
B).to(Ea),Ff=(new Location$$module$nodes(c,33,C,"Tornel Dead End")).overworld().trigger(qe,Ia,Ja).chest(W,23,105),dd=(new Location$$module$nodes(c,34,C,"Tunnel 2a (fork at start)")).cave().connect(Ea);(new Location$$module$nodes(c,34,C,"Tunnel 2b (left branch to dead end)")).cave().connect(dd,w).connectTo(Ef);const Gf=(new Location$$module$nodes(c,34,C,"Tunnel 2c (right branch to upper)")).cave().connect(dd,w),Hf=(new Location$$module$nodes(c,35,C,"Tunnel 3a (tunnel to upper, with herb chest)")).cave().connect(Gf).chest(E,
23,82),If=(new Location$$module$nodes(c,35,C,"Tunnel 3b (tunnel to upper, branch below ice)")).cave().connect(Hf,w).connectTo(cd),ed=(new Location$$module$nodes(c,36,C,"Tunnel 4a (branch to upper or Tornel)")).cave().connect(If);(new Location$$module$nodes(c,36,C,"Tunnel 4b (out to upper)")).cave().connect(ed,w).connectTo(cd);const Jf=(new Location$$module$nodes(c,37,C,"Tunnel 5 (tiny connector)")).cave().connect(ed),Kf=(new Location$$module$nodes(c,38,C,"Tunnel 6a (exit to Tornel, above ice)")).cave().connect(Jf);
(new Location$$module$nodes(c,38,C,"Tunnel 6b (exit to Tornel, below ice)")).cave().connect(Kf,w).connectTo(Ff);const Lf=(new Location$$module$nodes(c,39,C,"Tunnel 7a (tornado bracelet, lower)")).cave().connect(Df),Mf=(new Location$$module$nodes(c,39,C,"Tunnel 7b (tornado bracelet, middle)")).cave().connect(Lf,w);(new Location$$module$nodes(c,39,C,"Tunnel 7c (tornado bracelet, upper)")).cave().connect(Mf,w).chest(Ja,25);const hb=(new Location$$module$nodes(c,40,u,"Entrance")).overworld().connect(bd,
Le,Ke),ib=(new Location$$module$nodes(c,40,u,"Upper")).overworld().from(hb,B).to(hb),Ib=(new Location$$module$nodes(c,40,u,"Summit (boss)")).overworld().from(ib,B).to(ib).boss(bf),fd=(new Location$$module$nodes(c,41,u,"Connector")).overworld(),Jb=(new Location$$module$nodes(c,41,u,"Middle Left")).overworld(),Nf=(new Location$$module$nodes(c,41,u,"Middle Right")).overworld().from(Jb,Pc).to(Jb),gd=(new Location$$module$nodes(c,42,u,"Tunnel 2a (from entrance to connector)")).cave().connectTo(fd);(new Location$$module$nodes(c,
42,u,"Tunnel 2b (under bridge, to antidote)")).cave().connect(gd,w).chest(K,23,94);const hd=(new Location$$module$nodes(c,43,u,"Tunnel 3a (branch after connector)")).cave().connect(fd),Of=(new Location$$module$nodes(c,43,u,"Tunnel 3b (right branch)")).cave().connect(hd,w),Pf=(new Location$$module$nodes(c,43,u,"Tunnel 3c (upper branch)")).cave().connect(hd,w);(new Location$$module$nodes(c,44,u,"Tunnel 4 (over bridge, to middle)")).cave().connect(Pf).connectTo(Jb);const id=(new Location$$module$nodes(c,
45,u,"Tunnel 5a (E-shaped, from right branch)")).cave().connect(Of).connectTo(Nf);(new Location$$module$nodes(c,45,u,"Tunnel 5b (dead-end with herb)")).cave().connect(id,w).chest(E,22,83);const Qf=(new Location$$module$nodes(c,46,u,"Tunne; 6a (S-shaped hall, right)")).cave().connect(id),Rf=(new Location$$module$nodes(c,46,u,"Tunne; 6b (S-shaped hall, middle)")).cave().connect(Qf,w),Sf=(new Location$$module$nodes(c,46,u,"Tunnel 6c (S-shaped hall, left)")).cave().connect(Rf,w),jd=(new Location$$module$nodes(c,
47,u,"Prison (hallway)")).cave().connect(ib),Tf=(new Location$$module$nodes(c,48,u,"Left Cell (shopkeepers)")).cave().from(jd,w);(new Location$$module$nodes(c,49,u,"Left Cell 2 (back, with prison key)")).cave().from(Tf,w).chest(xb,13);const Uf=(new Location$$module$nodes(c,50,u,"Right Cell (villagers)")).cave().from(jd,w),Vf=(new Location$$module$nodes(c,51,u,"Tunnel 8 (behind right cell, toward summit)")).cave().from(Uf,w);(new Location$$module$nodes(c,52,u,"Tunnel 9 (connector to summit)")).cave().connect(Vf).connectTo(Ib);
const Wf=(new Location$$module$nodes(c,53,u,"Summit Cave (front)")).cave().from(Ib,xb).to(Ib),Xf=(new Location$$module$nodes(c,53,u,"Summit Cave (behind ice)")).cave().connect(Wf,w).trigger(re);(new Location$$module$nodes(c,56,u,"Tunnel 1 (leads from main entrance)")).cave().connect(hb).connectTo(gd);(new Location$$module$nodes(c,57,u,"Tunnel 7 (to upper)")).cave().connect(Sf).connectTo(ib);const Yf=(new Location$$module$nodes(c,60,db,"Inn")).shop(),Zf=(new Location$$module$nodes(c,61,db,"Tool Shop")).shop(E,
...
Slot$$module$nodes&&k.slots){var l=k.slotType[0];(h[l]=h[l]||[]).push(k)}k=[];l=[];var m={};for(var n of d.get("S")){var p=[];for(var r of n){m[r]=!0;for(var q of h[r])p.push(q),l.push([q,k.length])}k.push(p)}for(var x of"bc")if(m[x])for(n=h[x],r=n.map((a)=>[a.item,a.itemIndex]),b.shuffle(r),q=0;q<n.length;q++)n[q].set(r[q][0],r[q][1]);p=(a)=>g.find((b)=>b.vanillaItemName===a);h=p("Sword of Wind");x=p("Sword of Fire");n=p("Ball of Wind");r=p("Ball of Fire");q=p("Statue of Onyx");p=p("Gas Mask");m.k&&
b.nextInt(2)&&(x.swap(h),r.swap(n));m.k&&!b.nextInt(3)&&q.swap(p);m=[1,1,1,1,2,2,2,3,3,3,5,5,8,13];h=[];e&&e.addTasks(10);for(x=0;1E3>x;x++){e&&0===x%100&&(yield new Promise(requestAnimationFrame),e.addCompleted(1));b.shuffle(l);for(var z of k)b.shuffle(z);n=k.map(()=>-1);r=m[b.nextInt(m.length)];q=[];p=(a,b)=>a.itemIndex===b.itemIndex||a.isMimic()&&!b.canHoldMimic()||b.isMimic()&&!a.canHoldMimic()||a.needsChest()&&!b.isChest()||b.needsChest()&&!a.isChest()?!1:!0;for(let a=0;a<r;a++){const [b,c]=
l[a],d=k[c][++n[c]];p(b,d)&&(b.swap(d),q.push([b,d]))}if(!q.length)continue;const {win:a,path:c}=f.traverse({wanted:g,dfs:!1});if(a)h=c;else for(;q.length;){const [a,b]=q.pop();a.swap(b)}}a&&f.write();if(c){c.items=[];c.route=[];for(const a of h){const [b,d]=a;c.route.push(d);z=f.nodes[b];z instanceof Slot$$module$nodes&&!z.isFixed()&&null!=z.slotName&&(k=z.slotName,0>k.indexOf(z.vanillaItemName)&&(k+=` (normally ${z.vanillaItemName})`),c.items.push({itemIndex:z.itemIndex,itemName:z.item.name,origName:z.vanillaItemName,
...
{for(const c of a)p[c]=b});for(var r of m)n.set(r.item,r);const q=new Set,x=(a,b)=>{if(null==p[a.slotType?a.slotType[0]:"c"])return a===b;if(b.isMimic()&&!a.canHoldMimic()||"unique"!==b.item.inventoryRow&&a.requiresUnique||"armor"===b.item.inventoryRow&&!a.isChest())return!1;if(f.guaranteeSword()){if(0===a.slotIndex)return 4>b.item.id;if(4>b.item.id&&(q.add(b),4===q.size))return 0===a.slotIndex}return p[a.slotType?a.slotType[0]:"c"]===p[b.slotType?b.slotType[0]:"c"]};r=b.assumedFill(d,(a,b)=>x(a,
n.get(b)));if(null==r){g&&g.endState.key++;return!1;throw Error("Could not fill!");}const z=new Map;for(var V=0;V<r.length;V++){var va=r[V];if(null!=va){var oa=n.get(b.item(va));va=l.get(oa);l.delete(oa);oa=b.location(V);z.set(oa,va);m.delete(oa)}}V=(a,b,c)=>{c=void 0===c?"":c;for(const c of m)if(x(c,a))return z.set(c,b),m.delete(c),l.delete(a),!0;g&&("traps"===c?g.endState.traps++:"chest"===c?g.endState.chest++:g.endState.misc++);return!1};for(var F of l){const [a,b]=F;if(a.isMimic()&&!V(a,b,"traps"))return!1}for(const a of l){const [b,
c]=a;if(b.needsChest()&&!V(b,c,"chest"))return!1}for(const a of l){const [b,c]=a;if(!V(b,c))return!1}for(const a of z){const [b,c]=a;b.set(...c)}const {win:H,path:Qa}=a.traverse({wanted:k,dfs:!1});if(!H)return g&&g.endState.no_win++,!1;c&&a.write();if(!e)return!0;g&&(g.endState.success++,g.analyze(a,b,r));e.items=[];e.route=[];for(const b of Qa){const [c,d]=b;e.route.push(d);k=a.nodes[c];k instanceof Slot$$module$nodes&&!k.isFixed()&&(F=k.slotName,0>F.indexOf(k.vanillaItemName)&&(F+=` (normally ${k.vanillaItemName})`),
...
missing:r,words:q,chains:x}=g.shift();if(0>=p)break;h+=n.length+3;k=a.length;l=new Set;for(const a of q){m=b[a];for(const a of[m.mid,...d.get(m.mid)||[]])l.add(a)}a.push({bytes:128>k?[k+128]:[5,k-128],mids:l,str:n});for(const a of q)for(k=0;k<=x;k++){l=b[a+k];m=l.bytes-(k?0:r);for(const a of l.suffixes)a.saving-=m-l.used,e.add(a.str);l.used=m}if(128===a.length){for(const a of c.values())a.saving-=a.words.size;g.sort(f);e.clear()}}return a}write(a){const b=this;return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){function c(b,
...
0;l<p.length;l++)p[l]=d[p[l]];for(var r of a){d=this.tilesets[(r&127)>>>2];for(var q of b)for(n=0;n<q.length-1;n++){p=f(q[n]);l=f(q[n+1]);for(m=0;4>m;m++)d.tiles[m][p]=d.tiles[m][l];d.attrs[p]=d.attrs[l];if(32>l&&d.alternates[l]!==l){if(32<=p)throw Error(`Cannot unflag: ${r} ${p} ${l} ${d.alternates[l]}`);d.alternates[p]=d.alternates[l]}}for(var x of e){const [a,b]=x;d.alternates[a]=b}}for(const a of c){r=this.tileEffects[a-179];for(const a of b)for(q=0;q<a.length-1;q++)x=f(a[q]),d=f(a[q+1]),r.effects[x]=
...
f);b.remove();a(c)});d.readAsArrayBuffer(c)})})}const EXPECTED_CRC32$$module$rom=466849842,ADJUSTMENTS$$module$rom=new Map([[118533,72],[119569,160],[119570,0],[118777,224],[182928,0],[193907,0],[195300,0]]);var module$rom={};module$rom.Rom=Rom$$module$rom;module$rom.EXPECTED_CRC32=EXPECTED_CRC32$$module$rom;const STATUS$$module$version="unstable",VERSION$$module$version="latest",LABEL$$module$version="latest 6e4f928",HASH$$module$version="6e4f92808a6af69ce284593303a25aa0773636fc",DATE$$module$version=new Date(1558436176E3);var module$version={};module$version.STATUS=STATUS$$module$version;module$version.VERSION=VERSION$$module$version;module$version.LABEL=LABEL$$module$version;module$version.HASH=HASH$$module$version;module$version.DATE=DATE$$module$version;var $jscompDefaultExport$$module$patch={apply(a,b,c){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){let d;b.seed||(b.seed=parseSeed$$module$patch("").toString(16),window.location.hash+="&seed="+b.seed);d=b.flags?new FlagSet$$module$flagset(String(b.flags)):new FlagSet$$module$flagset("Em Gt Mr Rlpt Sbk Sct Sm Tasd");for(const a in b)"false"===b[a]&&(b[a]=!1);yield shuffle$$module$patch(a,parseSeed$$module$patch(String(b.seed)),d,new FetchReader$$module$fetchreader(c))})}};
...
m=new Rom$$module$rom(a);makeBraceletsProgressive$$module$patch(m);closeCaveEntrances$$module$patch(m,c);reversibleSwanGate$$module$patch(m);adjustGoaFortressTriggers$$module$patch(m);preventNpcDespawns$$module$patch(m,c);c.requireHealedDolphinToRide()&&requireHealedDolphin$$module$patch(m);c.saharaRabbitsRequireTelepathy()&&requireTelepathyForDeo$$module$patch(m);adjustItemNames$$module$patch(m,c);yield l("postparse.s");alarmFluteIsKeyItem$$module$patch(m);c.teleportOnThunderSword()?teleportOnThunderSword$$module$patch(m):
noTeleportOnThunderSword$$module$patch(m);m.scalingLevels=48;m.uniqueItemTableAddress=k.expand("KeyItemData");yield shuffle2$$module$depgraph(m,g,e,c,f);rescaleShops$$module$patch(m,k,c.bargainHunting()?g:void 0);rescaleMonsters$$module$patch(m);c.shuffleMonsters()&&shuffleMonsters$$module$patch(m,g);identifyKeyItemsForDifficultyBuffs$$module$patch(m);c.doubleBuffMedicalHerb()?(a[115996]*=2,a[115962]*=3):c.buffMedicalHerb()&&(a[115996]+=16,a[115962]*=2);c.connectLimeTreeToLeaf()&&connectLimeTreeToLeaf$$module$patch(m);
addCordelWestTriggers$$module$patch(m,c);c.disableRabbitSkip()&&fixRabbitSkip$$module$patch(m);c.storyMode()&&storyMode$$module$patch(m);c.chargeShotsOnly()&&disableStabs$$module$patch(m);c.orbsOptional()&&orbsOptional$$module$patch(m);misc$$module$patch(m,c);yield m.writeData();return yield postParsedShuffle$$module$patch(a,g,b,c,k,l)}),postParsedShuffle$$module$patch=(a,b,c,d,e,f)=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){yield f("postshuffle.s");updateDifficultyScalingTables$$module$patch(a,
d,e);updateCoinDrops$$module$patch(a,d);shuffleRandomNumbers$$module$patch(a,b);return stampVersionSeedAndHash$$module$patch(a,c,d)}),misc$$module$patch=(a,b)=>{a.messages.parts[2][2].text="\n{01:Akahana} is handed a statue.#\nThanks for finding that.\nI was totally gonna sell\nit for tons of cash.#\nHere, have this lame\n[29:Gas Mask] or something.";a.messages.parts[0][14].text="It's dangerous to go alone! Take this.";a.messages.parts[0][14].fixText();a&&b||console.log(a,b)};
function makeBraceletsProgressive$$module$patch(a){a=a.npcs[95];var b=a.localDialogs.get(33);b=[b[0],b[2],b[2],b[1]];b[1].condition=-519;b[2].condition=-518;b[3].condition=-1;a.localDialogs.set(33,b)}
...
36:80,38:300},rescaleMonsters$$module$patch=(a)=>{var b=new Set(Object.keys(a.objects).map(Number));for(var c of SCALED_MONSTERS$$module$patch){const [a]=c;b.delete(a)}for(const c of SCALED_MONSTERS$$module$patch){const [d,f]=c;for(const c of b)a.objects[d].base===a.objects[c].base&&(SCALED_MONSTERS$$module$patch.set(c,f),b.delete(d))}for(const d of SCALED_MONSTERS$$module$patch){const [e,{sdef:f,swrd:g,hits:h,satk:k,dgld:l,sexp:m}]=d;b=a.objects[e].data;c=[87,94,104,125,136,151,155,158].includes(e)?
1:0;b[2]|=128;b[6]=h;b[7]=k;b[8]=f|g<<4;b[9]=b[9]&224|c;b[16]=b[16]&15|l<<4;b[17]=m}a.objects[125].elements|=8},shuffleMonsters$$module$patch=(a,b)=>{const c=new MonsterPool$$module$patch({});for(const b of a.locations)b.used&&c.populate(b);c.shuffle(b)},identifyKeyItemsForDifficultyBuffs$$module$patch=(a)=>{for(let b=0;73>b;b++){const c=1<<(b&7),d=123152+(b>>>3);a.prg[d]=a.prg[d]&~c|(a.prg[135152+b]&64||49===b?c:0)}},SCALED_MONSTERS$$module$patch=new Map([[63,"p","Sorceror shot",,,,19,,,],[75,"m",
...
class MonsterPool$$module$patch{constructor(a){this.report=a;this.monsters=[];this.used=[];this.locations=[]}populate(a){var b=MONSTER_ADJUSTMENTS$$module$patch[a.id]||{},c=Object.assign({},b);b=void 0===b.skip?!1:b.skip;c=(delete c.maxFlyers,delete c.nonFlyers,delete c.skip,delete c.fixedSlots,c);for(var d of Object.keys(c))throw Error(`Unexpected property '${d}' in MONSTER_ADJUSTMENTS[${a.id}]`);if(!0!==b&&a.spritePatterns&&a.spritePalettes){d=[];c=[];var e=12;for(const b of a.spawns){++e;if(!b.isMonster())continue;
const h=b.monsterId;if(h in UNTOUCHED_MONSTERS$$module$patch||!SCALED_MONSTERS$$module$patch.has(h)||"m"!==SCALED_MONSTERS$$module$patch.get(h).type)continue;var f=a.rom.objects[h];if(!f)continue;const l=b.patternBank,m=a.spritePatterns[l];var g=f.palettes(!0);f=g.includes(2)?a.spritePalettes[0]:void 0;g=g.includes(3)?a.spritePalettes[1]:void 0;d.push({id:h,pat:m,pal2:f,pal3:g,patBank:l});(this.report[`start-${h.toString(16)}`]=this.report[`start-${h.toString(16)}`]||[]).push("$"+a.id.toString(16));
c.push(e)}d.length&&(b||this.locations.push({location:a,slots:c}),this.monsters.push(...d))}}shuffle(a){this.report["pre-shuffle locations"]=this.locations.map((a)=>a.location.id);this.report["pre-shuffle monsters"]=this.monsters.map((a)=>a.id);a.shuffle(this.locations);a.shuffle(this.monsters);this.report["post-shuffle locations"]=this.locations.map((a)=>a.location.id);for(this.report["post-shuffle monsters"]=this.monsters.map((a)=>a.id);this.locations.length;){const {location:d,slots:e}=this.locations.pop(),
f=this.report["$"+d.id.toString(16).padStart(2,"0")]=[],{maxFlyers:g=0,nonFlyers:h={},fixedSlots:k={}}=MONSTER_ADJUSTMENTS$$module$patch[d.id]||{};let l=k.pat0||null,m=k.pat1||null,n=k.pal2||null,p=k.pal3||null,r=g,q=!1;for(const a of d.spawns)if(a.isChest()&&(q=!0),a.isMonster()){var b=a.monsterId;126===b||127===b||159===b?m=98:143===b&&(l=97)}if(21===d.id||71===d.id)q=!1;f.push(`Initial pass: ${[q,l,m,n,p].join(", ")}`);b=(a)=>{var b=FLYERS$$module$patch.has(a.id),c=MOTHS_AND_BATS$$module$patch.has(a.id);
...
this.used.push(a);c--}for(c=0;c<this.used.length&&e.length;c++)b(this.used[c])&&(this.used.push(...this.used.splice(c,1)),c--);null!=l&&(d.spritePatterns[0]=l);null!=m&&(d.spritePatterns[1]=m);null!=n&&(d.spritePalettes[0]=n);null!=p&&(d.spritePalettes[1]=p);if(e.length){f.push(`Failed to fill location ${d.id.toString(16)}: ${e.length} remaining`);for(const a of e)b=d.spawns[a-13],b.x=b.y=0}}}}
