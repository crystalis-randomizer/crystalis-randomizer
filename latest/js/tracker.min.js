...
{section:"Debug Mode",text:"These options are helpful for exploring or debugging.  Note that,\n      while they do not directly affect any randomization, they\n      <i>do</i> factor into the seed to prevent cheating, and they\n      will remove the option to generate a seed for racing.",flags:[{flag:"Ds",name:"Generate a spoiler log"},{flag:"Di",name:"Player never dies"}]}],exclusiveFlags$$module$js$flagset=(b)=>b.startsWith("S")?new RegExp(`S.*[${b.substring(1)}]`):FLAG_CONFLICTS$$module$js$flagset[b],
FLAG_CONFLICTS$$module$js$flagset={Hm:/Em/,Hx:/Ex/,Em:/Hm/,Ex:/Hx/,Tw:/Gw/,Gw:/Tw/,Ft:/Gt/,Gt:/Ft/,Fc:/Gc/,Gc:/Fc/,Fp:/Gp/,Gp:/Fp/,Fr:/Gr/,Gr:/Fr/};
class FlagSet$$module$js$flagset{constructor(b="RtGftTab"){this.flags={};b=b.replace(/[^A-Za-z0-9!]/g,"");const a=/([A-Z])([a-z0-9!]+)/g;let c;for(;c=a.exec(b);){let [,a,b]=c;REPEATABLE_FLAGS$$module$js$flagset.has(a)&&(b=[b]);for(const c of b)this.set(a+c,!0)}}set(b,a){const c=b[0],d=b.substring(1);a?(this.removeConflicts(b),b=(this.flags[c]||[]).filter((a)=>a!==d),b.push(d),b.sort(),this.flags[c]=b):(b=(this.flags[c]||[]).filter((a)=>a!==d),b.length?this.flags[c]=b:delete this.flags[c])}check(b){const a=
...
.25:this.check("Ex")?2.5:1}removeConflicts(b){const a=exclusiveFlags$$module$js$flagset(b);if(a)for(const c in this.flags)b=this.flags[c].filter((b)=>!a.test(c+b)),b.length?this.flags[c]=b:delete this.flags[c]}toStringKey(b){return REPEATABLE_FLAGS$$module$js$flagset.has(b)?[...this.flags[b]].sort().map((a)=>b+a).join(" "):b+[...this.flags[b]].sort().join("")}toString(){const b=Object.keys(this.flags);b.sort();return b.map((a)=>this.toStringKey(a)).join(" ")}}var module$js$flagset={};
module$js$flagset.PRESETS=PRESETS$$module$js$flagset;module$js$flagset.FLAGS=FLAGS$$module$js$flagset;module$js$flagset.FlagSet=FlagSet$$module$js$flagset;const generate$$module$js$depgraph=(b)=>{b=void 0===b?new FlagSet$$module$js$flagset:b;const a=new WorldGraph$$module$js$nodes,c=(b,c)=>new Option$$module$js$nodes(a,b,void 0===c?!0:c),d=(b,c)=>new Item$$module$js$nodes(a,b,c,b,null),e=(b,c,...d)=>new Boss$$module$js$nodes(a,b,c,...d);new TrackerNode$$module$js$nodes(a,TrackerNode$$module$js$nodes.OFF_ROUTE,"Off-route");const f=new TrackerNode$$module$js$nodes(a,TrackerNode$$module$js$nodes.GLITCH,"Glitch"),g=new TrackerNode$$module$js$nodes(a,TrackerNode$$module$js$nodes.HARD,
"Hard"),k=(a)=>a.value?a:f,h=c("Leather Boots grant speed",b.leatherBootsGiveSpeed()),n=k(c("Assume ghetto flight",b.assumeGhettoFlight())),m=k(c("Assume statue glitch",b.assumeStatueGlitch())),q=c("Allow statue glitch",!b.disableStatueGlitch()),p=k(c("Assume teleport skip",b.assumeTeleportSkip())),l=c("Allow teleport skip",!b.disableTeleportSkip()),r=k(c("Assume rabbit skip",b.assumeRabbitSkip())),I=k(c("Allow rabbit skip",!b.disableRabbitSkip())),C=k(c("Assume sword charge glitch",b.assumeSwordChargeGlitch())),
...
d(62,"Bow of Moon").fromPerson("Aryllis",35).direct(251624).dialog(35,null,1).key(),pc=d(63,"Bow of Sun").chest().key(),qc=d(64,"Bow of Truth").fromPerson("Azteca",131).npcSpawn(131,156,1).dialog(131,null,0).key(),Ya=(new Magic$$module$js$nodes(a,65,"Refresh",65,null)).fromPerson("Zebu at windmill",94).requireUnique().direct(251665).dialog(94,16,3).trigger(180,1),Ja=(new Magic$$module$js$nodes(a,66,"Paralysis",66,null)).direct("Zebu at Mt. Sabre summit",251477).requireUnique().trigger(141).trigger(178),
oa=(new Magic$$module$js$nodes(a,67,"Telepathy",67,null)).direct("Tornel at Stom's house",223220).npcSpawn(95,30,1).trigger(133,1),rc=(new Magic$$module$js$nodes(a,68,"Teleport",68,null)).fromPerson("Tornel on Mt. Sabre",95).dialog(95,33,0),he=(new Magic$$module$js$nodes(a,69,"Recover",69,null)).direct("Asina in Portoa",250361),sc=(new Magic$$module$js$nodes(a,70,"Barrier",70,null)).direct("Asina on Angry sea",251609).requireUnique().trigger(132,0),pa=(new Magic$$module$js$nodes(a,71,"Change",71,
null)).direct("Kensu in Swan",251614).npcSpawn(116,241,1),z=(new Magic$$module$js$nodes(a,72,"Flight",72,null)).weight(15).direct("Kensu in Draygonia Fortress",250255),ie=L.bossDrop("Vampire 2",12,97).npcSpawn(204).key(),M=d(112,"Mimic"),tc=(new Trigger$$module$js$nodes(a,"Talked to Leaf Elder")).get(T),uc=(new Trigger$$module$js$nodes(a,"Talked to Leaf Student")).get(vb),zb=new Trigger$$module$js$nodes(a,"Talked to Zebu in cave"),je=(new Trigger$$module$js$nodes(a,"Woke up Windmill Guard")).get(gc),
Ka=new Trigger$$module$js$nodes(a,"Started Windmill"),vc=(new Trigger$$module$js$nodes(a,"Learned Refresh")).get(Ya),ke=(new Trigger$$module$js$nodes(a,"Gave Statue to Akahana")).get(Xa),le=(new Trigger$$module$js$nodes(a,"Fought Stom")).get(oa),wc=new Trigger$$module$js$nodes(a,"Visited Oak"),xc=new Trigger$$module$js$nodes(a,"Talked to Oak Mother"),Ab=new Trigger$$module$js$nodes(a,"Rescued Oak Child"),me=(new Trigger$$module$js$nodes(a,"Talked to Oak Mother Again")).get(dc),ne=(new Trigger$$module$js$nodes(a,
"Talked to Oak Elder")).get(U),oe=(new Trigger$$module$js$nodes(a,"Talked to Tornel on Mt Sabre")).get(rc),yc=new Trigger$$module$js$nodes(a,"Villagers Abducted"),zc=new Trigger$$module$js$nodes(a,"Talked to Rabbit in Leaf"),pe=(new Trigger$$module$js$nodes(a,"Learned Paralysis")).get(Ja),Ac=new Trigger$$module$js$nodes(a,"Talked to Portoa Queen"),Bc=new Trigger$$module$js$nodes(a,"Talked to Fortune Teller"),Cc=new Trigger$$module$js$nodes(a,"Visited Underground Channel"),qe=(new Trigger$$module$js$nodes(a,
"Sent to Waterfall Cave")).get(tb),re=(new Trigger$$module$js$nodes(a,"Cured Akahana")).get(fc),Dc=(new Trigger$$module$js$nodes(a,"Talked to Rage")).get(Ta),Ec=new Trigger$$module$js$nodes(a,"Mesia recording played"),Fc=(new Trigger$$module$js$nodes(a,"Talked to Asina")).get(he),Gc=(new Trigger$$module$js$nodes(a,"Healed Dolphin")).get(xb),Bb=new Trigger$$module$js$nodes(a,"Returned Fog Lamp"),Hc=new Trigger$$module$js$nodes(a,"Talked to Kensu in Cabin"),Ic=new Trigger$$module$js$nodes(a,"Talked to Joel Elder"),
se=(new Trigger$$module$js$nodes(a,"Talked to Clark")).get(jc),te=(new Trigger$$module$js$nodes(a,"Talked to Kensu in Lighthouse")).get(lc),Cb=new Trigger$$module$js$nodes(a,"Calmed the Angry Sea"),ue=(new Trigger$$module$js$nodes(a,"Learned Barrier")).get(sc),Jc=new Trigger$$module$js$nodes(a,"Talked to Stom in Swan Hut"),Kc=new Trigger$$module$js$nodes(a,"Talked to Kensu in Swan tavern"),Lc=new Trigger$$module$js$nodes(a,"Talked to Kensu at Swan dance"),ve=(new Trigger$$module$js$nodes(a,"Returned Kensu's love pendant")).get(pa),
we=(new Trigger$$module$js$nodes(a,"Talked to Amazones Queen")).get(oc),Db=new Trigger$$module$js$nodes(a,"Entered Shyron"),xe=(new Trigger$$module$js$nodes(a,"Talked to Zebu in Shyron")).get(hc),Mc=new Trigger$$module$js$nodes(a,"Shyron Massacre"),ye=(new Trigger$$module$js$nodes(a,"Saved Kensu")).get(z),ze=(new Trigger$$module$js$nodes(a,"Talked to Deo")).get(ge),Ae=(new Trigger$$module$js$nodes(a,"Talked to Akahana's Friend")).get(ee),Be=(new Trigger$$module$js$nodes(a,"Get Bow of Truth")).get(qc),
Ce=(new Trigger$$module$js$nodes(a,"Forged Crystalis")).get(ob),Za=(new Condition$$module$js$nodes(a,"Sword charge glitch")).option(C,S),De=(new Condition$$module$js$nodes(a,"Rabbit skip")).option(r,I),Ee=(new Condition$$module$js$nodes(a,"Teleport skip")).option(p,l),$a=(new Condition$$module$js$nodes(a,"Statue glitch")).option(m,q),Fe=(new Condition$$module$js$nodes(a,"Wild warp")).option(va,na),ab=(new Condition$$module$js$nodes(a,"Any level 2 sword")).option(T,Qa).option(T,Ia).option(U,Ra).option(U,
Sa).option(O,Ta).option(O,Ua).option(K,Va).option(K,Wa),v=(new Condition$$module$js$nodes(a,"Destroy stone")).option(T,Qa).option(T,Ia).option(Za,T,ab),w=(new Condition$$module$js$nodes(a,"Destroy ice")).option(U,Ra).option(U,Sa).option(Za,U,ab),t=(new Condition$$module$js$nodes(a,"Cross rivers")).option(O,Ta).option(O,Ua).option(z).option(Za,O,ab),xa=(new Condition$$module$js$nodes(a,"Destroy iron")).option(K,Va).option(K,Wa).option(Za,K,ab),aa=(new Condition$$module$js$nodes(a,"Any sword")).option(T).option(U).option(O).option(K),
Ge=(new Condition$$module$js$nodes(a,"Match insect sword (fire/water/thunder)")).option(U).option(O).option(K).option(Xa,G.value?G:g,T),He=(new Condition$$module$js$nodes(a,"Match vampire 2 sword (wind/water/thunder)")).option(T).option(O).option(K).option(G.value?G:g,U),Nc=(new Condition$$module$js$nodes(a,"Speed boots")).option(ec,h),Oc=(new Condition$$module$js$nodes(a,"Climb slopes")).option(ub).option(z).option(Nc),Ie=(new Condition$$module$js$nodes(a,"Enter Mt Sabre North (rabbit)")).option(zc).option(De),
Je=(new Condition$$module$js$nodes(a,"Enter Mt Sabre North (Teleport)")).option(rc).option(Ee,z),Pc=(new Condition$$module$js$nodes(a,"Asina in her room")).option(Ec),Ke=(new Condition$$module$js$nodes(a,"Paralysis or Asina revealed")).option(Ja).option(Pc).option($a),Qc=(new Condition$$module$js$nodes(a,"Ride dolphin")).option(xb,Hc),Le=(new Condition$$module$js$nodes(a,"Ghetto flight")).option(n,Qc,ub),ba=(new Condition$$module$js$nodes(a,"Cross sea")).option(Qc).option(z),Me=(new Condition$$module$js$nodes(a,
"Cross whirlpool")).option(Cb).option(z).option(Le),Q=(new Condition$$module$js$nodes(a,"Refresh if needed")).option(Wb.value?Wb:g).option(Ya),Rc=(new Condition$$module$js$nodes(a,"Wind magic")).option(E.value?E:g,Q).option(Qa,Ia,Q),Sc=(new Condition$$module$js$nodes(a,"Fire magic")).option(E.value?E:g,Q).option(Ra,Sa,Q),Tc=(new Condition$$module$js$nodes(a,"Water magic")).option(E.value?E:g,Q).option(E,Q).option(Ta,Ua,Q),Ne=(new Condition$$module$js$nodes(a,"Thunder magic")).option(E.value?E:g,Q).option(E,
Q).option(Va,Q).option(Wa,Q),Oe=(new Condition$$module$js$nodes(a,"Flute of lime or glitch")).option(tb).option($a),Pe=(new Condition$$module$js$nodes(a,"Change or glitch")).option(pa).option($a),Qe=(new Condition$$module$js$nodes(a,"Change or glitch or paralysis")).option(pa).option($a).option(Ja),Eb=(new Condition$$module$js$nodes(a,"Pass shooting statues")).option(sc).option(aa,Vb.value?Vb:g).option(Ya,fc),Re=(new Condition$$module$js$nodes(a,"Healed dolphin if required")).option(Gc).option(Od),
Uc=(new Condition$$module$js$nodes(a,"Match sword of wind")).option(T).option(G.value?G:g,aa),Vc=(new Condition$$module$js$nodes(a,"Match sword of fire")).option(U).option(G.value?G:g,aa),Wc=(new Condition$$module$js$nodes(a,"Match sword of water")).option(O).option(G.value?G:g,aa),Se=(new Condition$$module$js$nodes(a,"Match sword of thunder")).option(K).option(G.value?G:g,aa),Te=(new Condition$$module$js$nodes(a,"Swamp run possible")).option(aa).option(Nc).option(Ud).option(Ya),Ue=(new Condition$$module$js$nodes(a,
"Insect possible to kill")).option(Xa),bb=(new Condition$$module$js$nodes(a,"Travel swamp")).option(Xa).option(Pa.value?Pa:g,Te),Ve=(new Condition$$module$js$nodes(a,"Calmed sea if required")).option(Cb).option(Pd),We=(new Condition$$module$js$nodes(a,"Started windmill if required")).option(Ka).option(Rd),Xe=e(0,"Vampire 1",aa).get(ub),Ye=e(1,"Insect",dc,Ge,Ue).get(Ra),Ze=e(2,"Kelbesque 1",Uc,Rc).get(Sa),$e=e(12,"Vampire 2",He).get(ie),Xc=e(4,"Sabera 1",Vc,Sc).get(kc),af=e(5,"Mado 1",Wc,Tc).get(Va),
bf=e(6,"Kelbesque 2",Uc,Rc).get(sb),cf=e(7,"Sabera 2",Vc,Sc).get(rb),df=e(8,"Mado 2",Wc,Tc).get(pb),ef=e(9,"Karmine",Se,Ne).get(yb),ff=e(10,"Draygon 1",aa).get(ce),gf=e(null,"Statues",pc,oc),hf=e(11,"Draygon 2",aa,qc),jf=e(13,"Dyna",ob),kf=(new Condition$$module$js$nodes(a,"Opened prison if required")).option(wb).option(Qd),W=new Area$$module$js$nodes(a,"Leaf"),ya=new Area$$module$js$nodes(a,"Valley of Wind"),R=new Area$$module$js$nodes(a,"Sealed Cave"),qa=new Area$$module$js$nodes(a,"Cordel Plain"),
za=new Area$$module$js$nodes(a,"Brynmaer"),Aa=new Area$$module$js$nodes(a,"Amazones"),A=new Area$$module$js$nodes(a,"Mt Sabre West"),u=new Area$$module$js$nodes(a,"Mt Sabre North"),cb=new Area$$module$js$nodes(a,"Nadare's"),La=new Area$$module$js$nodes(a,"Oak"),fa=new Area$$module$js$nodes(a,"Waterfall Valley"),y=new Area$$module$js$nodes(a,"Portoa"),ha=new Area$$module$js$nodes(a,"Waterfall Cave"),B=new Area$$module$js$nodes(a,"Fog Lamp Cave"),ia=new Area$$module$js$nodes(a,"Kirisa Plant Cave"),
ra=new Area$$module$js$nodes(a,"Angry Sea"),ja=new Area$$module$js$nodes(a,"Joel"),H=new Area$$module$js$nodes(a,"Evil Spirit Island"),ka=new Area$$module$js$nodes(a,"Sabera's Castle"),X=new Area$$module$js$nodes(a,"Swan"),x=new Area$$module$js$nodes(a,"Mt Hydra"),la=new Area$$module$js$nodes(a,"Shyron"),sa=new Area$$module$js$nodes(a,"Styx"),ta=new Area$$module$js$nodes(a,"Goa"),db=new Area$$module$js$nodes(a,"Draygonia Fortress 1"),Y=new Area$$module$js$nodes(a,"Draygonia Fortress 2"),Ba=new Area$$module$js$nodes(a,
"Draygonia Fortress 3"),N=new Area$$module$js$nodes(a,"Draygonia Fortress 4"),F=new Area$$module$js$nodes(a,"Oasis Cave");new Area$$module$js$nodes(a,"Desert");const Z=new Area$$module$js$nodes(a,"Sahara"),Ca=new Area$$module$js$nodes(a,"Pyramid Front"),ca=new Area$$module$js$nodes(a,"Pyramid Back"),da=new Area$$module$js$nodes(a,"Tower"),Fb=(new Location$$module$js$nodes(a,0,W,"Start")).overworld().start(),lf=(new Location$$module$js$nodes(a,0,W,"Mezame Shrine")).overworld().from(Fb),mf=(new Location$$module$js$nodes(a,
1,W,"Outside Start")).overworld().connect(lf),ma=(new Location$$module$js$nodes(a,2,W,"Town")).town().connect(mf),Ma=(new Location$$module$js$nodes(a,3,ya,"Main")).overworld().connect(ma).trigger(vc,Ka),Yc=(new Location$$module$js$nodes(a,3,ya,"Outside Windmill")).overworld(),Zc=(new Location$$module$js$nodes(a,4,R,"Tunnel 1 (entrance)")).cave().from(Ma,Ka),nf=(new Location$$module$js$nodes(a,5,R,"Tunnel 2 (over bridge)")).cave().connect(Zc),of=(new Location$$module$js$nodes(a,6,R,"Tunnel 6 (herb dead end)")).cave().chest(D,
15),$c=(new Location$$module$js$nodes(a,7,R,"Tunnel 4a (ball corridor)")).cave().chest(D,20,80).chest(Qa,21);(new Location$$module$js$nodes(a,7,R,"Tunnel 4b (antidote dead end)")).cave().connect($c,v).chest(J,19);const pf=(new Location$$module$js$nodes(a,8,R,"Tunnel 5 (warp boots dead end)")).cave().chest(P,14),qf=(new Location$$module$js$nodes(a,9,R,"Tunnel 3a (branch, front)")).cave().connect(nf).connectTo($c).connectTo(pf),rf=(new Location$$module$js$nodes(a,9,R,"Tunnel 3b (branch, back)")).cave().connect(qf,
v).connectTo(of),sf=(new Location$$module$js$nodes(a,10,R,"Tunnel 7 (boss)")).cave().connect(rf).boss(Xe),tf=(new Location$$module$js$nodes(a,12,R,"Tunnel 8a (exit, above wall)")).cave().connect(sf),uf=(new Location$$module$js$nodes(a,12,R,"Tunnel 8b (exit, below wall")).cave().connect(tf,v);(new Location$$module$js$nodes(a,14,ya,"Windmill Cave")).cave().connect(Ma).connectTo(Yc).trigger(je,vb,zb);(new Location$$module$js$nodes(a,15,ya,"Windmill")).misc().connect(Yc).trigger(Ka,gc);const vf=(new Location$$module$js$nodes(a,
16,ya,"Zebu's Cave Front")).cave().connect(Ma).trigger(zb,tc,uc).trigger(vc,Ka,zb),wf=(new Location$$module$js$nodes(a,16,ya,"Zebu's Cave Back")).cave().trigger(yc).connect(vf,w),xf=(new Location$$module$js$nodes(a,17,A,"Tunnel 1 (to Zebu)")).cave().connect(wf),Da=(new Location$$module$js$nodes(a,20,qa,"West")).overworld().connect(uf,We),yf=(new Location$$module$js$nodes(a,20,qa,"South")).overworld().connect(Da,t),ad=(new Location$$module$js$nodes(a,21,qa,"East")).overworld().connect(Da).chest(cc,
24),Na=(new Location$$module$js$nodes(a,24,za,"Town")).town().connect(Da).trigger(ke,cc),zf=(new Location$$module$js$nodes(a,25,qa,"Outside Stom's House")).town().connect(Da),Gb=(new Location$$module$js$nodes(a,26,qa,"Swamp")).overworld().connect(ad,bb).trigger(Ab,xc,bb);(new Location$$module$js$nodes(a,26,qa,"Swamp Insect Area")).overworld().connect(Gb,bb).boss(Ye);const eb=(new Location$$module$js$nodes(a,27,Aa,"Town")).town().connect(yf),fb=(new Location$$module$js$nodes(a,28,La,"Town")).town().connect(Gb,
bb).trigger(wc);(new Location$$module$js$nodes(a,30,qa,"Stom's House")).house().connect(zf).trigger(le,wc);const Ea=(new Location$$module$js$nodes(a,32,A,"Entrance")).overworld().connect(Da).connect(xf),Af=(new Location$$module$js$nodes(a,32,A,"Up Slope")).overworld().from(Ea,Oc).to(Ea),Bf=(new Location$$module$js$nodes(a,32,A,"Dead End (warp boots)")).overworld().chest(P,24,106),bd=(new Location$$module$js$nodes(a,33,A,"Upper")).overworld().from(Ea,z).to(Ea),Cf=(new Location$$module$js$nodes(a,33,
A,"Tornel Dead End")).overworld().trigger(oe,Ia).chest(V,23,105),cd=(new Location$$module$js$nodes(a,34,A,"Tunnel 2a (fork at start)")).cave().connect(Ea);(new Location$$module$js$nodes(a,34,A,"Tunnel 2b (left branch to dead end)")).cave().connect(cd,w).connectTo(Bf);const Df=(new Location$$module$js$nodes(a,34,A,"Tunnel 2c (right branch to upper)")).cave().connect(cd,w),Ef=(new Location$$module$js$nodes(a,35,A,"Tunnel 3a (tunnel to upper, with herb chest)")).cave().connect(Df).chest(D,23,82),Ff=
(new Location$$module$js$nodes(a,35,A,"Tunnel 3b (tunnel to upper, branch below ice)")).cave().connect(Ef,w).connectTo(bd),dd=(new Location$$module$js$nodes(a,36,A,"Tunnel 4a (branch to upper or Tornel)")).cave().connect(Ff);(new Location$$module$js$nodes(a,36,A,"Tunnel 4b (out to upper)")).cave().connect(dd,w).connectTo(bd);const Gf=(new Location$$module$js$nodes(a,37,A,"Tunnel 5 (tiny connector)")).cave().connect(dd),Hf=(new Location$$module$js$nodes(a,38,A,"Tunnel 6a (exit to Tornel, above ice)")).cave().connect(Gf);
(new Location$$module$js$nodes(a,38,A,"Tunnel 6b (exit to Tornel, below ice)")).cave().connect(Hf,w).connectTo(Cf);const If=(new Location$$module$js$nodes(a,39,A,"Tunnel 7a (tornado bracelet, lower)")).cave().connect(Af),Jf=(new Location$$module$js$nodes(a,39,A,"Tunnel 7b (tornado bracelet, middle)")).cave().connect(If,w);(new Location$$module$js$nodes(a,39,A,"Tunnel 7c (tornado bracelet, upper)")).cave().connect(Jf,w).chest(Ia,25);const gb=(new Location$$module$js$nodes(a,40,u,"Entrance")).overworld().connect(ad,
Je,Ie),hb=(new Location$$module$js$nodes(a,40,u,"Upper")).overworld().from(gb,z).to(gb),Hb=(new Location$$module$js$nodes(a,40,u,"Summit (boss)")).overworld().from(hb,z).to(hb).boss(Ze),ed=(new Location$$module$js$nodes(a,41,u,"Connector")).overworld(),Ib=(new Location$$module$js$nodes(a,41,u,"Middle Left")).overworld(),Kf=(new Location$$module$js$nodes(a,41,u,"Middle Right")).overworld().from(Ib,Oc).to(Ib),fd=(new Location$$module$js$nodes(a,42,u,"Tunnel 2a (from entrance to connector)")).cave().connectTo(ed);
(new Location$$module$js$nodes(a,42,u,"Tunnel 2b (under bridge, to antidote)")).cave().connect(fd,w).chest(J,23,94);const gd=(new Location$$module$js$nodes(a,43,u,"Tunnel 3a (branch after connector)")).cave().connect(ed),Lf=(new Location$$module$js$nodes(a,43,u,"Tunnel 3b (right branch)")).cave().connect(gd,w),Mf=(new Location$$module$js$nodes(a,43,u,"Tunnel 3c (upper branch)")).cave().connect(gd,w);(new Location$$module$js$nodes(a,44,u,"Tunnel 4 (over bridge, to middle)")).cave().connect(Mf).connectTo(Ib);
const hd=(new Location$$module$js$nodes(a,45,u,"Tunnel 5a (E-shaped, from right branch)")).cave().connect(Lf).connectTo(Kf);(new Location$$module$js$nodes(a,45,u,"Tunnel 5b (dead-end with herb)")).cave().connect(hd,w).chest(D,22,83);const Nf=(new Location$$module$js$nodes(a,46,u,"Tunne; 6a (S-shaped hall, right)")).cave().connect(hd),Of=(new Location$$module$js$nodes(a,46,u,"Tunne; 6b (S-shaped hall, middle)")).cave().connect(Nf,w),Pf=(new Location$$module$js$nodes(a,46,u,"Tunnel 6c (S-shaped hall, left)")).cave().connect(Of,
w),id=(new Location$$module$js$nodes(a,47,u,"Prison (hallway)")).cave().connect(hb),Qf=(new Location$$module$js$nodes(a,48,u,"Left Cell (shopkeepers)")).cave().from(id,w);(new Location$$module$js$nodes(a,49,u,"Left Cell 2 (back, with prison key)")).cave().from(Qf,w).chest(wb,13);const Rf=(new Location$$module$js$nodes(a,50,u,"Right Cell (villagers)")).cave().from(id,w),Sf=(new Location$$module$js$nodes(a,51,u,"Tunnel 8 (behind right cell, toward summit)")).cave().from(Rf,w);(new Location$$module$js$nodes(a,
52,u,"Tunnel 9 (connector to summit)")).cave().connect(Sf).connectTo(Hb);const Tf=(new Location$$module$js$nodes(a,53,u,"Tunnel 10a (summit cave, front)")).cave().from(Hb,wb).to(Hb),Uf=(new Location$$module$js$nodes(a,53,u,"Tunnel 10b (summit cave, behind ice)")).cave().connect(Tf,w).trigger(pe);(new Location$$module$js$nodes(a,56,u,"Tunnel 1 (leads from main entrance)")).cave().connect(gb).connectTo(fd);(new Location$$module$js$nodes(a,57,u,"Tunnel 7 (to upper)")).cave().connect(Pf).connectTo(hb);
const Vf=(new Location$$module$js$nodes(a,60,cb,"Inn")).shop(),Wf=(new Location$$module$js$nodes(a,61,cb,"Tool Shop")).shop(D,J,L,P),Xf=(new Location$$module$js$nodes(a,62,cb,"Back Room")).house(),Jb=(new Location$$module$js$nodes(a,64,fa,"Summit")).overworld().connect(Uf,kf),ib=(new Location$$module$js$nodes(a,64,fa,"Northwest")).overworld().from(Jb).to(Jb,z),Yf=(new Location$$module$js$nodes(a,64,fa,"Northeast")).overworld().connect(ib,t),jd=(new Location$$module$js$nodes(a,65,fa,"Southwest")).overworld().connect(ib),
Zf=(new Location$$module$js$nodes(a,65,fa,"Southeast")).overworld().connect(jd,t),kd=(new Location$$module$js$nodes(a,66,fa,"Lime Tree Valley")).overworld().connect(jd).connect(Ma,Td),$f=(new Location$$module$js$nodes(a,67,fa,"Lime Tree Lake (Rage)")).cave().connect(kd).trigger(Dc,O),ag=(new Location$$module$js$nodes(a,68,ia,"Tunnel 1a (entrance)")).cave().connect(Zf),bg=(new Location$$module$js$nodes(a,68,ia,"Tunnel 1b (behind wall)")).cave().connect(ag,v),ld=(new Location$$module$js$nodes(a,69,
ia,"Tunnel 2a (main path, before wall)")).cave().connect(bg);(new Location$$module$js$nodes(a,69,ia,"Tunnel 2b (dead end, antidote)")).cave().connect(ld,v).chest(J,25,95);const cg=(new Location$$module$js$nodes(a,69,ia,"Tunnel 2c (main path, after wall)")).cave().connect(ld,v),dg=(new Location$$module$js$nodes(a,70,ia,"Tunnel 3a (last room, before wall)")).cave().connect(cg),eg=(new Location$$module$js$nodes(a,70,ia,"Tunnel 3b (last room, after wall)")).cave().connect(dg,v);(new Location$$module$js$nodes(a,
71,ia,"Meadow")).overworld().connect(eg).chest(nc,14);const fg=(new Location$$module$js$nodes(a,72,B,"Tunnel 1a (entrance)")).cave().connect(Yf),md=(new Location$$module$js$nodes(a,72,B,"Tunnel 1b (past wall)")).cave().connect(fg,v);(new Location$$module$js$nodes(a,72,B,"Tunnel 1c (dead end, lysisPlant)")).cave().connect(md,v).chest(wa,24);const gg=(new Location$$module$js$nodes(a,73,B,"Tunnel 2 (tiny connector)")).cave().connect(md),nd=(new Location$$module$js$nodes(a,74,B,"Tunnel 3a (upper branch)")).cave().connect(gg);
(new Location$$module$js$nodes(a,74,B,"Tunnel 3b (dead end, mimic)")).cave().connect(nd,v).chest(M,21,112);const od=(new Location$$module$js$nodes(a,74,B,"Tunnel 3c (short passage with mimic)")).cave().connect(nd,v).chest(M,22,113),hg=(new Location$$module$js$nodes(a,74,B,"Tunnel 3d (lower branch)")).cave().connect(od,v);(new Location$$module$js$nodes(a,75,B,"Tunnel 4 (dead end loop)")).cave().connect(hg);const pd=(new Location$$module$js$nodes(a,76,B,"Tunnel 5a (right branch over bridge)")).cave().connect(od),
ig=(new Location$$module$js$nodes(a,76,B,"Tunnel 5b (past wall over bridge)")).cave().connect(pd,v),jg=(new Location$$module$js$nodes(a,77,B,"Tunnel 6a (from left branch)")).cave().connect(pd),kg=(new Location$$module$js$nodes(a,77,B,"Tunnel 6b (reconvergence)")).cave().connect(jg,v).connect(ig),lg=(new Location$$module$js$nodes(a,77,B,"Tunnel 6c (between walls)")).cave().connect(kg,v),mg=(new Location$$module$js$nodes(a,77,B,"Tunnel 6d (under bridge)")).cave().connect(lg,v),ng=(new Location$$module$js$nodes(a,
78,B,"Tunnel 7a (over second bridge)")).cave().connect(mg),og=(new Location$$module$js$nodes(a,78,B,"Tunnel 7b (past wall)")).cave().connect(ng,v),pg=(new Location$$module$js$nodes(a,79,B,"Tunnel 8a (under second bridge)")).cave().connect(og);(new Location$$module$js$nodes(a,79,B,"Tunnel 8b (fog lamp)")).cave().connect(pg,v).chest(ic,19);const ua=(new Location$$module$js$nodes(a,80,y,"Town")).town().connect(ib),qd=(new Location$$module$js$nodes(a,81,y,"Fishherman Island")).town().connect(ua);(new Location$$module$js$nodes(a,
82,fa,"Mesia Shrine")).cave().trigger(Ec).connect($f,t,Dc);const rd=(new Location$$module$js$nodes(a,84,ha,"Tunnel 1a (entrance)")).cave().connect(ib);(new Location$$module$js$nodes(a,84,ha,"Tunnel 1b (dead end, mimic)")).cave().connect(rd,w).chest(M,19,114);const qg=(new Location$$module$js$nodes(a,84,ha,"Tunnel 1c (past ice)")).cave().connect(rd,w),rg=(new Location$$module$js$nodes(a,85,ha,"Tunnel 2 (stoned pair)")).cave().connect(qg),Kb=(new Location$$module$js$nodes(a,86,ha,"Tunnel 3 (wide medusa hallways)")).cave().connect(rg,
Oe),sg=(new Location$$module$js$nodes(a,87,ha,"Tunnel 4a (left entrance)")).cave().from(Kb,w).chest(Zb.chest().bonus(),25).trigger(re,tb);(new Location$$module$js$nodes(a,87,ha,"Tunnel 4b (right entrance)")).cave().from(Kb,w).connect(sg,z);(new Location$$module$js$nodes(a,87,ha,"Tunnel 4c (sword of water)")).cave().connect(Kb,w).chest(O,24);const sd=(new Location$$module$js$nodes(a,88,da,"Entrance")).misc(),tg=(new Location$$module$js$nodes(a,89,da,"Level 1")).misc().from(sd),ug=(new Location$$module$js$nodes(a,
90,da,"Level 2")).misc().from(tg),vg=(new Location$$module$js$nodes(a,91,da,"Level 3")).misc().from(ug),td=(new Location$$module$js$nodes(a,92,da,"Outside Mesia Room")).misc().from(vg),wg=(new Location$$module$js$nodes(a,93,da,"Outside Dyna Room")).misc().from(td,ob);(new Location$$module$js$nodes(a,94,da,"Mesia")).misc().connect(td).trigger(Ce);const xg=(new Location$$module$js$nodes(a,95,da,"Dyna")).misc().from(wg).boss(jf);(new Location$$module$js$nodes(a,95,da,"Win")).misc().from(xg).end();const Lb=
(new Location$$module$js$nodes(a,96,ra,"Cabin Beach")).sea().from(qd,Bb),Fa=(new Location$$module$js$nodes(a,96,ra,"South")).sea().connect(Lb,ba),yg=(new Location$$module$js$nodes(a,96,ra,"Joel Beach")).sea().connect(Fa,ba),ud=(new Location$$module$js$nodes(a,96,ja,"Outside Lighthouse")).sea();(new Location$$module$js$nodes(a,96,ra,"Altar")).sea().connect(Fa,ba).trigger(Cb,lc,kc);const zg=(new Location$$module$js$nodes(a,96,ra,"North")).sea().to(Fa,ba).from(Fa,ba,Me).trigger(ue,Ve),Ag=(new Location$$module$js$nodes(a,
96,ra,"Swan Beach")).sea().connect(zg,ba);(new Location$$module$js$nodes(a,97,ra,"Cabin")).misc().connect(Lb).trigger(Hc,Bb);(new Location$$module$js$nodes(a,98,ja,"Lighthouse")).misc().connect(ud).trigger(te,vb);const jb=(new Location$$module$js$nodes(a,100,y,"Underground Channel 1 (from throne room)")).sea().trigger(Cc),Bg=(new Location$$module$js$nodes(a,100,y,"Underground Channel 2 (to fortune teller)")).sea().connect(jb,t),vd=(new Location$$module$js$nodes(a,100,y,"Underground Channel 3 (from fortune teller)")).sea().connect(jb,
z),wd=(new Location$$module$js$nodes(a,100,y,"Underground Channel 4 (asina)")).sea().connect(vd,t),Cg=(new Location$$module$js$nodes(a,100,y,"Underground Channel 5 (dolphin)")).sea().connect(wd,t).trigger(Gc,D,Fc);(new Location$$module$js$nodes(a,100,y,"Underground Channel 6 (water)")).sea().connect(Cg,ba).connectTo(Fa,ba).chest(mc,17);const Mb=(new Location$$module$js$nodes(a,101,H,"Zombie Town")).town(),Dg=(new Location$$module$js$nodes(a,104,H,"Tunnel 1 (entrance)")).sea().connect(Fa,Ic,ba),xd=
(new Location$$module$js$nodes(a,105,H,"Tunnel 2a (start)")).cave().connect(Dg);(new Location$$module$js$nodes(a,105,H,"Tunnel 2b (dead end to left)")).cave().connect(xd,t);const Eg=(new Location$$module$js$nodes(a,105,H,"Tunnel 2c (across first river)")).cave().connect(xd,t),yd=(new Location$$module$js$nodes(a,105,H,"Tunnel 2d (across second river)")).cave().connect(Eg,t);(new Location$$module$js$nodes(a,105,H,"Tunnel 2e (dead end, magic ring)")).cave().connect(yd,v).chest(V,29);const Fg=(new Location$$module$js$nodes(a,
105,H,"Tunnel 2f (stair down)")).cave().connect(yd,v),Gg=(new Location$$module$js$nodes(a,106,H,"Tunnel 3a (main area)")).cave().connect(Fg).connectTo(Mb).chest(wa,25,92),Hg=(new Location$$module$js$nodes(a,106,H,"Tunnel 3b (left area toward items)")).cave().connect(Gg,v),Ig=(new Location$$module$js$nodes(a,107,H,"Tunnel 4a (right side, mimic)")).cave().connect(Hg).chest(M,14,115);(new Location$$module$js$nodes(a,107,H,"Tunnel 4b (left side, iron necklace)")).cave().connect(Ig,t).chest(fe,15);const Nb=
(new Location$$module$js$nodes(a,108,ka,"Floor 1")).fortress().connect(Mb),Jg=(new Location$$module$js$nodes(a,108,ka,"Miniboss")).fortress().boss($e).connect(Nb);(new Location$$module$js$nodes(a,109,ka,"Floor 2a (left stair)")).fortress().connect(Nb).chest(L,19);const Kg=(new Location$$module$js$nodes(a,109,ka,"Floor 2b (right stair)")).fortress().connect(Jg).chest(D,30,85),Lg=(new Location$$module$js$nodes(a,110,ka,"Floor 3a (toward boss)")).fortress().connect(Kg),zd=(new Location$$module$js$nodes(a,
110,ka,"Floor 3b (boss room)")).fortress().connect(Lg);(new Location$$module$js$nodes(a,110,ka,"Boss")).fortress().boss(Xc).connect(zd);(new Location$$module$js$nodes(a,110,ka,"Floor 3c (back room trap)")).fortress().from(zd).to(Nb);const Mg=(new Location$$module$js$nodes(a,112,ja,"Secret Passage")).cave().connectTo(ud),kb=(new Location$$module$js$nodes(a,113,ja,"Town")).town().connect(yg),ea=(new Location$$module$js$nodes(a,114,X,"Town")).town().connect(Ag),Ng=(new Location$$module$js$nodes(a,115,
X,"Inside Gate")).overworld().connect(ea),Og=(new Location$$module$js$nodes(a,115,X,"Outside Gate")).overworld().from(Ng,pa),lb=(new Location$$module$js$nodes(a,120,ta,"Valley")).overworld().connect(Og),Ob=(new Location$$module$js$nodes(a,124,x,"Entrance")).overworld().connect(lb),Pg=(new Location$$module$js$nodes(a,124,x,"Over first river toward Shyron")).overworld().connect(Ob,t),Ad=(new Location$$module$js$nodes(a,124,x,"After first tunnel")).overworld(),Bd=(new Location$$module$js$nodes(a,124,
x,"Door to Styx")).overworld().connect(Ad,t),Qg=(new Location$$module$js$nodes(a,124,x,"Dead end (no item)")).overworld(),Rg=(new Location$$module$js$nodes(a,124,x,"Dead end (fruit of lime)")).overworld().chest(qb,26),Sg=(new Location$$module$js$nodes(a,124,x,"Dead end (magic ring)")).overworld().chest(V,25,101),Cd=(new Location$$module$js$nodes(a,124,x,"Outside tunnel to bow")).overworld();(new Location$$module$js$nodes(a,124,x,"Floating island (bow of sun)")).overworld().connect(Cd,z).chest(pc,
24);const Tg=(new Location$$module$js$nodes(a,125,x,"Tunnel 1 (to Shyron)")).cave().connect(Pg),Ug=(new Location$$module$js$nodes(a,126,x,"Outside Shyron")).overworld().connect(Tg);(new Location$$module$js$nodes(a,127,x,"Tunnel 2 (fork)")).cave().connect(Ob).connectTo(Rg).connectTo(Ad);const Dd=(new Location$$module$js$nodes(a,128,x,"Tunnel 3 (caves)")).cave().connect(Bd).connect(Qg),Ed=(new Location$$module$js$nodes(a,129,x,"Tunnel 4 (left branch of cave)")).cave().connect(Dd);(new Location$$module$js$nodes(a,
130,x,"Tunnel 5 (dead end, medical herb)")).cave().connect(Ed).chest(D,15,86);const Vg=(new Location$$module$js$nodes(a,131,x,"Tunnel 6a (left-then-right)")).cave().connect(Ed),Wg=(new Location$$module$js$nodes(a,131,x,"Tunnel 6b (past wall)")).cave().connect(Vg,v),Xg=(new Location$$module$js$nodes(a,132,x,"Tunnel 7 (wide hall)")).cave().connect(Wg);(new Location$$module$js$nodes(a,133,x,"Tunnel 8 (red slimes)")).cave().from(Xg,v).connectTo(Cd).chest(M,23,116);const Yg=(new Location$$module$js$nodes(a,
134,x,"Tunnel 9 (right branch, infinite loop)")).cave().connect(Dd),Zg=(new Location$$module$js$nodes(a,135,x,"Tunnel 10a (toward magic ring)")).cave().connect(Yg);(new Location$$module$js$nodes(a,135,x,"Tunnel 10b (past wall)")).cave().connect(Zg,v).connectTo(Sg);const Fd=(new Location$$module$js$nodes(a,136,sa,"Entrance")).fortress().from(Bd,hc,Eb),$g=(new Location$$module$js$nodes(a,137,sa,"Left branch")).fortress().connect(Fd).chest(M,19,117),ah=(new Location$$module$js$nodes(a,137,sa,"Left branch, past one bridge")).fortress().connect($g,
t),bh=(new Location$$module$js$nodes(a,137,sa,"Left branch, past two bridges")).fortress().connect(ah,t).chest(D,29,87),ch=(new Location$$module$js$nodes(a,137,sa,"Right branch")).fortress().connect(Fd);(new Location$$module$js$nodes(a,137,sa,"Right branch, across water")).fortress().connect(ch,z).chest(M,20,118).chest(M,21,119).chest(Xd,28);(new Location$$module$js$nodes(a,138,sa,"Upper floor")).fortress().connect(bh).chest(K,27);const Ga=(new Location$$module$js$nodes(a,140,la,"Town")).town().connect(Ug,
Pe).trigger(Db),Oa=(new Location$$module$js$nodes(a,142,ta,"Town")).town().connect(lb),Gd=(new Location$$module$js$nodes(a,143,F,"Draygonia Fortress Basement 1 (front)")).fortress();(new Location$$module$js$nodes(a,143,F,"Draygonia Fortress Basement 2 (power ring)")).fortress().connect(Gd,xa).chest(de,15);const Pb=(new Location$$module$js$nodes(a,144,ta,"Desert 1")).overworld().connect(lb),mb=(new Location$$module$js$nodes(a,145,F,"Area 1 (from entrance)")).cave().chest(ec,26),Hd=(new Location$$module$js$nodes(a,
145,F,"Area 2 (across top bridge)")).cave().connect(mb,t);(new Location$$module$js$nodes(a,145,F,"Area 3 (dead-end across top-right bridge)")).cave().connect(Hd,t);const dh=(new Location$$module$js$nodes(a,145,F,"Area 4 (left across middle-right bridge)")).cave().connect(Hd,t),eh=(new Location$$module$js$nodes(a,145,F,"Area 5 (bottom edge)")).cave().connect(dh,t),fh=(new Location$$module$js$nodes(a,145,F,"Area 6 (bottom island)")).cave().connect(eh,t),gh=(new Location$$module$js$nodes(a,145,F,"Area 7 (bottom inner ring)")).cave().connect(fh,
t),hh=(new Location$$module$js$nodes(a,145,F,"Area 8 (left outer ring)")).cave().connect(gh,t).connect(mb,t).chest(L,27,100),ih=(new Location$$module$js$nodes(a,145,F,"Area 9 (top left inner ring)")).cave().connect(hh,t),jh=(new Location$$module$js$nodes(a,145,F,"Area 10 (top right inner ring)")).cave().connect(ih,t);(new Location$$module$js$nodes(a,145,F,"Area 11 (center)")).cave().connect(jh,t).connectTo(Gd);(new Location$$module$js$nodes(a,145,F,"Area 12 (top center islands)")).cave().connect(mb,
z).chest(be,28);const kh=(new Location$$module$js$nodes(a,146,Z,"Desert Cave 1")).cave().connect(Pb,z),Ha=(new Location$$module$js$nodes(a,147,Z,"Town")).town(),lh=(new Location$$module$js$nodes(a,148,Z,"Outside Cave")).overworld().connect(Ha),mh=(new Location$$module$js$nodes(a,149,Z,"Desert Cave 2")).cave().connect(lh);(new Location$$module$js$nodes(a,150,Z,"Meadow")).overworld().connect(kh).connectTo(Ha).trigger(ze,pa);const Qb=(new Location$$module$js$nodes(a,152,Z,"Desert 2")).overworld().connect(mh),
nh=(new Location$$module$js$nodes(a,156,Ca,"Entrance")).fortress().connect(Qb,z),oh=(new Location$$module$js$nodes(a,156,Ca,"Azteca")).fortress().trigger(Be),ph=(new Location$$module$js$nodes(a,157,Ca,"Fork")).fortress().connect(nh),Id=(new Location$$module$js$nodes(a,158,Ca,"Main")).fortress().connect(ph);(new Location$$module$js$nodes(a,158,Ca,"Treasure Chest (magic ring)")).fortress().connect(Id).chest(V,27,108);(new Location$$module$js$nodes(a,159,Ca,"Draygon")).fortress().connect(Id).to(oh).boss(ff);
const qh=(new Location$$module$js$nodes(a,160,ca,"Entrance")).fortress().connect(Qb,z),rh=(new Location$$module$js$nodes(a,160,ca,"Statues")).fortress().connect(qh).boss(gf),sh=(new Location$$module$js$nodes(a,161,ca,"Hall 1")).fortress().connect(rh),Rb=(new Location$$module$js$nodes(a,162,ca,"Branch")).fortress().connect(sh);(new Location$$module$js$nodes(a,163,ca,"Left Dead End")).fortress().connect(Rb).chest(M,13,120);(new Location$$module$js$nodes(a,164,ca,"Right Dead End")).fortress().connect(Rb);
const th=(new Location$$module$js$nodes(a,165,ca,"Hall 2")).fortress().connect(Rb).chest(sb,26,109),uh=(new Location$$module$js$nodes(a,166,ca,"Draygon 2")).fortress().connect(th).boss(hf);(new Location$$module$js$nodes(a,167,ca,"Teleporter")).fortress().from(uh).to(sd);const Jd=(new Location$$module$js$nodes(a,168,db,"Entrance")).fortress().connect(Oa,Eb).trigger(Mc,K),vh=(new Location$$module$js$nodes(a,169,db,"Main")).fortress().from(Jd,xa),wh=(new Location$$module$js$nodes(a,169,db,"Boss")).fortress().connect(vh).boss(bf),
xh=(new Location$$module$js$nodes(a,170,db,"Zebu")).fortress().connect(wh),nb=(new Location$$module$js$nodes(a,171,Y,"Entrance")).fortress().connect(xh);(new Location$$module$js$nodes(a,171,Y,"Dead End Behind Iron (fruit of power)")).fortress().connect(nb,xa).chest(L,28,98);(new Location$$module$js$nodes(a,171,Y,"Dead End Loop Across Closer Bridges")).fortress().connect(nb,t).connect(nb,t);const yh=(new Location$$module$js$nodes(a,171,Y,"Across First Bridge (fruit of repun)")).fortress().connect(nb,
t).chest(rb,30,102),Sb=(new Location$$module$js$nodes(a,171,Y,"Across Second Bridge")).fortress().connect(yh,t);(new Location$$module$js$nodes(a,171,Y,"Dead End Across Two Bridges ()")).fortress().connect(Sb,t).connect(Sb,t).chest(wa,29,93);const zh=(new Location$$module$js$nodes(a,171,Y,"Across Third Bridge")).fortress().connect(Sb,t),Ah=(new Location$$module$js$nodes(a,171,Y,"Exit Behind Iron Door")).fortress().connect(zh,xa),Bh=(new Location$$module$js$nodes(a,172,Y,"Boss")).fortress().connect(Ah).boss(cf),
Ch=(new Location$$module$js$nodes(a,172,Y,"Tornel")).fortress().connect(Bh),Kd=(new Location$$module$js$nodes(a,173,Ba,"Lower")).fortress().connect(Ch).chest(sb,26,99).chest(V,27,111),Dh=(new Location$$module$js$nodes(a,174,Ba,"Upper Loop")).fortress().connect(Kd).chest(J,22,96);(new Location$$module$js$nodes(a,174,Ba,"Upper Loop Behind Wall (magic ring)")).fortress().connect(Dh,xa).chest(V,23,107);const Eh=(new Location$$module$js$nodes(a,175,Ba,"Upper Passage (toward Mado)")).fortress().connect(Kd).chest(V,
27,84),Tb=(new Location$$module$js$nodes(a,176,N,"Initial Fork")).fortress(),Fh=(new Location$$module$js$nodes(a,177,N,"Left Branch")).fortress().connect(Tb),Ld=(new Location$$module$js$nodes(a,178,N,"Main Area (right branch, over bridges)")).fortress().connect(Tb),Gh=(new Location$$module$js$nodes(a,179,N,"U-shaped Passage (between floors)")).fortress().connect(Ld),Hh=(new Location$$module$js$nodes(a,180,N,"Main Area Lower (under bridge)")).fortress().connect(Fh).connect(Ld).connect(Gh),Md=(new Location$$module$js$nodes(a,
180,N,"Behind Iron Wall")).fortress().connect(Hh,xa),Ih=(new Location$$module$js$nodes(a,181,N,"Lower")).fortress().connect(Md).chest(M,13,121).chest(M,14,122).chest(M,15,123).chest(V,23,88).chest(P,24,110),Jh=(new Location$$module$js$nodes(a,182,N,"Boss Corridor")).fortress().connect(Ih),Kh=(new Location$$module$js$nodes(a,182,N,"Boss")).fortress().connect(Jh,Eb).boss(ef);(new Location$$module$js$nodes(a,182,N,"Behind Boss (stormBracelet)")).fortress().connect(Kh).chest(Wa,18);const Nd=(new Location$$module$js$nodes(a,
183,N,"Exit Stairs")).fortress(),Lh=(new Location$$module$js$nodes(a,184,F,"Entrance Back (behind river)")).cave().connect(Nd).chest(L,13,90);(new Location$$module$js$nodes(a,184,F,"Entrance Front")).cave().connect(Lh,z).connectTo(Pb).connectTo(mb);const Mh=(new Location$$module$js$nodes(a,185,Ba,"Boss")).fortress().connect(Eh).boss(df);(new Location$$module$js$nodes(a,185,Ba,"Asina")).fortress().connect(Mh).connectTo(Tb);(new Location$$module$js$nodes(a,186,N,"Kensu")).fortress().connect(Md).connectTo(Nd).trigger(ye,
yb);(new Location$$module$js$nodes(a,187,ta,"House")).house().connect(Oa).trigger(Ae,pa,yb);(new Location$$module$js$nodes(a,188,ta,"Inn")).shop().connect(Oa,Db);(new Location$$module$js$nodes(a,190,ta,"Tool Shop")).shop(D,J,P,L).connect(Oa,Db);(new Location$$module$js$nodes(a,191,ta,"Tavern")).shop().connect(Oa);(new Location$$module$js$nodes(a,192,W,"Elder House")).house().connect(ma).trigger(tc);(new Location$$module$js$nodes(a,193,W,"Rabbit Hut")).house().connect(ma).trigger(zc,yc,oa);(new Location$$module$js$nodes(a,
194,W,"Inn")).shop().connect(ma);(new Location$$module$js$nodes(a,195,W,"Tool Shop")).shop(D,J,P,L).connect(ma);(new Location$$module$js$nodes(a,196,W,"Armor Shop")).shop(Yd,Xb).connect(ma);(new Location$$module$js$nodes(a,197,W,"Student House")).house().connect(ma).trigger(uc);(new Location$$module$js$nodes(a,198,za,"Tavern")).house().connect(Na);(new Location$$module$js$nodes(a,199,za,"Pawn Shop")).shop().connect(Na);(new Location$$module$js$nodes(a,200,za,"Inn")).shop().connect(Na);(new Location$$module$js$nodes(a,
201,za,"Armor Shop")).shop(Zd,Xb,Vd).connect(Na);(new Location$$module$js$nodes(a,203,za,"Tool Shop")).shop(D,J,P).connect(Na);(new Location$$module$js$nodes(a,205,La,"Elder House")).house().from(fb,oa).trigger(ne,Ab);(new Location$$module$js$nodes(a,206,La,"Mother's House")).house().from(fb,oa).trigger(xc,oa).trigger(me,Ab);(new Location$$module$js$nodes(a,207,La,"Tool Shop")).shop(D,J,L).from(fb,oa);(new Location$$module$js$nodes(a,208,La,"Inn")).shop().from(fb,oa);(new Location$$module$js$nodes(a,
209,Aa,"Inn")).shop().connect(eb);(new Location$$module$js$nodes(a,210,Aa,"Tool Shop")).shop(P,L,wa).connect(eb);(new Location$$module$js$nodes(a,211,Aa,"Armor Shop")).shop(ac,Yb,Zb,pb).connect(eb);const Nh=(new Location$$module$js$nodes(a,212,Aa,"Queen's House")).house().from(eb,Qe).trigger(we,pa,nc);(new Location$$module$js$nodes(a,213,cb,"Nadare's")).house().connect(gb).connectTo(Vf).connectTo(Wf).connectTo(Xf);(new Location$$module$js$nodes(a,214,y,"Fisherman's House")).house().connect(qd).trigger(Bb,
ic,xb,Re);const Ub=(new Location$$module$js$nodes(a,215,y,"Palace Entrance")).house().connect(ua);(new Location$$module$js$nodes(a,216,y,"Fortune Teller Front")).house().connect(ua).trigger(Bc,Ac);(new Location$$module$js$nodes(a,216,y,"Fortune Teller Back")).house().connect(Bg).connect(vd);(new Location$$module$js$nodes(a,217,y,"Pawn Shop")).shop().connect(ua);(new Location$$module$js$nodes(a,218,y,"Armor Shop")).shop($d,ac,Yb).connect(ua);(new Location$$module$js$nodes(a,220,y,"Inn")).shop().connect(ua);
(new Location$$module$js$nodes(a,221,y,"Tool Shop")).shop(D,wa,qb,P).connect(ua);(new Location$$module$js$nodes(a,222,y,"Palace Left")).house().connect(Ub);(new Location$$module$js$nodes(a,223,y,"Palace Throne Room")).house().connect(Ub).to(jb,Ke).from(jb).trigger(Ac).trigger(qe,Bc,Cc);(new Location$$module$js$nodes(a,224,y,"Palace Right")).house().connect(Ub);(new Location$$module$js$nodes(a,225,y,"Asina's Room")).house().connect(wd).trigger(Fc,Pc);(new Location$$module$js$nodes(a,226,Aa,"Queen Downstairs")).house().connect(Nh).chest(Ua,
13);(new Location$$module$js$nodes(a,227,ja,"Elder's House")).house().connect(kb).trigger(Ic);(new Location$$module$js$nodes(a,228,ja,"Shed")).house().connect(kb).to(Mg,jc);(new Location$$module$js$nodes(a,229,ja,"Tool Shop")).shop(D,J,L,wa).connect(kb);(new Location$$module$js$nodes(a,231,ja,"Inn")).shop().connect(kb);const Oh=(new Location$$module$js$nodes(a,232,H,"Zombie Town House")).house().connect(Mb);(new Location$$module$js$nodes(a,233,H,"Zombie Town Basement")).house().connect(Oh).trigger(se,
Xc);(new Location$$module$js$nodes(a,235,X,"Tool Shop")).shop(D,J,L,P).connect(ea);(new Location$$module$js$nodes(a,236,X,"Stom's Hut")).house().connect(ea).trigger(Jc);(new Location$$module$js$nodes(a,237,X,"Inn")).shop().connect(ea);(new Location$$module$js$nodes(a,238,X,"Armor Shop")).shop(ae,bc,Wd,$b).connect(ea);(new Location$$module$js$nodes(a,239,X,"Tavern")).house().connect(ea).trigger(Kc,Jc,Ja);(new Location$$module$js$nodes(a,240,X,"Pawn Shop")).shop().connect(ea);(new Location$$module$js$nodes(a,
241,X,"Dance Hall")).house().connect(ea).trigger(Lc,Kc,Ja).trigger(ve,Lc,mc);const Ph=(new Location$$module$js$nodes(a,242,la,"Temple (pre-massacre)")).fortress().connect(Ga).from(Fb,K,Sd).trigger(xe);(new Location$$module$js$nodes(a,242,la,"Temple (post-massacre)")).fortress().from(Ph,Mc).boss(af);(new Location$$module$js$nodes(a,243,la,"Training Hall")).house().connect(Ga);(new Location$$module$js$nodes(a,244,la,"Hospital")).house().connect(Ga);(new Location$$module$js$nodes(a,245,la,"Armor Shop")).shop(bc,
pb,$b).connect(Ga);(new Location$$module$js$nodes(a,246,la,"Tool Shop")).shop(D,J,qb,V).connect(Ga);(new Location$$module$js$nodes(a,247,la,"Inn")).shop().connect(Ga);(new Location$$module$js$nodes(a,248,Z,"Inn")).shop().connect(Ha);(new Location$$module$js$nodes(a,249,Z,"Tool Shop")).shop(J,P,rb,V).connect(Ha);(new Location$$module$js$nodes(a,250,Z,"Elder's House")).house().connect(Ha);(new Location$$module$js$nodes(a,251,Z,"Pawn Shop")).house().connect(Ha);const Qh=[ma,Ma,Zc,Da,Gb,Ea,Jb,kd,Lb,ea,
lb,Ob,Pb,Jd,Qb];for(const a of Qh)a.from(Fb,Fe);return a},shuffle$$module$js$depgraph=(b,a,c,d,e)=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){const f=generate$$module$js$depgraph(d);d.shuffleShops()&&f.shuffleShops(a);const g=f.nodes.filter((a)=>a instanceof Slot$$module$js$nodes&&a.slots&&a.slotName);d||(d=new FlagSet$$module$js$flagset("Sbkm Sct"));var k={};for(var h of g)if(h instanceof Slot$$module$js$nodes&&h.slots){var n=h.slotType[0];(k[n]=k[n]||[]).push(h)}h=[];n=[];var m={};
for(var q of d.flags.S||[]){var p=[];for(var l of q){m[l]=!0;for(var r of k[l])p.push(r),n.push([r,h.length])}h.push(p)}for(var I of"bc")if(m[I])for(q=k[I],l=q.map((a)=>[a.item,a.itemIndex]),a.shuffle(l),r=0;r<q.length;r++)q[r].set(...l[r]);p=(a)=>g.find((b)=>b.vanillaItemName===a);k=p("Sword of Wind");I=p("Sword of Fire");q=p("Ball of Wind");l=p("Ball of Fire");r=p("Statue of Onyx");p=p("Gas Mask");m.k&&a.nextInt(2)&&(I.swap(k),l.swap(q));m.k&&!a.nextInt(3)&&r.swap(p);m=[1,1,1,1,2,2,2,3,3,3,5,5,
8,13];k=[];e&&e.addTasks(10);for(I=0;1E3>I;I++){e&&0==I%100&&(yield new Promise(requestAnimationFrame),e.addCompleted(1));a.shuffle(n);for(var C of h)a.shuffle(C);q=h.map(()=>-1);l=m[a.nextInt(m.length)];r=[];p=(a,b)=>a.itemIndex===b.itemIndex||a.isMimic()&&!b.canHoldMimic()||b.isMimic()&&!a.canHoldMimic()||a.needsChest()&&!b.isChest()||b.needsChest()&&!a.isChest()?!1:!0;for(let a=0;a<l;a++){const [b,c]=n[a],d=h[c][++q[c]];p(b,d)&&(b.swap(d),r.push([b,d]))}if(!r.length)continue;const {win:b,path:c}=
f.traverse({wanted:g,dfs:!1});if(b)k=c;else for(;r.length;){const [a,b]=r.pop();a.swap(b)}}b&&f.write(b);if(c){c.items=[];c.route=[];for(const a of k){const [b,d]=a;c.route.push(d);C=f.nodes[b];null!=C.slotName&&(h=C.slotName,0>h.indexOf(C.orig)&&(h+=` (normally ${C.vanillaItemName})`),c.items.push({slotIndex:C.slotIndex,itemIndex:C.itemIndex,origName:C.vanillaItemName,slotName:C.slotName,itemName:C.item.name,text:`${C.item.name}: ${h}`}))}}}),shuffle2$$module$js$depgraph=(b,a,c,d,e)=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){const f=
generate$$module$js$depgraph(d);d.shuffleShops()&&f.shuffleShops(a);const g=f.integrate();e&&e.addTasks(1E3);for(let k=0;1E3>k;k++){if(yield shuffle3$$module$js$depgraph(f,g,b,a,c,d,e)){console.log(`success after ${k} attempts`);return}e&&(e.addCompleted(1),0===k%50&&(yield new Promise(requestAnimationFrame)))}return shuffle$$module$js$depgraph(b,a,c,d,e)}),shuffle3$$module$js$depgraph=(b,a,c,d,e,f,g)=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){const g=e&&e.stats;var h=b.nodes.filter((a)=>
a instanceof Slot$$module$js$nodes&&a.slots&&a.slotName);const n=new Map(d.shuffle(h.map((a)=>[a,[a.item,a.itemIndex]]))),m=new Set(d.shuffle(h)),q=new Map,p={};g&&g.seeds++;f||(f=new FlagSet$$module$js$flagset("Sbkm Sct"));(f.flags.S||[]).forEach((a,b)=>{for(const c of a)p[c]=b});for(var l of m)q.set(l.item,l);let r=new Set;const I=(a,b)=>{if(null==p[a.slotType?a.slotType[0]:"c"])return a==b;if(b.isMimic()&&!a.canHoldMimic()||"unique"!=b.item.inventoryRow&&a.requiresUnique||"armor"==b.item.inventoryRow&&
!a.isChest())return!1;if(f.guaranteeSword()){if(0===a.slotIndex)return 4>b.item.id;if(4>b.item.id&&(r.add(b),4===r.size))return 0===a.slotIndex}return p[a.slotType?a.slotType[0]:"c"]==p[b.slotType?b.slotType[0]:"c"]};l=a.assumedFill(d,(a,b)=>I(a,q.get(b)));if(null==l){g&&g.endState.key++;return!1;throw Error("Could not fill!");}const C=new Map;for(var S=0;S<l.length;S++){var va=l[S];if(null!=va){var na=q.get(a.item(va));va=n.get(na);n.delete(na);na=a.location(S);C.set(na,va);m.delete(na)}}S=(a,b,
c)=>{c=void 0===c?"":c;for(const c of m)if(I(c,a))return C.set(c,b),m.delete(c),n.delete(a),!0;g&&("traps"==c?g.endState.traps++:"chest"==c?g.endState.chest++:g.endState.misc++);return!1};for(var E of n){const [a,b]=E;if(a.isMimic()&&!S(a,b,"traps"))return!1}for(const a of n){const [b,c]=a;if(b.needsChest()&&!S(b,c,"chest"))return!1}for(const a of n){const [b,c]=a;if(!S(b,c))return!1}for(const a of C){const [b,c]=a;b.set(...c)}const {win:G,path:Pa}=b.traverse({wanted:h,dfs:!1});if(!G)return g&&g.endState.no_win++,
!1;c&&b.write(c);if(!e)return!0;g&&(g.endState.success++,g.analyze(b,a,l));e.items=[];e.route=[];for(const a of Pa){const [c,d]=a;e.route.push(d);h=b.nodes[c];null!=h.slotName&&(E=h.slotName,0>E.indexOf(h.orig)&&(E+=` (normally ${h.vanillaItemName})`),e.items.push({slotIndex:h.slotIndex,itemIndex:h.itemIndex,origName:h.vanillaItemName,slotName:h.slotName,itemName:h.item.name,text:`${h.item.name}: ${E}`}))}return!0});var module$js$depgraph={};module$js$depgraph.generate=generate$$module$js$depgraph;
module$js$depgraph.shuffle=shuffle$$module$js$depgraph;module$js$depgraph.shuffle2=shuffle2$$module$js$depgraph;module$js$depgraph.shuffle3=shuffle3$$module$js$depgraph;class FetchReader$$module$js$fetchreader{constructor(b){this.path=void 0===b?"js/":b}read(b){const a=this;return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){return yield(yield fetch(a.path+b)).text()})}}var module$js$fetchreader={};module$js$fetchreader.FetchReader=FetchReader$$module$js$fetchreader;class Deque$$module$js$util{constructor(){this.buffer=Array(16);this.mask=15;this.size=this.end=this.start=0}get length(){return this.size}upsize(b){for(;this.mask<b;)this.end<=this.start&&(this.start+=this.mask+1),this.mask=this.mask<<1|1,this.buffer=this.buffer.concat(this.buffer);this.size=b}push(...b){this.upsize(this.size+b.length);for(const a of b)this.buffer[this.end]=a,this.end=this.end+1&this.mask}pop(){if(this.size)return this.end=this.end-1&this.mask,this.size--,this.buffer[this.end]}peek(){if(this.size)return this.buffer[this.end-
...
4<=e.sells.length?g.push(d):e.sells.push(d));for(const {shops:b}of[a,c])for(const a of b)a.sells.sort((a,b)=>a.id-b.id)}integrate(b={}){const {tracker:a=!1}=b;var c=!a;const d=new SparseDependencyGraph$$module$js$graph(this.nodes.length),e=[],f=[],g=new Set,k=new Map;var h=[],n=[];const m=[];var q=[];const p=[];b=new Set;for(var l of this.nodes)if(l instanceof Location$$module$js$nodes){if(l.isStart){const [a]=d.addRoute([l.uid]);k.set(a.label,a)}e[l.uid]=[];f[l.uid]=[];for(const a of l.connections)g.has(a)||
(g.add(a),a.bidi&&g.add(a.reverse()));for(const a of l.chests)d.addRoute([a.uid,l.uid])}else if(l instanceof ItemGet$$module$js$nodes&&"Medical Herb"===l.name&&(d.addRoute([l.uid]),d.finalize(l.uid)),l instanceof Option$$module$js$nodes)h.push(l);else if(l instanceof TrackerNode$$module$js$nodes)n.push(l);else if(l instanceof Condition$$module$js$nodes)m.push(l);else if(l instanceof Trigger$$module$js$nodes)q.push(l);else if(l instanceof ItemGet$$module$js$nodes)p.push(l);else if(l instanceof Slot$$module$js$nodes)b.add(l);
else if(!(l instanceof Area$$module$js$nodes))throw Error(`Unknown node type: ${l.nodeType}`);for(const a of g)e[a.from.uid].push(a),f[a.to.uid].push(a);l=(b)=>{for(const c of b){for(const b of c.edges({tracker:a}))d.addRoute(b);d.finalize(c.uid)}};l(h);c&&l(n);l(q);for(c=k.values();!(h=c.next()).done;){h=h.value;n=h.target;for(var r of e[n]){n=[r.to.uid,...h.deps];for(q=r.deps.length-1;0<=q;q--)n.push(r.deps[q].uid);r.from.bossNode&&n.push(r.from.bossNode.uid);for(const a of d.addRoute(n))k.has(a.label)||
k.set(a.label,a)}}l(m);for(r=0;r<this.nodes.length;r++)this.nodes[r]instanceof ItemGet$$module$js$nodes||this.nodes[r]instanceof TrackerNode$$module$js$nodes||d.finalize(r);r=new LocationList$$module$js$nodes(this);for(const a of b)for(const b of d.nodes[a.uid].values())r.addRoute([a.uid,...b]);return r}}
...
f.add(e);g=f[Symbol.iterator]();for(var k;!(k=g.next()).done;){k=k.value;f.delete(k);if(k===e){f.size&&f.add(e);c++;continue}if(null!=d[k])continue;const g=this.routes[k];for(let e=0;e<g.length;e++)if(Bits$$module$js$bits.containsAll(a,g[e])){d[k]=c;if(b[k]){a=Bits$$module$js$bits.with(a,b[k]);for(let a of this.unlocks[b[k]])f.add(a)}break}}return d}toString(){const b=[];for(let a=0;a<this.routes.length;a++){const c=this.worldGraph.nodes[this.locationToUid[a]],d=this.routes[a],e=[];for(let a=0;a<
...
[244]:"Shyron - Hospital",[245]:"Shyron - Armor Shop",[246]:"Shyron - Tool Shop",[247]:"Shyron - Inn",[248]:"Sahara - Inn",[249]:"Sahara - Tool Shop",[250]:"Sahara - Elder House",[251]:"Sahara - Pawn Shop"},BAD_SCREENS$$module$js$rom={36:[[3,4,128]],66:[[0,2,0]],145:[[0,11,128],[1,11,128],[2,11,128],[3,11,128],[4,11,128],[5,11,128],[6,11,128],[7,11,128]]};
class Entity$$module$js$rom{constructor(b,a){this.rom=b;this.id=a}toString(){return this.constructor.name+" $"+this.id.toString(16).padStart(2,0)}}
class Screen$$module$js$rom extends Entity$$module$js$rom{constructor(b,a){super(b,a);this.base=(255<a?64+a:a)<<8;this.tiles=seq$$module$js$rom(15,(a)=>slice$$module$js$rom(b.prg,this.base|a<<4,16))}metatile(b,a){return this.rom.metatiles[this.tiles[b][a]]}allTilesSet(){const b=new Set;for(const a of this.tiles)for(const c of a)b.add(c);return b}write(b){b=void 0===b?this.rom:b;let a=this.base;for(const c of this.tiles)for(const d of c)b[a++]=d}}
class Tileset$$module$js$rom extends Entity$$module$js$rom{constructor(b,a){super(b,a);a&=63;this.tileBase=65536|a<<8;this.attrBase=77824|a<<4;this.alternatesBase=81408|a<<3;this.tiles=seq$$module$js$rom(4,(a)=>slice$$module$js$rom(b.prg,this.tileBase|a<<8,256));this.attrs=seq$$module$js$rom(256,(a)=>b.prg[this.attrBase|a>>2]>>((a&3)<<1)&3);this.alternates=slice$$module$js$rom(b.prg,this.alternatesBase,32)}write(b){b=void 0===b?this.rom:b;for(var a=0;256>a;a++){32>a&&(b[this.alternatesBase+a]=this.alternates[a]);
for(var c=0;4>c;c++)b[this.tileBase+(c<<8)+a]=this.tiles[c][a]}for(a=0;64>a;a++)c=a<<2,b[this.attrBase+a]=this.attrs[c]&3|(this.attrs[c+1]&3)<<2|(this.attrs[c+2]&3)<<4|(this.attrs[c+3]&3)<<6}makeFlagTiles(b){}}class TileEffects$$module$js$rom extends Entity$$module$js$rom{constructor(b,a){super(b,a);this.base=a<<8&8191|73728;this.effects=slice$$module$js$rom(b.prg,this.base,256)}write(b){b=void 0===b?this.rom:b;for(let a=0;256>a;a++)b[this.base+a]=this.effects[a]}}
class Palette$$module$js$rom extends Entity$$module$js$rom{constructor(b,a){super(b,a);this.base=(a&3)<<2|(a&252)<<6|16624;this.colors=slice$$module$js$rom(b.prg,this.base,4)}color(b){return this.colors[b]&63}}
...
class Trigger$$module$js$rom extends Entity$$module$js$rom{constructor(b,a){super(b,a);this.used=!UNUSED_TRIGGERS$$module$js$rom.has(a);this.pointer=123258+((a&127)<<1);this.base=addr$$module$js$rom(b.prg,this.pointer,81920);this.conditions=[];this.message=null;this.flags=[];let c=this.base;do{a=b.prg[c]<<8|b.prg[c+1];var d=a&4095;this.conditions.push(a&8192?~d:d);c+=2}while(!(a&32768));this.message=MessageId$$module$js$rom.from(b.prg,c);do c+=2,a=b.prg[c]<<8|b.prg[c+1],d=a&4095,this.flags.push(a&
32768?~d:d);while(!(a&16384))}bytes(){const b=[];for(var a=0;a<this.conditions.length;a++){var c=this.conditions[a];0>c&&(c=~c|8192);a===this.conditions.length-1&&(c|=32768);b.push(c>>>8,c&255)}b.push(...this.message.bytes());for(a=0;a<this.flags.length;a++)c=this.flags[a],0>c&&(c=~c|32768),a===this.flags.length-1&&(c|=16384),b.push(c>>>8,c&255);return b}write(b,a){a=void 0===a?123258:a;const c=this;return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){const d=yield b.write(c.bytes());b.rom[a+
2*(c.id&127)]=d&255;b.rom[a+2*(c.id&127)+1]=(d>>>8)-64})}}class MessageId$$module$js$rom{constructor(b,a,c){this.action=b;this.part=a;this.index=c}static from(b,a){b=b[a]<<8|b[a+1];return new MessageId$$module$js$rom(b>>>11&31,b>>>5&63,b&31)}bytes(){const b=(this.action&31)<<11|(this.part&63)<<5|this.index&31;return[b>>>8,b&255]}}const UNUSED_NPCS$$module$js$rom=new Set([60,106,115,130,134,135,137,138,139,140,141]);
class NpcSpawn$$module$js$rom extends Entity$$module$js$rom{constructor(b,a){super(b,a);this.pointer=116192+(a<<1);this.base=addr$$module$js$rom(b.prg,this.pointer,81920);this.used=!UNUSED_NPCS$$module$js$rom.has(a)&&(143>a||192<=a);this.conditions={};a=this.base;let c;for(;this.used&&255!=(c=b.prg[a++]);){this.conditions[c]=[];let d;do{d=b.prg[a]<<8|b.prg[a+1];const e=d&4095;this.conditions[c].push(d&8192?~e:e);a+=2}while(!(d&32768))}}bytes(){const b=[];for(let a in this.conditions){a=Number(a);
b.push(a);for(let c=0;c<this.conditions[a].length;c++){let d=this.conditions[a][c];0>d&&(d=~d|8192);c===this.conditions[a].length-1&&(d|=32768);b.push(d>>>8,d&255)}}b.push(255);return b}write(b,a){a=void 0===a?116192:a;const c=this;return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){const d=yield b.write(c.bytes());b.rom[a+2*c.id]=d&255;b.rom[a+2*c.id+1]=(d>>>8)-64})}}
...
...b?b.palettes():[]])]}}ObjectData$$module$js$rom.setupProps();
...
class Rom$$module$js$rom{constructor(b){this.prg=b.subarray(16,262160);this.chr=b.subarray(262160);this.screens=seq$$module$js$rom(259,(a)=>new Screen$$module$js$rom(this,a));this.tilesets=seq$$module$js$rom(12,(a)=>new Tileset$$module$js$rom(this,a<<2|128));this.tileEffects=seq$$module$js$rom(11,(a)=>new TileEffects$$module$js$rom(this,a+179));this.triggers=seq$$module$js$rom(67,(a)=>new Trigger$$module$js$rom(this,128|a));this.patterns=seq$$module$js$rom(this.chr.length>>4,(a)=>new Pattern$$module$js$rom(this,
a));this.palettes=seq$$module$js$rom(256,(a)=>new Palette$$module$js$rom(this,a));this.locations=seq$$module$js$rom(256,(a)=>LOCATION_NAMES$$module$js$rom[a]?new Location$$module$js$rom(this,a):null);this.tileAnimations=seq$$module$js$rom(4,(a)=>new TileAnimation$$module$js$rom(this,a));this.hitboxes=seq$$module$js$rom(24,(a)=>new Hitbox$$module$js$rom(this,a));this.npcSpawns=seq$$module$js$rom(205,(a)=>new NpcSpawn$$module$js$rom(this,a));this.objects=seq$$module$js$rom(256,(a)=>new ObjectData$$module$js$rom(this,
a));this.adHocSpawns=seq$$module$js$rom(96,(a)=>new AdHocSpawn$$module$js$rom(this,a));this.metasprites=seq$$module$js$rom(256,(a)=>new Metasprite$$module$js$rom(this,a));this.messages=new Messages$$module$js$rom(this);this.itemGets=seq$$module$js$rom(113,(a)=>new ItemGet$$module$js$rom(this,a));this.npcs=seq$$module$js$rom(205,(a)=>new Npc$$module$js$rom(this,a))}get monsters(){let b=new Set;for(const a of this.locations)if(a&&a.objects)for(const c of a.objects)0==(c[2]&7)&&b.add(this.objects[c[3]+
80&255]);b=[...b];b.sort((a,b)=>a.id-b.id);return b}get projectiles(){let b=new Set;for(const a of this.monsters)a.child&&b.add(this.objects[this.adHocSpawns[a.child].object]);b=[...b];b.sort((a,b)=>a.id-b.id);return b}get monsterGraphics(){const b={};for(const a of this.locations)if(a&&a.objects)for(const c of a.objects)if(!(c[2]&7)){const d=c[2]&128?1:0,e=(c[3]+80).toString(16).padStart(2,0);(b[e]=b[e]||{})[`${d}:${a.spritePatterns[d].toString(16)}:${a.spritePalettes[d].toString(16)}`]={slot:d,
pat:a.spritePatterns[d],pal:a.spritePalettes[d]}}return b}get locationMonsters(){const b={};for(const a of this.locations){if(!a||!a.objects)continue;const c=b["$"+a.id.toString(16).padStart(2,0)]={};for(const b of a.objects)if(!(b[2]&7)){const a=b[2]&128?1:0,d=b[3]+80;c[`${a}:${d.toString(16)}`]=(c[`${a}:${d.toString(16)}`]||0)+1}}return b}static load(b){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){const a=yield pickFile$$module$js$rom();b&&(yield b(a));return new Rom$$module$js$rom(a)})}writeObjectData(){let b=
110080;const a={};for(const c of this.objects){const d=c.serialize(),e=d.join(" ");e in a?c.objectDataBase=a[e]:(c.objectDataBase=b,a[e]=b,b+=d.length);c.write()}return b}writeData(){const b=this;return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){b.prg[246387]=b.prg[246392]=190;b.prg[246399]=b.prg[246404]=191;var a=new Writer$$module$js$rom(b.prg,83192,97792);const c=new Writer$$module$js$rom(b.prg,103417,113408),d=[];for(const e of b.locations)e&&(d.push(e.writeMapData(a)),d.push(e.writeNpcData(c)));
for(const a of b.objects)a.write(c,114176);yield a.commit();yield c.commit();a=new Writer$$module$js$rom(b.prg,123392,123888);for(var e of b.triggers)e.used&&d.push(e.write(a));yield a.commit();e=new Writer$$module$js$rom(b.prg,116602,117085);for(const a of b.npcSpawns)a.used&&d.push(a.write(e));yield e.commit();for(const a of b.tilesets)a.write(b.prg);for(const a of b.tileEffects)a.write(b.prg);for(const a of b.screens)a.write(b.prg);return Promise.all(d).then(()=>{})})}analyzeTiles(){}disjointTilesets(){var b=
[];for(var a of this.locations)if(a){var c=a.tileset,d=a.extended?256:0;for(const e of a.screens)for(const a of e)(b[a+d]||(b[a+d]=new Set)).add(c)}a=Array(256).fill(0).map(()=>new UnionFind$$module$js$unionfind);for(c=0;c<b.length;c++)if(b[c]){d=new Set;for(var e of this.screens[c].tiles)for(const a of e)d.add(a);for(const e of d)a[e].union([...b[c]])}for(b=0;b<a.length;b++)e=a[b].sets().map((a)=>[...a].map((a)=>a.toString(16)).join(" ")).join(" | "),console.log(`Tile ${b.toString(16).padStart(2,
0)}: ${e}`)}swapMetatiles(b,...a){var c=new Map,d=seq$$module$js$rom(256);const e=new Map,f=(a)=>Array.isArray(a)?a[0]:0>a?~a:a;for(var g of a){for(var k=0;k<g.length-1;k++)Array.isArray(g[k])&&(e.set(g[k][0],g[k][1]),g[k]=g[k][0]);for(k=0;k<g.length-1;k++){let a=g[k],b=g[k+1];0>a||0>b||(c.set(b,a),d[b]=a)}}g=new Set;c=new Set;b=new Set(b);for(var h of this.locations)if(h&&b.has(h.tileset)){c.add(h.tileEffects);for(const a of h.allScreens())g.add(a)}for(var n of g)for(var m of n.tiles)for(h=0;h<m.length;h++)m[h]=
d[m[h]];for(var q of b){b=this.tilesets[(q&127)>>>2];for(var p of a)for(d=0;d<p.length-1;d++){n=f(p[d]);m=f(p[d+1]);for(h=0;4>h;h++)b.tiles[h][n]=b.tiles[h][m];b.attrs[n]=b.attrs[m];if(32>m&&b.alternates[m]!=m){if(32<=n)throw Error(`Cannot unflag: ${q} ${n} ${m} ${b.alternates[m]}`);b.alternates[n]=b.alternates[m]}}for(var l of e){const [a,c]=l;b.alternates[a]=c}}for(const d of c){q=this.tileEffects[d-179];for(const c of a)for(p=0;p<c.length-1;p++)l=f(c[p]),b=f(c[p+1]),q.effects[l]=q.effects[b];for(const a of e.keys())q.effects[a]|=
8}}}
...
return Math.min(AM$$module$js$random*this.iy,RNMX$$module$js$random)}nextInt(b){return Math.floor(this.next()*b)}shuffle(b){for(let a=b.length;a;){const c=this.nextInt(a--);[b[a],b[c]]=[b[c],b[a]]}return b}}var module$js$random={};module$js$random.Random=Random$$module$js$random;const STATUS$$module$js$version="unstable",VERSION$$module$js$version="latest",LABEL$$module$js$version="latest 570e289",HASH$$module$js$version="570e28983a2f02d026d0043e579e719d45ca809e",DATE$$module$js$version=new Date(1555232661E3);var module$js$version={};module$js$version.STATUS=STATUS$$module$js$version;module$js$version.VERSION=VERSION$$module$js$version;module$js$version.LABEL=LABEL$$module$js$version;module$js$version.HASH=HASH$$module$js$version;module$js$version.DATE=DATE$$module$js$version;var $jscompDefaultExport$$module$js$patch={apply(b,a,c){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){let d;a.seed||(a.seed=parseSeed$$module$js$patch("").toString(16),window.location.hash+="&seed="+a.seed);d=a.flags?new FlagSet$$module$js$flagset(a.flags):new FlagSet$$module$js$flagset("Em Gt Mr Rlpt Sbk Sct Sm Tasd");for(const b in a)"false"===a[b]&&(a[b]=!1);yield shuffle$$module$js$patch(b,parseSeed$$module$js$patch(a.seed),d,new FetchReader$$module$js$fetchreader(c))})}};
...
_SAHARA_RABBITS_REQUIRE_TELEPATHY:c.saharaRabbitsRequireTelepathy(),_TELEPORT_ON_THUNDER_SWORD:c.teleportOnThunderSword()},h=new Assembler$$module$js$6502,n=(a)=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){h.assemble(yield d.read(a),a);h.patchRom(b)});var m=Object.keys(k).filter((a)=>k[a]).map((a)=>`define ${a} 1\n`).join("");h.assemble(m,"flags.s");yield n("preshuffle.s");g=new Random$$module$js$random(g);yield shuffle2$$module$js$depgraph(b,g,e,c,f);rescaleShops$$module$js$patch(b,
h,c.bargainHunting()?g:null);m=new Rom$$module$js$rom(b);rescaleMonsters$$module$js$patch(b,m);c.shuffleMonsters()&&shuffleMonsters$$module$js$patch(b,m,g);identifyKeyItemsForDifficultyBuffs$$module$js$patch(m);c.doubleBuffMedicalHerb?(b[115996]*=2,b[115962]*=3):c.singleBuffMedicalHerb()&&(b[115996]+=16,b[115962]*=2);c.connectLimeTreeToLeaf()&&connectLimeTreeToLeaf$$module$js$patch(m);addCordelWestTriggers$$module$js$patch(m,c);c.disableRabbitSkip()&&fixRabbitSkip$$module$js$patch(m);c.storyMode()&&
storyMode$$module$js$patch(m);c.chargeShotsOnly()&&disableStabs$$module$js$patch(m);closeCaveEntrances$$module$js$patch(m,c);misc$$module$js$patch(m,c);yield n("postshuffle.s");updateDifficultyScalingTables$$module$js$patch(b,c,h);updateCoinDrops$$module$js$patch(b,c);shuffleRandomNumbers$$module$js$patch(b,g);yield m.writeData();return stampVersionSeedAndHash$$module$js$patch(b,a,c)}),misc$$module$js$patch=(b,a)=>{},closeCaveEntrances$$module$js$patch=(b,a)=>{b.swapMetatiles([144],[7,[1,0],-194],
[14,[2,0],-194],[32,[3,10],-216],[33,[4,10],-216]);b.swapMetatiles([148,156],[104,[1,0],-194],[131,[2,0],-194],[136,[3,10],-216],[137,[4,10],-216]);b.screens[10].tiles[3][8]=1;b.screens[10].tiles[3][9]=2;b.screens[10].tiles[4][8]=3;b.screens[10].tiles[4][9]=4;b.screens[21].tiles[7][9]=1;b.screens[21].tiles[7][10]=2;b.screens[21].tiles[8][9]=3;b.screens[21].tiles[8][10]=4;b.screens[25].tiles[4][8]=1;b.screens[25].tiles[4][9]=2;b.screens[25].tiles[5][8]=3;b.screens[25].tiles[5][9]=4;b.screens[62].tiles[5][6]=
1;b.screens[62].tiles[5][7]=2;b.screens[62].tiles[6][6]=3;b.screens[62].tiles[6][7]=4;var c=[[3,48],[20,48],[21,48],[64,0],[64,20],[65,116],[71,16],[148,0],[152,65]];for(const a of c){const [c,d]=a;b.locations[c].flags.push([239,d])}c=(a,c,f)=>{for(const d of b.locations[a].flags)if(d[1]==c){d[0]=f;return}throw Error(`Could not find flag to replace at ${a}:${c}`);};a.paralysisRequiresPrisonKey()&&(c(20,48,238),c(21,48,238),c(64,0,216),b.locations[64].objects.splice(1,0,[6,6,4,44]),b.locations[64].objects.push([7,
7,2,173]))},eastCave$$module$js$patch=(b)=>{},addCordelWestTriggers$$module$js$patch=(b,a)=>{for(const c of b.locations[21].objects)2===c[2]&&(128>c[3]||a.disableTeleportSkip())&&b.locations[20].objects.push([...c])},fixRabbitSkip$$module$js$patch=(b)=>{for(const a of b.locations[40].objects)2===a[2]&&134===a[3]&&116===a[1]&&(a[0]++,a[1]++)},storyMode$$module$js$patch=(b)=>{const a=b.npcSpawns[203].conditions[166];a.push(~b.npcSpawns[194].conditions[40][0]);a.push(~b.npcSpawns[132].conditions[110][0]);
a.push(~b.triggers[26].conditions[1]);a.push(~b.npcSpawns[197].conditions[169][0]);a.push(~b.npcSpawns[198].conditions[172][0]);a.push(~b.npcSpawns[199].conditions[185][0]);a.push(~b.npcSpawns[200].conditions[182][0]);a.push(~b.npcSpawns[203].conditions[159][0]);a.push(512);a.push(513);a.push(514);a.push(515)},disableStabs$$module$js$patch=(b)=>{for(const a of[8,9,39])b.objects[a].collisionPlane=0},connectLimeTreeToLeaf$$module$js$patch=(b)=>{const a=b.locations[3];b=b.locations[66];a.screens[5][4]=
16;b.screens[1][0]=26;b.screens[2][0]=12;const c=a.entrances.push([239,4,120,5])-1,d=b.entrances.push([16,0,192,1])-1;a.exits.push([79,86,66,d],[79,87,66,d]);b.exits.push([0,27,3,c],[0,28,3,c])},stampVersionSeedAndHash$$module$js$patch=(b,a,c)=>{const d=crc32$$module$js$crc32(b),e=d.toString(16).padStart(8,0).toUpperCase(),f="unstable"==STATUS$$module$js$version?HASH$$module$js$version.substring(0,7).padStart(7,0).toUpperCase()+"     ":VERSION$$module$js$version.substring(0,12).padEnd(12," ");a=a.toString(16).padStart(8,
0).toUpperCase();const g=(a,c)=>{for(let d=0;d<c.length;d++)b[a+16+d]=c.charCodeAt(d)},k=(a,b)=>{const c=[];for(let d=0;d<a.length||d<b.length;d++)c.push(a[d]||" "),c.push(b[d]||" ");return c.join("")};g(161743,k("  VERSION     SEED      ",`  ${f}${a}`));a=String(c);let h;if(46<a.length){if(92<a.length)throw Error("Flag string way too long!");h=a.substring(46,92).padEnd(46," ");a=a.substring(0,46)}a=a.padEnd(46," ");g(161791,k(a.substring(0,23),a.substring(23)));h&&g(161839,k(h.substring(0,23),h.substring(23)));
g(161925,k(e.substring(0,4),e.substring(4)));g(153366,"RANDOMIZER");"unstable"==STATUS$$module$js$version&&g(153404,"BETA");return d},patchBytes$$module$js$patch=(b,a,c)=>{for(let d=0;d<c.length;d++)b[a+16+d]=c[d]},patchWords$$module$js$patch=(b,a,c)=>{for(let d=0;d<2*c.length;d+=2)b[a+16+d]=c[d>>>1]&255,b[a+17+d]=c[d>>>1]>>>8},updateCoinDrops$$module$js$patch=(b,a)=>{a.disableShopGlitch()?patchWords$$module$js$patch(b,216030,[0,5,10,15,25,40,65,105,170,275,445,600,700,800,900,1E3]):patchWords$$module$js$patch(b,
216030,[0,1,2,4,8,16,30,50,100,200,300,400,500,600,700,800])},updateDifficultyScalingTables$$module$js$patch=(b,a,c)=>{const d=Array(48).fill(0).map((a,b)=>b);patchBytes$$module$js$patch(b,c.expand("DiffAtk"),d.map((a)=>Math.round(40+15*a/4)));patchBytes$$module$js$patch(b,c.expand("DiffDef"),d.map((a)=>4*a));const e=a.decreaseEnemyDamage()?16:48,f=a.decreaseEnemyDamage()?6:5.5;patchBytes$$module$js$patch(b,c.expand("DiffHP"),d.map((a)=>Math.min(255,e+Math.round(a*f))));const g=a.expScalingFactor();
patchBytes$$module$js$patch(b,c.expand("DiffExp"),d.map((a)=>{a=Math.floor(4*Math.pow(2,(16+9*a)/32)*g);return 128>a?a:Math.min(255,128+(a>>4))}));patchBytes$$module$js$patch(b,216E3,[0,2,6,10,14,18,32,24,20,0,2,6,10,14,18,16,32,20])},rescaleShops$$module$js$patch=(b,a,c)=>{const d=a.expand("BasePrices");var e=a.expand("InnPrices");const f={prices:138836,items:138792},g={prices:138704,items:138660},k=Array(48).fill(0).map((a,b)=>b);patchBytes$$module$js$patch(b,a.expand("ToolShopScaling"),k.map((a)=>
Math.round(8*Math.pow(2,a/10))));patchBytes$$module$js$patch(b,a.expand("ArmorShopScaling"),k.map((a)=>Math.round(8*Math.pow(2,(47-a)/12))));for(var h of[f,g]){const {prices:d,items:e}=h;for(a=0;44>a;a++)b[d+a+16]=BASE_PRICES$$module$js$patch[b[e+a+16]]?c?c.nextInt(32)+17:32:0}for(h=0;11>h;h++)b[e+h+16]=c?c.nextInt(40)+13:32;c=(a,c)=>{b[d+2*(a-13)+16]=c&255;b[d+2*(a-13)+17]=c>>>8};for(e=13;39>e;e++)c(e,BASE_PRICES$$module$js$patch[e]);c(39,20)},BASE_PRICES$$module$js$patch={13:4,14:16,15:50,16:325,
...
dgld:n,sexp:m}]=d;b=a.objects[e].objectData;c=[87,94,104,125,136,151,155,158].includes(e)?1:0;b[2]|=128;b[6]=k;b[7]=h;b[8]=f|g<<4;b[9]=b[9]&224|c;b[16]=b[16]&15|n<<4;b[17]=m}a.objects[125].objectData[16]|=8},shuffleMonsters$$module$js$patch=(b,a,c,d)=>{b=new MonsterPool$$module$js$patch({});for(const c of a.locations)c&&b.populate(c);b.shuffle(c)},identifyKeyItemsForDifficultyBuffs$$module$js$patch=(b)=>{for(const a of b.itemGets){const c=ITEMS$$module$js$patch.get(a.item);if(c&&c.key){if(!a.table)throw Error(`No table for ${c.name}`);
if(255==a.table[a.table.length-1])a.table[a.table.length-1]=254;else throw Error(`Expected FF at end of ItemGet table: ${a.id.toString(16)}: ${Array.from(a.table).map((a)=>a.toString(16).padStart(2,0)).join(" ")}`);a.write(b)}}},SCALED_MONSTERS$$module$js$patch=new Map([[63,"p","Sorceror shot",,,,19,,,],[75,"m","wraith??",2,,2,22,4,61],[79,"m","wraith",1,,2,20,4,61],[80,"m","Blue Slime",,,1,16,2,32],[81,"m","Weretiger",,,1,21,4,40],[82,"m","Green Jelly",4,,3,16,4,36],[83,"m","Red Slime",6,,4,16,4,
48],[84,"m","Rock Golem",6,,11,24,6,85],[85,"m","Blue Bat",,,,4,,32],[86,"m","Green Wyvern",4,,4,24,6,52],[87,"b","Vampire",3,,12,18,,,],[88,"m","Orc",3,,4,21,4,57],[89,"m","Red Flying Swamp Insect",3,,1,21,4,57],[90,"m","Blue Mushroom",2,,1,21,4,44],[91,"m","Swamp Tomato",3,,2,35,4,52],[92,"m","Flying Meadow Insect",3,,3,23,4,81],[93,"m","Swamp Plant",,,,,,36],[94,"b","Insect",,1,8,6,,,],[95,"m","Large Blue Slime",5,,3,20,4,52],[96,"m","Ice Zombie",5,,7,14,4,57],[97,"m","Green Living Rock",,,1,9,
4,28],[98,"m","Green Spider",4,,4,22,4,44],[99,"m","Red/Purple Wyvern",3,,4,30,4,65],[100,"m","Draygonia Soldier",6,,11,36,4,89],[101,"m","Ice Entity",3,,2,24,4,52],[102,"m","Red Living Rock",,,1,13,4,40],[103,"m","Ice Golem",7,2,11,28,4,81],[104,"b","Kelbesque",4,6,12,29,,,],[105,"m","Giant Red Slime",7,,40,90,4,102],[106,"m","Troll",2,,3,24,4,65],[107,"m","Red Jelly",2,,2,14,4,44],[108,"m","Medusa",3,,4,36,8,77],[109,"m","Red Crab",2,,1,21,4,44],[110,"m","Medusa Head",,,1,29,4,36],[111,"m","Evil Bird",
...
[245,"p","tower defense mech laser",,,,23,,,],[246,"p","tower sentinel laser",,,,8,,,],[247,"p","skeleton shot",,,,11,,,],[248,"p","lavaman shot",,,,14,,,],[249,"p","black knight flail",,,,18,,,],[250,"p","lizardman flail",,,,21,,,],[252,"p","mado shuriken",,,,36,,,],[253,"p","guardian statue missile",,,,23,,,],[254,"p","demon wall fire",,,,23,,,]].map((b)=>{var [a,c,d,e=0,f=0,g=0,k=0,h=0,n=0]=b;return[a,{id:a,type:c,name:d,sdef:e,swrd:f,hits:g,satk:k,dgld:h,sexp:n}]}));
class MonsterPool$$module$js$patch{constructor(b){this.report=b;this.monsters=[];this.used=[];this.locations=[]}populate(b){var a=MONSTER_ADJUSTMENTS$$module$js$patch[b.id]||{},c=Object.assign({},a);a=a.skip;c=(delete c.maxFlyers,delete c.nonFlyers,delete c.skip,delete c.fixedSlots,c);for(var d in c)throw Error(`Unexpected property '${d}' in MONSTER_ADJUSTMENTS[${b.id}]`);if(!0!==a&&b.spritePatterns&&b.spritePalettes){d=[];c=[];var e=12;for(const a of b.objects||[]){++e;if(a[2]&7)continue;const k=
a[3]+80;if(k in UNTOUCHED_MONSTERS$$module$js$patch||!SCALED_MONSTERS$$module$js$patch.has(k)||"m"!=SCALED_MONSTERS$$module$js$patch.get(k).type)continue;var f=b.rom.objects[k];if(!f)continue;const n=a[2]&128?1:0,m=b.spritePatterns[n];var g=f.palettes(!0);f=g.includes(2)?b.spritePalettes[0]:null;g=g.includes(3)?b.spritePalettes[1]:null;d.push({id:k,pat:m,pal2:f,pal3:g,patBank:n});(this.report[`start-${k.toString(16)}`]=this.report[`start-${k.toString(16)}`]||[]).push("$"+b.id.toString(16));c.push(e)}d.length&&
...
this.report["$"+d.id.toString(16).padStart(2,0)]=[];const {maxFlyers:g,nonFlyers:k={},fixedSlots:h={}}=MONSTER_ADJUSTMENTS$$module$js$patch[d.id]||{};let n=h.pat0||null,m=h.pat1||null,q=h.pal2||null,p=h.pal3||null,l=g,r=!1;for(const b of d.objects||[])if(2==(b[2]&7)&&(r=!0),!(b[2]&7)){var a=b[3]+80;126==a||127==a||159==a?m=98:143==a&&(n=97)}if(21==d.id||71==d.id)r=!1;f.push(`Initial pass: ${[r,n,m,q,p].join(", ")}`);a=(a)=>{var b=FLYERS$$module$js$patch.has(a.id),c=MOTHS_AND_BATS$$module$js$patch.has(a.id);
if(b){if(!l)return!1;--l}if(null!=q&&null!=a.pal2&&q!=a.pal2||null!=p&&null!=a.pal3&&p!=a.pal3)return!1;var g=!r||TREASURE_CHEST_BANKS$$module$js$patch.has(a.pat);let h;if(d.rom.objects[a.id].child||RETAIN_SLOTS$$module$js$patch.has(a.id)){const b=(h=a.patSlot?128:0)?m:n;if(null!=b&&b!=a.pat)return!1;if(h)m=a.pat;else if(g)n=a.pat;else return!1;f.push(`  Adding ${a.id.toString(16)}: pat(${h}) <-  ${a.pat.toString(16)}`)}else if(null==n&&g||n==a.pat)n=a.pat,h=0,f.push(`  Adding ${a.id.toString(16)}: pat0 <-  ${a.pat.toString(16)}`);
