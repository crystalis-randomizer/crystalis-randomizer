...
vague$$module$js$pass$fixdialog(f));f=a.prg[251902];d("02:02","a statue",`the ${commonNoun$$module$js$pass$fixdialog(f)}`);d("02:02","[29:Gas Mask]",c(f));f=a.prg[223220];65>f&&b("03:01");d("03:01","[43:Telepathy]",c(f));f=a.npcs[95];var g=findTornelTradeIn$$module$js$pass$fixdialog(f);d("03:01","[06:Tornado Bracelet]",c(g));d("05:0a","[06:Tornado Bracelet]",c(g));d("05:0a","[44:Teleport]",c(f.data[0]));f=e.get(100);null!=f&&(d("09:01","[35:Fog Lamp]",c(f)),d("09:04","[35:Fog Lamp]",c(f)),d("09:05",
"[35:Fog Lamp]",c(f)));d("0a:0c","[28:Flute of Lime]",c(a.npcs[56].data[0]));f=a.prg[250361];65>f&&b("0b:01");d("0b:01","[45:Recover]",c(f));f=a.prg[251609];65>f&&(d("0b:01","teach you\n the magic of","give you\n "),b("1d:12"));d("0b:01","[46:Barrier]",c(f));d("1d:12","[46:Barrier]",c(f));f=function(...b){for(const c of b){b=a.locations[c];for(const c of b.spawns)if(c.isChest()&&72>c.id&&a.items[a.itemGets[c.id].itemId].unique)return c.id}return-1}(68,69,70,71,72,73,74,75,76,77,78,79);0<=f?d("0d:00",
"[35:Fog Lamp]",c(f)):d("0d:00","that a [35:Fog Lamp] was","there was treasure");f=a.npcs[195].localDialogs.get(-1)[0].condition&255;g=a.prg[250679];d("0e:03","[02:Sword of Water]",c(f));d("0e:03","[09:Ball of Water]",c(g));d("10:0c","that's","is");d("10:0c",/, is in the\+lighthouse/,"");f=e.get(35);null!=f&&(d("12:05","[3c:Kirisa Plant]",c(f)),d("12:10","the plant",`the ${commonNoun$$module$js$pass$fixdialog(f)}`),d("12:10","[3c:Kirisa Plant]",c(f)),d("12:09",/s*\n.*/,"."),d("12:0a",/s*\n.*/,"."));
...
function findTornelTradeIn$$module$js$pass$fixdialog(a){for(const b of a.localDialogs.values())for(a=2;a<b.length;a++)if(3===b[a].message.action)return~b[a-2].condition&255;return 6}function vague$$module$js$pass$fixdialog(a){switch(a){case 37:return"an unusual statue";case 40:return"a rare instrument";case 53:return"a brilliant lamp";case 59:return"a beautiful charm";case 60:return"a fragrant plant";case 61:return"an exotic statue"}fail$$module$js$assert();return"a valuable item"}
...
(b)=>CSet$$module$js$rom$constraint.union(this.fixed[b],a.fixed[b]));if(this.float.length!=a.float.length)throw console.dir(this),console.dir(a),Error(`incompatible float: ${this.float} ${a.float}`);const c=seq$$module$js$rom$util(this.float.length,(b)=>CSet$$module$js$rom$constraint.union(this.float[b],a.float[b]));return new Constraint$$module$js$rom$constraint(b,c,this.shift|a.shift)}fix(a){const b=a?(b)=>a.nextInt(b):()=>0,c=[...this.fixed];if(this.float.length){const a=b(2),e=1-a;a<this.float.length&&
(c[0]=CSet$$module$js$rom$constraint.intersect(c[0],this.float[a]));e<this.float.length&&(c[1]=CSet$$module$js$rom$constraint.intersect(c[1],this.float[e]))}return c.map((a)=>{if(a===ALL$$module$js$rom$constraint)return 255;a=[...a];return a[b(a.length)]})}meet(a){const b=[];let c=this.shift|a.shift;for(var d=0;4>d;d++){var e=CSet$$module$js$rom$constraint.intersect(this.fixed[d],a.fixed[d]);if(!e.size)return;b.push(e)}e=new Map;d=[];for(var f of module$js$util.iters.concat(this.float,a.float)){if(f===
ALL$$module$js$rom$constraint)throw Error("Unexpected unconstrained float");a=!1;for(var g of f){const b=e.get(g);if(null!=b){for(var h of d[b])e.delete(h);d[b]=CSet$$module$js$rom$constraint.intersect(d[b],f);for(const a of d[b])e.set(a,b);a=!0;break}}if(a)break;for(const a of f)e.set(a,d.length);d.push(f);if(2<d.length)return}for(f=0;f<d.length;f++)for(g=0;2>g;g++)if(!CSet$$module$js$rom$constraint.intersect(d[f],b[g]).size){h=b[1-g]=CSet$$module$js$rom$constraint.intersect(d[f],b[1-g]);c|=1<<1-
g;if(!h.size)return;d.splice(f,1);f=-1;break}return new Constraint$$module$js$rom$constraint(b,d,c)}}const TREASURE_CHEST_BANKS$$module$js$rom$constraint=new Set([94,95,96,97,100,101,102,103,104,105,106,108,109,110,111,112,116,117,118,119]),COIN_BANKS$$module$js$rom$constraint=new Set([94,95,96,97,99,100,101,102,103,104,105,106,107,108,109,110,111,112,116,117,118,119]);var module$js$rom$constraint={};module$js$rom$constraint.Constraint=Constraint$$module$js$rom$constraint;class ObjectData$$module$js$rom$objectdata extends Entity$$module$js$rom$entity{constructor(a,b){super(a,b);this.used=!0;this.name="";this.pointer=109568+(b<<1);this.base=readLittleEndian$$module$js$rom$util(a.prg,this.pointer)+65536;this.sfx=a.prg[this.base];this.data=[];b=this.base+1;let c=0;for(let d=0;32>d;d++)d&7||(c=a.prg[b++]),this.data.push(c&128?a.prg[b++]:0),c<<=1}serialize(){const a=[this.sfx];for(let b=0;4>b;b++){const c=a.length;a.push(0);for(let d=0;8>d;d++)this.data[8*b+d]&&(a[c]|=
...
k)){c=d;break}(this.report[`mon-${a.id.toString(16)}`]=this.report[`mon-${a.id.toString(16)}`]||[]).push("$"+e.id.toString(16));d=f[c];h=e.spawns[d-13];d in k&&(h.y+=16*k[d][0],h.x+=16*k[d][1]);h.monsterId=a.id;g.push(`    slot ${d.toString(16)}: ${h}`);f.splice(c,1);return!0};if(l)for(var d=0;d<Math.min(40,this.monsters.length);d++)FLYERS$$module$js$patch.has(this.monsters[d].id)&&c(this.monsters[d])&&this.monsters.splice(d,1);for(d=0;d<this.monsters.length&&f.length;d++)if(c(this.monsters[d])){const [a]=
this.monsters.splice(d,1);FLYERS$$module$js$patch.has(a.id)||this.used.push(a);d--}for(d=0;d<this.used.length&&f.length;d++)c(this.used[d])&&(this.used.push(...this.used.splice(d,1)),d--);[e.spritePatterns[0],e.spritePatterns[1],e.spritePalettes[0],e.spritePalettes[1]]=n.fix(a);if(f.length){console.error(`Failed to fill location ${e.id.toString(16)}: ${f.length} remaining`);for(const a of f)c=e.spawns[a-13],c.x=c.y=0,c.id=176,c.data[0]=254}for(const a of e.spawns)b.configure(e,a)}}}
