...
return Math.min(AM$$module$js$random*this.iy,RNMX$$module$js$random)}nextInt(b){return Math.floor(this.next()*b)}shuffle(b){for(let a=b.length;a;){const c=this.nextInt(a--);[b[a],b[c]]=[b[c],b[a]]}return b}}var module$js$random={};module$js$random.Random=Random$$module$js$random;const STATUS$$module$js$version="unstable",VERSION$$module$js$version="latest",LABEL$$module$js$version="latest c8180da",HASH$$module$js$version="c8180dad59f84eb930db98f11b03bb902783cfd5",DATE$$module$js$version=new Date(1552782104E3);var module$js$version={};module$js$version.STATUS=STATUS$$module$js$version;module$js$version.VERSION=VERSION$$module$js$version;module$js$version.LABEL=LABEL$$module$js$version;module$js$version.HASH=HASH$$module$js$version;module$js$version.DATE=DATE$$module$js$version;var $jscompDefaultExport$$module$js$patch={apply(b,a){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){let c;a.seed||(a.seed=parseSeed$$module$js$patch("").toString(16),window.location.hash+="&seed="+a.seed);c=a.flags?new FlagSet$$module$js$flagset(a.flags):new FlagSet$$module$js$flagset("Em Gt Mr Rlpt Sbk Sct Sm Tasd");for(const b in a)"false"===a[b]&&(a[b]=!1);yield shuffle$$module$js$patch(b,parseSeed$$module$js$patch(a.seed),c)})}};
...
preventNpcDespawns$$module$js$patch.apply(b);allowTeleportOutOfTower$$module$js$patch.apply(b);c.check("Td")&&buffDeosPendant$$module$js$patch.apply(b);c.check("Rl")&&barrierRequiresCalmSea$$module$js$patch.apply(b);c.check("Fs")&&fixShopGlitch$$module$js$patch.apply(b);preventSoftlockFromZeroHpOrMp$$module$js$patch.apply(b);scaleDifficultyLib$$module$js$patch.apply(b);nerfArmors$$module$js$patch.apply(b);fixVampire$$module$js$patch.apply(b);displayDifficulty$$module$js$patch.apply(b);itemLib$$module$js$patch.apply(b);
c.check("Em")?buffMedicalHerb3x$$module$js$patch.apply(b):c.check("Hm")||buffMedicalHerb2x$$module$js$patch.apply(b);c.check("Rt")?asinaTeleportsToMezameShrine$$module$js$patch.apply(b):noTeleportOnThunderSword$$module$js$patch.apply(b);c.check("Ts")&&leatherBootsForSpeed$$module$js$patch.apply(b);c.check("Di")&&neverDie$$module$js$patch.apply(b);c.check("Ta")&&autoEquipBracelet$$module$js$patch.apply(b);c.check("Tw")&&nerfWildWarp$$module$js$patch.apply(b);return stampVersionSeedAndHash$$module$js$patch(b,
a,c)}),stampVersionSeedAndHash$$module$js$patch=(b,a,c)=>{const d=crc32$$module$js$crc32(b),e=d.toString(16).padStart(8,0).toUpperCase(),f="unstable"==STATUS$$module$js$version?HASH$$module$js$version.substring(0,7).padStart(7,0).toUpperCase()+"     ":VERSION$$module$js$version.substring(0,12).padEnd(12," ");a=a.toString(16).padStart(8,0).toUpperCase();const g=(a,c)=>{for(let d=0;d<c.length;d++)b[a+16+d]=c.charCodeAt(d)},k=(a,b)=>{const c=[];for(let d=0;d<a.length||d<b.length;d++)c.push(a[d]||" "),
