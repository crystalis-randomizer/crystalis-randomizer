...
ye=(new Trigger$$module$js$nodes(a,"Talked to Akahana's Friend")).get(ce),ze=(new Trigger$$module$js$nodes(a,"Get Bow of Truth")).get(oc),Ae=(new Trigger$$module$js$nodes(a,"Forged Crystalis")).get(nb),Ya=(new Condition$$module$js$nodes(a,"Sword charge glitch")).option(Qd,Rd),Be=(new Condition$$module$js$nodes(a,"Rabbit skip")).option(l),Ce=(new Condition$$module$js$nodes(a,"Teleport skip")).option(t),Cb=(new Condition$$module$js$nodes(a,"Statue glitch")).option(r,Pd),De=(new Condition$$module$js$nodes(a,
"Wild warp")).option(Nd,Od),Za=(new Condition$$module$js$nodes(a,"Any level 2 sword")).option(W,Na).option(W,Ha).option(X,Oa).option(X,Pa).option(Q,Qa).option(Q,Ra).option(L,Sa).option(L,Ta),v=(new Condition$$module$js$nodes(a,"Destroy stone")).option(W,Na).option(W,Ha).option(Ya,W,Za),w=(new Condition$$module$js$nodes(a,"Destroy ice")).option(X,Oa).option(X,Pa).option(Ya,X,Za),q=(new Condition$$module$js$nodes(a,"Cross rivers")).option(Q,Qa).option(Q,Ra).option(B).option(Ya,Q,Za),wa=(new Condition$$module$js$nodes(a,
...
(new Location$$module$js$nodes(a,20,oa,"West")).overworld().connect(nf),rf=(new Location$$module$js$nodes(a,20,oa,"South")).overworld().connect(Ca,q),$c=(new Location$$module$js$nodes(a,21,oa,"East")).overworld().connect(Ca).chest($b,24),Ka=(new Location$$module$js$nodes(a,24,ya,"Town")).town().connect(Ca).trigger(ie,$b),sf=(new Location$$module$js$nodes(a,25,oa,"Outside Stom's House")).town().connect(Ca),Fb=(new Location$$module$js$nodes(a,26,oa,"Swamp")).overworld().connect($c,$a).trigger(yb,vc,
...
B),Rf=(new Location$$module$js$nodes(a,64,ea,"Northeast")).overworld().connect(gb,q),id=(new Location$$module$js$nodes(a,65,ea,"Southwest")).overworld().connect(gb),Sf=(new Location$$module$js$nodes(a,65,ea,"Southeast")).overworld().connect(id,q),jd=(new Location$$module$js$nodes(a,66,ea,"Lime Tree Valley")).overworld().connect(id).connect(Ja,mb),Tf=(new Location$$module$js$nodes(a,67,ea,"Lime Tree Lake (Rage)")).cave().connect(jd).trigger(Bc,Q),Uf=(new Location$$module$js$nodes(a,68,ha,"Tunnel 1a (entrance)")).cave().connect(Sf),
...
80,z,"Town")).town().connect(gb),pd=(new Location$$module$js$nodes(a,81,z,"Fishherman Island")).town().connect(sa);(new Location$$module$js$nodes(a,82,ea,"Mesia Shrine")).cave().trigger(Cc).connect(Tf,q,Bc);const qd=(new Location$$module$js$nodes(a,84,fa,"Tunnel 1a (entrance)")).cave().connect(gb);(new Location$$module$js$nodes(a,84,fa,"Tunnel 1b (dead end, mimic)")).cave().connect(qd,w).chest(J,19,114);const jg=(new Location$$module$js$nodes(a,84,fa,"Tunnel 1c (past ice)")).cave().connect(qd,w),
...
100,z,"Underground Channel 2 (to fortune teller)")).sea().connect(hb,q),ud=(new Location$$module$js$nodes(a,100,z,"Underground Channel 3 (from fortune teller)")).sea().connect(hb,B),vd=(new Location$$module$js$nodes(a,100,z,"Underground Channel 4 (asina)")).sea().connect(ud,q),vg=(new Location$$module$js$nodes(a,100,z,"Underground Channel 5 (dolphin)")).sea().connect(vd,q).trigger(Ec,E,Dc);(new Location$$module$js$nodes(a,100,z,"Underground Channel 6 (water)")).sea().connect(vg,Z).connectTo(Ea,Z).chest(kc,
17);const Lb=(new Location$$module$js$nodes(a,101,G,"Zombie Town")).town(),wg=(new Location$$module$js$nodes(a,104,G,"Tunnel 1 (entrance)")).sea().connect(Ea,Gc,Z),wd=(new Location$$module$js$nodes(a,105,G,"Tunnel 2a (start)")).cave().connect(wg);(new Location$$module$js$nodes(a,105,G,"Tunnel 2b (dead end to left)")).cave().connect(wd,q);const xg=(new Location$$module$js$nodes(a,105,G,"Tunnel 2c (across first river)")).cave().connect(wd,q),xd=(new Location$$module$js$nodes(a,105,G,"Tunnel 2d (across second river)")).cave().connect(xg,
q);(new Location$$module$js$nodes(a,105,G,"Tunnel 2e (dead end, magic ring)")).cave().connect(xd,v).chest(R,29);const yg=(new Location$$module$js$nodes(a,105,G,"Tunnel 2f (stair down)")).cave().connect(xd,v),zg=(new Location$$module$js$nodes(a,106,G,"Tunnel 3a (main area)")).cave().connect(yg).connectTo(Lb).chest(ua,25,92),Ag=(new Location$$module$js$nodes(a,106,G,"Tunnel 3b (left area toward items)")).cave().connect(zg,v),Bg=(new Location$$module$js$nodes(a,107,G,"Tunnel 4a (right side, mimic)")).cave().connect(Ag).chest(J,
...
q),zd=(new Location$$module$js$nodes(a,124,y,"After first tunnel")).overworld(),Ad=(new Location$$module$js$nodes(a,124,y,"Door to Styx")).overworld().connect(zd,q),Jg=(new Location$$module$js$nodes(a,124,y,"Dead end (no item)")).overworld(),Kg=(new Location$$module$js$nodes(a,124,y,"Dead end (fruit of lime)")).overworld().chest(pb,26),Lg=(new Location$$module$js$nodes(a,124,y,"Dead end (magic ring)")).overworld().chest(R,25,101),Bd=(new Location$$module$js$nodes(a,124,y,"Outside tunnel to bow")).overworld();
...
q);const rh=(new Location$$module$js$nodes(a,171,U,"Across First Bridge (fruit of repun)")).fortress().connect(lb,q).chest(qb,30,102),Rb=(new Location$$module$js$nodes(a,171,U,"Across Second Bridge")).fortress().connect(rh,q);(new Location$$module$js$nodes(a,171,U,"Dead End Across Two Bridges ()")).fortress().connect(Rb,q).connect(Rb,q).chest(ua,29,93);const sh=(new Location$$module$js$nodes(a,171,U,"Across Third Bridge")).fortress().connect(Rb,q),th=(new Location$$module$js$nodes(a,171,U,"Exit Behind Iron Door")).fortress().connect(sh,
...
e,f,g)=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){const h=e&&e.stats;var g=b.nodes.filter((a)=>a instanceof Slot$$module$js$nodes&&a.slots&&a.slotName);const m=new Map(d.shuffle(g.map((a)=>[a,[a.item,a.itemIndex]]))),p=new Set(d.shuffle(g)),r=new Map,t={};h&&h.seeds++;f||(f=new FlagSet$$module$js$flagset("Sbkm Sct"));(f.flags.S||[]).forEach((a,b)=>{for(const c of a)t[c]=b});for(var l of p)r.set(l.item,l);let n=new Set;const x=(a,b)=>{if(null==t[a.slotType?a.slotType[0]:"c"])return a==
b;if(b.isMimic()&&!a.canHoldMimic()||"unique"!=b.item.inventoryRow&&a.requiresUnique||"armor"==b.item.inventoryRow&&!a.isChest())return!1;if(f.check("Es")){if(0===a.slotIndex)return 4>b.item.id;if(4>b.item.id&&(n.add(b),4===n.size))return 0===a.slotIndex}return t[a.slotType?a.slotType[0]:"c"]==t[b.slotType?b.slotType[0]:"c"]};l=a.assumedFill(d,(a,b)=>x(a,r.get(b)));if(null==l){h&&h.endState.key++;return!1;throw Error("Could not fill!");}const A=new Map;for(var P=0;P<l.length;P++){var ta=l[P];if(null!=
ta){var ma=r.get(a.item(ta));ta=m.get(ma);m.delete(ma);ma=a.location(P);A.set(ma,ta);p.delete(ma)}}P=(a,b,c)=>{c=void 0===c?"":c;for(const c of p)if(x(c,a))return A.set(c,b),p.delete(c),m.delete(a),!0;h&&("traps"==c?h.endState.traps++:"chest"==c?h.endState.chest++:h.endState.misc++);return!1};for(var da of m){const [a,b]=da;if(a.isMimic()&&!P(a,b,"traps"))return!1}for(const a of m){const [b,c]=a;if(b.needsChest()&&!P(b,c,"chest"))return!1}for(const a of m){const [b,c]=a;if(!P(b,c))return!1}for(const a of A){const [b,
...
return Math.min(AM$$module$js$random*this.iy,RNMX$$module$js$random)}nextInt(b){return Math.floor(this.next()*b)}shuffle(b){for(let a=b.length;a;){const c=this.nextInt(a--);[b[a],b[c]]=[b[c],b[a]]}return b}}var module$js$random={};module$js$random.Random=Random$$module$js$random;const STATUS$$module$js$version="unstable",VERSION$$module$js$version="latest",LABEL$$module$js$version="latest bdc4b57",HASH$$module$js$version="bdc4b57e89a11192dc98cdcddeb499e0b8233fbe",DATE$$module$js$version=new Date(1554527307E3);var module$js$version={};module$js$version.STATUS=STATUS$$module$js$version;module$js$version.VERSION=VERSION$$module$js$version;module$js$version.LABEL=LABEL$$module$js$version;module$js$version.HASH=HASH$$module$js$version;module$js$version.DATE=DATE$$module$js$version;var $jscompDefaultExport$$module$js$patch={apply(b,a){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){a.seed||(a.seed=parseSeed$$module$js$patch("").toString(16),window.location.hash+="&seed="+a.seed);a.flags?new FlagSet$$module$js$flagset(a.flags):new FlagSet$$module$js$flagset("Em Gt Mr Rlpt Sbk Sct Sm Tasd");for(const b in a)"false"===a[b]&&(a[b]=!1)})}};
...
_TELEPORT_ON_THUNDER_SWORD:c.check("Rt")},m=new Assembler$$module$js$6502,p=(a)=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){m.assemble(yield d.read(a),a);m.patchRom(b)}),r=Object.keys(k).filter((a)=>k[a]).map((a)=>`define ${a} 1\n`).join("");m.assemble(r,"flags.s");yield p("preshuffle.s");g=new Random$$module$js$random(g);yield shuffle2$$module$js$depgraph(b,g,e,c,f);h&&rescaleShops$$module$js$patch(b,m,c.check("Pb")?g:null);h=new Rom$$module$js$rom(b);rescaleMonsters$$module$js$patch(b,
h);c.check("Mr")&&shuffleMonsters$$module$js$patch(b,h,g);identifyKeyItemsForDifficultyBuffs$$module$js$patch(h);c.check("Em")?(b[115996]*=2,b[115962]*=3):c.check("Hm")||(b[115996]+=16,b[115962]*=2);yield p("postshuffle.s");updateDifficultyScalingTables$$module$js$patch(b,c,m);updateCoinDrops$$module$js$patch(b,c);shuffleRandomNumbers$$module$js$patch(b,g);return stampVersionSeedAndHash$$module$js$patch(b,a,c)}),stampVersionSeedAndHash$$module$js$patch=(b,a,c)=>{const d=crc32$$module$js$crc32(b),
e=d.toString(16).padStart(8,0).toUpperCase(),f="unstable"==STATUS$$module$js$version?HASH$$module$js$version.substring(0,7).padStart(7,0).toUpperCase()+"     ":VERSION$$module$js$version.substring(0,12).padEnd(12," ");a=a.toString(16).padStart(8,0).toUpperCase();const g=(a,c)=>{for(let d=0;d<c.length;d++)b[a+16+d]=c.charCodeAt(d)},h=(a,b)=>{const c=[];for(let d=0;d<a.length||d<b.length;d++)c.push(a[d]||" "),c.push(b[d]||" ");return c.join("")};g(161743,h("  VERSION     SEED      ",`  ${f}${a}`));
a=String(c);let k;if(46<a.length){if(92<a.length)throw Error("Flag string way too long!");k=a.substring(46,92).padEnd(46," ");a=a.substring(0,46)}a=a.padEnd(46," ");g(161791,h(a.substring(0,23),a.substring(23)));k&&g(161839,h(k.substring(0,23),k.substring(23)));g(161925,h(e.substring(0,4),e.substring(4)));g(153366,"RANDOMIZER");"unstable"==STATUS$$module$js$version&&g(153404,"BETA");return d},patchBytes$$module$js$patch=(b,a,c)=>{for(let d=0;d<c.length;d++)b[a+16+d]=c[d]},patchWords$$module$js$patch=
