...
descr:"Full shuffle but with no glitches.",flags:"Em Fcpstw Mert Ps Rprt Sckmt Tab Wtuw"},{title:"Advanced",descr:"A balanced randomization with quite a bit more difficulty.",flags:"Fsw Gfprt Hbdgw Mert Ps Roprst Sckt Sm Tabmp Wtuw"},{title:"Ludicrous",descr:"Pulls out all the stops, may require superhuman feats.",flags:"Fs Gcfprtw Hbdgmswxz Mert Ps Roprst Sckmt Tabmp Wtuw"}],PRESETS_BY_KEY$$module$js$flagset={};
...
d&&(0<=e.sells.indexOf(d)||4<=e.sells.length?g.push(d):e.sells.push(d));for(const {shops:a}of[b,c])for(const b of a)b.sells.sort((a,b)=>a.id-b.id)}integrate({tracker:a=!1}={}){var b=!a;const c=new SparseDependencyGraph$$module$js$graph(this.nodes.length);var d=[],e=[];const f=new Set,g=new Map;var h=[];const k=[],l=[],m=[],n=[],r=new Set;for(const a of this.nodes)if(a instanceof Location$$module$js$nodes){if(a.isStart){const [b]=c.addRoute([a.uid]);g.set(b.label,b)}d[a.uid]=[];e[a.uid]=[];for(const b of a.connections)f.has(b)||
(f.add(b),b.bidi&&f.add(b.reverse()));for(const b of a.chests)c.addRoute([b.uid,a.uid])}else if(a instanceof ItemGet$$module$js$nodes&&"Medical Herb"===a.name&&(c.addRoute([a.uid]),c.finalize(a.uid)),a instanceof Option$$module$js$nodes)h.push(a);else if(a instanceof TrackerNode$$module$js$nodes)k.push(a);else if(a instanceof Condition$$module$js$nodes)l.push(a);else if(a instanceof Trigger$$module$js$nodes)m.push(a);else if(a instanceof ItemGet$$module$js$nodes)n.push(a);else if(a instanceof Slot$$module$js$nodes)r.add(a);
...
a)}}e(l);for(d=0;d<this.nodes.length;d++)this.nodes[d]instanceof ItemGet$$module$js$nodes||this.nodes[d]instanceof TrackerNode$$module$js$nodes||c.finalize(d);d=new LocationList$$module$js$nodes(this);for(const a of r)for(const b of c.nodes[a.uid].values())d.addRoute([a.uid,...b]);return d}}
...
a.assumeGhettoFlight())),m=h(d("Assume statue glitch",a.assumeStatueGlitch())),n=d("Allow statue glitch",!a.disableStatueGlitch()),r=h(d("Assume teleport skip",a.assumeTeleportSkip())),p=d("Allow teleport skip",!a.disableTeleportSkip()),q=h(d("Assume rabbit skip",a.assumeRabbitSkip())),t=h(d("Allow rabbit skip",!a.disableRabbitSkip())),u=h(d("Assume sword charge glitch",a.assumeSwordChargeGlitch())),z=d("Allow sword charge glitch",!a.disableSwordChargeGlitch()),J=h(d("Assume wild warp",a.assumeWildWarp())),
w=d("Allow wild warp",a.allowWildWarp()),x=d("Sword magic optional",!a.guaranteeSwordMagic()),K=d("Matching sword optional",!a.guaranteeMatchingSword()),Qa=d("Gas mask optional",!a.guaranteeGasMask()),Pd=d("Healed dolphin optional",!a.requireHealedDolphinToRide()),Qd=d("Calm sea optional",!a.barrierRequiresCalmSea()),Rd=d("Prison key optional",!a.paralysisRequiresPrisonKey()),Sd=d("Windmill optional",!a.sealedCaveRequiresWindmill()),Td=d("Sword of Thunder teleports to Shyron",a.teleportOnThunderSword()),
...
Nc=new Trigger$$module$js$nodes(c,"Shyron Massacre"),Ae=(new Trigger$$module$js$nodes(c,"Saved Kensu")).get(E),Be=(new Trigger$$module$js$nodes(c,"Talked to Deo")).get(ie),Ce=(new Trigger$$module$js$nodes(c,"Talked to Akahana's Friend")).get(ge),De=(new Trigger$$module$js$nodes(c,"Get Bow of Truth")).get(rc),Ee=(new Trigger$$module$js$nodes(c,"Forged Crystalis")).get(pb),$a=(new Condition$$module$js$nodes(c,"Sword charge glitch")).option(u,z),Fe=(new Condition$$module$js$nodes(c,"Rabbit skip")).option(q,
t),Ge=(new Condition$$module$js$nodes(c,"Teleport skip")).option(r,p),ab=(new Condition$$module$js$nodes(c,"Statue glitch")).option(m,n),He=(new Condition$$module$js$nodes(c,"Wild warp")).option(J,w),bb=(new Condition$$module$js$nodes(c,"Any level 2 sword")).option(S,Ia).option(S,Ja).option(T,Sa).option(T,Ta).option(O,Ua).option(O,Va).option(M,Wa).option(M,Xa),A=(new Condition$$module$js$nodes(c,"Destroy stone")).option(S,Ra).option(S,Ia).option(S,Ja).option($a,S,bb),B=(new Condition$$module$js$nodes(c,
...
Xb:g).option(Za),Sc=(new Condition$$module$js$nodes(c,"Wind magic")).option(x.value?x:g,W).option(Ia,Ja,W),Tc=(new Condition$$module$js$nodes(c,"Fire magic")).option(x.value?x:g,W).option(Sa,Ta,W),Uc=(new Condition$$module$js$nodes(c,"Water magic")).option(x.value?x:g,W).option(x,W).option(Ua,Va,W),Pe=(new Condition$$module$js$nodes(c,"Thunder magic")).option(x.value?x:g,W).option(x,W).option(Wa,W).option(Xa,W),Qe=(new Condition$$module$js$nodes(c,"Flute of lime or glitch")).option(ub).option(ab),
...
34,F,"Tunnel 2a (fork at start)")).cave().connect(Ea);(new Location$$module$js$nodes(c,34,F,"Tunnel 2b (left branch to dead end)")).cave().connect(dd,B).connectTo(Ef);const Gf=(new Location$$module$js$nodes(c,34,F,"Tunnel 2c (right branch to upper)")).cave().connect(dd,B),Hf=(new Location$$module$js$nodes(c,35,F,"Tunnel 3a (tunnel to upper, with herb chest)")).cave().connect(Gf).chest(H,23,82),If=(new Location$$module$js$nodes(c,35,F,"Tunnel 3b (tunnel to upper, branch below ice)")).cave().connect(Hf,
B).connectTo(cd),ed=(new Location$$module$js$nodes(c,36,F,"Tunnel 4a (branch to upper or Tornel)")).cave().connect(If);(new Location$$module$js$nodes(c,36,F,"Tunnel 4b (out to upper)")).cave().connect(ed,B).connectTo(cd);const Jf=(new Location$$module$js$nodes(c,37,F,"Tunnel 5 (tiny connector)")).cave().connect(ed),Kf=(new Location$$module$js$nodes(c,38,F,"Tunnel 6a (exit to Tornel, above ice)")).cave().connect(Jf);(new Location$$module$js$nodes(c,38,F,"Tunnel 6b (exit to Tornel, below ice)")).cave().connect(Kf,
B).connectTo(Ff);const Lf=(new Location$$module$js$nodes(c,39,F,"Tunnel 7a (tornado bracelet, lower)")).cave().connect(Df),Mf=(new Location$$module$js$nodes(c,39,F,"Tunnel 7b (tornado bracelet, middle)")).cave().connect(Lf,B);(new Location$$module$js$nodes(c,39,F,"Tunnel 7c (tornado bracelet, upper)")).cave().connect(Mf,B).chest(Ja,25);const hb=(new Location$$module$js$nodes(c,40,y,"Entrance")).overworld().connect(bd,Le,Ke),ib=(new Location$$module$js$nodes(c,40,y,"Upper")).overworld().from(hb,E).to(hb),
Ib=(new Location$$module$js$nodes(c,40,y,"Summit (boss)")).overworld().from(ib,E).to(ib).boss(bf),fd=(new Location$$module$js$nodes(c,41,y,"Connector")).overworld(),Jb=(new Location$$module$js$nodes(c,41,y,"Middle Left")).overworld(),Nf=(new Location$$module$js$nodes(c,41,y,"Middle Right")).overworld().from(Jb,Pc).to(Jb),gd=(new Location$$module$js$nodes(c,42,y,"Tunnel 2a (from entrance to connector)")).cave().connectTo(fd);(new Location$$module$js$nodes(c,42,y,"Tunnel 2b (under bridge, to antidote)")).cave().connect(gd,
B).chest(N,23,94);const hd=(new Location$$module$js$nodes(c,43,y,"Tunnel 3a (branch after connector)")).cave().connect(fd),Of=(new Location$$module$js$nodes(c,43,y,"Tunnel 3b (right branch)")).cave().connect(hd,B),Pf=(new Location$$module$js$nodes(c,43,y,"Tunnel 3c (upper branch)")).cave().connect(hd,B);(new Location$$module$js$nodes(c,44,y,"Tunnel 4 (over bridge, to middle)")).cave().connect(Pf).connectTo(Jb);const id=(new Location$$module$js$nodes(c,45,y,"Tunnel 5a (E-shaped, from right branch)")).cave().connect(Of).connectTo(Nf);
(new Location$$module$js$nodes(c,45,y,"Tunnel 5b (dead-end with herb)")).cave().connect(id,B).chest(H,22,83);const Qf=(new Location$$module$js$nodes(c,46,y,"Tunne; 6a (S-shaped hall, right)")).cave().connect(id),Rf=(new Location$$module$js$nodes(c,46,y,"Tunne; 6b (S-shaped hall, middle)")).cave().connect(Qf,B),Sf=(new Location$$module$js$nodes(c,46,y,"Tunnel 6c (S-shaped hall, left)")).cave().connect(Rf,B),jd=(new Location$$module$js$nodes(c,47,y,"Prison (hallway)")).cave().connect(ib),Tf=(new Location$$module$js$nodes(c,
48,y,"Left Cell (shopkeepers)")).cave().from(jd,B);(new Location$$module$js$nodes(c,49,y,"Left Cell 2 (back, with prison key)")).cave().from(Tf,B).chest(xb,13);const Uf=(new Location$$module$js$nodes(c,50,y,"Right Cell (villagers)")).cave().from(jd,B),Vf=(new Location$$module$js$nodes(c,51,y,"Tunnel 8 (behind right cell, toward summit)")).cave().from(Uf,B);(new Location$$module$js$nodes(c,52,y,"Tunnel 9 (connector to summit)")).cave().connect(Vf).connectTo(Ib);const Wf=(new Location$$module$js$nodes(c,
...
f.shuffleShops(b);const g=f.nodes.filter((a)=>a instanceof Slot$$module$js$nodes&&a.slots&&a.slotName);var h={};for(var k of g)if(k instanceof Slot$$module$js$nodes&&k.slots){var l=k.slotType[0];(h[l]=h[l]||[]).push(k)}k=[];l=[];var m={};for(var n of d.get("S")){var r=[];for(var p of n){m[p]=!0;for(var q of h[p])r.push(q),l.push([q,k.length])}k.push(r)}for(var t of"bc")if(m[t])for(n=h[t],p=n.map((a)=>[a.item,a.itemIndex]),b.shuffle(p),q=0;q<n.length;q++)n[q].set(p[q][0],p[q][1]);r=(a)=>g.find((b)=>
b.vanillaItemName===a);h=r("Sword of Wind");t=r("Sword of Fire");n=r("Ball of Wind");p=r("Ball of Fire");q=r("Statue of Onyx");r=r("Gas Mask");m.k&&b.nextInt(2)&&(t.swap(h),p.swap(n));m.k&&!b.nextInt(3)&&q.swap(r);m=[1,1,1,1,2,2,2,3,3,3,5,5,8,13];h=[];e&&e.addTasks(10);for(t=0;1E3>t;t++){e&&0===t%100&&(yield new Promise(requestAnimationFrame),e.addCompleted(1));b.shuffle(l);for(var u of k)b.shuffle(u);n=k.map(()=>-1);p=m[b.nextInt(m.length)];q=[];r=(a,b)=>a.itemIndex===b.itemIndex||a.isMimic()&&!b.canHoldMimic()||
b.isMimic()&&!a.canHoldMimic()||a.needsChest()&&!b.isChest()||b.needsChest()&&!a.isChest()?!1:!0;for(let a=0;a<p;a++){const [b,c]=l[a],d=k[c][++n[c]];r(b,d)&&(b.swap(d),q.push([b,d]))}if(!q.length)continue;const {win:a,path:c}=f.traverse({wanted:g,dfs:!1});if(a)h=c;else for(;q.length;){const [a,b]=q.pop();a.swap(b)}}a&&f.write();if(c){c.items=[];c.route=[];for(const a of h){const [b,d]=a;c.route.push(d);u=f.nodes[b];u instanceof Slot$$module$js$nodes&&!u.isFixed()&&null!=u.slotName&&(k=u.slotName,
...
a.slots&&!a.isFixed());const l=new Map(d.shuffle(k.map((a)=>[a,[a.item,a.itemIndex]]))),m=new Set(d.shuffle(k)),n=new Map,r={};g&&g.seeds++;f.get("S").forEach((a,b)=>{for(const c of a)r[c]=b});for(var p of m)n.set(p.item,p);const q=new Set,t=(a,b)=>{if(null==r[a.slotType?a.slotType[0]:"c"])return a===b;if(b.isMimic()&&!a.canHoldMimic()||"unique"!==b.item.inventoryRow&&a.requiresUnique||"armor"===b.item.inventoryRow&&!a.isChest())return!1;if(f.guaranteeSword()){if(0===a.slotIndex)return 4>b.item.id;
if(4>b.item.id&&(q.add(b),4===q.size))return 0===a.slotIndex}return r[a.slotType?a.slotType[0]:"c"]===r[b.slotType?b.slotType[0]:"c"]};p=b.assumedFill(d,(a,b)=>t(a,n.get(b)));if(null==p){g&&g.endState.key++;return!1;throw Error("Could not fill!");}const u=new Map;for(var z=0;z<p.length;z++){var J=p[z];if(null!=J){var w=n.get(b.item(J));J=l.get(w);l.delete(w);w=b.location(z);u.set(w,J);m.delete(w)}}z=(a,b,c)=>{c=void 0===c?"":c;for(const c of m)if(t(c,a))return u.set(c,b),m.delete(c),l.delete(a),!0;
g&&("traps"===c?g.endState.traps++:"chest"===c?g.endState.chest++:g.endState.misc++);return!1};for(var x of l){const [a,b]=x;if(a.isMimic()&&!z(a,b,"traps"))return!1}for(const a of l){const [b,c]=a;if(b.needsChest()&&!z(b,c,"chest"))return!1}for(const a of l){const [b,c]=a;if(!z(b,c))return!1}for(const a of u){const [b,c]=a;b.set(...c)}const {win:K,path:Qa}=a.traverse({wanted:k,dfs:!1});if(!K)return g&&g.endState.no_win++,!1;c&&a.write();if(!e)return!0;g&&(g.endState.success++,g.analyze(a,b,p));e.items=
...
1,0),e++;for(;c(b)&32;)b=module$js$graph$geometry.TileId.add(b,-1,0),e++;6>e?a&=-33:9>e&&(a|=64)}return TERRAINS$$module$js$graph$overlay[a]}extraRoutes(){const a=[],b=(a,b)=>{a=this.rom.locations[a];return module$js$graph$geometry.TileId.from(a,a.entrances[void 0===b?0:b])};a.push({tile:b(0)});this.flags.teleportOnThunderSword()&&a.push({tile:b(242),condition:module$js$graph$condition.Item.SWORD_OF_THUNDER});if(this.flags.assumeWildWarp())for(const c of this.rom.wildWarp.locations)a.push({tile:b(c)});
return a}extraEdges(){const a=[];a.push({from:module$js$graph$geometry.TileId(5308552),to:module$js$graph$geometry.TileId(6325896),condition:module$js$graph$condition.Event.RETURNED_FOG_LAMP});return a}trigger(a){function b(a){if(0>a)return~b(~a);const c=FLAG_MAP$$module$js$graph$overlay.get(a);return null!=c?c[0][0]:a}switch(a){case 154:return{check:[{condition:meet$$module$js$graph$condition(module$js$graph$condition.Event.SHYRON_MASSACRE,this.bossRequirements(this.rom.bosses.mado1)),slot:module$js$graph$condition.Slot(module$js$graph$condition.Boss.MADO1)}]};
case 170:return{check:[{condition:and$$module$js$graph$condition(module$js$graph$condition.Event.DWARF_CHILD,module$js$graph$condition.Capability.BUY_WARP),slot:module$js$graph$condition.Slot(module$js$graph$condition.Event.RESCUED_CHILD)}]};case 173:return{check:[{condition:module$js$graph$condition.Item.KEY_TO_PRISON,slot:module$js$graph$condition.Slot(module$js$graph$condition.Event.OPENED_PRISON)}]};case 174:return{check:[{condition:module$js$graph$condition.Item.KEY_TO_STYX,slot:module$js$graph$condition.Slot(module$js$graph$condition.Event.OPENED_STYX)}]};
case 175:return{check:[{condition:module$js$graph$condition.Item.STATUE_OF_GOLD,slot:module$js$graph$condition.Slot(module$js$graph$condition.Event.CALMED_SEA)}]};case 177:return{check:[{condition:and$$module$js$graph$condition(module$js$graph$condition.Item.BOW_OF_SUN,module$js$graph$condition.Item.BOW_OF_MOON),slot:module$js$graph$condition.Slot(module$js$graph$condition.Event.OPENED_CRYPT)}]}}var c=this.rom.triggers[a&127];if(!c||!c.used)throw Error(`Unknown trigger: ${hex$$module$js$rom$util(a)}`);
a=(a)=>this.relevantFlags.has(a);const d=(a)=>0<a&&this.relevantFlags.has(a);var e=TRIGGER_ACTION_ITEMS$$module$js$graph$overlay[c.message.action];const f=and$$module$js$graph$condition(...c.conditions.map(b).filter(d).map(module$js$graph$condition.Condition));if(25===c.message.action){e={};134!==c.id||this.flags.disableRabbitSkip()||this.flags.assumeRabbitSkip()||(e.dx=[-32,-16,0,16]);186!==c.id||this.flags.disableTeleportSkip()||this.flags.assumeTeleportSkip()||(e.extraLocations=[this.rom.locations.cordelPlainsWest]);
const [d,...f]=c.conditions;if(!f.length&&0>d&&a(~b(d)))return Object.assign({},e,{terrain:{exit:module$js$graph$condition.Condition(~b(d))}})}else if(null!=e)return{check:[{condition:f,slot:e}]};c=c.flags.filter(d);return c.length?{check:c.map((a)=>({condition:f,slot:module$js$graph$condition.Slot(a)}))}:{}}npc(a,b){function c(...a){if(!f.terrain)throw Error("Missing terrain for guard");f.terrain.exit=or$$module$js$graph$condition(f.terrain.exit||[],...a)}const d=this.rom.npcs[a];if(!d||!d.used)throw Error(`Unknown trigger: ${hex$$module$js$rom$util(a)}`);
...
null),null!=a&&g.push(a))};for(var k of e)h(k);var l=this.tradeIns.get(a);if(null!=l){k=function(a,...c){c=and$$module$js$graph$condition(...g,b,...c);f.check.push({slot:a,condition:c})};const b=l;l=k;this.tracker&&this.flags.randomizeTrades()&&(l=(a,...b)=>{const c=or$$module$js$graph$condition(...[module$js$graph$condition.Item.STATUE_OF_ONYX,module$js$graph$condition.Item.FOG_LAMP,module$js$graph$condition.Item.LOVE_PENDANT,module$js$graph$condition.Item.KIRISA_PLANT,module$js$graph$condition.Item.IVORY_STATUE].map((a)=>
and$$module$js$graph$condition(...g,a,...b)));f.check.push({slot:a,condition:c})});switch(a){case 21:k(module$js$graph$condition.Slot(module$js$graph$condition.Item.WINDMILL_KEY));break;case 35:f.hitbox={x0:-1,x1:2,y0:-1,y1:2};l(module$js$graph$condition.Slot(module$js$graph$condition.Item.BOW_OF_MOON),module$js$graph$condition.Magic.CHANGE);break;case 99:f.hitbox={x0:-1,x1:2,y0:-1,y1:2};k(module$js$graph$condition.Slot(module$js$graph$condition.Event.HEALED_DOLPHIN));k(module$js$graph$condition.Slot(module$js$graph$condition.Item.SHELL_FLUTE));
break;case 100:l(module$js$graph$condition.Slot(module$js$graph$condition.Event.RETURNED_FOG_LAMP),...this.flags.requireHealedDolphinToRide()?[module$js$graph$condition.Event.HEALED_DOLPHIN]:[]);break;case 107:k(module$js$graph$condition.Slot(module$js$graph$condition.Item.GLOWING_LAMP));break;case 117:l(module$js$graph$condition.Slot(module$js$graph$condition.Magic.FLIGHT));break;case 116:l(module$js$graph$condition.Slot(module$js$graph$condition.Magic.CHANGE),module$js$graph$condition.Magic.PARALYSIS,
module$js$graph$condition.Event.FOUND_KENSU);break;case 130:l(module$js$graph$condition.Slot(module$js$graph$condition.Item.GAS_MASK));break;case 136:k(module$js$graph$condition.Slot(module$js$graph$condition.Item.SHIELD_RING))}}if(132===a)return{check:[{condition:this.bossRequirements(this.rom.bosses.sabera1),slot:module$js$graph$condition.Slot(module$js$graph$condition.Boss.SABERA1)}]};if(29===a)return a=module$js$graph$condition.Slot(module$js$graph$condition.Item.SWORD_OF_FIRE),{check:[{condition:and$$module$js$graph$condition(module$js$graph$condition.Magic.TELEPATHY,
module$js$graph$condition.Boss.INSECT),slot:a},{condition:module$js$graph$condition.Event.RESCUED_CHILD,slot:a}]};if(31===a)return(a=this.rom.npcs[a].spawnConditions.get(b.id))&&a.includes(69)?{}:{check:[{condition:module$js$graph$condition.Event.DWARF_MOTHER,slot:module$js$graph$condition.Slot(module$js$graph$condition.Event.DWARF_CHILD)}]};for(var m of d.globalDialogs)h(~m.condition);for(const c of d.localDialogs.get(b.id)||d.localDialogs.get(-1)||[])if(!e.includes(~c.condition)){b=(b=FLAG_MAP$$module$js$graph$overlay.get(c.condition))?
[b]:this.relevantFlags.has(c.condition)?[module$js$graph$condition.Condition(c.condition)]:[];h=and$$module$js$graph$condition(...b,...g);m=FLAG_MAP$$module$js$graph$overlay.get(~c.condition)||(this.relevantFlags.has(~c.condition)?module$js$graph$condition.Condition(~c.condition):null);null!=m&&g.push(m);m=c.message.action;3===m?f.check.push({slot:module$js$graph$condition.Slot.item(d.data[0]),condition:h}):17===m||9===m?f.check.push({slot:module$js$graph$condition.Slot.item(d.data[1]),condition:h}):
16===m?f.check.push({slot:module$js$graph$condition.Slot(module$js$graph$condition.Magic.RECOVER),condition:h}):8===m&&45===a&&f.check.push({slot:module$js$graph$condition.Slot(module$js$graph$condition.Event.OPENED_SWAN),condition:h});for(const a of c.flags)(m=(m=FLAG_MAP$$module$js$graph$overlay.get(a))?m:this.relevantFlags.has(a)?module$js$graph$condition.Condition(a):null)&&f.check.push({slot:module$js$graph$condition.Slot(m),condition:h});if(b.length&&e.includes(c.condition))break}return f}capabilities(){var a=
module$js$graph$condition.Item.SWORD_OF_WIND;let b=module$js$graph$condition.Item.SWORD_OF_FIRE,c=module$js$graph$condition.Item.SWORD_OF_WATER,d=module$js$graph$condition.Item.SWORD_OF_THUNDER;if(!this.flags.orbsOptional()&&(a=or$$module$js$graph$condition(and$$module$js$graph$condition(module$js$graph$condition.Item.SWORD_OF_WIND,module$js$graph$condition.Item.ORB_OF_WIND),and$$module$js$graph$condition(module$js$graph$condition.Item.SWORD_OF_WIND,module$js$graph$condition.Item.TORNADO_BRACELET)),
b=or$$module$js$graph$condition(and$$module$js$graph$condition(module$js$graph$condition.Item.SWORD_OF_FIRE,module$js$graph$condition.Item.ORB_OF_FIRE),and$$module$js$graph$condition(module$js$graph$condition.Item.SWORD_OF_FIRE,module$js$graph$condition.Item.FLAME_BRACELET)),c=or$$module$js$graph$condition(and$$module$js$graph$condition(module$js$graph$condition.Item.SWORD_OF_WATER,module$js$graph$condition.Item.ORB_OF_WATER),and$$module$js$graph$condition(module$js$graph$condition.Item.SWORD_OF_WATER,
module$js$graph$condition.Item.BLIZZARD_BRACELET)),d=or$$module$js$graph$condition(and$$module$js$graph$condition(module$js$graph$condition.Item.SWORD_OF_THUNDER,module$js$graph$condition.Item.ORB_OF_THUNDER),and$$module$js$graph$condition(module$js$graph$condition.Item.SWORD_OF_THUNDER,module$js$graph$condition.Item.STORM_BRACELET)),this.flags.assumeSwordChargeGlitch())){var e=function(a){const b=a[0][0];return f.map((a)=>a[0]===b?a:[b,...a])};const f=or$$module$js$graph$condition(a,b,c,d);a=e(module$js$graph$condition.Item.SWORD_OF_WIND);
b=e(module$js$graph$condition.Item.SWORD_OF_FIRE);c=e(module$js$graph$condition.Item.SWORD_OF_WATER);d=e(module$js$graph$condition.Item.SWORD_OF_THUNDER)}a=[[module$js$graph$condition.Capability.SWORD,module$js$graph$condition.Item.SWORD_OF_WIND,module$js$graph$condition.Item.SWORD_OF_FIRE,module$js$graph$condition.Item.SWORD_OF_WATER,module$js$graph$condition.Item.SWORD_OF_THUNDER],[module$js$graph$condition.Capability.BREAK_STONE,a],[module$js$graph$condition.Capability.BREAK_ICE,b],[module$js$graph$condition.Capability.FORM_BRIDGE,
c],[module$js$graph$condition.Capability.BREAK_IRON,d],[module$js$graph$condition.Capability.MONEY,module$js$graph$condition.Capability.SWORD],[module$js$graph$condition.Capability.CLIMB_WATERFALL,module$js$graph$condition.Magic.FLIGHT],[module$js$graph$condition.Capability.SHOOTING_STATUE,module$js$graph$condition.Magic.BARRIER],[module$js$graph$condition.Capability.CLIMB_SLOPE,module$js$graph$condition.Item.RABBIT_BOOTS,module$js$graph$condition.Magic.FLIGHT],[module$js$graph$condition.Event.GENERALS_DEFEATED,
module$js$graph$condition.Item.IVORY_STATUE],[module$js$graph$condition.Event.OPENED_SEALED_CAVE,module$js$graph$condition.Event.STARTED_WINDMILL]];this.flags.assumeGhettoFlight()&&a.push([module$js$graph$condition.Capability.CLIMB_WATERFALL,and$$module$js$graph$condition(module$js$graph$condition.Event.RIDE_DOLPHIN,module$js$graph$condition.Item.RABBIT_BOOTS)]);this.flags.guaranteeBarrier()||a.push([module$js$graph$condition.Capability.SHOOTING_STATUE,and$$module$js$graph$condition(module$js$graph$condition.Capability.MONEY,
module$js$graph$condition.Capability.BUY_HEALING),and$$module$js$graph$condition(module$js$graph$condition.Capability.MONEY,module$js$graph$condition.Item.SHIELD_RING),and$$module$js$graph$condition(module$js$graph$condition.Capability.MONEY,module$js$graph$condition.Magic.REFRESH)]);this.flags.leatherBootsGiveSpeed()&&a.push([module$js$graph$condition.Capability.CLIMB_SLOPE,module$js$graph$condition.Item.LEATHER_BOOTS]);for(const b of this.rom.bosses)null!=b.kill&&null!=b.drop&&a.push([module$js$graph$condition.Item(b.drop),
module$js$graph$condition.Boss(b.kill)]);a.push([module$js$graph$condition.Item.ORB_OF_WATER,module$js$graph$condition.Boss.RAGE]);this.flags.guaranteeGasMask()?a.push([module$js$graph$condition.Capability.TRAVEL_SWAMP,module$js$graph$condition.Item.GAS_MASK]):a.push([module$js$graph$condition.Capability.TRAVEL_SWAMP,or$$module$js$graph$condition(module$js$graph$condition.Item.GAS_MASK,and$$module$js$graph$condition(module$js$graph$condition.Capability.MONEY,module$js$graph$condition.Item.MEDICAL_HERB),
and$$module$js$graph$condition(module$js$graph$condition.Capability.MONEY,module$js$graph$condition.Magic.REFRESH))]);return a.map((a)=>{var [b,...c]=a;return{capability:b,condition:or$$module$js$graph$condition(...c)}})}wallCapability(a){return{flag:[module$js$graph$condition.Capability.BREAK_STONE,module$js$graph$condition.Capability.BREAK_ICE,module$js$graph$condition.Capability.FORM_BRIDGE,module$js$graph$condition.Capability.BREAK_IRON][a][0][0]}}}
...
function build$$module$js$graph$world(a,b,c){b=void 0===b?new FlagSet$$module$js$flagset("@FullShuffle"):b;c=void 0===c?!1:c;const d=new Overlay$$module$js$graph$overlay(a,b,c),e=new Map,f=new Map,g=new Map,h=new Map,k=new DefaultMap$$module$js$util(()=>new Set),l=new Map,m=new Set;for(const b of a.locations){var n=function(a,b){const c=e.get(a);c&&e.set(a,module$js$graph$condition.Terrain.meet(c,b))};if(!b.used)continue;const c=b.extended?256:0,q=a.tilesets[(b.tileset&127)>>2],u=a.tileEffects[b.tileEffects-
179];for(const a of b.spawns)a.isWall()&&f.set(module$js$graph$geometry.ScreenId.from(b,a),a.id&3);for(let g=0,h=b.height;g<h;g++){const h=b.screens[g],k=b.id<<8|g<<4;for(let g=0,l=b.width;g<l;g++){var r=function(a,b){if(!p)throw Error("flag expected");return b?a?t(a,b):module$js$graph$condition.Terrain.meet({enter:module$js$graph$condition.Condition(p.flag)},b):a};const l=a.screens[h[g]|c],n=module$js$graph$geometry.ScreenId(k|g),m=n&255,z=f.get(n),p=null!=z?d.wallCapability(z):b.flags.find((a)=>
a.yx===m),t=memoize2$$module$js$graph$condition((a,b)=>{if(!p)throw Error("flag expected");b=Object.assign({},b,{enter:meet$$module$js$graph$condition(b.enter||[[]],module$js$graph$condition.Condition(p.flag))});return module$js$graph$condition.Terrain.join(a,b)});for(let a=0;240>a;a++){const b=module$js$graph$geometry.TileId(n<<8|a);let f=l.tiles[a];p&&751===p.flag&&32>f&&(f=q.alternates[f]);let g=d.makeTerrain(c?0:u.effects[f]&38,b);if(32>f&&q.alternates[f]!=f&&p&&751!==p.flag){const a=d.makeTerrain(u.effects[q.alternates[f]],
b);g=r(g,a)}g&&e.set(b,g)}}}for(const a of b.exits)if(a.entrance&32){const c=module$js$graph$geometry.TileId.from(b,a);m.add(c);const d=e.get(c);d&&e.set(c,module$js$graph$condition.Terrain.seamless(d))}for(const c of b.spawns)if(c.isTrigger()){const a=d.trigger(c.id);if(a.terrain||a.check){let {x:d,y:e}=c;d+=8;for(const c of[b,...a.extraLocations||[]])for(const b of a.dx||[-16,0])for(const f of[-16,0]){const g=module$js$graph$geometry.TileId.from(c,{x:d+b,y:e+f});a.terrain&&n(g,a.terrain);for(const b of a.check||
...
{exit:h,exitSouth:h})),n=c&-256,m=d.bossRequirements(g),q={slot:module$js$graph$condition.Slot(h),condition:m};for(let a=0;240>a;a++){const b=module$js$graph$geometry.TileId(n|a),c=e.get(b);c&&(e.set(b,l(c)),k.get(b).add(q))}}}for(const a of d.locations()||[])k.get(a.tile).add(a);const p=new UnionFind$$module$js$unionfind;for(const a of e){const [b,c]=a;{const a=module$js$graph$geometry.TileId.add(b,0,1);e.get(a)===c&&p.union([b,a]);const d=module$js$graph$geometry.TileId.add(b,1,0);e.get(d)===c&&
p.union([b,d])}}const q=new Set;for(const b of a.locations)if(b.used)for(const c of b.exits){const {dest:d,entrance:e}=c,f=module$js$graph$geometry.TileId.from(b,c),g=e&32?module$js$graph$geometry.TileId(f&65535|d<<16):module$js$graph$geometry.TileId.from({id:d},a.locations[d].entrances[e]);q.add(module$js$graph$geometry.TilePair.of(p.find(f),p.find(g)))}for(const a of q){const [b,c]=module$js$graph$geometry.TilePair.split(a);if(e.get(b)!==e.get(c))continue;const d=module$js$graph$geometry.TilePair.of(c,
b);q.has(d)&&(p.union([b,c]),q.delete(a),q.delete(d))}const t=new Neighbors$$module$js$graph$geometry(p,m);for(const a of e){const [b,c]=a;{const a=module$js$graph$geometry.TileId.add(b,0,1),d=e.get(a);d&&d!==c&&t.addAdjacent(b,a,!1);const f=module$js$graph$geometry.TileId.add(b,1,0),g=e.get(f);g&&g!==c&&t.addAdjacent(b,f,!0)}}for(const a of q){const [b,c]=module$js$graph$geometry.TilePair.split(a);e.has(b)&&e.has(c)&&t.addExit(b,c)}const u=new Routes$$module$js$graph$routes;for(const a of d.extraRoutes())for(const b of a.condition||
[[]])u.addRoute(p.find(a.tile),b);for(const a of d.extraEdges()){const {from:b,to:c,condition:d}=a;for(const a of d||[[]])u.addEdge(p.find(c),p.find(b),a)}for(const a of t){const {from:b,to:c,south:d}=a;{const a=e.get(b),f=e.get(c);if(!a||!f)throw Error(`missing terrain ${a?c:b}`);for(const e of(d?a.exitSouth:a.exit)||[[]])for(const a of f.enter||[[]])u.addEdge(c,b,[...a,...e])}}const z=new DefaultMap$$module$js$util(()=>new MutableRequirement$$module$js$graph$condition);for(const a of d.capabilities()){const {condition:b=
[[]],capability:c}=a;z.get(module$js$graph$condition.Slot(c)).addAll(b)}for(const a of k){const [b,c]=a;{const a=p.find(b);if(b!==a){for(const b of c)k.get(a).add(b);k.delete(b)}}}for(const a of k){const [b,c]=a;for(const a of c){const {slot:c,condition:d=[[]]}=a;{const a=z.get(c);for(const c of d)for(const d of u.routes.get(b)||[])a.addList([...c,...d])}}}if(DEBUG$$module$js$graph$world&&"undefined"!==typeof window){var J=function(a,b){b=void 0===b?w:b;a=[...a];return a.length?"("+a.map((a)=>[...a].map(b).join(" & ")).join(") |\n     (")+
")":"no route"},w=function(a){return 0>a?"~"+(~a).toString(16).padStart(2,"0"):a.toString(16).padStart(3,"0")};const b=window;console.log(b.roots=(b.tiles=p).roots());console.log([...b.neighbors=t]);console.log(b.ll=u);b.area=(a,b)=>{b=void 0===b?w:b;const c=[...p.sets().filter((b)=>b.has(a))[0]].map((a)=>a.toString(16).padStart(6,"0")),d=J(u.routes.get(p.find(a)),b),e=[],f=p.find(a);for(const a of(u.edges.get(f)||new Map).values())e.push(`\nto ${a.target.toString(16)} if (${[...a.deps].map(b).join(" & ")})`);
for(const a of u.edges){const [c,d]=a;for(const a of d.values())a.target===f&&e.push(`\nfrom ${c.toString(16)} if (${[...a.deps].map(b).join(" & ")})`)}return`${hex$$module$js$rom$util(f)}\n${(function(a,b,c){const d=[];for(let e=0;e<a.length;e+=b)d.push(a.slice(e,e+b).join(c));return d})(c,16," ").join("\n")}\ncount = ${c.length}\nroutes = ${d}${e.join("")}`};b.whatFlag=(b)=>conditionName$$module$js$graph$world(b,a);b.reqs=z;console.log("reqs\n"+[...z].sort((a,b)=>{[a]=a;[b]=b;return a-b}).map((a)=>
{var [c,d]=a;return`${b.whatFlag(c)}: ${J(d,b.whatFlag)}`}).join("\n"));b.reqs.check=(a,b)=>{b=void 0===b?w:b;return`${b(a)}: ${J(z.get(module$js$graph$condition.Slot(a)),b)}`};b.reqs.check2=(a)=>b.reqs.check(a,b.whatFlag)}let x=isItem$$module$js$graph$world;c&&(x=function(a){return isItem$$module$js$graph$world(a)||b.shuffleBossElements()&&isBoss$$module$js$graph$world(a)?!0:!1});return new World$$module$js$graph$world(a,p,makeGraph$$module$js$graph$world(z,a,x))}
function isBoss$$module$js$graph$world(a){return 256<=~a&&272>~a}function isItem$$module$js$graph$world(a){return 512<=a&&640>a}
function makeGraph$$module$js$graph$world(a,b,c){function d(a,c){return{name:conditionName$$module$js$graph$world(a,b),condition:a,index:c}}function e(a,b){return Object.assign(d(a,b+n),{item:a&127})}function f(a){const c=r.get(a);if(null==c)throw Error(`Missing item $${a.toString(16)}: ${conditionName$$module$js$graph$world(a,b)}`);return c}var g=new Set,h=new Set;for(var k of a){const [a,b]=k;h.add(a);for(var l of b)for(var m of l)g.add(m)}g.add(module$js$graph$condition.Boss.DRAYGON2[0][0]);k=
[...g].filter((a)=>!c(a)).sort();g=[...g].filter(c).sort();l=[...h].filter(c).sort();const n=k.length;h=k.map(d);g=g.map(e);m=l.map(e);g=[...h,...g];k=[...h,...m];const r=new Map(g.map((a,b)=>[a.condition,b])),p=new Map(k.map((a,b)=>[a.condition,b]));l=[];const q=[];for(var t of a){const [c,d]=t;a=p.get(c);if(null==a)MAYBE_MISSING_SLOTS$$module$js$graph$world.has(c)||console.error(`Nothing depended on $${c.toString(16)}: ${conditionName$$module$js$graph$world(c,b)}`);else for(const b of d){const c=
[...b].map(f);(l[a]||(l[a]=[])).push(Bits$$module$js$bits.from(c));for(const b of c)(q[b]||(q[b]=new Set)).add(a)}}for(var u of[...h,...m])l[u.index]&&l[u.index].length||(t=u.condition,console.error(`Nothing provided $${t.toString(16)}: ${conditionName$$module$js$graph$world(t,b)} (index ${u.index})`));DEBUG$$module$js$graph$world&&console.log(l);u=q.map((a)=>[...a]);return{fixed:n,slots:k,items:g,graph:l,unlocks:u}}const MAYBE_MISSING_SLOTS$$module$js$graph$world=new Set([37,38,169,580]);
...
h=[...concatIterables$$module$js$rom$util(b.exits),128|(b.pits.length?64:0)|b.entrances.length];var k=[...concatIterables$$module$js$rom$util(b.flags),255],l=concatIterables$$module$js$rom$util(b.pits),[m,n,r,p,q,t]=yield Promise.all([d(e,"Layout"),d(f,"Graphics"),d(g,"Entrances"),d(h,"Exits"),d(k,"Flags"),...l.length?[d(l,"Pits")]:[]]);e=[m&255,(m>>>8)-192,n&255,(n>>>8)-192,r&255,(r>>>8)-192,p&255,(p>>>8)-192,q&255,(q>>>8)-192,...t?[t&255,(t>>8)-192]:[]];d=yield d(e,"MapData");writeLittleEndian$$module$js$rom$util(a.rom,
...
Scr$$module$js$maze$maze.edge(c[1],c[2]);yield c}}*eligible(a,b){b=void 0===b?{}:b;const {skipAlternates:c,maxExits:d,edge:e,fuzzy:f,stair:g}=b;var h=null!=e?e|e<<4|e<<8|e<<12:void 0;let k=b=0,l=0;for(var m of Dir$$module$js$maze$maze.ALL){var n=Dir$$module$js$maze$maze.edgeMask(m),r=Dir$$module$js$maze$maze.edgeMask(Dir$$module$js$maze$maze.inv(m));let c=this.get(a,m);null==c&&(c=h);null!=c&&(l|=((c&r)>>>8|(c&r)<<8)&65535,b|=n,f&&this.isFixed(Pos$$module$js$maze$maze.plus(a,m))&&(k|=n))}f||(k=b);
a=l&k;m=[];h=Infinity;for(const e of this.screens){const [q,p]=e;if(!(p.fixed||c&&p.edges&-65536||null!=g&&!p.stairs.some((a)=>a.dir===g)||null==g&&p.stairs.length||(q&k)!==a||d&&Scr$$module$js$maze$maze.numExits(q)>d))if(f){n=0;r=q&b^l;for(const a of Dir$$module$js$maze$maze.ALL)r&15<<Dir$$module$js$maze$maze.shift(a)&&n++;n<h?(h=n,m=[q]):n===h&&m.push(q)}else yield q}f&&(yield*m)}fill(a,b){b=void 0===b?{}:b;if(b.force&&b.fuzzy)throw Error("invalid");const c=[...this.eligible(a,b)];if(!c.length)return!1;
...
new Set([77]),ALL$$module$js$rom$constraint,ALL$$module$js$rom$constraint],[],0)}static get KENSU_CHEST(){return new Constraint$$module$js$rom$constraint([new Set([81]),ALL$$module$js$rom$constraint,ALL$$module$js$rom$constraint,ALL$$module$js$rom$constraint],[],0)}static forLocation(a){switch(a){case 3:return new Constraint$$module$js$rom$constraint([ALL$$module$js$rom$constraint,bit$$module$js$rom$constraint(96),ALL$$module$js$rom$constraint,bit$$module$js$rom$constraint(32)],[],0);case 96:case 100:case 104:return new Constraint$$module$js$rom$constraint([ALL$$module$js$rom$constraint,
bit$$module$js$rom$constraint(82),ALL$$module$js$rom$constraint,bit$$module$js$rom$constraint(8)],[],0)}return Constraint$$module$js$rom$constraint.ALL}static fromSpawn(a,b,c,d,e){const [f,...g]=b;(e=e&&2===f&&!g.length)&&d.patternBank&&(b=new Set([3]));const h=e||!b.has(2)?ALL$$module$js$rom$constraint:bit$$module$js$rom$constraint(c.spritePatterns[0]);b=e||!b.has(3)?ALL$$module$js$rom$constraint:bit$$module$js$rom$constraint(c.spritePatterns[1]);d=e?[bit$$module$js$rom$constraint(c.spritePatterns[d.patternBank])]:
[];e=a.has(2)?bit$$module$js$rom$constraint(c.spritePalettes[0]):ALL$$module$js$rom$constraint;a=a.has(3)?bit$$module$js$rom$constraint(c.spritePalettes[1]):ALL$$module$js$rom$constraint;return new Constraint$$module$js$rom$constraint([h,b,e,a],d,0)}ignorePalette(){return new Constraint$$module$js$rom$constraint([this.fixed[0],this.fixed[1],ALL$$module$js$rom$constraint,ALL$$module$js$rom$constraint],this.float,this.shift)}shufflePalette(a,b){const c=[...this.fixed];for(let d=2;4>d;d++){if(c[d]===
...
const c=seq$$module$js$rom$util(this.float.length,(b)=>CSet$$module$js$rom$constraint.union(this.float[b],a.float[b]));return new Constraint$$module$js$rom$constraint(b,c,this.shift|a.shift)}fix(a,b){var c=b?(a)=>b.nextInt(a):()=>0;const d=b?(a)=>b.pick([...a]):(a)=>a[Symbol.iterator]().next().value,e=[...this.fixed];if(this.float.length){c=c(2);const a=1-c;c<this.float.length&&(e[0]=CSet$$module$js$rom$constraint.intersect(e[0],this.float[c]));a<this.float.length&&(e[1]=CSet$$module$js$rom$constraint.intersect(e[1],
this.float[a]))}e[0]!==ALL$$module$js$rom$constraint&&(a.spritePatterns[0]=d([...e[0]]));e[1]!==ALL$$module$js$rom$constraint&&(a.spritePatterns[1]=d([...e[1]]));e[2]!==ALL$$module$js$rom$constraint&&(a.spritePalettes[0]=d([...e[2]]));e[3]!==ALL$$module$js$rom$constraint&&(a.spritePalettes[1]=d([...e[3]]))}meet(a,b=!1){a=this.tryMeet(a,b);if(!a)throw Error("Could not reconcile patterns");return a}tryMeet(a,b=!1){const c=[];let d=this.shift|a.shift;for(var e=0;4>e;e++){var f=CSet$$module$js$rom$constraint.intersect(this.fixed[e],
a.fixed[e]);if(!f.size){if(!b||2>e)return;f=CSet$$module$js$rom$constraint.union(this.fixed[e],a.fixed[e])}c.push(f)}e=new Map;b=[];for(var g of module$js$util.iters.concat(this.float,a.float)){if(g===ALL$$module$js$rom$constraint)throw Error("Unexpected unconstrained float");a=!1;for(var h of g)if(f=e.get(h),null!=f){for(var k of b[f])e.delete(k);b[f]=CSet$$module$js$rom$constraint.intersect(b[f],g);for(const a of b[f])e.set(a,f);a=!0;break}if(a)break;for(const a of g)e.set(a,b.length);b.push(g);
if(2<b.length)return}for(g=0;g<b.length;g++)for(h=0;2>h;h++)if(!CSet$$module$js$rom$constraint.intersect(b[g],c[h]).size){k=c[1-h]=CSet$$module$js$rom$constraint.intersect(b[g],c[1-h]);d|=1<<1-h;if(!k.size)return;b.splice(g,1);g=-1;break}return new Constraint$$module$js$rom$constraint(c,b,d)}}
...
function surveyMap$$module$js$maze$cave(a){function b(b){b=a.entrances[b];for(const c of a.exits)if(20>Math.abs(c.x-b.x)&&20>Math.abs(c.y-b.y))return c.dest<<8|c.entrance;throw Error("Could not find exit for given entrance");}let c=0,d=0,e=0,f=0;const g=new Map,h=new Map,k=new Map,l=new Multiset$$module$js$util,m=new Set([...a.reachableTiles(!0).keys()].map((a)=>a>>>8));for(var n=0;n<a.entrances.length;n++){var r=a.entrances[n],p=STAIRS_BY_TILE$$module$js$maze$cave.get(a.screens[r.screen>>>4][r.screen&
15]<<8|r.tile);null!=p&&g.set(r.screen,{entrance:n,exit:b(n),dir:p})}for(n=0;n<a.height;n++)for(r=0,n||(r|=1),n===a.height-1&&(r|=4),p=0;p<a.width;p++){const z=n<<4|p;if(!m.has(z))continue;r&=-11;p||(r|=8);p===a.width-1&&(r|=2);var q=a.screens[n][p];l.add(q);if(128===q)continue;c++;let J=EDGES_BY_TILE$$module$js$maze$cave.get(q);if(null==J)throw Error(`Bad tile: ${hex$$module$js$rom$util(q)}`);var t=BITCOUNT$$module$js$maze$cave[J]+(g.has(z)?1:0);154===q&&(t=2);if(155===q||156==q||240==q||241==q)t=
1;1===t&&d++;2<t&&(e+=t-2);WALLS$$module$js$maze$cave.has(q)&&f++;t=FIXED_TILES$$module$js$maze$cave.get(q);DEAD_ENDS$$module$js$maze$cave.has(q)&&(J=0);for(const c of module$js$maze$maze.Dir.ALL)if(J&r&1<<c){q=null;for(var u=0;u<a.entrances.length;u++)if(a.entrances[u].screen===z&&matchesDir$$module$js$maze$cave(a.entrances[u].tile,c)){q=u;break}null!=q&&(u=b(q),h.set(z,{entrance:q,exit:u,dir:c}))}null!=t&&k.set(z,t)}return{size:c,deadEnds:d,branches:e,walls:f,stairs:g,edges:h,fixed:k,tiles:l}}
...
f[0]);g=a.entrances[c.entrance];k=g.tile&15;l=g.tile>>>4;g.screen=f[0];g.coord=f[1].entrance;m.set(b,[(g.tile>>>4)-l,(g.tile&15)-k]);for(var q of f[1].exits)a.exits.push(Exit$$module$js$rom$location.of({screen:f[0],tile:q,dest:c.exit>>>8,entrance:c.exit&255}))}q=new Map;h=a.monsterPlacer(b);for(const c of a.spawns)if(3!==c.type)if(c.isMonster())f=a.rom.objects[c.monsterId],f instanceof Monster$$module$js$rom$monster&&(g=h(f),null==g?(console.error(`no valid location for ${hex$$module$js$rom$util(f.id)} in ${hex$$module$js$rom$util(a.id)}`),
c.used=!1):(c.screen=g>>>8,c.tile=g&255));else if(f=c.screen,g=e.get(f),k=q.get(c.y<<12|c.x),null!=k)c.y=k>>>12,c.x=k&4095;else if(null!=g){if(c.screen=g,f=m.get(f),null!=f){const [a,b]=f;c.yt+=a;c.xt+=b}}else if(c.isTrigger())140===c.id?c.screen=e.get(33)-16:console.error(`unhandled trigger: ${c.id}`);else{f=[...d.keys()].sort((a,b)=>a-b);if(!f.length)throw Error("no poi");for(const a of f)if(f=d.get(a),f.length){g=c.y<<12|c.x;k=b.nextInt(f.length);[[c.y,c.x]]=f.splice(k,1);q.set(g,c.y<<12|c.x);
...
k=Math.min(a.width+1,8),l=Math.min(a.height+1,16),m=seq$$module$js$rom$util(k*l,(a)=>a%k|Math.floor(a/k)<<4),n=surveyMap$$module$js$maze$cave(a),r=n.size/k/l;let p;p=function(a){let {walls:f}=n;var h=new Set;for(const [b,c]of n.edges)for(;;){var q=e(c.dir);if(!h.has(q)){h.add(q);var p=n.fixed.get(b);null==p?a.setBorder(q,c.dir,6):(a.setBorder(q,c.dir,p.edges>>>module$js$maze$maze.Dir.shift(c.dir)&15),fixBorders$$module$js$maze$cave(a,q,p.edges),a.set(q,p.edges),p.wall&&f--);break}}for(const [c,d]of n.fixed)if(!n.edges.has(c))for(const c of b.ishuffle(m))if(!h.has(c)&&
a.saveExcursion(()=>{fixBorders$$module$js$maze$cave(a,c,d.edges);return a.trySet(c,d.edges)})){h.add(c);d.wall&&f--;break}q=[...n.stairs.values()];p=0;for(var w=0;10>p&&w<q.length;p++){const b=a.randomPos();!h.has(b)&&a.fill(b,{stair:q[w].dir})&&(h.add(b),p=0,w++)}if(10<=p)return fail$$module$js$maze$cave("could not add all stairs");if(!a.fillAll({edge:1,fuzzy:1,shuffleOrder:!0,skipAlternates:!0}))return fail$$module$js$maze$cave("could not fill open");if(!g(a))return fail$$module$js$maze$cave("check failed after initial setup");
const x={skipAlternates:!0};for(const [c]of b.shuffle([...a])){if(a.density()<=r)break;!a.isFixed(c)&&a.saveExcursion(()=>a.setAndUpdate(c,0,x)&&g(a))}if(!c.allowTightCycles)for(h=1;h<l;h++)for(q=1;q<k;q++)if(p=h<<4|q,isTightCycle$$module$js$maze$cave(a,p)){w=!1;for(const c of b.ishuffle(module$js$maze$maze.Dir.ALL)){const b=2>c?p-1:p-16;a.saveExcursion(()=>a.replaceEdge(b,c,0)&&g(a))&&(w=!0)}if(!w)return fail$$module$js$maze$cave("failed to remove tight cycle")}h=new Set;q=[...a.alternates()];for(const c of[140])if(n.tiles.count(c)){p=
b.shuffle(q.filter((a)=>a[3].tile===c));if(p.length<n.tiles.count(c))return fail$$module$js$maze$cave("could not add stair hallway");for(w=n.tiles.count(c)-1;0<=w;w--)a.replace(p[w][0],p[w][2]),h.add(p[w][0])}q=b.shuffle(q.filter((a)=>a[3].wall));for(p=0;p<f;p++){w=q.pop();if(null==w)return fail$$module$js$maze$cave(`could not add wall ${p}`);h.has(w[0])?p--:(a.replace(w[0],w[2]),h.add(w[0]))}return d(a)};for(let a=0;a<f;a++){const a=new Maze$$module$js$maze$maze(b,l,k,h);if(p(a))return}throw Error("Could not shuffle");
...
c|b);if(k())return!0;m.replace(a,c);return!1},k=function(){const a=m.traverse(),b=a.get(q);return!b||!t.filter((a)=>b.has(a)).length||b.size<.8*a.size?!1:!0},l=function(a,b){m.set(a,b,{force:!0})};const m=new Maze$$module$js$maze$maze(b,f,e,g),r=f-1<<4|b.nextInt(e),p=b.nextInt(e),q=r<<8|2,t=[p+32<<8|1,p+32<<8|3];l(r,61681);l(r-1,240);l(r+1,61440);l(p,65520);l(p-1,4080);l(p+1,65280);l(p+16,61951);l(p+15,255);l(p+17,61455);if(!m.connect(r,0,p+16,2))continue a;if(257===m.get(p+32)||257===m.get(r-16))continue a;
console.log("first",m.show());for(l=0;10>l&&.65>m.density();l++)m.addLoop()&&(l=0);console.log(`loop\n${m.show()}`);if(.45>m.density())continue a;if(!k())continue a;for(const [a,c]of m)if(257===c&&257===m.get(a+16)&&(l=b.shuffle([0,16]),!h(a+l[0],131072)&&!h(a+l[1],131072)))continue a;for(const [a,c]of b.shuffle([...m.alternates()]))h(a,c);m.fillAll({edge:0});d.moveScreen(6,p);d.moveScreen(131,r);m.write(d,new Set);b=d.monsterPlacer(b);for(const f of d.spawns)f.isMonster()&&(c=a.objects[f.monsterId],
...
function tryShuffleSwamp$$module$js$maze$swamp(a,b,c,d,e){function f(){const a=d.traverse(),b=a.get(m);return b&&b.size===a.size||!1}const g=d.width,h=d.height,[k,l]=(()=>{let a,c;do a=b.nextInt(g),c=b.nextInt(h);while(2>a||2>c);return[a,c<<4]})();d.setBorder(l,module$js$maze$maze.Dir.LEFT,1);d.set(k,61936,{force:!0});d.set(k-1,240,{force:!0});d.set(k+1,61440,{force:!0});d.fillAll({edge:1});const m=l<<8|16;var n=[...d].map((a)=>a[0]);for(var r=Math.floor(.6*g*h);r;r--){const a=b.pick(n),c=b.pick(module$js$maze$maze.Dir.ALL),
e=module$js$maze$maze.Pos.plus(a,c);!d.isFixed(a)&&!d.isFixed(e)&&d.get(a)&module$js$maze$maze.Dir.edgeMask(c)?d.saveExcursion(()=>d.replaceEdge(a,c,0)?!!(d.get(a)&&d.get(e)&&f()):!1):r++}if(!d.consolidate(e,f,a))return!1;const [p,q]=b.pick([...d.alternates()]);if(65536!=q)throw Error(`unexpected alt: ${hex$$module$js$rom$util(q)}`);d.replace(p,d.get(p)|q);c.moveScreen(48,l);c.moveScreen(4,k);d.write(c,new Set);e=[];n=[];for(const [a,b]of d)a!==k&&a!==p&&(1!==b&&16!==b&&256!==b&&4096!==b||e.push(a),
17!==b&&272!==b&&4352!==b&&4097!==b||n.push(a));var t=a.screens[c.screens[p>>4][p&15]].tiles;r=0;for(let a=0;240>a;a++)if(3===t[a]){r=a;break}c.entrances[1].screen=p;c.entrances[1].tile=r+17;for(t=0;2>t;t++)c.exits[5+t].screen=p,c.exits[5+t].tile=r+t;e=b.pick(e.length?e:n);c.spawns[1].screen=e;e=c.monsterPlacer(b);for(const b of c.spawns)b.isMonster()&&(n=a.objects[b.monsterId],n instanceof Monster$$module$js$rom$monster&&(r=e(n),null==r?(console.error(`no valid location for ${hex$$module$js$rom$util(n.id)} in ${hex$$module$js$rom$util(c.id)}`),
b.used=!1):(b.screen=r>>>8,b.tile=r&255)));a.spoiler&&a.spoiler.addMaze(c.id,c.name,d.show());return!0}
...
0;g.length&&h<MAX_TABLE_LENGTH$$module$js$rom$messages;){e.has(g[0].str)&&(g.sort(f),e.clear());const {str:n,saving:r,missing:p,words:q,chains:t}=g.shift();if(0>=r)break;h+=n.length+3;k=a.length;l=new Set;for(const a of q){m=b[a];for(const a of[m.mid,...d.get(m.mid)||[]])l.add(a)}a.push({bytes:128>k?[k+128]:[5,k-128],mids:l,str:n});for(const a of q)for(k=0;k<=t;k++){l=b[a+k];m=l.bytes-(k?0:p);for(const a of l.suffixes)a.saving-=m-l.used,e.add(a.str);l.used=m}if(128===a.length){for(const a of c.values())a.saving-=
...
16);return`[${"{"===b?6:7}][${a}]${d}`});for(var r of f.get(n.mid())||[]){const {str:a,bytes:b}=r;l=l.replace(new RegExp(a+"( [ &0-9]|.|$)","g"),(a,c)=>{if(c&&!PUNCTUATION$$module$js$rom$messages[c])return a;" "===c&&(c="");return b.map((a)=>`[${a}]`).join("")+c})}m=["[01]"];d=[];d.push(1);for(let a=0,b=l.length;a<b;a++)if(e=l[a],e===Messages$$module$js$rom$messages.CONTINUED)d.push(3,1),m.push("[03][01]"),"\n"===l[a+1]&&a++;else if("\n"===e)d.push(2)," "===l[a+1]&&a++,m.push("[02]");else if("["===
...
a-c)}n=yield Promise.all(k.map((a)=>Promise.all(a)));r=163840;for(f=0;f<n.length;f++){d=[];for(k=0;k<n[f].length;k++)writeLittleEndian$$module$js$rom$util(d,2*k,n[f][k]);a.rom.subarray(r,r+d.length).set(d);writeLittleEndian$$module$js$rom$util(a.rom,164898+2*f,r-131072);r+=d.length}for(n=0;n<b.banks.length;n++)a.rom[164862+n]=b.banks[n];yield Promise.all([])})}}Messages$$module$js$rom$messages.CONTINUED="#";
...
for(var l of this.locations)if(l.used&&a.has(l.tileset)){c.add(l.tileEffects);for(var m of l.allScreens())g.add(m)}for(var n of g)for(let a=0,b=n.tiles.length;a<b;a++)n.tiles[a]=d[n.tiles[a]];for(var r of a){d=this.tilesets[(r&127)>>>2];for(var p of b)for(l=0;l<p.length-1;l++){m=f(p[l]);n=f(p[l+1]);for(a=0;4>a;a++)d.tiles[a][m]=d.tiles[a][n];d.attrs[m]=d.attrs[n];if(32>n&&d.alternates[n]!==n){if(32<=m)throw Error(`Cannot unflag: ${r} ${m} ${n} ${d.alternates[n]}`);d.alternates[m]=d.alternates[n]}}for(var q of e){const [a,
b]=q;d.alternates[a]=b}}for(const a of c){r=this.tileEffects[a-179];for(const a of b)for(p=0;p<a.length-1;p++)q=f(a[p]),d=f(a[p+1]),r.effects[q]=r.effects[d];for(const a of e.keys())r.effects[a]|=8}}static load(a,b){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){const c=yield pickFile$$module$js$rom(b);a&&(yield a(c));return new Rom$$module$js$rom(c)})}}Rom$$module$js$rom.OMIT_ITEM_GET_DATA_SUFFIX=module$js$rom$option.RomOption.bit(82624,0);
...
c,h);c.randomizeWildWarp()&&shuffleWildWarp$$module$js$patch(m,c,h);rescaleMonsters$$module$js$patch(m,c,h);unidentifiedItems$$module$js$pass$unidentifieditems(m,c,h);shuffleTrades$$module$js$pass$shuffletrades(m,c,h);c.randomizeMaps()&&shuffleMazes$$module$js$pass$shufflemazes(m,h);const n=World$$module$js$graph$world.build(m,c),r=yield(new AssumedFill$$module$js$graph$shuffle(m,c)).shuffle(n.graph,h,f);if(r)n.traverse(n.graph,r),update$$module$js$rom$slots(m,r.slots);else return-1;rescaleShops$$module$js$patch(m,
...
new Set([80,83,95,105]);for(const h of SCALED_MONSTERS$$module$js$patch){const [k,{sdef:l,swrd:m,hits:n,satk:r,dgld:p,sexp:q}]=h;f=a.objects[k].data;g=e.has(k)?1:0;f[2]|=128;f[6]=n;f[7]=r;f[8]=l|m<<4;f[16]=f[16]&15|p<<4;f[17]=q;(g?b.shuffleBossElements():b.shuffleMonsterElements())&&!d.has(k)&&(f=[...a.objects[k].elements.toString(2).padStart(4,"0")],c.shuffle(f),a.objects[k].elements=Number.parseInt(f.join(""),2))}if(b.shuffleMonsterElements()){b=c.nextInt(4);a.prg[217645]=b+1;for(const c of d)a.objects[c].elements=
1<<b}}
...
!0):n.meet(Constraint$$module$js$rom$constraint.MIMIC,!0);else if(a.isNpc()||a.isBoss()){var c=b.getNpcConstraint(e.id,a.id);n=n.meet(c,!0);a.isNpc()&&107===a.id&&(n=n.meet(Constraint$$module$js$rom$constraint.KENSU_CHEST,!0))}else a.isMonster()&&UNTOUCHED_MONSTERS$$module$js$patch[a.monsterId]&&(c=b.getMonsterConstraint(e.id,a.monsterId),n=n.meet(c,!0));g.push(`Initial pass: ${n.fixed.map((a)=>Infinity>a.size?"["+[...a].join(", ")+"]":"all")}`);const r=new Map;c=(a)=>{var c=e.rom.objects[a.id];if(c.monsterClass){var d=
r.get(c.monsterClass);if(null!=d&&d!==a.id)return!1}d=FLYERS$$module$js$patch.has(a.id);var h=MOTHS_AND_BATS$$module$js$patch.has(a.id);if(d){if(!m)return!1;--m}const l=b.getMonsterConstraint(e.id,a.id);let q=n.tryMeet(l);!q&&Infinity>n.pal2.size&&Infinity>n.pal3.size&&this.flags.shuffleSpritePalettes()&&(q=n.tryMeet(l,!0));if(!q)return!1;if(p){var x=e.rom.objects[a.id];if(!(x instanceof Monster$$module$js$rom$monster))throw Error(`non-monster: ${x}`);x=p(x);if(null==x)return!1}g.push(`  Adding ${a.id.toString(16)}: ${q}`);
n=q;c.monsterClass&&r.set(c.monsterClass,a.id);c=0;if(d||h)for(d=0;d<f.length;d++){if(f[d]in k){c=d;break}}else for(d=0;d<f.length;d++)if(!(f[d]in k)){c=d;break}(this.report[`mon-${a.id.toString(16)}`]=this.report[`mon-${a.id.toString(16)}`]||[]).push("$"+e.id.toString(16));d=f[c];h=e.spawns[d-13];p?(h.screen=x>>>8,h.tile=x&255):d in k&&(h.y+=16*k[d][0],h.x+=16*k[d][1]);h.monsterId=a.id;g.push(`    slot ${d.toString(16)}: ${h}`);f.splice(c,1);return!0};const p=f.length&&this.flags.randomizeMaps()?
e.monsterPlacer(a):null;if(m&&f.length)for(var d=0;d<Math.min(40,this.monsters.length);d++)FLYERS$$module$js$patch.has(this.monsters[d].id)&&c(this.monsters[d])&&this.monsters.splice(d,1);for(d=0;d<this.monsters.length&&f.length;d++)if(c(this.monsters[d])){const [a]=this.monsters.splice(d,1);FLYERS$$module$js$patch.has(a.id)||this.used.push(a);d--}for(d=0;d<this.used.length&&f.length;d++)c(this.used[d])&&(this.used.push(...this.used.splice(d,1)),d--);n.fix(e,a);if(f.length){console.error(`Failed to fill location ${e.id.toString(16)}: ${f.length} remaining`);
