...
for(const a of r.routes.edges){const [c,d]=a;for(const a of d.values())a.target===f&&e.push(`\nfrom ${c.toString(16)} if (${[...a.deps].map(b).join(" & ")})`)}return`${hex$$module$js$rom$util(f)}\n${(function(a,b,c){const d=[];for(let e=0;e<a.length;e+=b)d.push(a.slice(e,e+b).join(c));return d})(c,16," ").join("\n")}\ncount = ${c.length}\nroutes = ${d}${e.join("")}`};b.whatFlag=(b)=>conditionName$$module$js$graph$world(b,a);b.reqs=t;console.log("reqs\n"+[...t].sort((a,b)=>{[a]=a;[b]=b;return a-b}).map((a)=>
{var [c,d]=a;return`${b.whatFlag(c)}: ${u(d,b.whatFlag)}`}).join("\n"));b.reqs.check=(a,b)=>{b=void 0===b?z:b;return`${b(a)}: ${u(t.get(module$js$graph$condition.Slot(a)),b)}`};b.reqs.check2=(a)=>b.reqs.check(a,b.whatFlag)}this.graph=makeGraph$$module$js$graph$world(t,a)}traverse(a,b){const {spoiler:c}=this.rom;if(c){for(var d=-512;768>d;d++)c.addCondition(d,conditionName$$module$js$graph$world(d,this.rom));for(const e of traverseFill$$module$js$graph$shuffle(a,b)){const [f,...g]=e;{d=a.slots[f].condition;
const e=g.map((b)=>a.items[b].condition);c.addCheck(d,e,b.slots[a.slots[f].item])}}}}}
...
class BossDropSlot$$module$js$rom$slots{constructor(a,b){this.slot=a;this.boss=b}set(a,b){const c=readLittleEndian$$module$js$rom$util(a.prg,129415+2*this.boss)+81920;if(112<=b)throw Error("no mimics on bosses");a.prg[c+4]=b;a.spoiler&&a.spoiler.addSlot(this.slot,a.bosses.fromBossKill(this.boss).name,b)}}
...
class Slot$$module$js$rom$spoiler{constructor(a,b,c,d){this.slot=b;this.slotName=c;this.item=d;this.itemName=slotToItem$$module$js$rom$spoiler(a,d);this.originalItem=slotToItem$$module$js$rom$spoiler(a,b)}toString(){return`${this.itemName}: ${this.slotName} (${this.originalItem})`}}function slotToItem$$module$js$rom$spoiler(a,b){return 112<=b?"Mimic":a.items[a.itemGets[b].itemId].messageName}var module$js$rom$spoiler={};module$js$rom$spoiler.Spoiler=Spoiler$$module$js$rom$spoiler;const STATUS$$module$js$version="unstable",VERSION$$module$js$version="latest",LABEL$$module$js$version="latest 2e22824",HASH$$module$js$version="2e22824cf9f6612db9e8f6705a5d388544fdbb5d",DATE$$module$js$version=new Date(1561172594E3);var module$js$version={};module$js$version.STATUS=STATUS$$module$js$version;module$js$version.VERSION=VERSION$$module$js$version;module$js$version.LABEL=LABEL$$module$js$version;module$js$version.HASH=HASH$$module$js$version;module$js$version.DATE=DATE$$module$js$version;var $jscompDefaultExport$$module$js$patch={apply(a,b,c){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){let d;b.seed||(b.seed=parseSeed$$module$js$patch("").toString(16),window.location.hash+="&seed="+b.seed);d=b.flags?new FlagSet$$module$js$flagset(String(b.flags)):new FlagSet$$module$js$flagset("Em Gt Mr Rlpt Sbk Sct Sm Tasd");for(const a in b)"false"===b[a]&&(b[a]=!1);yield shuffle$$module$js$patch(a,parseSeed$$module$js$patch(String(b.seed)),d,new FetchReader$$module$js$fetchreader(c))})}};
