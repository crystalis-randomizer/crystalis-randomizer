...
new Set([77]),ALL$$module$js$rom$constraint,ALL$$module$js$rom$constraint],[],0)}static get GUARDIAN_STATUE(){return new Constraint$$module$js$rom$constraint([new Set([116]),new Set([98]),ALL$$module$js$rom$constraint,ALL$$module$js$rom$constraint],[],0)}static get SHOOTING_WALL(){return new Constraint$$module$js$rom$constraint([new Set([97]),ALL$$module$js$rom$constraint,ALL$$module$js$rom$constraint],[],0)}static get KENSU_CHEST(){return new Constraint$$module$js$rom$constraint([new Set([81]),ALL$$module$js$rom$constraint,
ALL$$module$js$rom$constraint,ALL$$module$js$rom$constraint],[],0)}static forLocation(a){switch(a){case 3:return new Constraint$$module$js$rom$constraint([ALL$$module$js$rom$constraint,bit$$module$js$rom$constraint(96),ALL$$module$js$rom$constraint,bit$$module$js$rom$constraint(32)],[],0);case 96:case 100:case 104:return new Constraint$$module$js$rom$constraint([ALL$$module$js$rom$constraint,bit$$module$js$rom$constraint(82),ALL$$module$js$rom$constraint,bit$$module$js$rom$constraint(8)],[],0)}return Constraint$$module$js$rom$constraint.ALL}static fromSpawn(a,
b,c,d,e){const [f,...g]=b;(e=e&&2===f&&!g.length)&&d.patternBank&&(b=new Set([3]));const h=e||!b.has(2)?ALL$$module$js$rom$constraint:bit$$module$js$rom$constraint(c.spritePatterns[0]);b=e||!b.has(3)?ALL$$module$js$rom$constraint:bit$$module$js$rom$constraint(c.spritePatterns[1]);d=e?[bit$$module$js$rom$constraint(c.spritePatterns[d.patternBank])]:[];e=a.has(2)?bit$$module$js$rom$constraint(c.spritePalettes[0]):ALL$$module$js$rom$constraint;a=a.has(3)?bit$$module$js$rom$constraint(c.spritePalettes[1]):
ALL$$module$js$rom$constraint;return new Constraint$$module$js$rom$constraint([h,b,e,a],d,0)}ignorePalette(){return new Constraint$$module$js$rom$constraint([this.fixed[0],this.fixed[1],ALL$$module$js$rom$constraint,ALL$$module$js$rom$constraint],this.float,this.shift)}shufflePalette(a,b){const c=[...this.fixed];for(let d=2;4>d;d++){if(c[d]===ALL$$module$js$rom$constraint)continue;const e=Math.floor(5-Math.log2(a.nextInt(15)+2)),f=c[d]=new Set;for(let c=0;c<e;c++)f.add(a.pick(b))}return new Constraint$$module$js$rom$constraint(c,
this.float,this.shift)}shifted(){return new Constraint$$module$js$rom$constraint(this.fixed,this.float,this.shift|2)}join(a){const b=seq$$module$js$rom$util(4,(b)=>CSet$$module$js$rom$constraint.union(this.fixed[b],a.fixed[b]));if(this.float.length!=a.float.length)throw console.dir(this),console.dir(a),Error(`incompatible float: ${this.float} ${a.float}`);const c=seq$$module$js$rom$util(this.float.length,(b)=>CSet$$module$js$rom$constraint.union(this.float[b],a.float[b]));return new Constraint$$module$js$rom$constraint(b,
c,this.shift|a.shift)}fix(a,b){var c=b?(a)=>b.nextInt(a):()=>0;const d=b?(a)=>b.pick([...a]):(a)=>a[Symbol.iterator]().next().value,e=[...this.fixed];if(this.float.length){c=c(2);const a=1-c;c<this.float.length&&(e[0]=CSet$$module$js$rom$constraint.intersect(e[0],this.float[c]));a<this.float.length&&(e[1]=CSet$$module$js$rom$constraint.intersect(e[1],this.float[a]))}e[0]!==ALL$$module$js$rom$constraint&&(a.spritePatterns[0]=d([...e[0]]));e[1]!==ALL$$module$js$rom$constraint&&(a.spritePatterns[1]=
d([...e[1]]));e[2]!==ALL$$module$js$rom$constraint&&(a.spritePalettes[0]=d([...e[2]]));e[3]!==ALL$$module$js$rom$constraint&&(a.spritePalettes[1]=d([...e[3]]))}meet(a,b=!1){a=this.tryMeet(a,b);if(!a)throw Error("Could not reconcile patterns");return a}tryMeet(a,b=!1){const c=[];let d=this.shift|a.shift;for(var e=0;4>e;e++){var f=CSet$$module$js$rom$constraint.intersect(this.fixed[e],a.fixed[e]);if(!f.size){if(!b||2>e)return;f=CSet$$module$js$rom$constraint.union(this.fixed[e],a.fixed[e])}c.push(f)}e=
new Map;b=[];for(var g of module$js$util.iters.concat(this.float,a.float)){if(g===ALL$$module$js$rom$constraint)throw Error("Unexpected unconstrained float");a=!1;for(var h of g)if(f=e.get(h),null!=f){for(var k of b[f])e.delete(k);b[f]=CSet$$module$js$rom$constraint.intersect(b[f],g);for(const a of b[f])e.set(a,f);a=!0;break}if(a)break;for(const a of g)e.set(a,b.length);b.push(g);if(2<b.length)return}for(g=0;g<b.length;g++)for(h=0;2>h;h++)if(!CSet$$module$js$rom$constraint.intersect(b[g],c[h]).size){k=
c[1-h]=CSet$$module$js$rom$constraint.intersect(b[g],c[1-h]);d|=1<<1-h;if(!k.size)return;b.splice(g,1);g=-1;break}return new Constraint$$module$js$rom$constraint(c,b,d)}}const TREASURE_CHEST_BANKS$$module$js$rom$constraint=new Set([94,95,96,97,100,101,102,103,104,105,106,108,109,110,111,112,116,117,118,119]),COIN_BANKS$$module$js$rom$constraint=new Set([94,95,96,97,99,100,101,102,103,104,105,106,107,108,109,110,111,112,116,117,118,119]);var module$js$rom$constraint={};
module$js$rom$constraint.Constraint=Constraint$$module$js$rom$constraint;class ObjectData$$module$js$rom$objectdata extends Entity$$module$js$rom$entity{constructor(a,b){super(a,b);this.used=!0;this.name="";this.pointer=109568+(b<<1);this.base=readLittleEndian$$module$js$rom$util(a.prg,this.pointer)+65536;this.sfx=a.prg[this.base];this.data=[];b=this.base+1;let c=0;for(let d=0;32>d;d++)d&7||(c=a.prg[b++]),this.data.push(c&128?a.prg[b++]:0),c<<=1}serialize(){const a=[this.sfx];for(let b=0;4>b;b++){const c=a.length;a.push(0);for(let d=0;8>d;d++)this.data[8*b+d]&&(a[c]|=
