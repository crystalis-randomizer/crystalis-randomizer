...
this.fixed[1],ALL$$module$js$rom$constraint,ALL$$module$js$rom$constraint],this.float,this.shift)}shifted(){return new Constraint$$module$js$rom$constraint(this.fixed,this.float,this.shift|2)}join(a){const b=seq$$module$js$rom$util(4,(b)=>CSet$$module$js$rom$constraint.union(this.fixed[b],a.fixed[b]));if(this.float.length!=a.float.length)throw console.dir(this),console.dir(a),Error(`incompatible float: ${this.float} ${a.float}`);const c=seq$$module$js$rom$util(this.float.length,(b)=>CSet$$module$js$rom$constraint.union(this.float[b],
a.float[b]));return new Constraint$$module$js$rom$constraint(b,c,this.shift|a.shift)}fix(a){const b=a?(b)=>a.nextInt(b):()=>0,c=[...this.fixed];if(this.float.length){const a=b(2),e=1-a;a<this.float.length&&(c[0]=CSet$$module$js$rom$constraint.intersect(c[0],this.float[a]));e<this.float.length&&(c[1]=CSet$$module$js$rom$constraint.intersect(c[1],this.float[e]))}return c.map((a)=>{if(a===ALL$$module$js$rom$constraint)return 255;a=[...a];return a[b(a.length)]})}meet(a){const b=[];let c=this.shift|a.shift;
for(var d=0;4>d;d++){var e=CSet$$module$js$rom$constraint.intersect(this.fixed[d],a.fixed[d]);if(!e.size)return;b.push(e)}e=new Map;d=[];for(var f of module$js$util.iters.concat(this.float,a.float)){if(f===ALL$$module$js$rom$constraint)throw Error("Unexpected unconstrained float");a=!1;for(var g of f){const b=e.get(g);if(null!=b){for(var h of d[b])e.delete(h);d[b]=CSet$$module$js$rom$constraint.intersect(d[b],f);for(const a of d[b])e.set(a,b);a=!0;break}}if(a)break;for(const a of f)e.set(a,d.length);
d.push(f);if(2<d.length)return}for(f=0;f<d.length;f++)for(g=0;2>g;g++)if(!CSet$$module$js$rom$constraint.intersect(d[f],b[g]).size){h=b[1-g]=CSet$$module$js$rom$constraint.intersect(d[f],b[1-g]);c|=1<<1-g;if(!h.size)return;d.splice(f,1);f=-1;break}return new Constraint$$module$js$rom$constraint(b,d,c)}}
...
77,23],dynaCounter:["Dyna Counter",184,47],dynaLaser:["Dyna Laser",185,47],dynaBubble:["Dyna Bubble",186,47],vampire2Bat:["Vampire 2 Bat",188,28],brownRobotLaserSource:["Brown Robot Laser Source",190,47],draygon2Fireball:["Draygon 2 Fireball",191,47],vampire1Bat:["Vampire 1 Bat",193,5],giantInsectFireball:["Giant Insect Fireball",195,11],greenMosquito:["Green Monsquito",196,11],kelbesque1Rock:["Kelbesque 1 Rock",197,15],sabera1Balls:["Sabera 1 Balls",198,29],kelbesque2Fire:["Kelbesque 2 Fire",199,
41],sabera2Fire:["Sabera 2 Fire",200,41],sabera2Balls:["Sabera 2 Balls",201,41],karmineBalls:["Karmine Balls",202,41],statueBalls:["Statue Balls",203,47],draygon1Lightning:["Draygon 1 Lightning",204,45],draygon2Laser:["Draygon 2 Laser",205,47],draygon2Breath:["Draygon 2 Breath",206,47],birdBomb:["Bird Bomb",224,33],greenMosquitoShot:["Green Mosquito Shot",226,11],paralysisBeam:["Paralysis Beam",227,25],stoneGaze:["Stone Gaze",228,19],rockGolemRock:["Rock Golem Rock",229,4],curseBeam:["Curse Beam",
230,41],mpDrainWeb:["MP Drain Web",231,41],fishmanTrident:["Fishman Triden",232,25],orcAxe:["Orc Axe",233,6],swampPollen:["Swamp Pollen",234,10],paralysisPowder:["Paralysis Powder",235,23],soldierSword:["Soldier Sword",236,14],iceGolemRock:["Ice Golem Rock",237,14],trollAxe:["Troll Axe",238,18],krakenInk:["Kraken Ink",239,25],archerArrow:["Archer Arrow",240,33],knightSword:["Knight Sword",242,41],mothResidue:["Moth Residue",243,28],brownRobotLaser:["Brown Robot Laser",244,47],whiteRobotLaser:["White Robot Laser",
...
d&&(b=d)}this.monsterConstraints.set(c.id,b);c.constraint=b}}getMonsterConstraint(a,b){const c=this.monsterConstraints.get(b)||Constraint$$module$js$rom$constraint.NONE;return 88!==(a&88)&&this.rom.objects[b].goldDrop?c.meet(Constraint$$module$js$rom$constraint.COIN)||Constraint$$module$js$rom$constraint.NONE:c}configure(a,b){const c=b.isMonster()?this.monsterConstraints.get(b.monsterId):b.isNpc()?this.npcConstraints.get(b.id):b.isChest()?112>b.id?Constraint$$module$js$rom$constraint.TREASURE_CHEST:
...
function*allObjects$$module$js$rom$graphics(a,b){yield b;var c=b.spawnedReplacement();c&&(yield*allObjects$$module$js$rom$graphics(a,c));(c=b.spawnedChild())&&(yield*allObjects$$module$js$rom$graphics(a,c));80===b.id&&(yield a.objects[95]);83===b.id&&(yield a.objects[105])}
...
return c}var module$js$rom$graphics={};module$js$rom$graphics.Graphics=Graphics$$module$js$rom$graphics;var $jscompDefaultExport$$module$js$patch={apply(a,b,c){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){let d;b.seed||(b.seed=parseSeed$$module$js$patch("").toString(16),window.location.hash+="&seed="+b.seed);d=b.flags?new FlagSet$$module$js$flagset(String(b.flags)):new FlagSet$$module$js$flagset("Em Gt Mr Rlpt Sbk Sct Sm Tasd");for(const a in b)"false"===b[a]&&(b[a]=!1);yield shuffle$$module$js$patch(a,parseSeed$$module$js$patch(String(b.seed)),d,new FetchReader$$module$js$fetchreader(c))})}};
...
simplifyInvisibleChests$$module$js$patch(l);addCordelWestTriggers$$module$js$patch(l,c);c.disableRabbitSkip()&&fixRabbitSkip$$module$js$patch(l);c.shuffleShops()&&shuffleShops$$module$js$patch(l,c,h);c.randomizeWildWarp()&&shuffleWildWarp$$module$js$patch(l,c,h);rescaleMonsters$$module$js$patch(l,c,h);const n=World$$module$js$graph$world.build(l,c),p=yield(new AssumedFill$$module$js$graph$shuffle(l,c)).shuffle(n.graph,h,f);if(p)n.traverse(n.graph,p),update$$module$js$rom$slots(l,p.slots);else return-1;
rescaleShops$$module$js$patch(l,m,c.bargainHunting()?h:void 0);normalizeSwords$$module$js$patch(l,c,h);c.shuffleMonsters()&&shuffleMonsters$$module$js$patch(l,c,h);identifyKeyItemsForDifficultyBuffs$$module$js$patch(l);c.doubleBuffMedicalHerb()?(a[115996]*=2,a[115962]*=3):c.buffMedicalHerb()&&(a[115996]+=16,a[115962]*=2);c.storyMode()&&storyMode$$module$js$patch(l);c.chargeShotsOnly()&&disableStabs$$module$js$patch(l);c.orbsOptional()&&orbsOptional$$module$js$patch(l);shuffleMusic$$module$js$patch(l,
...
-28]);d(52)[1].condition=27;d(56)[3].message.action=3;d(56)[4].flags.push(156);e(56,223)[1]=-28;e(56,225)[0]=27;d(56)[1].condition=27;e(57,216)[1]=-28;a.npcs[68].spawnConditions.set(233,[-142]);a.npcs[68].spawnConditions.set(228,[141]);a.npcs[94].localDialogs.set(16,[LocalDialog$$module$js$rom$npc.of(-59,[0,26],[58]),LocalDialog$$module$js$rom$npc.of(13,[0,29]),LocalDialog$$module$js$rom$npc.of(56,[0,28]),LocalDialog$$module$js$rom$npc.of(57,[0,29]),LocalDialog$$module$js$rom$npc.of(10,[0,27,3]),
LocalDialog$$module$js$rom$npc.of(-1,[0,29])]);c(e(94,16),-82);a.npcs[95].spawnConditions.delete(33);a.npcs[96].spawnConditions.delete(30);g=a.npcs[98];g.data[1]=40;d(g.id,225)[0].message.action=17;d(g.id,225)[2].message.action=17;c(e(g.id,225),-144);a.npcs[104].spawnConditions.set(97,[-156,33]);d(104)[0].message.action=2;d(195)[0].condition=514;a.npcs[196].spawnConditions.delete(242);a.npcs[203].spawnConditions.set(166,[-654]);g=a.npcs[94].localDialogs.get(242);g.unshift(...g.splice(1,1));a.trigger(128).conditions=
[-40,59,515];a.trigger(129).conditions=[];b.barrierRequiresCalmSea()&&a.trigger(132).conditions.push(643);a.trigger(140).conditions.push(58);a.trigger(186).conditions[0]=-581;a.trigger(187).conditions[1]=-28;({zombieTown:b}=a.locations);b.spawns=b.spawns.filter((a)=>!a.isTrigger()||138!=a.id);for(const b of a.npcs)for(const a of b.allDialogs())14===a.condition&&(a.condition=579),-15===a.condition&&(a.condition=-580)}
function fixMimics$$module$js$patch(a){let b=112;for(const c of a.locations)for(const a of c.spawns)a.isChest()&&(a.timed=!1,112<=a.id&&(a.id=b++))}
...
for(this.report["post-shuffle monsters"]=this.monsters.map((a)=>a.id);this.locations.length;){const {location:e,slots:f}=this.locations.pop(),g=this.report["$"+e.id.toString(16).padStart(2,"0")]=[],{maxFlyers:h=0,nonFlyers:k={},tower:m=!1}=MONSTER_ADJUSTMENTS$$module$js$patch[e.id]||{};if(m)continue;let l=h,n=Constraint$$module$js$rom$constraint.forLocation(e.id);null!=e.bossId()&&(n=n.meet(Constraint$$module$js$rom$constraint.BOSS)||n);for(const a of e.spawns)if(!a.isChest()||a.data[2]&32)if(a.isNpc()){var c=
b.npcConstraints.get(a.id);c&&(n=n.meet(c)||n)}else a.isMonster()&&UNTOUCHED_MONSTERS$$module$js$patch[a.monsterId]&&(c=b.monsterConstraints.get(a.monsterId))&&(n=n.meet(c)||n);else n=112>a.id?n.meet(Constraint$$module$js$rom$constraint.TREASURE_CHEST)||n:n.meet(Constraint$$module$js$rom$constraint.MIMIC)||n;g.push(`Initial pass: ${n.fixed.map((a)=>Infinity>a.size?"["+[...a].join(", ")+"]":"all")}`);c=(a)=>{var c=FLYERS$$module$js$patch.has(a.id),d=MOTHS_AND_BATS$$module$js$patch.has(a.id);if(c){if(!l)return!1;
--l}var h=b.getMonsterConstraint(e.id,a.id);h=n.meet(h);if(!h)return!1;g.push(`  Adding ${a.id.toString(16)}: ${h}`);n=h;h=0;if(c||d)for(c=0;c<f.length;c++){if(f[c]in k){h=c;break}}else for(c=0;c<f.length;c++)if(!(f[c]in k)){h=c;break}(this.report[`mon-${a.id.toString(16)}`]=this.report[`mon-${a.id.toString(16)}`]||[]).push("$"+e.id.toString(16));c=f[h];d=e.spawns[c-13];c in k&&(d.y+=16*k[c][0],d.x+=16*k[c][1]);d.monsterId=a.id;g.push(`    slot ${c.toString(16)}: ${d}`);f.splice(h,1);return!0};if(l)for(var d=
0;d<Math.min(40,this.monsters.length);d++)FLYERS$$module$js$patch.has(this.monsters[d].id)&&c(this.monsters[d])&&this.monsters.splice(d,1);for(d=0;d<this.monsters.length&&f.length;d++)if(c(this.monsters[d])){const [a]=this.monsters.splice(d,1);FLYERS$$module$js$patch.has(a.id)||this.used.push(a);d--}for(d=0;d<this.used.length&&f.length;d++)c(this.used[d])&&(this.used.push(...this.used.splice(d,1)),d--);[e.spritePatterns[0],e.spritePatterns[1],e.spritePalettes[0],e.spritePalettes[1]]=n.fix(a);if(f.length){console.error(`Failed to fill location ${e.id.toString(16)}: ${f.length} remaining`);
