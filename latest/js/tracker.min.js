...
a=[...a];return a[b(a.length)]})}meet(a){const b=[];let c=this.shift;for(var d=0;4>d;d++){var e=CSet$$module$js$rom$constraint.intersect(this.fixed[d],a.fixed[d]);if(!e.size)return;b.push(e)}e=new Map;d=[];for(var f of module$js$util.iters.concat(this.float,a.float)){if(f===ALL$$module$js$rom$constraint)throw Error("Unexpected unconstrained float");a=!1;for(var g of f){const b=e.get(g);if(null!=b){d[b]=CSet$$module$js$rom$constraint.intersect(d[b],f);a=!0;break}}if(a)break;d.push(f);if(2<d.length)return}for(f=
0;f<d.length;f++)for(g=0;2>g;g++)if(!CSet$$module$js$rom$constraint.intersect(d[f],b[g]).size){a=b[1-g]=CSet$$module$js$rom$constraint.intersect(d[f],b[1-g]);c|=1<<1-g;if(!a.size)return;d.splice(f,1);f=-1;break}return new Constraint$$module$js$rom$constraint(b,d,c)}}const TREASURE_CHEST_BANKS$$module$js$rom$constraint=new Set([94,95,96,97,100,101,102,103,104,105,106,108,109,110,111,112,116,117,118,119]);var module$js$rom$constraint={};module$js$rom$constraint.Constraint=Constraint$$module$js$rom$constraint;class ObjectData$$module$js$rom$objectdata extends Entity$$module$js$rom$entity{constructor(a,b){super(a,b);this.used=!0;this.name="";this.pointer=109568+(b<<1);this.base=readLittleEndian$$module$js$rom$util(a.prg,this.pointer)+65536;this.sfx=a.prg[this.base];this.data=[];b=this.base+1;let c=0;for(let d=0;32>d;d++)d&7||(c=a.prg[b++]),this.data.push(c&128?a.prg[b++]:0),c<<=1}serialize(){const a=[this.sfx];for(let b=0;4>b;b++){const c=a.length;a.push(0);for(let d=0;8>d;d++)this.data[8*b+d]&&(a[c]|=
...
--l}var h=b.monsterConstraints.get(a.id);h=h&&n.meet(h);if(!h)return!1;g.push(`  Adding ${a.id.toString(16)}: ${h}`);n=h;h=0;if(c||d)for(c=0;c<f.length;c++){if(f[c]in k){h=c;break}}else for(c=0;c<f.length;c++)if(!(f[c]in k)){h=c;break}(this.report[`mon-${a.id.toString(16)}`]=this.report[`mon-${a.id.toString(16)}`]||[]).push("$"+e.id.toString(16));c=f[h];d=e.spawns[c-13];c in k&&(d.y+=16*k[c][0],d.x+=16*k[c][1]);d.monsterId=a.id;g.push(`    slot ${c.toString(16)}: ${d}`);f.splice(h,1);return!0};if(l)for(var d=
0;d<Math.min(40,this.monsters.length);d++)FLYERS$$module$js$patch.has(this.monsters[d].id)&&c(this.monsters[d])&&this.monsters.splice(d,1),a.shuffle(this.monsters);for(d=0;d<this.monsters.length&&f.length;d++)if(c(this.monsters[d])){const [a]=this.monsters.splice(d,1);FLYERS$$module$js$patch.has(a.id)||this.used.push(a);d--}for(d=0;d<this.used.length&&f.length;d++)c(this.used[d])&&(this.used.push(...this.used.splice(d,1)),d--);[e.spritePatterns[0],e.spritePatterns[1],e.spritePalettes[0],e.spritePalettes[1]]=
