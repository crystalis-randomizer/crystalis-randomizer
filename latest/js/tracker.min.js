...
Ce=(new Trigger$$module$js$nodes(a,"Talked to Akahana's Friend")).get(ge),De=(new Trigger$$module$js$nodes(a,"Get Bow of Truth")).get(rc),Ee=(new Trigger$$module$js$nodes(a,"Forged Crystalis")).get(pb),$a=(new Condition$$module$js$nodes(a,"Sword charge glitch")).option(C,U),Fe=(new Condition$$module$js$nodes(a,"Rabbit skip")).option(r,I),Ge=(new Condition$$module$js$nodes(a,"Teleport skip")).option(p,l),ab=(new Condition$$module$js$nodes(a,"Statue glitch")).option(m,q),He=(new Condition$$module$js$nodes(a,
...
h.slots){var n=h.slotType[0];(k[n]=k[n]||[]).push(h)}h=[];n=[];var m={};for(var q of d.flags.S||[]){var p=[];for(var l of q){m[l]=!0;for(var r of k[l])p.push(r),n.push([r,h.length])}h.push(p)}for(var I of"bc")if(m[I])for(q=k[I],l=q.map((a)=>[a.item,a.itemIndex]),a.shuffle(l),r=0;r<q.length;r++)q[r].set(...l[r]);p=(a)=>g.find((b)=>b.vanillaItemName===a);k=p("Sword of Wind");I=p("Sword of Fire");q=p("Ball of Wind");l=p("Ball of Fire");r=p("Statue of Onyx");p=p("Gas Mask");m.k&&a.nextInt(2)&&(I.swap(k),
...
b;if(b.isMimic()&&!a.canHoldMimic()||"unique"!=b.item.inventoryRow&&a.requiresUnique||"armor"==b.item.inventoryRow&&!a.isChest())return!1;if(f.guaranteeSword()){if(0===a.slotIndex)return 4>b.item.id;if(4>b.item.id&&(r.add(b),4===r.size))return 0===a.slotIndex}return p[a.slotType?a.slotType[0]:"c"]==p[b.slotType?b.slotType[0]:"c"]};l=a.assumedFill(d,(a,b)=>I(a,q.get(b)));if(null==l){g&&g.endState.key++;return!1;throw Error("Could not fill!");}const C=new Map;for(var U=0;U<l.length;U++){var va=l[U];
if(null!=va){var oa=q.get(a.item(va));va=n.get(oa);n.delete(oa);oa=a.location(U);C.set(oa,va);m.delete(oa)}}U=(a,b,c)=>{c=void 0===c?"":c;for(const c of m)if(I(c,a))return C.set(c,b),m.delete(c),n.delete(a),!0;g&&("traps"==c?g.endState.traps++:"chest"==c?g.endState.chest++:g.endState.misc++);return!1};for(var E of n){const [a,b]=E;if(a.isMimic()&&!U(a,b,"traps"))return!1}for(const a of n){const [b,c]=a;if(b.needsChest()&&!U(b,c,"chest"))return!1}for(const a of n){const [b,c]=a;if(!U(b,c))return!1}for(const a of C){const [b,
...
return Math.min(AM$$module$js$random*this.iy,RNMX$$module$js$random)}nextInt(b){return Math.floor(this.next()*b)}shuffle(b){for(let a=b.length;a;){const c=this.nextInt(a--);[b[a],b[c]]=[b[c],b[a]]}return b}}var module$js$random={};module$js$random.Random=Random$$module$js$random;const STATUS$$module$js$version="unstable",VERSION$$module$js$version="latest",LABEL$$module$js$version="latest 151080c",HASH$$module$js$version="151080c662a4454f927e0c32dae5d6dd654375b3",DATE$$module$js$version=new Date(1555287158E3);var module$js$version={};module$js$version.STATUS=STATUS$$module$js$version;module$js$version.VERSION=VERSION$$module$js$version;module$js$version.LABEL=LABEL$$module$js$version;module$js$version.HASH=HASH$$module$js$version;module$js$version.DATE=DATE$$module$js$version;var $jscompDefaultExport$$module$js$patch={apply(b,a,c){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){let d;a.seed||(a.seed=parseSeed$$module$js$patch("").toString(16),window.location.hash+="&seed="+a.seed);d=a.flags?new FlagSet$$module$js$flagset(a.flags):new FlagSet$$module$js$flagset("Em Gt Mr Rlpt Sbk Sct Sm Tasd");for(const b in a)"false"===a[b]&&(a[b]=!1);yield shuffle$$module$js$patch(b,parseSeed$$module$js$patch(a.seed),d,new FetchReader$$module$js$fetchreader(c))})}};
