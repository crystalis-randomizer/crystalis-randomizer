...
InjuredDolphin:gb,LeadingChild:Sb,LeatherBoots:Tb,Money:Oa,OpenedCrypt:bd,RabbitBoots:kc,Refresh:qb,RepairedStatue:Dc,RescuedChild:Cb,ShellFlute:lc,ShieldRing:Ec,ShootingStatue:mc,StormBracelet:Fc,Sword:cd,SwordOfFire:bb,SwordOfThunder:nc,SwordOfWater:oc,SwordOfWind:pc,TornadoBracelet:Gc,TravelSwamp:Hc,WildWarp:dd},items:{MedicalHerb:Ub,WarpBoots:I}}=this.rom,ba=this.entrance(b);var Ca=this.entrance(c);this.addCheck([ba],Od(z,B),[bd.id]);this.addCheck([ba],Od(e,lc),[Rb.id]);this.addCheck([Ca],Od(Sb),
[Cb.id]);this.addItemCheck([ba],Od(Ra,ja),Dc.id,{lossy:!0,unique:!0});for(var Ja of this.rom.shops)if(Ja.location!==a.id&&Ja.location!==d.id&&Ja.used&&Ja.type===Wc.TOOL){Ca=[fc(Ja.location<<16|136)];for(var rb of Ja.contents)rb===Ub.id?this.addCheck(Ca,Oa.r,[O.id]):rb===I.id&&this.addCheck(Ca,Oa.r,[T.id])}Ja=pc.r;rb=bb.r;Ca=oc.r;let qc=nc.r;if(!this.flagset.orbsOptional()){var Vb=Pd(l,Gc);const a=Pd(f,fb),b=Pd(h,p),c=Pd(g,Fc);Ja=D.meet(Ja,Vb);rb=D.meet(rb,a);Ca=D.meet(Ca,b);qc=D.meet(qc,c);if(this.flagset.assumeSwordChargeGlitch()){Vb=
function(b){return a.map(a=>a[0]===b.c?a:[b.c,...a])};const a=D.or(Ja,rb,Ca,qc);Ja=Vb(pc);rb=Vb(bb);Ca=Vb(oc);qc=Vb(nc)}}this.addCheck([ba],Ja,[A.id]);this.addCheck([ba],rb,[w.id]);this.addCheck([ba],Ca,[pb.id]);this.addCheck([ba],qc,[S.id]);this.addCheck([ba],Pd(pc,bb,oc,nc),[cd.id]);this.addCheck([ba],ab.r,[X.id]);this.addCheck([ba],Pd(ab,kc),[oa.id]);this.addCheck([ba],Pd(ab,kc),[xb.id]);this.addCheck([ba],q.r,[mc.id]);this.addCheck([ba],Bb.r,[Hc.id]);this.flagset.leatherBootsGiveSpeed()&&this.addCheck([ba],
Tb.r,[oa.id]);this.flagset.assumeGhettoFlight()&&this.addCheck([ba],Od(Rb,kc),[X.id]);this.flagset.fogLampNotRequired()&&(Ja=this.flagset.requireHealedDolphinToRide(),this.addCheck([ba],Ja?gb.r:[[]],[e.id]));this.flagset.guaranteeBarrier()||this.addCheck([ba],[[Oa.c,O.c],[Oa.c,Ec.c],[Oa.c,qb.c]],[mc.id]);this.flagset.assumeFlightStatueSkip()||this.addCheck([ba],[[Oa.c,ab.c]],[mc.id]);this.flagset.guaranteeGasMask()||this.addCheck([ba],[[Oa.c,O.c],[Oa.c,qb.c]],[Hc.id]);this.flagset.assumeWildWarp()&&
this.addCheck([ba],D.OPEN,[dd.id])}addExtraRoutes(){var a;const {flags:{BuyWarp:b,SwordOfThunder:c,Teleport:d,WildWarp:e},locations:{MezameShrine:f}}=this.rom;this.addRoute(new jc(this.entrance(f),[]));if(this.flagset.teleportOnThunderSword()){var g=this.rom.townWarp.thunderSwordWarp;this.addRoute(new jc(this.entrance(g[0],g[1]&31),[c.c,b.c]));this.addRoute(new jc(this.entrance(g[0],g[1]&31),[c.c,d.c]))}if(this.flagset.assumeWildWarp())for(const b of this.rom.wildWarp.locations){if(b===this.rom.locations.UndergroundChannel.id)continue;
g=this.entrance(b);const c=null!==(a=this.terrains.get(g))&&void 0!==a?a:aa("bad entrance");for(const a of c.enter)this.addRoute(new jc(g,[e.c,...a]))}}consolidateChecks(){for(const [a,b]of this.checks){const c=this.tiles.find(a);if(a!==c){for(const a of b)this.checks.get(c).add(a);this.checks.delete(a)}}}buildRequirementMap(){for(const [a,b]of this.checks)for(const {checks:c,requirement:d}of b)for(const b of c){const c=this.requirementMap.get(b);for(const b of d)for(const d of this.routes.get(a)||
...
4&61440|b&3840|a&240|b>>4&15)};for(var X of z)if(X.data.match(ja,null!=p)){w=X;break}}w||(w=O[0])}else throw Error("impossible");if(!w)throw Error("impossible");S[A]=w}T=new na;for(const b of a.exits)if(255!==b.dest){X=b.screen;A=b.tile;!b.isSeamless()||b.yt&15||88===(a.id&88)||(X-=16,A|=240);if(!q.has(X))throw Error("impossible?");w=S[X];p=w.findExitType(A,1===l,!!(b.entrance&32));A=null===p||void 0===p?void 0:p.type;if(!A)Yd.has(a.id<<16|X<<8|b.tile)||(A=null===(d=w.data.exits)||void 0===d?void 0:
d.map(a=>a.type+": "+a.exits.map(x).join(", ")).join("\n  "),console.warn(`Unknown exit ${x(b.tile)}: ${w.name} in ${a} @ ${x(X)}:\n  ${A}`));else if(!T.has(X,A))if(w=g.locations[b.dest],A.startsWith("seamless"))z="seamless:down"===A,T.set(X,A,[w.id<<8|X+(0>p.exits[0]+(z?-16:16)?-16:0),z?"seamless:up":"seamless:down"]);else if(O=w.entrances[b.entrance&31],p=O.screen,z=O.coord,"door"===A&&0===(O.y&240)&&(p-=16,z+=65536),O=w.screens[p>>4][p&15],ja=c(w,O,z))T.set(X,A,[w.id<<8|p,ja]);else{X=[];for(const a of g.metascreens.getById(O,
w.tileset))for(const b of null!==(e=a.data.exits)&&void 0!==e?e:[])b.type.startsWith("seamless")||X.push(`  ${a.name} ${b.type}: ${x(b.entrance)}`);console.warn(`Bad entrance ${x(z)}: raw ${x(O)} in ${w} @ ${x(p)}\n${X.join("\n")}`)}}d=new Map;for(var oa of a.pits)d.set(oa.fromScreen,oa.dest<<8|oa.toScreen);oa=new Xd(a.id,b,l,h);oa._screens=S;oa._exits=T;oa._pits=d;for(const b of a.flags)a=oa._screens[b.screen],(null===(f=a.flag)||void 0===f?0:f.startsWith("custom"))?oa.customFlags.set(b.screen,g.flags[b.flag]):
...
a)){b=Error;a=[g.name,f,Zd[a],e.name,d];d="bad neighbor %s (%02x) %s %s (%02x)".split(/%/g);e=0;f=d[0];for(g=1;g<d.length;g++){if(!d[g]){f+="%"+d[++g];continue}c=/([-+]*)([0\D]?)(\d*)([dxs])/.exec(d[g]);if(!c){f+=a[e++]+d[g];continue}var h=parseInt(c[3])||0;const b=c[2]||" ",q=a[e++];let p="x"===c[4]?Number(q).toString(16):String(q);"s"!==c[4]&&/\+/.test(c[1])&&0<=Number(q)&&(p="+"+p);p.length<h&&(h=b.repeat(h-p.length),p=/-/.test(c[1])?p+h:h+p);f+=p+d[g].substring(c[0].length)}a=f;throw b(a);}}return!0}spliceColumns(a,
...
new Set;for(var c of this._pits){var [,d]=c;b.add(this.rom.locations[d>>>8].id)}this._pits.clear();c=new m(()=>[]);for(var e of this.allPos())if(this.get(e).hasFeature("pit")){var f=[-1,this,Infinity];for(var g of this._exits){const [a,,[c]]=g;d=$d(e,a);b.has(c>>>8)&&d<f[2]&&(f=this.rom.locations[c>>>8].meta,f=[ae(e,c&255,a,f),f,d])}0<=f[0]&&c.get(f[1]).push([e,f[0]])}for(var h of b)b=c.get(this.rom.locations[h].meta),b.length||b.push([0,240]);for(const f of c){const [l,p]=f;h=[[],[]];b=new Map;for(const a of l.allPos())e=
l.get(a),e.hasFeature("river")||e.hasFeature("empty")||(g=(e.data.edges||"").split("").map(a=>" "===a?"":a),g[0]&&g[2]&&h[0].push(a),(g[1]&&g[3]||e.hasFeature("spikes"))&&h[1].push(a),e.hasFeature("spikes")&&b.set(a,[...g].filter(a=>"s"===a).length));g=[0,0];for(const f of p){const [p,q]=f;e=this.get(p).data.edges||"";c="c"===e[1]&&"c"===e[3]?1:0;e=[-1,Infinity,0];g=ae(q,g[0],g[1],l);for(const f of h[c])c=null!==(a=b.get(f))&&void 0!==a?a:0,c<e[2]||(d=$d(g,f),d<e[1]&&(e=[f,d,c]));e=e[0];if(0>e)throw Error("no eligible dest");
g=[e,g];this._pits.set(p,l.id<<8|e)}}}}transferExits(a,b){var c;const d=new m(()=>[]),e=new m(()=>new Set);for(var f of this.allPos()){var g=this._screens[f];for(var h of null!==(c=g.data.exits)&&void 0!==c?c:[])({type:g}=h),"edge:top"===g&&f>>>4||"edge:left"===g&&f&15||"edge:bottom"===g&&f>>>4<this.height-1||"edge:right"===g&&(f&15)<this.width-1||d.get(g).push(f)}for(var l of d.values())b.shuffle(l);for(const g of a._exits){const [p,q,B]=g;if(!e.get(q).has(p)){f=d.get(q).pop();if(null==f)throw Error(`Could not transfer exit ${q} in ${this.rom.locations[this.id]}: no eligible screen\n${this.show()}`);
...
d)}d=a.locations;d.GoaFortress_Kelbesque.spawns[0].x-=16;d.GoaFortress_Zebu.spawns.splice(1,1);d.GoaFortress_Tornel.spawns.splice(2,1);d.GoaFortress_Asina.spawns.splice(2,1);d.GoaFortress_Kensu.spawns.splice(3,1);d.GoaFortress_Kensu.spawns.splice(1,1);Ye(a,b);a.npcs[13].localDialogs.get(53)[0].message.action=23;if(b.requireHealedDolphinToRide()){{const {flags:{InjuredDolphin:b,ShellFlute:c},npcs:{Fisherman:e,FishermanDaughter:f}}=a;e.spawnConditions.set(214,[c.id,b.id]);d=f.localDialogs.get(-1);d.unshift(d[0].clone());
...
a.items[d].messageName=a.items[d].messageName.replace("Ball","Orb");{const {items:{AlarmFlute:b},locations:{WaterfallCave4:c},npcs:{WindmillGuard:l}}=a;a.itemGets[49].inventoryRowStart=32;b.unique=!0;b.basePrice=0;l.data[1]=49;d=[[33,.72],[31,.9]];e=0;for(var f of a.shops)if(f.type===Wc.TOOL)for(let b=0,c=f.contents.length;b<c;b++){if(49!==f.contents[b])continue;const [c,g]=d[e++%d.length];f.contents[b]=c;a.shopDataTablesAddress&&(f.prices[b]=Math.round(f.prices[b]*g))}a.itemGets[91].itemId=29;c.spawn(25).id=
...
WaterfallCave4:X,WaterfallValleyNorth:oa,ZebuCave:xb,ZombieTown_HouseBasement:Rb},items:{GlowingLamp:ab,KeyToPrison:fb,LovePendant:pb,StatueOfOnyx:Bb},npcs:{Akahana:Ra,AkahanaInBrynmaer:gb,Asina:Sb,AztecaInShyron:Tb,Clark:Oa,Draygon:bd,FortuneTeller:kc,Kensu:qb,KensuInCabin:Dc,KensuInSwan:Cb,LeafRabbit:lc,OakChild:Ec,OakElder:mc,OakMother:Fc,PortoaPalaceFrontGuard:cd,PortoaQueen:bb,PortoaThroneRoomBackDoorGuard:nc,Rage:oc,Stom:pc,StonedAkahana:Gc,Tornel:Hc,WindmillGuard:dd,Zebu:Ub},flags:I}=a;qb.localDialogs.delete(O.id);
Cb.link(qb.id);Cb.used=!0;Cb.data=[...qb.data];qb.data[0]=ab.id;ja.spawns.find(a=>a.isNpc()&&a.id===qb.id).id=Cb.id;pb.itemUseData[0].want=256|Cb.id;Gc.linkDialog(Ra.id);gb.used=!0;gb.link(Ra.id);gb.data=[...Ra.data];f.spawns.find(a=>a.isNpc()&&a.id===Ra.id).id=gb.id;Bb.itemUseData[0].want=256|gb.id;lc.dialog()[2].condition=I.RescuedLeafElder.id;lc.dialog()[2].flags.push(I.TalkedToLeafRabbit.id);lc.dialog()[3].flags.push(I.TalkedToLeafRabbit.id);dd.spawns(T)[1]=~I.WindmillGuardAlarmFluteTradein.id;
c(Ra.spawns(X),~I.BehindWhirlpool.id);c(Gc.spawns(X),~I.BehindWhirlpool.id);for(var ba=0;4>ba;ba++){var Ca=mc.dialog()[ba];Ca.condition!==a.flags.OakElder.id&&(Ca.message.action=3)}(()=>{const [a,b,c,d]=Fc.dialog();d.condition=~I.RescuedChild.id;b.condition=-1;Fc.dialog().splice(0,4,d,c,a,b)})();for(const b of[32,33,34,124,125])d(a.npcs[b].dialog());Ec.dialog().unshift(...Ec.dialog().splice(1,1));nc.spawnConditions.set(p.id,[~I.QueenNotInThroneRoom.id,~I.MesiaRecording.id]);cd.dialog()[1].condition=
I.MesiaRecording.id;bb.dialog()[3].condition=I.SwordOfWater.id;bb.dialog()[3].message.action=3;bb.dialog()[4].flags.push(I.PortoaQueenGoingAway.id);bb.spawns(p)[1]=~I.MesiaRecording.id;bb.spawns(B)[0]=I.MesiaRecording.id;bb.dialog()[1].condition=I.MesiaRecording.id;kc.spawns(A)[1]=~I.MesiaRecording.id;Oa.spawnConditions.set(Rb.id,[~I.Clark.id]);Oa.spawnConditions.set(h.id,[I.Clark.id]);Ub.localDialogs.set(xb.id,[Ne.of(~I.TalkedToZebuInCave.id,[0,26],[I.TalkedToZebuInCave.id]),Ne.of(I.LeafVillagersRescued.id,
[0,29]),Ne.of(I.LeafAbduction.id,[0,28]),Ne.of(I.ZebuAtWindmill.id,[0,29]),Ne.of(I.UsedWindmillKey.id,[0,27,3]),Ne.of(-1,[0,29])]);c(Ub.spawns(xb),~I.BehindWhirlpool.id);Hc.spawnConditions.delete(q.id);pc.spawnConditions.delete(S.id);c(Sb.spawns(B),~I.CalmedAngrySea.id);ba=a.npcs[52];ba.spawnConditions.set(p.id,[I.MesiaRecording.id,~I.PortoaQueen.id]);ba.localDialogs.set(z.id,ba.localDialogs.get(-1));ba.data[0]=a.items.FluteOfLime.id;Ca=a.messages.alloc();Ca.text="The queen left this for you.";ba.localDialogs.set(p.id,
[Ne.of(~I.PortoaQueen.id,[Ca.part,Ca.id,3]),Ne.of(-1,[10,14])]);p.spawns.push(Ud.of({yt:3,xt:12,type:1,patternBank:1,id:ba.id}));Dc.spawnConditions.set(e.id,[~I.AbleToRideDolphin.id,I.ReturnedFogLamp.id]);Dc.dialog()[0].message.action=2;Tb.spawns(w).push(~I.ShyronMassacre.id);a.trigger(130).conditions.push(~I.ShyronMassacre.id);oc.dialog()[0].condition=I.SwordOfWater.id;bd.spawnConditions.set(g.id,[~I.Draygon2.id]);Ub.dialog(w).unshift(...Ub.dialog(w).splice(1,1));a.trigger(128).conditions=[~I.ShyronMassacre.id,
...
const Jg={"edge:top":0,"edge:left":1,"edge:bottom":2,"edge:right":3},Fg=["|:","|:=-","|","|="],Gg=new Set(["|",":","-","="]),Kg=new Set("arena portoa1 portoa2 portoa3 lake overpass underpass lighthouse cabin windmill altar pyramid crypt".split(" ")),Lg=new Set("pit spikes bridge wall ramp whirlpool".split(" ")),Hg=[...Kg].map(a=>ug[a]).reduce((a,b)=>a|b),Ig=[...Lg].map(a=>ug[a]).reduce((a,b)=>a|b);class Mg{constructor(a){this.rom=a;this.length=0;this.screensByFix=new m(()=>[]);this.screensById=new m(()=>[]);this.overworldEmpty=this.metascreen({id:0,icon:U`
      ...
      | \u250a |
      |\u2500\u2534\u2500|
      |   |`,tile:" t |ttt|   ",tilesets:{tower:{}},edges:"st t",exits:[yg({left:8}),Dg(8,2)]});this.towerRobotDoor_downStair=this.metascreen({id:57,icon:U`\u252c
      | \u2229 |
      |\u2500\u252c\u2500|
...
[106976,64,192],[106977,61,52],[118533,71,72],[119569,32,160],[119570,48,0],[118777,96,224],[182928,2,0],[193907,2,0],[195300,95,0]];Array.prototype.flatMap||Object.defineProperties(Array.prototype,{flatMap:{value(a){const b=[];let c=0;for(const d of this){let e=a(d,c++);"function"!==typeof e[Symbol.iterator]&&(e=[e]);e=[...e];e.length&&b.push(...e)}return b}}});const Nh=[[/\b(sort)\b/,"sword"],[/\b(sorta)\b/,"sword of"],[/\b(win)\b/,"wind"],[/\bketostix\b/,"key 2 styx"],[/\bketo\b/,"key 2"],[/\b(sticks|stick c|stix e|stick sea|dixie|sticks e|stick see|sexy|60|sixty)\b/,"styx"],[/\b(arraylist|gorilla[sz]|aurelius|h?airless|a realist|arl[eiy]ss?)\b/,"aryllis"],[/\b(amazon us|amazon[ae]?ss?|amazon)\b/,"amazones"],[/\bamazones basement\b/,"aryllis basement"],[/\b(deal|dil|diehl)\b/,"deo"],[/\b(ds|deals|diaz|delos|drose|theos)\b/,"deos"],[/\bone\b/,"1"],[/\b(two|ii|to|too)\b/,
