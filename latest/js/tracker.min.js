...
vb=new Trigger$$module$js$nodes(a,"Entered Shyron"),je=(new Trigger$$module$js$nodes(a,"Talked to Zebu in Shyron")).get(Xb),Cc=new Trigger$$module$js$nodes(a,"Shyron Massacre"),ke=(new Trigger$$module$js$nodes(a,"Saved Kensu")).get(A),le=(new Trigger$$module$js$nodes(a,"Talked to Deo")).get(Td),me=(new Trigger$$module$js$nodes(a,"Talked to Akahana's Friend")).get(Rd),ne=(new Trigger$$module$js$nodes(a,"Get Bow of Truth")).get(fc),oe=(new Trigger$$module$js$nodes(a,"Forged Crystalis")).get(hb),Sa=
(new Condition$$module$js$nodes(a,"Sword charge glitch")).option(Kd),pe=(new Condition$$module$js$nodes(a,"Rabbit skip")).option(p),qe=(new Condition$$module$js$nodes(a,"Teleport skip")).option(l),wb=(new Condition$$module$js$nodes(a,"Statue glitch")).option(t),re=(new Condition$$module$js$nodes(a,"Wild warp")).option(Id,Jd),Ta=(new Condition$$module$js$nodes(a,"Any level 2 sword")).option(Q,Ha).option(Q,za).option(R,Ia).option(R,Ja).option(L,Ka).option(L,La).option(I,Ma).option(I,Na),v=(new Condition$$module$js$nodes(a,
"Destroy stone")).option(Q,Ha).option(Q,za).option(Sa,Q,Ta),w=(new Condition$$module$js$nodes(a,"Destroy ice")).option(R,Ia).option(R,Ja).option(Sa,R,Ta),q=(new Condition$$module$js$nodes(a,"Cross rivers")).option(L,Ka).option(L,La).option(A).option(Sa,L,Ta),oa=(new Condition$$module$js$nodes(a,"Destroy iron")).option(I,Ma).option(I,Na).option(Sa,I,Ta),Z=(new Condition$$module$js$nodes(a,"Any sword")).option(Q).option(R).option(L).option(I),se=(new Condition$$module$js$nodes(a,"Match insect sword (fire/water/thunder)")).option(R).option(L).option(I).option(lb,
...
return Math.min(AM$$module$js$random*this.iy,RNMX$$module$js$random)}nextInt(b){return Math.floor(this.next()*b)}shuffle(b){for(let a=b.length;a;){const c=this.nextInt(a--);[b[a],b[c]]=[b[c],b[a]]}return b}}var module$js$random={};module$js$random.Random=Random$$module$js$random;const STATUS$$module$js$version="unstable",VERSION$$module$js$version="latest",LABEL$$module$js$version="latest de60453",HASH$$module$js$version="de60453b86f4a1d6d2a4d3db9a21a3e877c268b4",DATE$$module$js$version=new Date(1552185053E3);var module$js$version={};module$js$version.STATUS=STATUS$$module$js$version;module$js$version.VERSION=VERSION$$module$js$version;module$js$version.LABEL=LABEL$$module$js$version;module$js$version.HASH=HASH$$module$js$version;module$js$version.DATE=DATE$$module$js$version;var $jscompDefaultExport$$module$js$patch={apply(b,a){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){let c;a.seed||(a.seed=parseSeed$$module$js$patch("").toString(16),window.location.hash+="&seed="+a.seed);c=a.flags?new FlagSet$$module$js$flagset(a.flags):new FlagSet$$module$js$flagset("Em Gt Mr Rlpt Sbk Sct Sm Tasd");for(const b in a)"false"===a[b]&&(a[b]=!1);yield shuffle$$module$js$patch(b,parseSeed$$module$js$patch(a.seed),c)})}};
