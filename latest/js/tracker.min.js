...
_REVERSIBLE_SWAN_GATE:!0,_SAHARA_RABBITS_REQUIRE_TELEPATHY:c.saharaRabbitsRequireTelepathy(),_SIMPLIFY_INVISIBLE_CHESTS:!0,_TELEPORT_ON_THUNDER_SWORD:c.teleportOnThunderSword(),_TRAINER:c.trainer(),_UNIDENTIFIED_ITEMS:c.unidentifiedItems()},l=new Assembler$$module$js$6502;deterministicPreParse$$module$js$pass$deterministic(a.subarray(16));var m=Object.keys(k).filter((a)=>k[a]).map((a)=>`define ${a} 1\n`).join("");l.assemble(m,"flags.s");yield g("preshuffle.s");h=new Random$$module$js$random(h);m=
new Rom$$module$js$rom(a);"object"==typeof window&&(window.rom=m);m.spoiler=new Spoiler$$module$js$rom$spoiler(m);e&&(e.spoiler=m.spoiler);deterministic$$module$js$pass$deterministic(m,c);yield g("postparse.s");m.scalingLevels=48;m.uniqueItemTableAddress=l.expand("KeyItemData");c.shuffleShops()&&shuffleShops$$module$js$patch(m,c,h);randomizeWalls$$module$js$patch(m,c,h);c.randomizeWildWarp()&&shuffleWildWarp$$module$js$patch(m,c,h);rescaleMonsters$$module$js$patch(m,c,h);unidentifiedItems$$module$js$pass$unidentifieditems(m,
c,h);shuffleTrades$$module$js$pass$shuffletrades(m,c,h);c.randomizeMaps()&&shuffleMazes$$module$js$pass$shufflemazes(m,h);const n=World$$module$js$graph$world.build(m,c),p=yield(new AssumedFill$$module$js$graph$shuffle(m,c)).shuffle(n.graph,h,f);if(p)n.traverse(n.graph,p),update$$module$js$rom$slots(m,p.slots);else return[a,-1];rescaleShops$$module$js$patch(m,l,c.bargainHunting()?h:void 0);c.shuffleMonsters()&&shuffleMonsters$$module$js$patch(m,c,h);identifyKeyItemsForDifficultyBuffs$$module$js$patch(m);
c.doubleBuffMedicalHerb()?(a[115996]*=2,a[115962]*=3):c.buffMedicalHerb()&&(a[115996]+=16,a[115962]*=2);c.storyMode()&&storyMode$$module$js$patch(m);shuffleMusic$$module$js$patch(m,c,h);shufflePalettes$$module$js$pass$shufflepalettes(m,c,h);c.blackoutMode()&&blackoutMode$$module$js$patch(m);misc$$module$js$patch(m,c,h);fixDialog$$module$js$pass$fixdialog(m);c.buffDyna()&&buffDyna$$module$js$patch(m,c);c.trainer()&&(m.wildWarp.locations=[10,26,53,72,109,110,140,170,172,176,182,159,166,88,92,0]);yield m.writeData();
buffDyna$$module$js$patch(m,c);h=yield postParsedShuffle$$module$js$patch(a,h,b,c,l,g);EXPAND_PRG$$module$js$patch&&(m=a.subarray(16),m.subarray(507904,524288).set(m.subarray(245760,262144)));return[a,h]})}
