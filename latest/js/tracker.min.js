...
function disableStabs$$module$js$pass$deterministic(a){for(const b of[8,9,39])a.objects[b].collisionPlane=0;a.npcs[84].data[0]=32}function orbsOptional$$module$js$pass$deterministic(a){for(const b of[16,20,24,29])a.objects[b].terrainSusceptibility&=-5,a.objects[b].level=2}var module$js$pass$deterministic={};module$js$pass$deterministic.deterministicPreParse=deterministicPreParse$$module$js$pass$deterministic;module$js$pass$deterministic.deterministic=deterministic$$module$js$pass$deterministic;function shuffleTrades$$module$js$pass$shuffletrades(a,b,c){if(b.randomizeTrades()){var {StatueOfOnyx:d,FogLamp:e,LovePendant:f,KirisaPlant:g,IvoryStatue:h}=a.items,k=new Map;b=[[d,0,"Akahana"],[e,0,"Fisherman"],[f,0,"Kensu"],[g,0,"Aryllis"],[h,0,"Slimed Kensu"]];var l=b.map(([a,b,c])=>{if(0>a.trades.indexOf(b)||b>=a.itemUseData.length)throw Error(`not a trade: ${a} ${b}`);return[a.itemUseData[b],a.id,c]});c.shuffle(l);for(const [c,d]of b){const [b,e,f]=l.pop();c.itemUseData[d]=b;a.spoiler&&a.spoiler.addTrade(c.id,
