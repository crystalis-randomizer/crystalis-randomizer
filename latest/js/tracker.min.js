...
PRESETS_BY_KEY$$module$js$flagset={};for(const {title:a,flags:b}of PRESETS$$module$js$flagset)PRESETS_BY_KEY$$module$js$flagset[`@${a.replace(/ /g,"").toLowerCase()}`]=b;
...
module$js$graph$condition.Item.BOW_OF_MOON),slot:module$js$graph$condition.Slot(module$js$graph$condition.Event.OPENED_CRYPT)}]}}var c=this.rom.triggers[a&127];if(!c||!c.used)throw Error(`Unknown trigger: ${hex$$module$js$rom$util(a)}`);a=(a)=>this.relevantFlags.has(a);const d=(a)=>0<a&&this.relevantFlags.has(a);var e=TRIGGER_ACTION_ITEMS$$module$js$graph$overlay[c.message.action];const f=and$$module$js$graph$condition(...c.conditions.map(b).filter(d).map(module$js$graph$condition.Condition));if(25===
c.message.action){e={};134!==c.id||this.flags.assumeRabbitSkip()||(e.dx=[-32,-16,0,16]);186!==c.id||this.flags.disableTeleportSkip()||this.flags.assumeTeleportSkip()||(e.extraLocations=[this.rom.locations.cordelPlainsWest]);const [d,...f]=c.conditions;if(!f.length&&0>d&&a(~b(d)))return Object.assign({},e,{terrain:{exit:module$js$graph$condition.Condition(~b(d))}})}else if(null!=e)return{check:[{condition:f,slot:e}]};c=c.flags.filter(d);return c.length?{check:c.map((a)=>({condition:f,slot:module$js$graph$condition.Slot(a)}))}:
{}}npc(a,b){function c(...a){if(!f.terrain)throw Error("Missing terrain for guard");f.terrain.exit=or$$module$js$graph$condition(f.terrain.exit||[],...a)}const d=this.rom.npcs[a];if(!d||!d.used)throw Error(`Unknown trigger: ${hex$$module$js$rom$util(a)}`);const e=d.spawnConditions.get(b.id)||[],f={check:[]};d.data[2]&4&&(f.terrain={exit:this.flags.assumeStatueGlitch()?[[]]:or$$module$js$graph$condition(...e.map((a)=>FLAG_MAP$$module$js$graph$overlay.get(a)||(this.relevantFlags.has(a)?module$js$graph$condition.Condition(a):
[])))});switch(a){case 20:if(b.spawns.find((a)=>a.isNpc()&&21===a.id))return{};case 37:f.hitbox={x0:0,x1:2,y0:0,y1:1};c(module$js$graph$condition.Magic.CHANGE,module$js$graph$condition.Magic.PARALYSIS);break;case 45:delete f.terrain;break;case 51:c(module$js$graph$condition.Magic.PARALYSIS);break;case 56:223===b.id&&(f.hitbox={x0:0,x1:1,y0:2,y1:3});break;case 78:f.hitbox={x0:-1,x1:2,y0:0,y1:1};c(module$js$graph$condition.Magic.CHANGE,module$js$graph$condition.Event.ENTERED_SHYRON);break;case 128:c(...e.map((a)=>
module$js$graph$condition.Condition(~a)));break;case 133:c(module$js$graph$condition.Item.FLUTE_OF_LIME)}const g=[];var h=(a)=>{0>=a||(a=FLAG_MAP$$module$js$graph$overlay.get(a)||(this.relevantFlags.has(a)?module$js$graph$condition.Condition(a):null),null!=a&&g.push(a))};for(var k of e)h(k);var l=this.tradeIns.get(a);if(null!=l){k=function(a,...c){c=and$$module$js$graph$condition(...g,b,...c);f.check.push({slot:a,condition:c})};const b=l;l=k;this.tracker&&this.flags.randomizeTrades()&&(l=(a,...b)=>
{const c=or$$module$js$graph$condition(...[module$js$graph$condition.Item.STATUE_OF_ONYX,module$js$graph$condition.Item.FOG_LAMP,module$js$graph$condition.Item.LOVE_PENDANT,module$js$graph$condition.Item.KIRISA_PLANT,module$js$graph$condition.Item.IVORY_STATUE].map((a)=>and$$module$js$graph$condition(...g,a,...b)));f.check.push({slot:a,condition:c})});switch(a){case 21:k(module$js$graph$condition.Slot(module$js$graph$condition.Item.WINDMILL_KEY));break;case 35:f.hitbox={x0:-1,x1:2,y0:-1,y1:2};l(module$js$graph$condition.Slot(module$js$graph$condition.Item.BOW_OF_MOON),
module$js$graph$condition.Magic.CHANGE);break;case 99:f.hitbox={x0:-1,x1:2,y0:-1,y1:2};k(module$js$graph$condition.Slot(module$js$graph$condition.Event.HEALED_DOLPHIN));k(module$js$graph$condition.Slot(module$js$graph$condition.Item.SHELL_FLUTE));break;case 100:l(module$js$graph$condition.Slot(module$js$graph$condition.Event.RETURNED_FOG_LAMP),...this.flags.requireHealedDolphinToRide()?[module$js$graph$condition.Event.HEALED_DOLPHIN]:[]);break;case 107:k(module$js$graph$condition.Slot(module$js$graph$condition.Item.GLOWING_LAMP));
break;case 117:l(module$js$graph$condition.Slot(module$js$graph$condition.Magic.FLIGHT));break;case 116:l(module$js$graph$condition.Slot(module$js$graph$condition.Magic.CHANGE),module$js$graph$condition.Magic.PARALYSIS,module$js$graph$condition.Event.FOUND_KENSU);break;case 130:l(module$js$graph$condition.Slot(module$js$graph$condition.Item.GAS_MASK));break;case 136:k(module$js$graph$condition.Slot(module$js$graph$condition.Item.SHIELD_RING))}}if(132===a)return{check:[{condition:this.bossRequirements(this.rom.bosses.sabera1),
slot:module$js$graph$condition.Slot(module$js$graph$condition.Boss.SABERA1)}]};if(29===a)return a=module$js$graph$condition.Slot(module$js$graph$condition.Item.SWORD_OF_FIRE),{check:[{condition:and$$module$js$graph$condition(module$js$graph$condition.Magic.TELEPATHY,module$js$graph$condition.Boss.INSECT),slot:a},{condition:module$js$graph$condition.Event.RESCUED_CHILD,slot:a}]};if(31===a)return(a=this.rom.npcs[a].spawnConditions.get(b.id))&&a.includes(69)?{}:{check:[{condition:module$js$graph$condition.Event.DWARF_MOTHER,
slot:module$js$graph$condition.Slot(module$js$graph$condition.Event.DWARF_CHILD)}]};for(var m of d.globalDialogs)h(~m.condition);for(const c of d.localDialogs.get(b.id)||d.localDialogs.get(-1)||[])if(!e.includes(~c.condition)){b=(b=FLAG_MAP$$module$js$graph$overlay.get(c.condition))?[b]:this.relevantFlags.has(c.condition)?[module$js$graph$condition.Condition(c.condition)]:[];h=and$$module$js$graph$condition(...b,...g);m=FLAG_MAP$$module$js$graph$overlay.get(~c.condition)||(this.relevantFlags.has(~c.condition)?
module$js$graph$condition.Condition(~c.condition):null);null!=m&&g.push(m);m=c.message.action;3===m?f.check.push({slot:module$js$graph$condition.Slot.item(d.data[0]),condition:h}):17===m||9===m?f.check.push({slot:module$js$graph$condition.Slot.item(d.data[1]),condition:h}):16===m?f.check.push({slot:module$js$graph$condition.Slot(module$js$graph$condition.Magic.RECOVER),condition:h}):8===m&&45===a&&f.check.push({slot:module$js$graph$condition.Slot(module$js$graph$condition.Event.OPENED_SWAN),condition:h});
for(const a of c.flags)(m=(m=FLAG_MAP$$module$js$graph$overlay.get(a))?m:this.relevantFlags.has(a)?module$js$graph$condition.Condition(a):null)&&f.check.push({slot:module$js$graph$condition.Slot(m),condition:h});if(b.length&&e.includes(c.condition))break}return f}capabilities(){var a=module$js$graph$condition.Item.SWORD_OF_WIND;let b=module$js$graph$condition.Item.SWORD_OF_FIRE,c=module$js$graph$condition.Item.SWORD_OF_WATER,d=module$js$graph$condition.Item.SWORD_OF_THUNDER;if(!this.flags.orbsOptional()&&
(a=or$$module$js$graph$condition(and$$module$js$graph$condition(module$js$graph$condition.Item.SWORD_OF_WIND,module$js$graph$condition.Item.ORB_OF_WIND),and$$module$js$graph$condition(module$js$graph$condition.Item.SWORD_OF_WIND,module$js$graph$condition.Item.TORNADO_BRACELET)),b=or$$module$js$graph$condition(and$$module$js$graph$condition(module$js$graph$condition.Item.SWORD_OF_FIRE,module$js$graph$condition.Item.ORB_OF_FIRE),and$$module$js$graph$condition(module$js$graph$condition.Item.SWORD_OF_FIRE,
module$js$graph$condition.Item.FLAME_BRACELET)),c=or$$module$js$graph$condition(and$$module$js$graph$condition(module$js$graph$condition.Item.SWORD_OF_WATER,module$js$graph$condition.Item.ORB_OF_WATER),and$$module$js$graph$condition(module$js$graph$condition.Item.SWORD_OF_WATER,module$js$graph$condition.Item.BLIZZARD_BRACELET)),d=or$$module$js$graph$condition(and$$module$js$graph$condition(module$js$graph$condition.Item.SWORD_OF_THUNDER,module$js$graph$condition.Item.ORB_OF_THUNDER),and$$module$js$graph$condition(module$js$graph$condition.Item.SWORD_OF_THUNDER,
module$js$graph$condition.Item.STORM_BRACELET)),this.flags.assumeSwordChargeGlitch())){var e=function(a){const b=a[0][0];return f.map((a)=>a[0]===b?a:[b,...a])};const f=or$$module$js$graph$condition(a,b,c,d);a=e(module$js$graph$condition.Item.SWORD_OF_WIND);b=e(module$js$graph$condition.Item.SWORD_OF_FIRE);c=e(module$js$graph$condition.Item.SWORD_OF_WATER);d=e(module$js$graph$condition.Item.SWORD_OF_THUNDER)}a=[[module$js$graph$condition.Capability.SWORD,module$js$graph$condition.Item.SWORD_OF_WIND,
module$js$graph$condition.Item.SWORD_OF_FIRE,module$js$graph$condition.Item.SWORD_OF_WATER,module$js$graph$condition.Item.SWORD_OF_THUNDER],[module$js$graph$condition.Capability.BREAK_STONE,a],[module$js$graph$condition.Capability.BREAK_ICE,b],[module$js$graph$condition.Capability.FORM_BRIDGE,c],[module$js$graph$condition.Capability.BREAK_IRON,d],[module$js$graph$condition.Capability.MONEY,module$js$graph$condition.Capability.SWORD],[module$js$graph$condition.Capability.CLIMB_WATERFALL,module$js$graph$condition.Magic.FLIGHT],
[module$js$graph$condition.Capability.SHOOTING_STATUE,module$js$graph$condition.Magic.BARRIER],[module$js$graph$condition.Capability.CLIMB_SLOPE,module$js$graph$condition.Item.RABBIT_BOOTS,module$js$graph$condition.Magic.FLIGHT],[module$js$graph$condition.Event.GENERALS_DEFEATED,module$js$graph$condition.Item.IVORY_STATUE],[module$js$graph$condition.Event.OPENED_SEALED_CAVE,module$js$graph$condition.Event.STARTED_WINDMILL]];this.flags.assumeGhettoFlight()&&a.push([module$js$graph$condition.Capability.CLIMB_WATERFALL,
and$$module$js$graph$condition(module$js$graph$condition.Event.RIDE_DOLPHIN,module$js$graph$condition.Item.RABBIT_BOOTS)]);this.flags.guaranteeBarrier()||a.push([module$js$graph$condition.Capability.SHOOTING_STATUE,and$$module$js$graph$condition(module$js$graph$condition.Capability.MONEY,module$js$graph$condition.Capability.BUY_HEALING),and$$module$js$graph$condition(module$js$graph$condition.Capability.MONEY,module$js$graph$condition.Item.SHIELD_RING),and$$module$js$graph$condition(module$js$graph$condition.Capability.MONEY,
module$js$graph$condition.Magic.REFRESH)]);this.flags.leatherBootsGiveSpeed()&&a.push([module$js$graph$condition.Capability.CLIMB_SLOPE,module$js$graph$condition.Item.LEATHER_BOOTS]);for(const b of this.rom.bosses)null!=b.kill&&null!=b.drop&&a.push([module$js$graph$condition.Item(b.drop),module$js$graph$condition.Boss(b.kill)]);a.push([module$js$graph$condition.Item.ORB_OF_WATER,module$js$graph$condition.Boss.RAGE]);this.flags.guaranteeGasMask()?a.push([module$js$graph$condition.Capability.TRAVEL_SWAMP,
module$js$graph$condition.Item.GAS_MASK]):a.push([module$js$graph$condition.Capability.TRAVEL_SWAMP,or$$module$js$graph$condition(module$js$graph$condition.Item.GAS_MASK,and$$module$js$graph$condition(module$js$graph$condition.Capability.MONEY,module$js$graph$condition.Item.MEDICAL_HERB),and$$module$js$graph$condition(module$js$graph$condition.Capability.MONEY,module$js$graph$condition.Magic.REFRESH))]);return a.map((a)=>{var [b,...c]=a;return{capability:b,condition:or$$module$js$graph$condition(...c)}})}wallCapability(a){return{flag:[module$js$graph$condition.Capability.BREAK_STONE,
module$js$graph$condition.Capability.BREAK_ICE,module$js$graph$condition.Capability.FORM_BRIDGE,module$js$graph$condition.Capability.BREAK_IRON][a][0][0]}}}
...
c.shuffleShops()&&shuffleShops$$module$js$patch(m,c,h);randomizeWalls$$module$js$patch(m,c,h);c.randomizeWildWarp()&&shuffleWildWarp$$module$js$patch(m,c,h);rescaleMonsters$$module$js$patch(m,c,h);unidentifiedItems$$module$js$pass$unidentifieditems(m,c,h);shuffleTrades$$module$js$pass$shuffletrades(m,c,h);c.randomizeMaps()&&shuffleMazes$$module$js$pass$shufflemazes(m,h);const n=World$$module$js$graph$world.build(m,c),r=yield(new AssumedFill$$module$js$graph$shuffle(m,c)).shuffle(n.graph,h,f);if(r)n.traverse(n.graph,
r),update$$module$js$rom$slots(m,r.slots);else return-1;rescaleShops$$module$js$patch(m,l,c.bargainHunting()?h:void 0);normalizeSwords$$module$js$patch(m,c,h);c.shuffleMonsters()&&shuffleMonsters$$module$js$patch(m,c,h);identifyKeyItemsForDifficultyBuffs$$module$js$patch(m);c.doubleBuffMedicalHerb()?(a[115996]*=2,a[115962]*=3):c.buffMedicalHerb()&&(a[115996]+=16,a[115962]*=2);c.storyMode()&&storyMode$$module$js$patch(m);shuffleMusic$$module$js$patch(m,c,h);shufflePalettes$$module$js$pass$shufflepalettes(m,
