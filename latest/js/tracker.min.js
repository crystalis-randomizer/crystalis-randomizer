...
0;a.trigger(186).conditions[0]=-581;a.trigger(187).conditions[1]=-28;for(const b of a.npcs)for(const a of b.allDialogs())14===a.condition&&(a.condition=579),-15===a.condition&&(a.condition=-580)}function disableStabs$$module$js$pass$deterministic(a){for(const b of[8,9,39])a.objects[b].collisionPlane=0}function orbsOptional$$module$js$pass$deterministic(a){for(const b of[16,20,24,29])a.objects[b].terrainSusceptibility&=-5,a.objects[b].level=2}var module$js$pass$deterministic={};
module$js$pass$deterministic.deterministicPreParse=deterministicPreParse$$module$js$pass$deterministic;module$js$pass$deterministic.deterministic=deterministic$$module$js$pass$deterministic;function shuffleTrades$$module$js$pass$shuffletrades(a,b,c){if(b.randomizeTrades()){b=[[a.items[37],0,"Akahana"],[a.items[53],0,"Fisherman"],[a.items[59],0,"Kensu"],[a.items[60],0,"Aryllis"],[a.items[61],0,"Slimed Kensu"]];var d=[];for(const [a,c,g]of b){if(!a.tradeIn)throw Error(`Expected trade-in for ${a.id}`);d.push([g,a.tradeIn.slice(c,c+6)])}c.shuffle(d);for(const [c,f]of b){const [b,e]=d.pop();c.tradeIn.splice(f,6,...e);a.spoiler&&a.spoiler.addTrade(c.id,c.messageName,b);35===e[0]&&(a.prg[251061]=
