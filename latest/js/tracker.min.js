...
for(this.report["post-shuffle monsters"]=this.monsters.map((a)=>a.id);this.locations.length;){const {location:e,slots:f}=this.locations.pop(),g=this.report["$"+e.id.toString(16).padStart(2,"0")]=[],{maxFlyers:h=0,nonFlyers:k={},tower:m=!1}=MONSTER_ADJUSTMENTS$$module$js$patch[e.id]||{};if(m)continue;let l=h,n=Constraint$$module$js$rom$constraint.forLocation(e.id);e.bossId();for(const a of e.spawns)if(a.isChest()&&!a.isInvisible())n=112>a.id?n.meet(Constraint$$module$js$rom$constraint.TREASURE_CHEST,
!0):n.meet(Constraint$$module$js$rom$constraint.MIMIC,!0);else if(a.isNpc()){var c=b.npcConstraints.get(a.id);c&&(n=n.meet(c,!0))}else a.isMonster()&&UNTOUCHED_MONSTERS$$module$js$patch[a.monsterId]&&(c=b.monsterConstraints.get(a.monsterId))&&(n=n.meet(c,!0));g.push(`Initial pass: ${n.fixed.map((a)=>Infinity>a.size?"["+[...a].join(", ")+"]":"all")}`);const p=new Map;c=(a)=>{var c=e.rom.objects[a.id];if(c.monsterClass){var d=p.get(c.monsterClass);if(null!=d&&d!==a.id)return!1}d=FLYERS$$module$js$patch.has(a.id);
var h=MOTHS_AND_BATS$$module$js$patch.has(a.id);if(d){if(!l)return!1;--l}const m=b.getMonsterConstraint(e.id,a.id);let q=n.tryMeet(m);!q&&Infinity>n.pal2.size&&Infinity>n.pal3.size&&this.flags.shuffleSpritePalettes()&&(q=n.tryMeet(m,!0));if(!q)return!1;g.push(`  Adding ${a.id.toString(16)}: ${q}`);n=q;c.monsterClass&&p.set(c.monsterClass,a.id);c=0;if(d||h)for(d=0;d<f.length;d++){if(f[d]in k){c=d;break}}else for(d=0;d<f.length;d++)if(!(f[d]in k)){c=d;break}(this.report[`mon-${a.id.toString(16)}`]=
this.report[`mon-${a.id.toString(16)}`]||[]).push("$"+e.id.toString(16));d=f[c];h=e.spawns[d-13];d in k&&(h.y+=16*k[d][0],h.x+=16*k[d][1]);h.monsterId=a.id;g.push(`    slot ${d.toString(16)}: ${h}`);f.splice(c,1);return!0};if(l&&f.length)for(var d=0;d<Math.min(40,this.monsters.length);d++)FLYERS$$module$js$patch.has(this.monsters[d].id)&&c(this.monsters[d])&&this.monsters.splice(d,1);for(d=0;d<this.monsters.length&&f.length;d++)if(c(this.monsters[d])){const [a]=this.monsters.splice(d,1);FLYERS$$module$js$patch.has(a.id)||
