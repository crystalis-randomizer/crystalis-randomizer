...
function rescaleMonsters$$module$js$patch(a,b,c){var d=new Set(seq$$module$js$rom$util(256,(a)=>a).filter((b)=>b in a.objects));for(var e of SCALED_MONSTERS$$module$js$patch){var [f]=e;d.delete(f)}for(var g of SCALED_MONSTERS$$module$js$patch){const [b,c]=g;for(const e of d)a.objects[b].base===a.objects[e].base&&(SCALED_MONSTERS$$module$js$patch.set(e,c),d.delete(b))}a.objects[249].attackType=255;a.objects[250].attackType=255;a.objects[125].elements|=8;a.objects[200].attackType=255;a.objects[200].statusEffect=
0;e=new Set([87,94,104,125,136,151,155,158]);d=new Set([80,83,95,105]);for(const h of SCALED_MONSTERS$$module$js$patch){const [k,{sdef:l,swrd:m,hits:n,satk:p,dgld:q,sexp:r}]=h;f=a.objects[k].data;g=e.has(k)?1:0;f[2]|=128;f[6]=n;f[7]=p;f[8]=l|m<<4;f[16]=f[16]&15|q<<4;f[17]=r;(g?b.shuffleBossElements():b.shuffleMonsterElements())&&!d.has(k)&&(f=[...a.objects[k].elements.toString(2).padStart(4,"0")],c.shuffle(f),a.objects[k].elements=Number.parseInt(f.join(""),2))}if(b.shuffleMonsterElements()){b=c.nextInt(4);
a.prg[217645]=b+1;for(const c of d)a.objects[c].elements=1<<b}}
