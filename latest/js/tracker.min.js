...
Ce=(new Trigger$$module$js$nodes(a,"Talked to Akahana's Friend")).get(ge),De=(new Trigger$$module$js$nodes(a,"Get Bow of Truth")).get(rc),Ee=(new Trigger$$module$js$nodes(a,"Forged Crystalis")).get(pb),$a=(new Condition$$module$js$nodes(a,"Sword charge glitch")).option(C,V),Fe=(new Condition$$module$js$nodes(a,"Rabbit skip")).option(r,I),Ge=(new Condition$$module$js$nodes(a,"Teleport skip")).option(p,l),ab=(new Condition$$module$js$nodes(a,"Statue glitch")).option(m,q),He=(new Condition$$module$js$nodes(a,
...
b;if(b.isMimic()&&!a.canHoldMimic()||"unique"!=b.item.inventoryRow&&a.requiresUnique||"armor"==b.item.inventoryRow&&!a.isChest())return!1;if(f.guaranteeSword()){if(0===a.slotIndex)return 4>b.item.id;if(4>b.item.id&&(r.add(b),4===r.size))return 0===a.slotIndex}return p[a.slotType?a.slotType[0]:"c"]==p[b.slotType?b.slotType[0]:"c"]};l=a.assumedFill(d,(a,b)=>I(a,q.get(b)));if(null==l){g&&g.endState.key++;return!1;throw Error("Could not fill!");}const C=new Map;for(var V=0;V<l.length;V++){var va=l[V];
if(null!=va){var oa=q.get(a.item(va));va=n.get(oa);n.delete(oa);oa=a.location(V);C.set(oa,va);m.delete(oa)}}V=(a,b,c)=>{c=void 0===c?"":c;for(const c of m)if(I(c,a))return C.set(c,b),m.delete(c),n.delete(a),!0;g&&("traps"==c?g.endState.traps++:"chest"==c?g.endState.chest++:g.endState.misc++);return!1};for(var E of n){const [a,b]=E;if(a.isMimic()&&!V(a,b,"traps"))return!1}for(const a of n){const [b,c]=a;if(b.needsChest()&&!V(b,c,"chest"))return!1}for(const a of n){const [b,c]=a;if(!V(b,c))return!1}for(const a of C){const [b,
...
return Math.min(AM$$module$js$random*this.iy,RNMX$$module$js$random)}nextInt(b){return Math.floor(this.next()*b)}shuffle(b){for(let a=b.length;a;){const c=this.nextInt(a--);[b[a],b[c]]=[b[c],b[a]]}return b}}var module$js$random={};module$js$random.Random=Random$$module$js$random;const STATUS$$module$js$version="unstable",VERSION$$module$js$version="latest",LABEL$$module$js$version="latest b11a30a",HASH$$module$js$version="b11a30ae18c87c5d4ef4f01b67bd8eefda84aa6d",DATE$$module$js$version=new Date(1556779673E3);var module$js$version={};module$js$version.STATUS=STATUS$$module$js$version;module$js$version.VERSION=VERSION$$module$js$version;module$js$version.LABEL=LABEL$$module$js$version;module$js$version.HASH=HASH$$module$js$version;module$js$version.DATE=DATE$$module$js$version;var $jscompDefaultExport$$module$js$patch={apply(b,a,c){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){let d;a.seed||(a.seed=parseSeed$$module$js$patch("").toString(16),window.location.hash+="&seed="+a.seed);d=a.flags?new FlagSet$$module$js$flagset(a.flags):new FlagSet$$module$js$flagset("Em Gt Mr Rlpt Sbk Sct Sm Tasd");for(const b in a)"false"===a[b]&&(a[b]=!1);yield shuffle$$module$js$patch(b,parseSeed$$module$js$patch(a.seed),d,new FetchReader$$module$js$fetchreader(c))})}};
