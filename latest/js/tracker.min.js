...
a.assumeGhettoFlight())),l=h(d("Assume statue glitch",a.assumeStatueGlitch())),n=d("Allow statue glitch",!a.disableStatueGlitch()),p=h(d("Assume teleport skip",a.assumeTeleportSkip())),q=d("Allow teleport skip",!a.disableTeleportSkip()),r=h(d("Assume rabbit skip",a.assumeRabbitSkip())),u=h(d("Allow rabbit skip",!a.disableRabbitSkip())),t=h(d("Assume sword charge glitch",a.assumeSwordChargeGlitch())),I=d("Allow sword charge glitch",!a.disableSwordChargeGlitch()),W=h(d("Assume wild warp",a.assumeWildWarp())),
...
if(4>b.item.id&&(r.add(b),4===r.size))return 0===a.slotIndex}return p[a.slotType?a.slotType[0]:"c"]===p[b.slotType?b.slotType[0]:"c"]};q=b.assumedFill(d,(a,b)=>u(a,n.get(b)));if(null==q){g&&g.endState.key++;return!1;throw Error("Could not fill!");}const t=new Map;for(var I=0;I<q.length;I++){var W=q[I];if(null!=W){var pa=n.get(b.item(W));W=m.get(pa);m.delete(pa);pa=b.location(I);t.set(pa,W);l.delete(pa)}}I=(a,b,c)=>{c=void 0===c?"":c;for(const c of l)if(u(c,a))return t.set(c,b),l.delete(c),m.delete(a),
!0;g&&("traps"===c?g.endState.traps++:"chest"===c?g.endState.chest++:g.endState.misc++);return!1};for(var B of m){const [a,b]=B;if(a.isMimic()&&!I(a,b,"traps"))return!1}for(const a of m){const [b,c]=a;if(b.needsChest()&&!I(b,c,"chest"))return!1}for(const a of m){const [b,c]=a;if(!I(b,c))return!1}for(const a of t){const [b,c]=a;b.set(...c)}const {win:G,path:Qa}=a.traverse({wanted:k,dfs:!1});if(!G)return g&&g.endState.no_win++,!1;c&&a.write();if(!e)return!0;g&&(g.endState.success++,g.analyze(a,b,q));
...
b.reqs.check=(a,b)=>{b=void 0===b?W:b;return`${b(a)}: ${I(t.get(module$js$graph$condition.Slot(a)),b)}`};b.reqs.check2=(a)=>b.reqs.check(a,b.whatFlag)}return new World$$module$js$graph$world(a,p,makeGraph$$module$js$graph$world(t,a))}
...
b);c.constraint=b}}getMonsterConstraint(a,b){const c=this.monsterConstraints.get(b)||Constraint$$module$js$rom$constraint.NONE;return 88!==(a&88)&&this.rom.objects[b].goldDrop?c.meet(Constraint$$module$js$rom$constraint.COIN)||Constraint$$module$js$rom$constraint.NONE:c}shufflePalettes(a){function b(b){const d=[...b.fixed];for(let b=2;4>b;b++)if(Infinity>d[b].size){var f=b;{var g=Math.floor(5-Math.log2(a.nextInt(15)+2));const b=new Set;for(let d=0;d<g;d++)b.add(c[a.nextInt(c.length)]);g=b}d[f]=g}return new Constraint$$module$js$rom$constraint(d,
...
~d.id:d.id,e);b=void 0;for(let [f,g]of a){a=this.rom.locations[0>f?~f:f];for(const b of e)1<b&&this.allSpritePalettes.add(a.spritePalettes[b-2]);a=Constraint$$module$js$rom$constraint.fromSpawn(e,d,a,g,c);b=b?b.join(a):a;!c&&g.patternBank&&(b=b.shifted())}if(!b)throw Error("Expected child to appear");return b}}
...
--l}var h=b.getMonsterConstraint(e.id,a.id);let m=n.meet(h);m||this.flags.shuffleSpritePalettes()&&(m=n.meet(h.ignorePalette()));if(!m)return!1;g.push(`  Adding ${a.id.toString(16)}: ${m}`);n=m;h=0;if(c||d)for(c=0;c<f.length;c++){if(f[c]in k){h=c;break}}else for(c=0;c<f.length;c++)if(!(f[c]in k)){h=c;break}(this.report[`mon-${a.id.toString(16)}`]=this.report[`mon-${a.id.toString(16)}`]||[]).push("$"+e.id.toString(16));c=f[h];d=e.spawns[c-13];c in k&&(d.y+=16*k[c][0],d.x+=16*k[c][1]);d.monsterId=a.id;
g.push(`    slot ${c.toString(16)}: ${d}`);f.splice(h,1);return!0};if(l)for(var d=0;d<Math.min(40,this.monsters.length);d++)FLYERS$$module$js$patch.has(this.monsters[d].id)&&c(this.monsters[d])&&this.monsters.splice(d,1);for(d=0;d<this.monsters.length&&f.length;d++)if(c(this.monsters[d])){const [a]=this.monsters.splice(d,1);FLYERS$$module$js$patch.has(a.id)||this.used.push(a);d--}for(d=0;d<this.used.length&&f.length;d++)c(this.used[d])&&(this.used.push(...this.used.splice(d,1)),d--);[e.spritePatterns[0],
e.spritePatterns[1],e.spritePalettes[0],e.spritePalettes[1]]=n.fix(a);if(f.length){console.error(`Failed to fill location ${e.id.toString(16)}: ${f.length} remaining`);for(const a of f)c=e.spawns[a-13],c.x=c.y=0,c.id=176,c.data[0]=254}for(const a of e.spawns)b.configure(e,a)}}}
