...
else if(a=/^\s*\.assert\s+(\S+)/i.exec(b))this.addLine(new AssertLine$$module$6502(this.pc=parseNumber$$module$6502(a[1])));else if(a=/^\s*\.bank\s+(\S+)\s+(\S+)\s*:\s*(\S+)/i.exec(b)){const [,b,c,f]=a;this.addLine(new BankLine$$module$6502(parseNumber$$module$6502(b),parseNumber$$module$6502(c),parseNumber$$module$6502(f)))}else if(a=/^\s*\.(byte|word)\s+(.*)/i.exec(b))a=("word"==a[1]?WordLine$$module$6502:ByteLine$$module$6502).parse(a[2]),this.addLine(a),this.pc+=a.size();else if(a=/^define\s+(\S+)\s+(.*)/.exec(b))this.addLabel(a[1],
parseNumber$$module$6502(a[2]));else{if(a=/^(\S+?):(.*)$/.exec(b)){var c=a[1];b=" "+a[2];this.addLabel(c,~this.pc)}else if(a=/^((?:[-+]+\s+)+)(.*)$/.exec(b)){b=a[1];for(c of b.trim().split(" "))this.addLabel(c,~this.pc);b=" "+a[2]}if(a=/^\s+([a-z]{3})(\s+.*)?$/.exec(b))a=new Opcode$$module$6502(a[1],(a[2]||"").trim(),this.pc),this.addLine(a),this.pc+=a.size();else if(/\S/.test(b))throw Error(`Could not parse line ${b} at ${this.filename}:${this.lineNumber}`);}}assemble(){var b=new Context$$module$6502(this.labels),
a=[];for(var c of this.lines){try{c.expand(b)}catch(e){throw a=e.stack.replace(`Error: ${e.message}`,""),Error(`${e.message}${` from line ${c.origLineNumber}: \`${c.origContent}\``}${a}\n================`);}if(c instanceof OrgLine$$module$6502&&null!=a[c.pc])throw Error(`Collision at $${c.pc.toString(16)}`);for(const d of c.bytes()){if(null!=a[b.pc])throw Error(`Collision at $${b.pc.toString(16)}`);a[b.pc++]=d}}b=[];for(var d in a)d-1 in a||b.push(Number(d));c=[];for(const e of b){d=[];for(b=e;b in
a;b++)d.push(a[b]);c.push(new Chunk$$module$6502(e,d))}if(this.conditions.length)throw Error("Unterminated .if");return c}}class AbstractLine$$module$6502{orig(b,a){this.origLineNumber=b;this.origContent=a;return this}expand(){throw Error(`abstract: ${this.constructor}`);}bytes(){throw Error(`abstract: ${this.constructor}`);}}
...
class OrgLine$$module$6502 extends AbstractLine$$module$6502{constructor(b){super();this.pc=b}bytes(){return[]}size(){return 0}expand(b){b.pc=this.pc}}class AssertLine$$module$6502 extends AbstractLine$$module$6502{constructor(b){super();this.pc=b}bytes(){return[]}size(){return 0}expand(b){if(b.pc!=this.pc)throw Error(`Misalignment: expected $${this.pc.toString(16)} but was $${b.pc.toString(16)}`);}}
...
me=(new Trigger$$module$nodes(a,"Saved Kensu")).get(A),ne=(new Trigger$$module$nodes(a,"Talked to Deo")).get(Vd),oe=(new Trigger$$module$nodes(a,"Talked to Akahana's Friend")).get(Td),pe=(new Trigger$$module$nodes(a,"Get Bow of Truth")).get(fc),qe=(new Trigger$$module$nodes(a,"Forged Crystalis")).get(jb),Ua=(new Condition$$module$nodes(a,"Sword charge glitch")).option(Ld),re=(new Condition$$module$nodes(a,"Rabbit skip")).option(p),se=(new Condition$$module$nodes(a,"Teleport skip")).option(l),wb=(new Condition$$module$nodes(a,
"Statue glitch")).option(t),te=(new Condition$$module$nodes(a,"Wild warp")).option(Jd,Kd),Va=(new Condition$$module$nodes(a,"Any level 2 sword")).option(Q,Ha).option(Q,za).option(R,Ia).option(R,Ja).option(L,Ka).option(L,La).option(I,Ma).option(I,Na),v=(new Condition$$module$nodes(a,"Destroy stone")).option(Q,Ha).option(Q,za).option(Ua,Q,Va),w=(new Condition$$module$nodes(a,"Destroy ice")).option(R,Ia).option(R,Ja).option(Ua,R,Va),q=(new Condition$$module$nodes(a,"Cross rivers")).option(L,Ka).option(L,
La).option(A).option(Ua,L,Va),oa=(new Condition$$module$nodes(a,"Destroy iron")).option(I,Ma).option(I,Na).option(Ua,I,Va),S=(new Condition$$module$nodes(a,"Any sword")).option(Q).option(R).option(L).option(I),ue=(new Condition$$module$nodes(a,"Match insect sword (fire/water/thunder)")).option(R).option(L).option(I).option(Pa,D.value?D:h,Q),Dc=(new Condition$$module$nodes(a,"Speed boots")).option(Tb,r),Ec=(new Condition$$module$nodes(a,"Climb slopes")).option(nb).option(A).option(Dc),ve=(new Condition$$module$nodes(a,
"Enter Mt Sabre North (rabbit)")).option(pc).option(re),we=(new Condition$$module$nodes(a,"Enter Mt Sabre North (Teleport)")).option(gc).option(se,A),Fc=(new Condition$$module$nodes(a,"Asina in her room")).option(uc),xe=(new Condition$$module$nodes(a,"Paralysis or Ball of Water")).option(Ra).option(Fc).option(wb),Gc=(new Condition$$module$nodes(a,"Ride dolphin")).option(pb,xc),ye=(new Condition$$module$nodes(a,"Ghetto flight")).option(n,Gc,nb),T=(new Condition$$module$nodes(a,"Cross sea")).option(Gc).option(A),
...
q),Sc=(new Location$$module$nodes(a,21,ia,"East")).overworld().connect(ua).chest(Rb,24),Ea=(new Location$$module$nodes(a,24,qa,"Town")).town().connect(ua).trigger(Zd,Rb),jf=(new Location$$module$nodes(a,25,ia,"Outside Stom's House")).town().connect(ua),zb=(new Location$$module$nodes(a,26,ia,"Swamp")).overworld().connect(Sc,Wa).trigger(sb,nc,Wa);(new Location$$module$nodes(a,26,ia,"Swamp Insect Area")).overworld().connect(zb,Wa).boss(Je);const Za=(new Location$$module$nodes(a,27,ra,"Town")).town().connect(hf),
...
61,Xa,"Tool Shop")).house(),Hf=(new Location$$module$nodes(a,62,Xa,"Back Room")).house(),Cb=(new Location$$module$nodes(a,64,Z,"Summit")).overworld().connect(Ef),cb=(new Location$$module$nodes(a,64,Z,"Northwest")).overworld().from(Cb).to(Cb,A),If=(new Location$$module$nodes(a,64,Z,"Northeast")).overworld().connect(cb,q),ad=(new Location$$module$nodes(a,65,Z,"Southwest")).overworld().connect(cb),Jf=(new Location$$module$nodes(a,65,Z,"Southeast")).overworld().connect(ad,q),bd=(new Location$$module$nodes(a,
...
(new Location$$module$nodes(a,79,C,"Tunnel 8b (fog lamp)")).cave().connect($f,v).chest(Yb,19);const ma=(new Location$$module$nodes(a,80,z,"Town")).town().connect(cb),hd=(new Location$$module$nodes(a,81,z,"Fishherman Island")).town().connect(ma);(new Location$$module$nodes(a,82,Z,"Mesia Shrine")).cave().trigger(uc).connect(Kf,q,tc);const id=(new Location$$module$nodes(a,84,aa,"Tunnel 1a (entrance)")).cave().connect(cb);(new Location$$module$nodes(a,84,aa,"Tunnel 1b (dead end, mimic)")).cave().connect(id,
...
114,N,"Town")).town().connect(jg),wg=(new Location$$module$nodes(a,115,N,"Inside Gate")).overworld().connect(W),xg=(new Location$$module$nodes(a,115,N,"Outside Gate")).overworld().from(wg,na),fb=(new Location$$module$nodes(a,120,la,"Valley")).overworld().connect(xg),Hb=(new Location$$module$nodes(a,124,x,"Entrance")).overworld().connect(fb),yg=(new Location$$module$nodes(a,124,x,"Over first river toward Shyron")).overworld().connect(Hb,q),sd=(new Location$$module$nodes(a,124,x,"After first tunnel")).overworld(),
td=(new Location$$module$nodes(a,124,x,"Door to Styx")).overworld().connect(sd,q),zg=(new Location$$module$nodes(a,124,x,"Dead end (no item)")).overworld(),Ag=(new Location$$module$nodes(a,124,x,"Dead end (fruit of lime)")).overworld().chest(Rd,26),Bg=(new Location$$module$nodes(a,124,x,"Dead end (magic ring)")).overworld().chest(Y,25,101),ud=(new Location$$module$nodes(a,124,x,"Outside tunnel to bow")).overworld();(new Location$$module$nodes(a,124,x,"Floating island (bow of sun)")).overworld().connect(ud,
...
q);(new Location$$module$nodes(a,145,E,"Area 3 (dead-end across top-right bridge)")).cave().connect(zd,q);const Ng=(new Location$$module$nodes(a,145,E,"Area 4 (left across middle-right bridge)")).cave().connect(zd,q),Og=(new Location$$module$nodes(a,145,E,"Area 5 (bottom edge)")).cave().connect(Ng,q),Pg=(new Location$$module$nodes(a,145,E,"Area 6 (bottom island)")).cave().connect(Og,q),Qg=(new Location$$module$nodes(a,145,E,"Area 7 (bottom inner ring)")).cave().connect(Pg,q),Rg=(new Location$$module$nodes(a,
145,E,"Area 8 (left outer ring)")).cave().connect(Qg,q).connect(gb,q).chest(Aa,27,100),Sg=(new Location$$module$nodes(a,145,E,"Area 9 (top left inner ring)")).cave().connect(Rg,q),Tg=(new Location$$module$nodes(a,145,E,"Area 10 (top right inner ring)")).cave().connect(Sg,q);(new Location$$module$nodes(a,145,E,"Area 11 (center)")).cave().connect(Tg,q).connectTo(yd);(new Location$$module$nodes(a,145,E,"Area 12 (top center islands)")).cave().connect(gb,A).chest(Pd,28);const Ug=(new Location$$module$nodes(a,
...
(new Location$$module$nodes(a,171,O,"Dead End Behind Iron (fruit of power)")).fortress().connect(hb,oa).chest(Aa,28,98);(new Location$$module$nodes(a,171,O,"Dead End Loop Across Closer Bridges")).fortress().connect(hb,q).connect(hb,q);const hh=(new Location$$module$nodes(a,171,O,"Across First Bridge (fruit of repun)")).fortress().connect(hb,q).chest(Qb,30,102),Lb=(new Location$$module$nodes(a,171,O,"Across Second Bridge")).fortress().connect(hh,q);(new Location$$module$nodes(a,171,O,"Dead End Across Two Bridges ()")).fortress().connect(Lb,
q).connect(Lb,q).chest(kb,29,93);const ih=(new Location$$module$nodes(a,171,O,"Across Third Bridge")).fortress().connect(Lb,q),jh=(new Location$$module$nodes(a,171,O,"Exit Behind Iron Door")).fortress().connect(ih,oa),kh=(new Location$$module$nodes(a,172,O,"Boss")).fortress().connect(jh).boss(Oe),lh=(new Location$$module$nodes(a,172,O,"Tornel")).fortress().connect(kh),Cd=(new Location$$module$nodes(a,173,sa,"Lower")).fortress().connect(lh).chest(mb,26,99).chest(Y,27,111),mh=(new Location$$module$nodes(a,
...
n=(a)=>f.nodes.find((b)=>b instanceof Slot$$module$nodes&&b.name===a);g=n("Sword of Wind");p=n("Sword of Fire");r=n("Ball of Wind");t=n("Ball of Fire");l=n("Statue of Onyx");n=n("Gas Mask");m.k&&a.nextInt(2)&&(p.swap(g),t.swap(r));m.k&&!a.nextInt(3)&&l.swap(n);m=[1,1,1,1,2,2,2,3,3,3,5,5,8,13];g=[];e&&e.addTasks(10);for(p=0;1E3>p;p++){e&&0==p%100&&(yield new Promise(requestAnimationFrame),e.addCompleted(1));a.shuffle(h);for(var y of k)a.shuffle(y);r=k.map(()=>-1);t=m[a.nextInt(m.length)];l=[];n=(a,
b)=>a.itemIndex===b.itemIndex||a.isMimic()&&!b.canHoldMimic()||b.isMimic()&&!a.canHoldMimic()||a.needsChest()&&!b.isChest()||b.needsChest()&&!a.isChest()?!1:!0;for(let a=0;a<t;a++){const [b,c]=h[a],d=k[c][++r[c]];n(b,d)&&(b.swap(d),l.push([b,d]))}if(!l.length)continue;const {win:b,path:c}=f.traverse();if(b)g=c;else for(;l.length;){const [a,b]=l.pop();a.swap(b)}}for(const a of f.nodes)a instanceof Slot$$module$nodes&&a.write(b);if(c){c.items=[];c.route=[];for(const a of g){const [b,d]=a;c.route.push(d);
...
return}e&&(e.addCompleted(1),0===k%50&&(yield new Promise(requestAnimationFrame)))}throw Error("failed");}),shuffle3$$module$depgraph=(b,a,c,d,e,f,g)=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){var g=b.nodes.filter((a)=>a instanceof Slot$$module$nodes&&a.slots&&a.slotName),h=new Map(d.shuffle(g.map((a)=>[a,[a.item,a.itemIndex]])));const m=new Set(d.shuffle(g)),r=new Map,n={};f||(f=new FlagSet$$module$flagset("Sbkm Sct"));f.flags.S.forEach((a,b)=>{for(const c of a)n[c]=b});for(var t of m)r.set(t.item,
t);const l=(a,b)=>null==n[a.slotType?a.slotType[0]:"c"]?a==b:b.isMimic()&&!a.canHoldMimic()||"unique"!=b.item.inventoryRow&&a.requiresUnique||"armor"==b.item.inventoryRow&&!a.isChest()?!1:n[a.slotType?a.slotType[0]:"c"]==n[b.slotType?b.slotType[0]:"c"];t=a.assumedFill(d,(a,b)=>l(a,r.get(b)));if(null==t)return!1;const p=new Map;for(let b=0;b<t.length;b++){var y=t[b];if(null!=y){var D=r.get(a.item(y));y=h.get(D);h.delete(D);D=a.location(b);p.set(D,y);m.delete(D)}}t=(a,b)=>{for(const c of m)if(l(c,a)){p.set(c,
b);m.delete(c);return}return!1};for(const a of h){const [b,c]=a;b.isMimic()&&(t(b,c),h.delete(b))}for(const a of h){const [b,c]=a;b.needsChest()&&(t(b,c),h.delete(b))}for(const a of h){const [b,c]=a;t(b,c)}for(const a of p){const [b,c]=a;b.set(...c)}const {win:Ga,path:ib}=b.traverse({wanted:g,dfs:!1});if(!Ga)return!1;if(c)for(const a of b.nodes)a instanceof Slot$$module$nodes&&a.write(c);if(!e)return!0;e.items=[];e.route=[];for(const a of ib){const [c,d]=a;e.route.push(d);g=b.nodes[c];null!=g.slotName&&
...
class WorldGraph$$module$nodes extends Graph$$module$graph{integrate(b={}){const {tracker:a=!1}=b;var c=!a;const d=new SparseDependencyGraph$$module$graph(this.nodes.length),e=[],f=[],g=new Set,k=new Map;var h=[],m=[];const r=[];var n=[];const t=[];b=new Set;for(var l of this.nodes)if(l instanceof Location$$module$nodes){if(l.isStart){const [a]=d.addRoute([l.uid]);k.set(a.label,a)}e[l.uid]=[];f[l.uid]=[];for(const a of l.connections)g.has(a)||(g.add(a),a.bidi&&g.add(a.reverse()));for(const a of l.chests)d.addRoute([a.uid,
l.uid])}else if(l instanceof Slot$$module$nodes&&"Alarm Flute"===l.item.name)d.addRoute([l.item.uid]),d.finalize(l.item.uid);else if(l instanceof ItemGet$$module$nodes&&"Medical Herb"===l.name&&(d.addRoute([l.uid]),d.finalize(l.uid)),l instanceof Option$$module$nodes)h.push(l);else if(l instanceof TrackerNode$$module$nodes)m.push(l);else if(l instanceof Condition$$module$nodes)r.push(l);else if(l instanceof Trigger$$module$nodes)n.push(l);else if(l instanceof ItemGet$$module$nodes)t.push(l);else if(l instanceof
Slot$$module$nodes)b.add(l);else if(!(l instanceof Area$$module$nodes))throw Error(`Unknown node type: ${l.nodeType}`);for(const a of g)e[a.from.uid].push(a),f[a.to.uid].push(a);l=(b)=>{for(const c of b){for(const b of c.edges({tracker:a}))d.addRoute(b);d.finalize(c.uid)}};l(h);c&&l(m);l(n);for(c=k.values();!(h=c.next()).done;){h=h.value;m=h.target;for(var p of e[m]){m=[p.to.uid,...h.deps];for(n=p.deps.length-1;0<=n;n--)m.push(p.deps[n].uid);p.from.bossNode&&m.push(p.from.bossNode.uid);for(const a of d.addRoute(m))k.has(a.label)||
k.set(a.label,a)}}l(r);for(p=0;p<this.nodes.length;p++)this.nodes[p]instanceof ItemGet$$module$nodes||this.nodes[p]instanceof TrackerNode$$module$nodes||d.finalize(p);p=new LocationList$$module$nodes(this);for(const a of b)for(const b of d.nodes[a.uid].values())p.addRoute([a.uid,...b]);return p}}
...
RNMX$$module$random)}nextInt(b){return Math.floor(this.next()*b)}shuffle(b){for(let a=b.length;a;){const c=this.nextInt(a--);[b[a],b[c]]=[b[c],b[a]]}return b}}var module$random={};module$random.Random=Random$$module$random;const STATUS$$module$version="unstable",VERSION$$module$version="refactor-asm",LABEL$$module$version="refactor-asm e9aecee",HASH$$module$version="e9aeceeb809ad0b1cc049cfe454f5061d95e45d5",DATE$$module$version=new Date(1553056224E3);var module$version={};module$version.STATUS=STATUS$$module$version;module$version.VERSION=VERSION$$module$version;module$version.LABEL=LABEL$$module$version;module$version.HASH=HASH$$module$version;module$version.DATE=DATE$$module$version;var $jscompDefaultExport$$module$patch={apply(b,a){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){let c;a.seed||(a.seed=parseSeed$$module$patch("").toString(16),window.location.hash+="&seed="+a.seed);c=a.flags?new FlagSet$$module$flagset(a.flags):new FlagSet$$module$flagset("Em Gt Mr Rlpt Sbk Sct Sm Tasd");for(const b in a)"false"===a[b]&&(a[b]=!1);yield shuffle$$module$patch(b,parseSeed$$module$patch(a.seed),c)})}};
...
const shuffle$$module$patch=(b,a,c,d,e)=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){if("number"!==typeof a)throw Error("Bad seed");var f=crc32$$module$crc32(a.toString(16).padStart(8,0)+String(c))>>>0;watchForFlag0$$module$patch.apply(b);rearrangeTriggersAndNpcs$$module$patch.apply(b);c.check("Rd")&&requireHealedDolphinToRide$$module$patch.apply(b);c.check("Rp")&&connectLeafToLimeTree$$module$patch.apply(b);openSwanFromEitherSide$$module$patch.apply(b);f=new Random$$module$random(f);
yield shuffle2$$module$depgraph(b,f,d,c,e);const g=new Rom$$module$rom(b);rescaleMonsters$$module$patch(b,g);c.check("Mr")&&shuffleMonsters$$module$patch(b,g,f);identifyKeyItemsForDifficultyBuffs$$module$patch(g);fixShaking$$module$patch.apply(b);fixOpelStatue$$module$patch.apply(b);upgradeBallsToBracelets$$module$patch.apply(b);preventNpcDespawns$$module$patch.apply(b);allowTeleportOutOfTower$$module$patch.apply(b);c.check("Td")&&buffDeosPendant$$module$patch.apply(b);c.check("Rl")&&barrierRequiresCalmSea$$module$patch.apply(b);
c.check("Fs")&&fixShopGlitch$$module$patch.apply(b);preventSoftlockFromZeroHpOrMp$$module$patch.apply(b);scaleDifficultyLib$$module$patch.apply(b);nerfArmors$$module$patch.apply(b);fixVampire$$module$patch.apply(b);displayDifficulty$$module$patch.apply(b);itemLib$$module$patch.apply(b);c.check("Em")?buffMedicalHerb3x$$module$patch.apply(b):c.check("Hm")||buffMedicalHerb2x$$module$patch.apply(b);c.check("Rt")?asinaTeleportsToMezameShrine$$module$patch.apply(b):noTeleportOnThunderSword$$module$patch.apply(b);
c.check("Ts")&&leatherBootsForSpeed$$module$patch.apply(b);c.check("Di")&&neverDie$$module$patch.apply(b);c.check("Ta")&&autoEquipBracelet$$module$patch.apply(b);c.check("Tw")&&nerfWildWarp$$module$patch.apply(b);return stampVersionSeedAndHash$$module$patch(b,a,c)}),stampVersionSeedAndHash$$module$patch=(b,a,c)=>{const d=crc32$$module$crc32(b),e=d.toString(16).padStart(8,0).toUpperCase(),f="unstable"==STATUS$$module$version?HASH$$module$version.substring(0,7).padStart(7,0).toUpperCase()+"     ":VERSION$$module$version.substring(0,
12).padEnd(12," ");a=a.toString(16).padStart(8,0).toUpperCase();const g=(a,c)=>{for(let d=0;d<c.length;d++)b[a+16+d]=c.charCodeAt(d)},k=(a,b)=>{const c=[];for(let d=0;d<a.length||d<b.length;d++)c.push(a[d]||" "),c.push(b[d]||" ");return c.join("")};g(161743,k("  VERSION     SEED      ",`  ${f}${a}`));a=String(c);let h;if(46<a.length){if(92<a.length)throw Error("Flag string way too long!");h=a.substring(46,92).padEnd(46," ");a=a.substring(0,46)}a=a.padEnd(46," ");g(161791,k(a.substring(0,23),a.substring(23)));
h&&g(161839,k(h.substring(0,23),h.substring(23)));g(161925,k(e.substring(0,4),e.substring(4)));g(153366,"RANDOMIZER");"unstable"==STATUS$$module$version&&g(153404,"BETA");return d},watchForFlag0$$module$patch=buildRomPatch$$module$6502(assemble$$module$6502("\n.bank $3c000 $c000:$4000\n\n;;;; TODO - REMOVE THIS ONCE IT'S SOLVED\n.org $3cb62 ; main game mode jump 08\n  jsr CheckFlag0\n.org $3fe80\nReadControllersWithDirections:\n.org $3d347\nInitiateDialog:\n\n.org $3fdd0 ; !!! - note itemLib (I think) uses this space, too\nCheckFlag0:\n  lda $6480\n  lsr\n  bcc +\n   asl\n   sta $6480\n   lda #$00\n   sta $20\n   sta $21\n   ldx #$0c\n-   lda $6140,x\n    eor #$ff\n    sta $6140,x\n    dex\n   bpl -\n   jsr InitiateDialog\n+ jmp ReadControllersWithDirections\n\n",
"watchForFlag0")),fixShopGlitch$$module$patch=buildRomPatch$$module$6502(assemble$$module$6502("\n.bank $20000 $8000:$2000\n\n.org $217cd\nShop_NothingPressed:\n\n.org $21812\n  jmp Shop_NothingPressed\n","fixShopGlitch")),preventSoftlockFromZeroHpOrMp$$module$patch=buildRomPatch$$module$6502(assemble$$module$6502("\n.bank $3c000 $c000:$4000 ; fixed bank\n.bank $2e000 $a000:$2000\n.org $2fbd5 ; start of unused block\nCheckForLowHpMp:\n  cmp #$05\n  bcs +\n   lda #$05\n+ sta $03c1\n  ;; Check if we've ever found any swords\n  lda $64c0\n  and #$0f\n  ;; If this is zero then we have no swords and should give 20 MP.\n  ;; If it's nonzero, set it to -19 and then we'll add 20 unconditionally.\n  beq +\n   lda #$ed\n+ clc\n  adc #$14\n  ;; Now compare with MP - if it's less, set the minimum.\n  cmp $0708\n  bcc +\n   sta $0708\n+ rts\n.org $2fbf3 ; tight bound, in case we need space on this page later\n.org $2fc00 ; end of unused block\n\n.org $2fd82 ; normally \"sta $03c1\"\n  jsr CheckForLowHpMp\n\n",
"preventSoftlockFromZeroHpOrMp")),openSwanFromEitherSide$$module$patch=buildRomPatch$$module$6502(assemble$$module$6502("\n.bank $3c000 $c000:$4000\n.bank $18000 $8000:$4000\n\n.org $192e7\n  .byte $a3,$ab  ; 73 swan gate\n.org $1aba3 ; empty space at end of npcdata\n  .byte $00,$ff,$09,$6b,$ff\n  .byte $04,$01,$02,$b3\n  .byte $04,$0a,$04,$2c\n  .byte $07,$06,$01,$2d\n  .byte $07,$09,$01,$2d\n  .byte $02,$0a,$01,$2d ; new soldier (they need to come in pairs)\n  .byte $02,$0b,$01,$2d ; new soldier\n  .byte $0a,$0e,$02,$b3 ; new trigger to erase guards\n  .byte $ff\n.org $1ac00\n",
...
,,,8,,,],[247,"p","skeleton shot",,,,11,,,],[248,"p","lavaman shot",,,,14,,,],[249,"p","black knight flail",,,,18,,,],[250,"p","lizardman flail",,,,21,,,],[252,"p","mado shuriken",,,,36,,,],[253,"p","guardian statue missile",,,,23,,,],[254,"p","demon wall fire",,,,23,,,]].map((b)=>{var [a,c,d,e=0,f=0,g=0,k=0,h=0,m=0]=b;return[a,{id:a,type:c,name:d,sdef:e,swrd:f,hits:g,satk:k,dgld:h,sexp:m}]}));
...
this.report["$"+d.id.toString(16).padStart(2,0)]=[];const {maxFlyers:g,nonFlyers:k={},fixedSlots:h={}}=MONSTER_ADJUSTMENTS$$module$patch[d.id]||{};let m=h.pat0||null,r=h.pat1||null,n=h.pal2||null,t=h.pal3||null,l=g,p=!1;for(const b of d.objects||[])if(2==(b[2]&7)&&(p=!0),!(b[2]&7)){var a=b[3]+80;126==a||127==a||159==a?r=98:143==a&&(m=97)}if(21==d.id||71==d.id)p=!1;f.push(`Initial pass: ${[p,m,r,n,t].join(", ")}`);a=(a)=>{var b=FLYERS$$module$patch.has(a.id),c=MOTHS_AND_BATS$$module$patch.has(a.id);
if(b){if(!l)return!1;--l}if(null!=n&&null!=a.pal2&&n!=a.pal2||null!=t&&null!=a.pal3&&t!=a.pal3)return!1;var g=!p||TREASURE_CHEST_BANKS$$module$patch.has(a.pat);let h;if(d.rom.objects[a.id].child||RETAIN_SLOTS$$module$patch.has(a.id)){const b=(h=a.patSlot?128:0)?r:m;if(null!=b&&b!=a.pat)return!1;if(h)r=a.pat;else if(g)m=a.pat;else return!1;f.push(`  Adding ${a.id.toString(16)}: pat(${h}) <-  ${a.pat.toString(16)}`)}else if(null==m&&g||m==a.pat)m=a.pat,h=0,f.push(`  Adding ${a.id.toString(16)}: pat0 <-  ${a.pat.toString(16)}`);
...
module$patch.watchForFlag0=watchForFlag0$$module$patch;module$patch.fixShopGlitch=fixShopGlitch$$module$patch;module$patch.preventSoftlockFromZeroHpOrMp=preventSoftlockFromZeroHpOrMp$$module$patch;module$patch.openSwanFromEitherSide=openSwanFromEitherSide$$module$patch;module$patch.connectLeafToLimeTree=connectLeafToLimeTree$$module$patch;module$patch.rearrangeTriggersAndNpcs=rearrangeTriggersAndNpcs$$module$patch;module$patch.fixOpelStatue=fixOpelStatue$$module$patch;
module$patch.requireHealedDolphinToRide=requireHealedDolphinToRide$$module$patch;module$patch.buffDeosPendant=buffDeosPendant$$module$patch;module$patch.allowTeleportOutOfTower=allowTeleportOutOfTower$$module$patch;module$patch.preventNpcDespawns=preventNpcDespawns$$module$patch;module$patch.barrierRequiresCalmSea=barrierRequiresCalmSea$$module$patch;module$patch.noTeleportOnThunderSword=noTeleportOnThunderSword$$module$patch;module$patch.asinaTeleportsToMezameShrine=asinaTeleportsToMezameShrine$$module$patch;
module$patch.saharaBunniesRequireTelepathy=saharaBunniesRequireTelepathy$$module$patch;module$patch.fixShaking=fixShaking$$module$patch;module$patch.displayDifficulty=displayDifficulty$$module$patch;module$patch.buffMedicalHerb3x=buffMedicalHerb3x$$module$patch;module$patch.buffMedicalHerb2x=buffMedicalHerb2x$$module$patch;module$patch.removeWildWarp=removeWildWarp$$module$patch;module$patch.nerfWildWarp=nerfWildWarp$$module$patch;module$patch.preventSwordClobber=preventSwordClobber$$module$patch;
module$patch.upgradeBallsToBracelets=upgradeBallsToBracelets$$module$patch;module$patch.neverDie=neverDie$$module$patch;module$patch.fixVampire=fixVampire$$module$patch;module$patch.itemLib=itemLib$$module$patch;module$patch.quickChangeItems=quickChangeItems$$module$patch;module$patch.leatherBootsForSpeed=leatherBootsForSpeed$$module$patch;module$patch.autoEquipBracelet=autoEquipBracelet$$module$patch;module$patch.scaleDifficultyLib=scaleDifficultyLib$$module$patch;class ProgressTracker$$module$progress{constructor(){this.completed=this.tasks=0}addTasks(b){this.tasks+=b}addCompleted(b){this.completed+=b}value(){return this.tasks&&this.completed/this.tasks}}var module$progress={};module$progress.ProgressTracker=ProgressTracker$$module$progress;let flags$$module$main,seed$$module$main,rom$$module$main,romName$$module$main,race$$module$main=!1;
...
k();f=yield shuffle$$module$patch(d,b,f,g,c);e=!0;document.body.classList.remove("shuffling");g&&(replaceSpoiler$$module$main("spoiler-items",g.items.map((a)=>a.text)),replaceSpoiler$$module$main("spoiler-route",g.route));document.getElementById("checksum").textContent=read$$module$main(d,161941,4)+read$$module$main(d,161942,4);return[d,f]}),replaceSpoiler$$module$main=(b,a)=>{for(b=document.getElementById(b);b.children.length;)b.lastChild.remove();for(const c of a)a=document.createElement("li"),
a.textContent=c,b.appendChild(a)},setPresets$$module$main=()=>{var b=document.querySelector("#preset :checked");document.getElementById("preset-explanation").textContent=b.dataset.text;b=b.dataset.flags;"unstable"==VERSION$$module$version&&(b+=" Ds");document.getElementById("flagstring").value=b},makeCheckbox$$module$main=(b)=>{const a=b.getElementsByClassName("flag-body")[0];a&&a.remove();const [c,d]=b.textContent.split(/:\s*/),e=document.createElement("input");e.type="checkbox";e.id=`flag-${c}`;
e.dataset.flag=c;b.parentElement.insertBefore(e,b);const f=document.createElement("label");f.textContent=c;f.htmlFor=e.id;b.parentElement.insertBefore(f,b);const g=document.createElement("div");b.parentElement.insertBefore(g,b);const k=document.createElement("label");k.textContent=d;k.htmlFor=e.id;g.appendChild(k);a&&g.appendChild(a);b.classList.contains("hard")&&(f.classList.add("hard"),k.classList.add("hard"),k.textContent+=" *");b.remove();e.addEventListener("change",()=>{window.FLAGS=flags$$module$main;
flags$$module$main.set(c,e.checked);updateDom$$module$main()})},updateDom$$module$main=()=>{for(var b of document.querySelectorAll("input[data-flag]"))b.checked=flags$$module$main.check(b.dataset.flag);b=String(flags$$module$main).replace(/ /g,"");document.getElementById("seed").value=seed$$module$main||"";var a=["#flags=",b];seed$$module$main&&a.push("&seed=",encodeURIComponent(seed$$module$main));history.replaceState({flags:flags$$module$main.flags,seed:seed$$module$main},"",String(window.location).replace(/#.*/,
"")+a.join(""));if("stable"==STATUS$$module$version||"rc"==STATUS$$module$version){a=seed$$module$main||Math.floor(4294967296*Math.random()).toString(16);const c=VERSION$$module$version;document.getElementById("race").href=`/${c}/race#flags=${b}&seed=${a}`}updateRaceDom$$module$main()},updateRaceDom$$module$main=()=>{const b=String(flags$$module$main);document.body.classList.toggle("spoiled",flags$$module$main.check("Ds"));document.body.classList.toggle("debug-mode",/D/.test(b));document.getElementById("flagstring-out").textContent=
b;document.getElementById("track-url").href=`track#flags=${b.replace(/ /g,"")}`},loadRomFromStorage$$module$main=()=>{const b=window.localStorage.getItem("name"),a=window.localStorage.getItem("rom"),c=document.getElementById("pick-file"),d=()=>{document.body.classList.add("rom-uploaded");document.body.classList.toggle("rom-broken",crc32$$module$crc32(rom$$module$main)!=EXPECTED_CRC32$$module$rom)};b&&a&&(romName$$module$main=b,rom$$module$main=Uint8Array.from(Array(a.length/2).fill(0).map((b,c)=>
Number.parseInt(a[2*c]+a[2*c+1],16))),d());c.addEventListener("change",()=>{const a=c.files[0],b=new FileReader;b.addEventListener("loadend",()=>{const c=new Uint8Array(b.result),e=Array.from(c,(a)=>a.toString(16).padStart(2,0)).join("");window.localStorage.setItem("rom",e);window.localStorage.setItem("name",a.name);rom$$module$main=c.slice(0,393232);d();romName$$module$main=a.name});b.readAsArrayBuffer(a)})},download$$module$main=(b,a)=>{const c=document.createElement("a");document.body.appendChild(c);
c.style="display: none";b=new Blob([b],{type:"octet/stream"});b=window.URL.createObjectURL(b);c.href=b;c.download=a;c.click();window.URL.revokeObjectURL(b);c.remove()};main$$module$main();var module$main={};
}).call(this)
