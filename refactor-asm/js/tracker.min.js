...
return Math.min(AM$$module$js$random*this.iy,RNMX$$module$js$random)}nextInt(b){return Math.floor(this.next()*b)}shuffle(b){for(let a=b.length;a;){const c=this.nextInt(a--);[b[a],b[c]]=[b[c],b[a]]}return b}}var module$js$random={};module$js$random.Random=Random$$module$js$random;const STATUS$$module$js$version="unstable",VERSION$$module$js$version="refactor-asm",LABEL$$module$js$version="refactor-asm e9aecee",HASH$$module$js$version="e9aeceeb809ad0b1cc049cfe454f5061d95e45d5",DATE$$module$js$version=new Date(1553056224E3);var module$js$version={};module$js$version.STATUS=STATUS$$module$js$version;module$js$version.VERSION=VERSION$$module$js$version;module$js$version.LABEL=LABEL$$module$js$version;module$js$version.HASH=HASH$$module$js$version;module$js$version.DATE=DATE$$module$js$version;var $jscompDefaultExport$$module$js$patch={apply(b,a){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){let c;a.seed||(a.seed=parseSeed$$module$js$patch("").toString(16),window.location.hash+="&seed="+a.seed);c=a.flags?new FlagSet$$module$js$flagset(a.flags):new FlagSet$$module$js$flagset("Em Gt Mr Rlpt Sbk Sct Sm Tasd");for(const b in a)"false"===a[b]&&(a[b]=!1);yield shuffle$$module$js$patch(b,parseSeed$$module$js$patch(a.seed),c)})}};
...
const shuffle$$module$js$patch=(b,a,c,d,e)=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){if("number"!==typeof a)throw Error("Bad seed");var f=crc32$$module$js$crc32(a.toString(16).padStart(8,0)+String(c))>>>0;watchForFlag0$$module$js$patch.apply(b);rearrangeTriggersAndNpcs$$module$js$patch.apply(b);c.check("Rd")&&requireHealedDolphinToRide$$module$js$patch.apply(b);c.check("Rp")&&connectLeafToLimeTree$$module$js$patch.apply(b);openSwanFromEitherSide$$module$js$patch.apply(b);f=new Random$$module$js$random(f);
yield shuffle2$$module$js$depgraph(b,f,d,c,e);const g=new Rom$$module$js$rom(b);rescaleMonsters$$module$js$patch(b,g);c.check("Mr")&&shuffleMonsters$$module$js$patch(b,g,f);identifyKeyItemsForDifficultyBuffs$$module$js$patch(g);fixShaking$$module$js$patch.apply(b);fixOpelStatue$$module$js$patch.apply(b);upgradeBallsToBracelets$$module$js$patch.apply(b);preventNpcDespawns$$module$js$patch.apply(b);allowTeleportOutOfTower$$module$js$patch.apply(b);c.check("Td")&&buffDeosPendant$$module$js$patch.apply(b);
c.check("Rl")&&barrierRequiresCalmSea$$module$js$patch.apply(b);c.check("Fs")&&fixShopGlitch$$module$js$patch.apply(b);preventSoftlockFromZeroHpOrMp$$module$js$patch.apply(b);scaleDifficultyLib$$module$js$patch.apply(b);nerfArmors$$module$js$patch.apply(b);fixVampire$$module$js$patch.apply(b);displayDifficulty$$module$js$patch.apply(b);itemLib$$module$js$patch.apply(b);c.check("Em")?buffMedicalHerb3x$$module$js$patch.apply(b):c.check("Hm")||buffMedicalHerb2x$$module$js$patch.apply(b);c.check("Rt")?
asinaTeleportsToMezameShrine$$module$js$patch.apply(b):noTeleportOnThunderSword$$module$js$patch.apply(b);c.check("Ts")&&leatherBootsForSpeed$$module$js$patch.apply(b);c.check("Di")&&neverDie$$module$js$patch.apply(b);c.check("Ta")&&autoEquipBracelet$$module$js$patch.apply(b);c.check("Tw")&&nerfWildWarp$$module$js$patch.apply(b);return stampVersionSeedAndHash$$module$js$patch(b,a,c)}),stampVersionSeedAndHash$$module$js$patch=(b,a,c)=>{const d=crc32$$module$js$crc32(b),e=d.toString(16).padStart(8,
0).toUpperCase(),f="unstable"==STATUS$$module$js$version?HASH$$module$js$version.substring(0,7).padStart(7,0).toUpperCase()+"     ":VERSION$$module$js$version.substring(0,12).padEnd(12," ");a=a.toString(16).padStart(8,0).toUpperCase();const g=(a,c)=>{for(let d=0;d<c.length;d++)b[a+16+d]=c.charCodeAt(d)},k=(a,b)=>{const c=[];for(let d=0;d<a.length||d<b.length;d++)c.push(a[d]||" "),c.push(b[d]||" ");return c.join("")};g(161743,k("  VERSION     SEED      ",`  ${f}${a}`));a=String(c);let h;if(46<a.length){if(92<
a.length)throw Error("Flag string way too long!");h=a.substring(46,92).padEnd(46," ");a=a.substring(0,46)}a=a.padEnd(46," ");g(161791,k(a.substring(0,23),a.substring(23)));h&&g(161839,k(h.substring(0,23),h.substring(23)));g(161925,k(e.substring(0,4),e.substring(4)));g(153366,"RANDOMIZER");"unstable"==STATUS$$module$js$version&&g(153404,"BETA");return d},watchForFlag0$$module$js$patch=buildRomPatch$$module$js$6502(assemble$$module$js$6502("\n.bank $3c000 $c000:$4000\n\n;;;; TODO - REMOVE THIS ONCE IT'S SOLVED\n.org $3cb62 ; main game mode jump 08\n  jsr CheckFlag0\n.org $3fe80\nReadControllersWithDirections:\n.org $3d347\nInitiateDialog:\n\n.org $3fdd0 ; !!! - note itemLib (I think) uses this space, too\nCheckFlag0:\n  lda $6480\n  lsr\n  bcc +\n   asl\n   sta $6480\n   lda #$00\n   sta $20\n   sta $21\n   ldx #$0c\n-   lda $6140,x\n    eor #$ff\n    sta $6140,x\n    dex\n   bpl -\n   jsr InitiateDialog\n+ jmp ReadControllersWithDirections\n\n",
"watchForFlag0")),fixShopGlitch$$module$js$patch=buildRomPatch$$module$js$6502(assemble$$module$js$6502("\n.bank $20000 $8000:$2000\n\n.org $217cd\nShop_NothingPressed:\n\n.org $21812\n  jmp Shop_NothingPressed\n","fixShopGlitch")),preventSoftlockFromZeroHpOrMp$$module$js$patch=buildRomPatch$$module$js$6502(assemble$$module$js$6502("\n.bank $3c000 $c000:$4000 ; fixed bank\n.bank $2e000 $a000:$2000\n.org $2fbd5 ; start of unused block\nCheckForLowHpMp:\n  cmp #$05\n  bcs +\n   lda #$05\n+ sta $03c1\n  ;; Check if we've ever found any swords\n  lda $64c0\n  and #$0f\n  ;; If this is zero then we have no swords and should give 20 MP.\n  ;; If it's nonzero, set it to -19 and then we'll add 20 unconditionally.\n  beq +\n   lda #$ed\n+ clc\n  adc #$14\n  ;; Now compare with MP - if it's less, set the minimum.\n  cmp $0708\n  bcc +\n   sta $0708\n+ rts\n.org $2fbf3 ; tight bound, in case we need space on this page later\n.org $2fc00 ; end of unused block\n\n.org $2fd82 ; normally \"sta $03c1\"\n  jsr CheckForLowHpMp\n\n",
...
module$js$patch.stampVersionSeedAndHash=stampVersionSeedAndHash$$module$js$patch;module$js$patch.watchForFlag0=watchForFlag0$$module$js$patch;module$js$patch.fixShopGlitch=fixShopGlitch$$module$js$patch;module$js$patch.preventSoftlockFromZeroHpOrMp=preventSoftlockFromZeroHpOrMp$$module$js$patch;module$js$patch.openSwanFromEitherSide=openSwanFromEitherSide$$module$js$patch;module$js$patch.connectLeafToLimeTree=connectLeafToLimeTree$$module$js$patch;module$js$patch.rearrangeTriggersAndNpcs=rearrangeTriggersAndNpcs$$module$js$patch;
module$js$patch.fixOpelStatue=fixOpelStatue$$module$js$patch;module$js$patch.requireHealedDolphinToRide=requireHealedDolphinToRide$$module$js$patch;module$js$patch.buffDeosPendant=buffDeosPendant$$module$js$patch;module$js$patch.allowTeleportOutOfTower=allowTeleportOutOfTower$$module$js$patch;module$js$patch.preventNpcDespawns=preventNpcDespawns$$module$js$patch;module$js$patch.barrierRequiresCalmSea=barrierRequiresCalmSea$$module$js$patch;module$js$patch.noTeleportOnThunderSword=noTeleportOnThunderSword$$module$js$patch;
module$js$patch.asinaTeleportsToMezameShrine=asinaTeleportsToMezameShrine$$module$js$patch;module$js$patch.saharaBunniesRequireTelepathy=saharaBunniesRequireTelepathy$$module$js$patch;module$js$patch.fixShaking=fixShaking$$module$js$patch;module$js$patch.displayDifficulty=displayDifficulty$$module$js$patch;module$js$patch.buffMedicalHerb3x=buffMedicalHerb3x$$module$js$patch;module$js$patch.buffMedicalHerb2x=buffMedicalHerb2x$$module$js$patch;module$js$patch.removeWildWarp=removeWildWarp$$module$js$patch;
module$js$patch.nerfWildWarp=nerfWildWarp$$module$js$patch;module$js$patch.preventSwordClobber=preventSwordClobber$$module$js$patch;module$js$patch.upgradeBallsToBracelets=upgradeBallsToBracelets$$module$js$patch;module$js$patch.neverDie=neverDie$$module$js$patch;module$js$patch.fixVampire=fixVampire$$module$js$patch;module$js$patch.itemLib=itemLib$$module$js$patch;module$js$patch.quickChangeItems=quickChangeItems$$module$js$patch;module$js$patch.leatherBootsForSpeed=leatherBootsForSpeed$$module$js$patch;
module$js$patch.autoEquipBracelet=autoEquipBracelet$$module$js$patch;module$js$patch.scaleDifficultyLib=scaleDifficultyLib$$module$js$patch;const BOXES$$module$js$tracker="\nsword-of-wind $00 WIND\nsword-of-fire $01 FIRE\nsword-of-water $02 WATER\nsword-of-thunder $03 THUN\nwindmill-key $32 MILL\nstatue-of-onyx $25 ONYX\ninsect-flute $27 INSECT\nkey-to-prison $33 PRISON\nflute-of-lime $28 LIME\n# flute-of-lime $5b LIME\n\nball-of-wind $05 \nball-of-fire $07\nball-of-water $09\nball-of-thunder $0b\nkirisa-plant $3c KIRISA\nfog-lamp $35 FOG\nshell-flute $36 SHELL\nbroken-statue $38 BRKN\neye-glasses $37 EYE\nglowing-lamp $39 GLOW\n\ntornado-bracelet $06\nflame-bracelet $08\nblizzard-bracelet $0a\nstorm-bracelet $0c\nlove-pendant $3b LOVE\nkey-to-styx $34 STXY\nopel-statue $26 OPEL\nfruit-of-repun $23 REPUN\nsacred-shield $12 SCRD\nivory-statue $3d IVORY\n\nrabbit-boots $2e RABBIT\ngas-mask $29 GAS\nshield-ring $30 SHIELD\niron-necklace $2c IRON\nleather-boots $2f LTHR\npower-ring $2a POWER\nwarrior-ring $2b WARR\ndeos-pendant $2d DEO\nbow-of-moon $3e MOON\nbow-of-sun $3f SUN\n\nrefresh $41\nparalysis $42\ntelepathy $43\nteleport $44\nrecover $45\nbarrier $46\nchange $47\nflight $48\npsycho-armor $1c PSYCH\nbow-of-truth $40 TRUTH\n",
