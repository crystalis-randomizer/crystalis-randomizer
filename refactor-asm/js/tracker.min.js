...
$jscomp.polyfill("Array.prototype.includes",function(b){return b?b:function(a,b){var c=this;c instanceof String&&(c=String(c));var e=c.length;b=b||0;for(0>b&&(b=Math.max(b+e,0));b<e;b++){var f=c[b];if(f===a||Object.is(f,a))return!0}return!1}},"es7","es3");const LOG$$module$js$6502=!1;
class Assembler$$module$js$6502{constructor(){this.labels={};this.allChunks=[]}assemble(b,a="input"){a=new File$$module$js$6502(this.labels,a);for(const c of b.split("\n"))a.ingest(c);b=a.assemble();this.allChunks.push(...b)}chunks(){return[...this.allChunks]}patch(){return Patch$$module$js$6502.from(this.allChunks)}patchRom(b){buildRomPatch$$module$js$6502(this.patch()).apply(b);this.allChunks=[]}}
...
else if(a=/^\s*\.assert\s+(<\s*)?(\S+)/i.exec(b))this.addLine(new AssertLine$$module$js$6502(this.pc=parseNumber$$module$js$6502(a[2]),!a[1]));else if(a=/^\s*\.bank\s+(\S+)\s+(\S+)\s*:\s*(\S+)/i.exec(b)){const [,b,c,f]=a;this.addLine(new BankLine$$module$js$6502(parseNumber$$module$js$6502(b),parseNumber$$module$js$6502(c),parseNumber$$module$js$6502(f)))}else if(a=/^\s*\.(byte|word)\s+(.*)/i.exec(b))a=("word"==a[1]?WordLine$$module$js$6502:ByteLine$$module$js$6502).parse(a[2]),this.addLine(a),this.pc+=
a.size();else if(a=/^define\s+(\S+)\s+(.*)/.exec(b))this.addLabel(a[1],parseNumber$$module$js$6502(a[2]));else{if(a=/^(\S+?):(.*)$/.exec(b)){var c=a[1];b=" "+a[2];this.addLabel(c,~this.pc)}else if(a=/^((?:[-+]+\s+)+)(.*)$/.exec(b)){b=a[1];for(c of b.trim().split(" "))this.addLabel(c,~this.pc);b=" "+a[2]}if(a=/^\s+([a-z]{3})(\s+.*)?$/.exec(b))a=new Opcode$$module$js$6502(a[1],(a[2]||"").trim(),this.pc),this.addLine(a),this.pc+=a.size();else if(/\S/.test(b))throw Error(`Could not parse line ${b} at ${this.filename}:${this.lineNumber}`);
}}assemble(){var b=new Context$$module$js$6502(this.labels),a=[];for(var c of this.lines){try{c.expand(b)}catch(e){throw a=e.stack.replace(`Error: ${e.message}`,""),Error(`${e.message}${` from line ${c.origLineNumber+1}: \`${c.origContent}\``}${a}\n================`);}if(c instanceof OrgLine$$module$js$6502&&null!=a[c.pc])throw Error(`Collision at $${c.pc.toString(16)}`);for(const d of c.bytes()){if(null!=a[b.pc])throw Error(`Collision at $${b.pc.toString(16)}`);a[b.pc++]=d}}b=[];for(var d in a)d-
1 in a||b.push(Number(d));c=[];for(const e of b){d=[];for(b=e;b in a;b++)d.push(a[b]);c.push(new Chunk$$module$js$6502(e,d))}if(this.conditions.length)throw Error("Unterminated .if");return c}}class AbstractLine$$module$js$6502{orig(b,a){this.origLineNumber=b;this.origContent=a;return this}expand(){throw Error(`abstract: ${this.constructor}`);}bytes(){throw Error(`abstract: ${this.constructor}`);}}
...
(new Condition$$module$js$nodes(a,"Sword charge glitch")).option(Ld),re=(new Condition$$module$js$nodes(a,"Rabbit skip")).option(q),se=(new Condition$$module$js$nodes(a,"Teleport skip")).option(l),wb=(new Condition$$module$js$nodes(a,"Statue glitch")).option(t),te=(new Condition$$module$js$nodes(a,"Wild warp")).option(Jd,Kd),Va=(new Condition$$module$js$nodes(a,"Any level 2 sword")).option(Q,Ha).option(Q,za).option(R,Ia).option(R,Ja).option(L,Ka).option(L,La).option(I,Ma).option(I,Na),v=(new Condition$$module$js$nodes(a,
"Destroy stone")).option(Q,Ha).option(Q,za).option(Ua,Q,Va),w=(new Condition$$module$js$nodes(a,"Destroy ice")).option(R,Ia).option(R,Ja).option(Ua,R,Va),r=(new Condition$$module$js$nodes(a,"Cross rivers")).option(L,Ka).option(L,La).option(A).option(Ua,L,Va),oa=(new Condition$$module$js$nodes(a,"Destroy iron")).option(I,Ma).option(I,Na).option(Ua,I,Va),S=(new Condition$$module$js$nodes(a,"Any sword")).option(Q).option(R).option(L).option(I),ue=(new Condition$$module$js$nodes(a,"Match insect sword (fire/water/thunder)")).option(R).option(L).option(I).option(Pa,
D.value?D:h,Q),Dc=(new Condition$$module$js$nodes(a,"Speed boots")).option(Tb,n),Ec=(new Condition$$module$js$nodes(a,"Climb slopes")).option(nb).option(A).option(Dc),ve=(new Condition$$module$js$nodes(a,"Enter Mt Sabre North (rabbit)")).option(pc).option(re),we=(new Condition$$module$js$nodes(a,"Enter Mt Sabre North (Teleport)")).option(gc).option(se,A),Fc=(new Condition$$module$js$nodes(a,"Asina in her room")).option(uc),xe=(new Condition$$module$js$nodes(a,"Paralysis or Ball of Water")).option(Ra).option(Fc).option(wb),
Gc=(new Condition$$module$js$nodes(a,"Ride dolphin")).option(pb,xc),ye=(new Condition$$module$js$nodes(a,"Ghetto flight")).option(p,Gc,nb),T=(new Condition$$module$js$nodes(a,"Cross sea")).option(Gc).option(A),ze=(new Condition$$module$js$nodes(a,"Cross whirlpool")).option(ub).option(A).option(ye),J=(new Condition$$module$js$nodes(a,"Refresh if needed")).option(Pb.value?Pb:h).option(Qa),Hc=(new Condition$$module$js$nodes(a,"Wind magic")).option(y.value?y:h,J).option(Ha,za,J),Ic=(new Condition$$module$js$nodes(a,
...
r),Sc=(new Location$$module$js$nodes(a,21,ia,"East")).overworld().connect(ua).chest(Rb,24),Ea=(new Location$$module$js$nodes(a,24,qa,"Town")).town().connect(ua).trigger(Zd,Rb),jf=(new Location$$module$js$nodes(a,25,ia,"Outside Stom's House")).town().connect(ua),zb=(new Location$$module$js$nodes(a,26,ia,"Swamp")).overworld().connect(Sc,Wa).trigger(sb,nc,Wa);(new Location$$module$js$nodes(a,26,ia,"Swamp Insect Area")).overworld().connect(zb,Wa).boss(Je);const Za=(new Location$$module$js$nodes(a,27,
...
const Ff=(new Location$$module$js$nodes(a,60,Xa,"Inn")).town(),Gf=(new Location$$module$js$nodes(a,61,Xa,"Tool Shop")).house(),Hf=(new Location$$module$js$nodes(a,62,Xa,"Back Room")).house(),Cb=(new Location$$module$js$nodes(a,64,Z,"Summit")).overworld().connect(Ef),cb=(new Location$$module$js$nodes(a,64,Z,"Northwest")).overworld().from(Cb).to(Cb,A),If=(new Location$$module$js$nodes(a,64,Z,"Northeast")).overworld().connect(cb,r),ad=(new Location$$module$js$nodes(a,65,Z,"Southwest")).overworld().connect(cb),
Jf=(new Location$$module$js$nodes(a,65,Z,"Southeast")).overworld().connect(ad,r),bd=(new Location$$module$js$nodes(a,66,Z,"Lime Tree Valley")).overworld().connect(ad).connect(Da,Id),Kf=(new Location$$module$js$nodes(a,67,Z,"Lime Tree Lake (Rage)")).cave().connect(bd).trigger(tc,L),Lf=(new Location$$module$js$nodes(a,68,ba,"Tunnel 1a (entrance)")).cave().connect(Jf),Mf=(new Location$$module$js$nodes(a,68,ba,"Tunnel 1b (behind wall)")).cave().connect(Lf,v),cd=(new Location$$module$js$nodes(a,69,ba,
...
na),fb=(new Location$$module$js$nodes(a,120,la,"Valley")).overworld().connect(xg),Hb=(new Location$$module$js$nodes(a,124,x,"Entrance")).overworld().connect(fb),yg=(new Location$$module$js$nodes(a,124,x,"Over first river toward Shyron")).overworld().connect(Hb,r),sd=(new Location$$module$js$nodes(a,124,x,"After first tunnel")).overworld(),td=(new Location$$module$js$nodes(a,124,x,"Door to Styx")).overworld().connect(sd,r),zg=(new Location$$module$js$nodes(a,124,x,"Dead end (no item)")).overworld(),
...
171,O,"Dead End Behind Iron (fruit of power)")).fortress().connect(hb,oa).chest(Aa,28,98);(new Location$$module$js$nodes(a,171,O,"Dead End Loop Across Closer Bridges")).fortress().connect(hb,r).connect(hb,r);const hh=(new Location$$module$js$nodes(a,171,O,"Across First Bridge (fruit of repun)")).fortress().connect(hb,r).chest(Qb,30,102),Lb=(new Location$$module$js$nodes(a,171,O,"Across Second Bridge")).fortress().connect(hh,r);(new Location$$module$js$nodes(a,171,O,"Dead End Across Two Bridges ()")).fortress().connect(Lb,
r).connect(Lb,r).chest(kb,29,93);const ih=(new Location$$module$js$nodes(a,171,O,"Across Third Bridge")).fortress().connect(Lb,r),jh=(new Location$$module$js$nodes(a,171,O,"Exit Behind Iron Door")).fortress().connect(ih,oa),kh=(new Location$$module$js$nodes(a,172,O,"Boss")).fortress().connect(jh).boss(Oe),lh=(new Location$$module$js$nodes(a,172,O,"Tornel")).fortress().connect(kh),Cd=(new Location$$module$js$nodes(a,173,sa,"Lower")).fortress().connect(lh).chest(mb,26,99).chest(Y,27,111),mh=(new Location$$module$js$nodes(a,
...
var g={};for(var k of f.nodes)if(k instanceof Slot$$module$js$nodes&&k.slots){var h=k.slotType[0];(g[h]=g[h]||[]).push(k)}k=[];h=[];var m={};for(var n of d.flags.S){var p=[];for(var t of n){m[t]=!0;for(var l of g[t])p.push(l),h.push([l,k.length])}k.push(p)}for(var q of"bc")if(m[q])for(n=g[q],t=n.map((a)=>[a.item,a.itemIndex]),a.shuffle(t),l=0;l<n.length;l++)n[l].set(...t[l]);p=(a)=>f.nodes.find((b)=>b instanceof Slot$$module$js$nodes&&b.name===a);g=p("Sword of Wind");q=p("Sword of Fire");n=p("Ball of Wind");
t=p("Ball of Fire");l=p("Statue of Onyx");p=p("Gas Mask");m.k&&a.nextInt(2)&&(q.swap(g),t.swap(n));m.k&&!a.nextInt(3)&&l.swap(p);m=[1,1,1,1,2,2,2,3,3,3,5,5,8,13];g=[];e&&e.addTasks(10);for(q=0;1E3>q;q++){e&&0==q%100&&(yield new Promise(requestAnimationFrame),e.addCompleted(1));a.shuffle(h);for(var y of k)a.shuffle(y);n=k.map(()=>-1);t=m[a.nextInt(m.length)];l=[];p=(a,b)=>a.itemIndex===b.itemIndex||a.isMimic()&&!b.canHoldMimic()||b.isMimic()&&!a.canHoldMimic()||a.needsChest()&&!b.isChest()||b.needsChest()&&
!a.isChest()?!1:!0;for(let a=0;a<t;a++){const [b,c]=h[a],d=k[c][++n[c]];p(b,d)&&(b.swap(d),l.push([b,d]))}if(!l.length)continue;const {win:b,path:c}=f.traverse();if(b)g=c;else for(;l.length;){const [a,b]=l.pop();a.swap(b)}}for(const a of f.nodes)a instanceof Slot$$module$js$nodes&&a.write(b);if(c){c.items=[];c.route=[];for(const a of g){const [b,d]=a;c.route.push(d);null!=b.slotName&&(y=b.slotName,0>y.indexOf(b.orig)&&(y+=` (normally ${b.orig})`),c.items.push({slotIndex:b.slotIndex,itemIndex:b.itemIndex,
...
}),shuffle3$$module$js$depgraph=(b,a,c,d,e,f,g)=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){var g=b.nodes.filter((a)=>a instanceof Slot$$module$js$nodes&&a.slots&&a.slotName),h=new Map(d.shuffle(g.map((a)=>[a,[a.item,a.itemIndex]])));const m=new Set(d.shuffle(g)),n=new Map,p={};f||(f=new FlagSet$$module$js$flagset("Sbkm Sct"));f.flags.S.forEach((a,b)=>{for(const c of a)p[c]=b});for(var t of m)n.set(t.item,t);const l=(a,b)=>null==p[a.slotType?a.slotType[0]:"c"]?a==b:b.isMimic()&&!a.canHoldMimic()||
"unique"!=b.item.inventoryRow&&a.requiresUnique||"armor"==b.item.inventoryRow&&!a.isChest()?!1:p[a.slotType?a.slotType[0]:"c"]==p[b.slotType?b.slotType[0]:"c"];t=a.assumedFill(d,(a,b)=>l(a,n.get(b)));if(null==t)return!1;const q=new Map;for(let b=0;b<t.length;b++){var y=t[b];if(null!=y){var D=n.get(a.item(y));y=h.get(D);h.delete(D);D=a.location(b);q.set(D,y);m.delete(D)}}t=(a,b)=>{for(const c of m)if(l(c,a)){q.set(c,b);m.delete(c);return}return!1};for(const a of h){const [b,c]=a;b.isMimic()&&(t(b,
c),h.delete(b))}for(const a of h){const [b,c]=a;b.needsChest()&&(t(b,c),h.delete(b))}for(const a of h){const [b,c]=a;t(b,c)}for(const a of q){const [b,c]=a;b.set(...c)}const {win:Ga,path:ib}=b.traverse({wanted:g,dfs:!1});if(!Ga)return!1;if(c)for(const a of b.nodes)a instanceof Slot$$module$js$nodes&&a.write(c);if(!e)return!0;e.items=[];e.route=[];for(const a of ib){const [c,d]=a;e.route.push(d);g=b.nodes[c];null!=g.slotName&&(h=g.slotName,0>h.indexOf(g.orig)&&(h+=` (normally ${g.vanillaItemName})`),
...
class WorldGraph$$module$js$nodes extends Graph$$module$js$graph{integrate(b={}){const {tracker:a=!1}=b;var c=!a;const d=new SparseDependencyGraph$$module$js$graph(this.nodes.length),e=[],f=[],g=new Set,k=new Map;var h=[],m=[];const n=[];var p=[];const t=[];b=new Set;for(var l of this.nodes)if(l instanceof Location$$module$js$nodes){if(l.isStart){const [a]=d.addRoute([l.uid]);k.set(a.label,a)}e[l.uid]=[];f[l.uid]=[];for(const a of l.connections)g.has(a)||(g.add(a),a.bidi&&g.add(a.reverse()));for(const a of l.chests)d.addRoute([a.uid,
l.uid])}else if(l instanceof Slot$$module$js$nodes&&"Alarm Flute"===l.item.name)d.addRoute([l.item.uid]),d.finalize(l.item.uid);else if(l instanceof ItemGet$$module$js$nodes&&"Medical Herb"===l.name&&(d.addRoute([l.uid]),d.finalize(l.uid)),l instanceof Option$$module$js$nodes)h.push(l);else if(l instanceof TrackerNode$$module$js$nodes)m.push(l);else if(l instanceof Condition$$module$js$nodes)n.push(l);else if(l instanceof Trigger$$module$js$nodes)p.push(l);else if(l instanceof ItemGet$$module$js$nodes)t.push(l);
else if(l instanceof Slot$$module$js$nodes)b.add(l);else if(!(l instanceof Area$$module$js$nodes))throw Error(`Unknown node type: ${l.nodeType}`);for(const a of g)e[a.from.uid].push(a),f[a.to.uid].push(a);l=(b)=>{for(const c of b){for(const b of c.edges({tracker:a}))d.addRoute(b);d.finalize(c.uid)}};l(h);c&&l(m);l(p);for(c=k.values();!(h=c.next()).done;){h=h.value;m=h.target;for(var q of e[m]){m=[q.to.uid,...h.deps];for(p=q.deps.length-1;0<=p;p--)m.push(q.deps[p].uid);q.from.bossNode&&m.push(q.from.bossNode.uid);
for(const a of d.addRoute(m))k.has(a.label)||k.set(a.label,a)}}l(n);for(q=0;q<this.nodes.length;q++)this.nodes[q]instanceof ItemGet$$module$js$nodes||this.nodes[q]instanceof TrackerNode$$module$js$nodes||d.finalize(q);q=new LocationList$$module$js$nodes(this);for(const a of b)for(const b of d.nodes[a.uid].values())q.addRoute([a.uid,...b]);return q}}
...
return Math.min(AM$$module$js$random*this.iy,RNMX$$module$js$random)}nextInt(b){return Math.floor(this.next()*b)}shuffle(b){for(let a=b.length;a;){const c=this.nextInt(a--);[b[a],b[c]]=[b[c],b[a]]}return b}}var module$js$random={};module$js$random.Random=Random$$module$js$random;const STATUS$$module$js$version="unstable",VERSION$$module$js$version="refactor-asm",LABEL$$module$js$version="refactor-asm bfacb3e",HASH$$module$js$version="bfacb3e66c873acac889efcd3740db76911a553b",DATE$$module$js$version=new Date(1553158147E3);var module$js$version={};module$js$version.STATUS=STATUS$$module$js$version;module$js$version.VERSION=VERSION$$module$js$version;module$js$version.LABEL=LABEL$$module$js$version;module$js$version.HASH=HASH$$module$js$version;module$js$version.DATE=DATE$$module$js$version;const parseSeed$$module$js$patch=(b)=>b?/^[0-9a-f]{1,8}$/i.test(b)?Number.parseInt(b,16):crc32$$module$js$crc32(b):Math.floor(4294967296*Math.random());class Reader$$module$js$patch{read(b){}}
...
_EXTRA_PITY_MP:!0,_FIX_OPEL_STATUE:!0,_FIX_SHAKING:!0,_FIX_VAMPIRE:!0,_LEATHER_BOOTS_GIVE_SPEED:c.check("Ts"),_NERF_WILD_WARP:c.check("Tw"),_NEVER_DIE:c.check("Di"),_PITY_HP_AND_MP:!0,_PROGRESSIVE_BRACELET:!0,_REVERSIBLE_SWAN_GATE:!0,_REQUIRE_HEALED_DOLPHIN_TO_RIDE:c.check("Rd"),_SAHARA_RABBITS_REQUIRE_TELEPATHY:!1,_TELEPORT_ON_THUNDER_SWORD:c.check("Rt")},h=new Assembler$$module$js$6502,m=(a)=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){h.assemble(yield d.read(a),a);h.patchRom(b)});
var n=Object.keys(k).filter((a)=>k[a]).map((a)=>`define ${a} 1\n`).join("");h.assemble(n,"flags.s");yield m("preshuffle.s");g=new Random$$module$js$random(g);yield shuffle2$$module$js$depgraph(b,g,e,c,f);n=new Rom$$module$js$rom(b);rescaleMonsters$$module$js$patch(b,n);c.check("Mr")&&shuffleMonsters$$module$js$patch(b,n,g);identifyKeyItemsForDifficultyBuffs$$module$js$patch(n);c.check("Em")?b[115962]*=3:c.check("Hm")||(b[115962]*=2);yield m("postshuffle.s");updateDifficultyScalingTables$$module$js$patch(b);
updateCoinDrops$$module$js$patch(b);return stampVersionSeedAndHash$$module$js$patch(b,a,c)}),stampVersionSeedAndHash$$module$js$patch=(b,a,c)=>{const d=crc32$$module$js$crc32(b),e=d.toString(16).padStart(8,0).toUpperCase(),f="unstable"==STATUS$$module$js$version?HASH$$module$js$version.substring(0,7).padStart(7,0).toUpperCase()+"     ":VERSION$$module$js$version.substring(0,12).padEnd(12," ");a=a.toString(16).padStart(8,0).toUpperCase();const g=(a,c)=>{for(let d=0;d<c.length;d++)b[a+16+d]=c.charCodeAt(d)},
k=(a,b)=>{const c=[];for(let d=0;d<a.length||d<b.length;d++)c.push(a[d]||" "),c.push(b[d]||" ");return c.join("")};g(161743,k("  VERSION     SEED      ",`  ${f}${a}`));a=String(c);let h;if(46<a.length){if(92<a.length)throw Error("Flag string way too long!");h=a.substring(46,92).padEnd(46," ");a=a.substring(0,46)}a=a.padEnd(46," ");g(161791,k(a.substring(0,23),a.substring(23)));h&&g(161839,k(h.substring(0,23),h.substring(23)));g(161925,k(e.substring(0,4),e.substring(4)));g(153366,"RANDOMIZER");"unstable"==
STATUS$$module$js$version&&g(153404,"BETA");return d},patchBytes$$module$js$patch=(b,a,c)=>{for(let d=0;d<c.length;d++)b[a+16+d]=c[d]},patchWords$$module$js$patch=(b,a,c)=>{for(let d=0;d<2*c.length;d+=2)b[a+16+d]=c[d>>>1]&255,b[a+17+d]=c[d>>>1]>>>8},updateCoinDrops$$module$js$patch=(b)=>{patchWords$$module$js$patch(b,216030,[0,1,2,4,8,16,30,50,100,200,300,400,500,600,700,800])},updateDifficultyScalingTables$$module$js$patch=(b)=>{const a=Array(48).fill(0).map((a,b)=>b);patchBytes$$module$js$patch(b,
113408,a.map((a)=>Math.round(40+15*a/4)));patchBytes$$module$js$patch(b,113456,a.map((a)=>8+2*a));patchBytes$$module$js$patch(b,113504,a.map((a)=>Math.min(255,48+Math.round(11*a/2))));patchBytes$$module$js$patch(b,113552,a.map((a)=>{a=Math.floor(4*Math.pow(2,(16+9*a)/32));return 128>a?a:Math.min(255,128+(a>>4))}));patchBytes$$module$js$patch(b,216E3,[0,1,3,5,7,9,12,10,16,0,1,3,4,6,9,8,12,16])},rescaleMonsters$$module$js$patch=(b,a)=>{for(var c of SCALED_MONSTERS$$module$js$patch.keys())for(const b in a.objects){if(SCALED_MONSTERS$$module$js$patch.has(b))return;
...
this.report["$"+d.id.toString(16).padStart(2,0)]=[];const {maxFlyers:g,nonFlyers:k={},fixedSlots:h={}}=MONSTER_ADJUSTMENTS$$module$js$patch[d.id]||{};let m=h.pat0||null,n=h.pat1||null,p=h.pal2||null,t=h.pal3||null,l=g,q=!1;for(const b of d.objects||[])if(2==(b[2]&7)&&(q=!0),!(b[2]&7)){var a=b[3]+80;126==a||127==a||159==a?n=98:143==a&&(m=97)}if(21==d.id||71==d.id)q=!1;f.push(`Initial pass: ${[q,m,n,p,t].join(", ")}`);a=(a)=>{var b=FLYERS$$module$js$patch.has(a.id),c=MOTHS_AND_BATS$$module$js$patch.has(a.id);
if(b){if(!l)return!1;--l}if(null!=p&&null!=a.pal2&&p!=a.pal2||null!=t&&null!=a.pal3&&t!=a.pal3)return!1;var g=!q||TREASURE_CHEST_BANKS$$module$js$patch.has(a.pat);let h;if(d.rom.objects[a.id].child||RETAIN_SLOTS$$module$js$patch.has(a.id)){const b=(h=a.patSlot?128:0)?n:m;if(null!=b&&b!=a.pat)return!1;if(h)n=a.pat;else if(g)m=a.pat;else return!1;f.push(`  Adding ${a.id.toString(16)}: pat(${h}) <-  ${a.pat.toString(16)}`)}else if(null==m&&g||m==a.pat)m=a.pat,h=0,f.push(`  Adding ${a.id.toString(16)}: pat0 <-  ${a.pat.toString(16)}`);
