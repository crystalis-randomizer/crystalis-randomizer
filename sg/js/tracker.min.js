...
for(const {title:b,flags:a}of PRESETS$$module$js$flagset)PRESETS_BY_KEY$$module$js$flagset[`@${b.replace(/ /g,"").toLowerCase()}`]=a;
...
text:'Less grinding will be required to "keep up" with the game difficulty.'}]},{section:"Debug Mode",text:"These options are helpful for exploring or debugging.  Note that,\n      while they do not directly affect any randomization, they\n      <i>do</i> factor into the seed to prevent cheating, and they\n      will remove the option to generate a seed for racing.",flags:[{flag:"Ds",name:"Generate a spoiler log"},{flag:"Di",name:"Player never dies"}]}],exclusiveFlags$$module$js$flagset=(b)=>b.startsWith("S")?
new RegExp(`S.*[${b.substring(1)}]`):FLAG_CONFLICTS$$module$js$flagset[b],FLAG_CONFLICTS$$module$js$flagset={Hm:/Em/,Hx:/Ex/,Em:/Hm/,Ex:/Hx/,Fw:/Gw/,Gw:/Fw/,Ft:/Gt/,Gt:/Ft/,Fc:/Gc/,Gc:/Fc/,Fp:/Gp/,Gp:/Fp/,Fr:/Gr/,Gr:/Fr/};
class FlagSet$$module$js$flagset{constructor(b="RtGftTab"){if(b.startsWith("@")){var a=PRESETS_BY_KEY$$module$js$flagset[b.toLowerCase()];if(!a)throw`Unknown preset: ${b}`;b=a}this.flags={};b=b.replace(/[^A-Za-z0-9!]/g,"");a=/([A-Z])([a-z0-9!]+)/g;let c;for(;c=a.exec(b);){let [,a,b]=c;REPEATABLE_FLAGS$$module$js$flagset.has(a)&&(b=[b]);for(const c of b)this.set(a+c,!0)}}set(b,a){const c=b[0],d=b.substring(1);a?(this.removeConflicts(b),b=(this.flags[c]||[]).filter((a)=>a!==d),b.push(d),b.sort(),this.flags[c]=
...
oc=d(60,"Kirisa Plant").chest("Kirisa meadow").invisible(254886).key(),zb=d(61,"Ivory Statue").bossDrop("Karmine",9).npcSpawn(200).key(),pc=d(62,"Bow of Moon").fromPerson("Aryllis",35).direct(251624).dialog(35,null,1).key(),qc=d(63,"Bow of Sun").chest().key(),rc=d(64,"Bow of Truth").fromPerson("Azteca",131).npcSpawn(131,156,1).dialog(131,null,0).key(),Za=(new Magic$$module$js$nodes(a,65,"Refresh",65,null)).fromPerson("Zebu at windmill",94).requireUnique().direct(251665).dialog(94,16,3).trigger(180,
1),Ja=(new Magic$$module$js$nodes(a,66,"Paralysis",66,null)).direct("Zebu at Mt. Sabre summit",251477).requireUnique().trigger(141).trigger(178),fa=(new Magic$$module$js$nodes(a,67,"Telepathy",67,null)).direct("Tornel at Stom's house",223220).npcSpawn(95,30,1).trigger(133,1),sc=(new Magic$$module$js$nodes(a,68,"Teleport",68,null)).fromPerson("Tornel on Mt. Sabre",95).dialog(95,33,0),je=(new Magic$$module$js$nodes(a,69,"Recover",69,null)).direct("Asina in Portoa",250361),tc=(new Magic$$module$js$nodes(a,
70,"Barrier",70,null)).direct("Asina on Angry sea",251609).requireUnique().trigger(132,0),pa=(new Magic$$module$js$nodes(a,71,"Change",71,null)).direct("Kensu in Swan",251614).npcSpawn(116,241,1),z=(new Magic$$module$js$nodes(a,72,"Flight",72,null)).weight(15).direct("Kensu in Draygonia Fortress",250255),ke=M.bossDrop("Vampire 2",12,97).npcSpawn(204).key(),N=d(112,"Mimic"),uc=(new Trigger$$module$js$nodes(a,"Talked to Leaf Elder")).get(P),vc=(new Trigger$$module$js$nodes(a,"Talked to Leaf Student")).get(wb),
Ab=new Trigger$$module$js$nodes(a,"Talked to Zebu in cave"),le=(new Trigger$$module$js$nodes(a,"Woke up Windmill Guard")).get(hc),Ka=new Trigger$$module$js$nodes(a,"Started Windmill"),wc=(new Trigger$$module$js$nodes(a,"Learned Refresh")).get(Za),me=(new Trigger$$module$js$nodes(a,"Gave Statue to Akahana")).get(Ya),ne=(new Trigger$$module$js$nodes(a,"Fought Stom")).get(fa),xc=new Trigger$$module$js$nodes(a,"Visited Oak"),yc=new Trigger$$module$js$nodes(a,"Talked to Oak Mother"),Bb=new Trigger$$module$js$nodes(a,
"Rescued Oak Child"),oe=(new Trigger$$module$js$nodes(a,"Talked to Oak Mother Again")).get(ec),pe=(new Trigger$$module$js$nodes(a,"Talked to Oak Elder")).get(Q),qe=(new Trigger$$module$js$nodes(a,"Talked to Tornel on Mt Sabre")).get(sc),zc=new Trigger$$module$js$nodes(a,"Villagers Abducted"),Ac=new Trigger$$module$js$nodes(a,"Talked to Rabbit in Leaf"),re=(new Trigger$$module$js$nodes(a,"Learned Paralysis")).get(Ja),Bc=new Trigger$$module$js$nodes(a,"Talked to Portoa Queen"),Cc=new Trigger$$module$js$nodes(a,
"Talked to Fortune Teller"),Dc=new Trigger$$module$js$nodes(a,"Visited Underground Channel"),se=(new Trigger$$module$js$nodes(a,"Sent to Waterfall Cave")).get(ub),te=(new Trigger$$module$js$nodes(a,"Cured Akahana")).get(gc),Ec=(new Trigger$$module$js$nodes(a,"Talked to Rage")).get(Ua),Fc=new Trigger$$module$js$nodes(a,"Mesia recording played"),Gc=(new Trigger$$module$js$nodes(a,"Talked to Asina")).get(je),Hc=(new Trigger$$module$js$nodes(a,"Healed Dolphin")).get(yb),Cb=new Trigger$$module$js$nodes(a,
"Returned Fog Lamp"),Ic=new Trigger$$module$js$nodes(a,"Talked to Kensu in Cabin"),Jc=new Trigger$$module$js$nodes(a,"Talked to Joel Elder"),ue=(new Trigger$$module$js$nodes(a,"Talked to Clark")).get(kc),ve=(new Trigger$$module$js$nodes(a,"Talked to Kensu in Lighthouse")).get(mc),Db=new Trigger$$module$js$nodes(a,"Calmed the Angry Sea"),we=(new Trigger$$module$js$nodes(a,"Learned Barrier")).get(tc),Kc=new Trigger$$module$js$nodes(a,"Talked to Stom in Swan Hut"),Lc=new Trigger$$module$js$nodes(a,"Talked to Kensu in Swan tavern"),
Mc=new Trigger$$module$js$nodes(a,"Talked to Kensu at Swan dance"),xe=(new Trigger$$module$js$nodes(a,"Returned Kensu's love pendant")).get(pa),ye=(new Trigger$$module$js$nodes(a,"Talked to Amazones Queen")).get(pc),Eb=new Trigger$$module$js$nodes(a,"Entered Shyron"),ze=(new Trigger$$module$js$nodes(a,"Talked to Zebu in Shyron")).get(ic),Nc=new Trigger$$module$js$nodes(a,"Shyron Massacre"),Ae=(new Trigger$$module$js$nodes(a,"Saved Kensu")).get(z),Be=(new Trigger$$module$js$nodes(a,"Talked to Deo")).get(ie),
Ce=(new Trigger$$module$js$nodes(a,"Talked to Akahana's Friend")).get(ge),De=(new Trigger$$module$js$nodes(a,"Get Bow of Truth")).get(rc),Ee=(new Trigger$$module$js$nodes(a,"Forged Crystalis")).get(pb),$a=(new Condition$$module$js$nodes(a,"Sword charge glitch")).option(C,V),Fe=(new Condition$$module$js$nodes(a,"Rabbit skip")).option(r,I),Ge=(new Condition$$module$js$nodes(a,"Teleport skip")).option(p,m),ab=(new Condition$$module$js$nodes(a,"Statue glitch")).option(l,q),He=(new Condition$$module$js$nodes(a,
"Wild warp")).option(va,oa),bb=(new Condition$$module$js$nodes(a,"Any level 2 sword")).option(P,Ra).option(P,Ia).option(Q,Sa).option(Q,Ta).option(L,Ua).option(L,Va).option(J,Wa).option(J,Xa),v=(new Condition$$module$js$nodes(a,"Destroy stone")).option(P,Qa).option(P,Ra).option(P,Ia).option($a,P,bb),w=(new Condition$$module$js$nodes(a,"Destroy ice")).option(Q,Qa).option(Q,Sa).option(Q,Ta).option($a,Q,bb),t=(new Condition$$module$js$nodes(a,"Cross rivers")).option(L,Qa).option(L,Ua).option(L,Va).option(z).option($a,
L,bb),xa=(new Condition$$module$js$nodes(a,"Destroy iron")).option(J,Qa).option(J,Wa).option(J,Xa).option($a,J,bb),S=(new Condition$$module$js$nodes(a,"Any sword")).option(P).option(Q).option(L).option(J),Ie=(new Condition$$module$js$nodes(a,"Match insect sword (fire/water/thunder)")).option(Q).option(L).option(J).option(Ya,G.value?G:g,P),Je=(new Condition$$module$js$nodes(a,"Match vampire 2 sword (wind/water/thunder)")).option(P).option(L).option(J).option(G.value?G:g,Q),Oc=(new Condition$$module$js$nodes(a,
"Speed boots")).option(fc,h),Pc=(new Condition$$module$js$nodes(a,"Climb slopes")).option(vb).option(z).option(Oc),Ke=(new Condition$$module$js$nodes(a,"Enter Mt Sabre North (rabbit)")).option(Ac).option(Fe),Le=(new Condition$$module$js$nodes(a,"Enter Mt Sabre North (Teleport)")).option(sc).option(Ge,z),Qc=(new Condition$$module$js$nodes(a,"Asina in her room")).option(Fc),Me=(new Condition$$module$js$nodes(a,"Paralysis or Asina revealed")).option(Ja).option(Qc).option(ab),Rc=(new Condition$$module$js$nodes(a,
"Ride dolphin")).option(yb,Ic),Ne=(new Condition$$module$js$nodes(a,"Ghetto flight")).option(n,Rc,vb),ba=(new Condition$$module$js$nodes(a,"Cross sea")).option(Rc).option(z),Oe=(new Condition$$module$js$nodes(a,"Cross whirlpool")).option(Db).option(z).option(Ne),T=(new Condition$$module$js$nodes(a,"Refresh if needed")).option(Xb.value?Xb:g).option(Za),Sc=(new Condition$$module$js$nodes(a,"Wind magic")).option(E.value?E:g,T).option(Ra,Ia,T),Tc=(new Condition$$module$js$nodes(a,"Fire magic")).option(E.value?
E:g,T).option(Sa,Ta,T),Uc=(new Condition$$module$js$nodes(a,"Water magic")).option(E.value?E:g,T).option(E,T).option(Ua,Va,T),Pe=(new Condition$$module$js$nodes(a,"Thunder magic")).option(E.value?E:g,T).option(E,T).option(Wa,T).option(Xa,T),Qe=(new Condition$$module$js$nodes(a,"Flute of lime or glitch")).option(ub).option(ab),Re=(new Condition$$module$js$nodes(a,"Change or glitch")).option(pa).option(ab),Se=(new Condition$$module$js$nodes(a,"Change or glitch or paralysis")).option(pa).option(ab).option(Ja),
Fb=(new Condition$$module$js$nodes(a,"Pass shooting statues")).option(tc).option(S,Wb.value?Wb:g).option(Za,gc),Te=(new Condition$$module$js$nodes(a,"Healed dolphin if required")).option(Hc).option(Pd),Vc=(new Condition$$module$js$nodes(a,"Match sword of wind")).option(P).option(G.value?G:g,S),Wc=(new Condition$$module$js$nodes(a,"Match sword of fire")).option(Q).option(G.value?G:g,S),Xc=(new Condition$$module$js$nodes(a,"Match sword of water")).option(L).option(G.value?G:g,S),Ue=(new Condition$$module$js$nodes(a,
"Match sword of thunder")).option(J).option(G.value?G:g,S),Ve=(new Condition$$module$js$nodes(a,"Swamp run possible")).option(S,Oc).option(S,Wd).option(S,Za),We=(new Condition$$module$js$nodes(a,"Insect possible to kill")).option(Ya),cb=(new Condition$$module$js$nodes(a,"Travel swamp")).option(Ya).option(Pa.value?Pa:g,Ve),Xe=(new Condition$$module$js$nodes(a,"Calmed sea if required")).option(Db).option(Qd),Ye=(new Condition$$module$js$nodes(a,"Started windmill if required")).option(Ka).option(Sd),
Ze=(new Condition$$module$js$nodes(a,"Telepathy if required for Deo")).option(fa).option(Ud),$e=e(0,"Vampire 1",S).get(vb),af=e(1,"Insect",ec,Ie,We).get(Sa),bf=e(2,"Kelbesque 1",Vc,Sc).get(Ta),cf=e(12,"Vampire 2",Je).get(ke),Yc=e(4,"Sabera 1",Wc,Tc).get(lc),df=e(5,"Mado 1",Xc,Uc).get(Wa),ef=e(6,"Kelbesque 2",Vc,Sc).get(tb),ff=e(7,"Sabera 2",Wc,Tc).get(sb),gf=e(8,"Mado 2",Xc,Uc).get(qb),hf=e(9,"Karmine",Ue,Pe).get(zb),jf=e(10,"Draygon 1",S).get(ee),kf=e(null,"Statues",qc,pc),lf=e(11,"Draygon 2",S,
rc),mf=e(13,"Dyna",pb),nf=(new Condition$$module$js$nodes(a,"Opened prison if required")).option(xb).option(Rd),X=new Area$$module$js$nodes(a,"Leaf"),ya=new Area$$module$js$nodes(a,"Valley of Wind"),U=new Area$$module$js$nodes(a,"Sealed Cave"),qa=new Area$$module$js$nodes(a,"Cordel Plain"),za=new Area$$module$js$nodes(a,"Brynmaer"),Aa=new Area$$module$js$nodes(a,"Amazones"),A=new Area$$module$js$nodes(a,"Mt Sabre West"),u=new Area$$module$js$nodes(a,"Mt Sabre North"),db=new Area$$module$js$nodes(a,
"Nadare's"),La=new Area$$module$js$nodes(a,"Oak"),ha=new Area$$module$js$nodes(a,"Waterfall Valley"),y=new Area$$module$js$nodes(a,"Portoa"),ia=new Area$$module$js$nodes(a,"Waterfall Cave"),B=new Area$$module$js$nodes(a,"Fog Lamp Cave"),ja=new Area$$module$js$nodes(a,"Kirisa Plant Cave"),ra=new Area$$module$js$nodes(a,"Angry Sea"),ka=new Area$$module$js$nodes(a,"Joel"),H=new Area$$module$js$nodes(a,"Evil Spirit Island"),la=new Area$$module$js$nodes(a,"Sabera's Castle"),Y=new Area$$module$js$nodes(a,
"Swan"),x=new Area$$module$js$nodes(a,"Mt Hydra"),ma=new Area$$module$js$nodes(a,"Shyron"),sa=new Area$$module$js$nodes(a,"Styx"),ta=new Area$$module$js$nodes(a,"Goa"),eb=new Area$$module$js$nodes(a,"Draygonia Fortress 1"),Z=new Area$$module$js$nodes(a,"Draygonia Fortress 2"),Ba=new Area$$module$js$nodes(a,"Draygonia Fortress 3"),O=new Area$$module$js$nodes(a,"Draygonia Fortress 4"),F=new Area$$module$js$nodes(a,"Oasis Cave");new Area$$module$js$nodes(a,"Desert");const aa=new Area$$module$js$nodes(a,
"Sahara"),Ca=new Area$$module$js$nodes(a,"Pyramid Front"),ca=new Area$$module$js$nodes(a,"Pyramid Back"),da=new Area$$module$js$nodes(a,"Tower"),Gb=(new Location$$module$js$nodes(a,0,X,"Start")).overworld().start(),of=(new Location$$module$js$nodes(a,0,X,"Mezame Shrine")).overworld().from(Gb),pf=(new Location$$module$js$nodes(a,1,X,"Outside Start")).overworld().connect(of),na=(new Location$$module$js$nodes(a,2,X,"Town")).town().connect(pf),Ma=(new Location$$module$js$nodes(a,3,ya,"Main")).overworld().connect(na).trigger(wc,
Ka),Zc=(new Location$$module$js$nodes(a,3,ya,"Outside Windmill")).overworld(),$c=(new Location$$module$js$nodes(a,4,U,"Tunnel 1 (entrance)")).cave().from(Ma,Ka),qf=(new Location$$module$js$nodes(a,5,U,"Tunnel 2 (over bridge)")).cave().connect($c),rf=(new Location$$module$js$nodes(a,6,U,"Tunnel 6 (herb dead end)")).cave().chest(D,15),ad=(new Location$$module$js$nodes(a,7,U,"Tunnel 4a (ball corridor)")).cave().chest(D,20,80).chest(Ra,21);(new Location$$module$js$nodes(a,7,U,"Tunnel 4b (antidote dead end)")).cave().connect(ad,
v).chest(K,19);const sf=(new Location$$module$js$nodes(a,8,U,"Tunnel 5 (warp boots dead end)")).cave().chest(R,14),tf=(new Location$$module$js$nodes(a,9,U,"Tunnel 3a (branch, front)")).cave().connect(qf).connectTo(ad).connectTo(sf),uf=(new Location$$module$js$nodes(a,9,U,"Tunnel 3b (branch, back)")).cave().connect(tf,v).connectTo(rf),vf=(new Location$$module$js$nodes(a,10,U,"Tunnel 7 (boss)")).cave().connect(uf).boss($e),wf=(new Location$$module$js$nodes(a,12,U,"Tunnel 8a (exit, above wall)")).cave().connect(vf),
xf=(new Location$$module$js$nodes(a,12,U,"Tunnel 8b (exit, below wall")).cave().connect(wf,v);(new Location$$module$js$nodes(a,14,ya,"Windmill Cave")).cave().connect(Ma).connectTo(Zc).trigger(le,wb,Ab);(new Location$$module$js$nodes(a,15,ya,"Windmill")).misc().connect(Zc).trigger(Ka,hc);const yf=(new Location$$module$js$nodes(a,16,ya,"Zebu's Cave Front")).cave().connect(Ma).trigger(Ab,uc,vc).trigger(wc,Ka,Ab),zf=(new Location$$module$js$nodes(a,16,ya,"Zebu's Cave Back")).cave().trigger(zc).connect(yf,
w),Af=(new Location$$module$js$nodes(a,17,A,"Tunnel 1 (to Zebu)")).cave().connect(zf),Da=(new Location$$module$js$nodes(a,20,qa,"West")).overworld().connect(xf,Ye),Bf=(new Location$$module$js$nodes(a,20,qa,"South")).overworld().connect(Da,t),bd=(new Location$$module$js$nodes(a,21,qa,"East")).overworld().connect(Da).chest(dc,24),Na=(new Location$$module$js$nodes(a,24,za,"Town")).town().connect(Da).trigger(me,dc),Cf=(new Location$$module$js$nodes(a,25,qa,"Outside Stom's House")).town().connect(Da),
Hb=(new Location$$module$js$nodes(a,26,qa,"Swamp")).overworld().connect(bd,cb).trigger(Bb,yc,cb);(new Location$$module$js$nodes(a,26,qa,"Swamp Insect Area")).overworld().connect(Hb,cb).boss(af);const fb=(new Location$$module$js$nodes(a,27,Aa,"Town")).town().connect(Bf),gb=(new Location$$module$js$nodes(a,28,La,"Town")).town().connect(Hb,cb).trigger(xc);(new Location$$module$js$nodes(a,30,qa,"Stom's House")).house().connect(Cf).trigger(ne,xc);const Ea=(new Location$$module$js$nodes(a,32,A,"Entrance")).overworld().connect(Da).connect(Af),
Df=(new Location$$module$js$nodes(a,32,A,"Up Slope")).overworld().from(Ea,Pc).to(Ea),Ef=(new Location$$module$js$nodes(a,32,A,"Dead End (warp boots)")).overworld().chest(R,24,106),cd=(new Location$$module$js$nodes(a,33,A,"Upper")).overworld().from(Ea,z).to(Ea),Ff=(new Location$$module$js$nodes(a,33,A,"Tornel Dead End")).overworld().trigger(qe,Ia).chest(W,23,105),dd=(new Location$$module$js$nodes(a,34,A,"Tunnel 2a (fork at start)")).cave().connect(Ea);(new Location$$module$js$nodes(a,34,A,"Tunnel 2b (left branch to dead end)")).cave().connect(dd,
w).connectTo(Ef);const Gf=(new Location$$module$js$nodes(a,34,A,"Tunnel 2c (right branch to upper)")).cave().connect(dd,w),Hf=(new Location$$module$js$nodes(a,35,A,"Tunnel 3a (tunnel to upper, with herb chest)")).cave().connect(Gf).chest(D,23,82),If=(new Location$$module$js$nodes(a,35,A,"Tunnel 3b (tunnel to upper, branch below ice)")).cave().connect(Hf,w).connectTo(cd),ed=(new Location$$module$js$nodes(a,36,A,"Tunnel 4a (branch to upper or Tornel)")).cave().connect(If);(new Location$$module$js$nodes(a,
36,A,"Tunnel 4b (out to upper)")).cave().connect(ed,w).connectTo(cd);const Jf=(new Location$$module$js$nodes(a,37,A,"Tunnel 5 (tiny connector)")).cave().connect(ed),Kf=(new Location$$module$js$nodes(a,38,A,"Tunnel 6a (exit to Tornel, above ice)")).cave().connect(Jf);(new Location$$module$js$nodes(a,38,A,"Tunnel 6b (exit to Tornel, below ice)")).cave().connect(Kf,w).connectTo(Ff);const Lf=(new Location$$module$js$nodes(a,39,A,"Tunnel 7a (tornado bracelet, lower)")).cave().connect(Df),Mf=(new Location$$module$js$nodes(a,
39,A,"Tunnel 7b (tornado bracelet, middle)")).cave().connect(Lf,w);(new Location$$module$js$nodes(a,39,A,"Tunnel 7c (tornado bracelet, upper)")).cave().connect(Mf,w).chest(Ia,25);const hb=(new Location$$module$js$nodes(a,40,u,"Entrance")).overworld().connect(bd,Le,Ke),ib=(new Location$$module$js$nodes(a,40,u,"Upper")).overworld().from(hb,z).to(hb),Ib=(new Location$$module$js$nodes(a,40,u,"Summit (boss)")).overworld().from(ib,z).to(ib).boss(bf),fd=(new Location$$module$js$nodes(a,41,u,"Connector")).overworld(),
Jb=(new Location$$module$js$nodes(a,41,u,"Middle Left")).overworld(),Nf=(new Location$$module$js$nodes(a,41,u,"Middle Right")).overworld().from(Jb,Pc).to(Jb),gd=(new Location$$module$js$nodes(a,42,u,"Tunnel 2a (from entrance to connector)")).cave().connectTo(fd);(new Location$$module$js$nodes(a,42,u,"Tunnel 2b (under bridge, to antidote)")).cave().connect(gd,w).chest(K,23,94);const hd=(new Location$$module$js$nodes(a,43,u,"Tunnel 3a (branch after connector)")).cave().connect(fd),Of=(new Location$$module$js$nodes(a,
43,u,"Tunnel 3b (right branch)")).cave().connect(hd,w),Pf=(new Location$$module$js$nodes(a,43,u,"Tunnel 3c (upper branch)")).cave().connect(hd,w);(new Location$$module$js$nodes(a,44,u,"Tunnel 4 (over bridge, to middle)")).cave().connect(Pf).connectTo(Jb);const id=(new Location$$module$js$nodes(a,45,u,"Tunnel 5a (E-shaped, from right branch)")).cave().connect(Of).connectTo(Nf);(new Location$$module$js$nodes(a,45,u,"Tunnel 5b (dead-end with herb)")).cave().connect(id,w).chest(D,22,83);const Qf=(new Location$$module$js$nodes(a,
46,u,"Tunne; 6a (S-shaped hall, right)")).cave().connect(id),Rf=(new Location$$module$js$nodes(a,46,u,"Tunne; 6b (S-shaped hall, middle)")).cave().connect(Qf,w),Sf=(new Location$$module$js$nodes(a,46,u,"Tunnel 6c (S-shaped hall, left)")).cave().connect(Rf,w),jd=(new Location$$module$js$nodes(a,47,u,"Prison (hallway)")).cave().connect(ib),Tf=(new Location$$module$js$nodes(a,48,u,"Left Cell (shopkeepers)")).cave().from(jd,w);(new Location$$module$js$nodes(a,49,u,"Left Cell 2 (back, with prison key)")).cave().from(Tf,
w).chest(xb,13);const Uf=(new Location$$module$js$nodes(a,50,u,"Right Cell (villagers)")).cave().from(jd,w),Vf=(new Location$$module$js$nodes(a,51,u,"Tunnel 8 (behind right cell, toward summit)")).cave().from(Uf,w);(new Location$$module$js$nodes(a,52,u,"Tunnel 9 (connector to summit)")).cave().connect(Vf).connectTo(Ib);const Wf=(new Location$$module$js$nodes(a,53,u,"Tunnel 10a (summit cave, front)")).cave().from(Ib,xb).to(Ib),Xf=(new Location$$module$js$nodes(a,53,u,"Tunnel 10b (summit cave, behind ice)")).cave().connect(Wf,
w).trigger(re);(new Location$$module$js$nodes(a,56,u,"Tunnel 1 (leads from main entrance)")).cave().connect(hb).connectTo(gd);(new Location$$module$js$nodes(a,57,u,"Tunnel 7 (to upper)")).cave().connect(Sf).connectTo(ib);const Yf=(new Location$$module$js$nodes(a,60,db,"Inn")).shop(),Zf=(new Location$$module$js$nodes(a,61,db,"Tool Shop")).shop(D,K,M,R),$f=(new Location$$module$js$nodes(a,62,db,"Back Room")).house(),Kb=(new Location$$module$js$nodes(a,64,ha,"Summit")).overworld().connect(Xf,nf),jb=
(new Location$$module$js$nodes(a,64,ha,"Northwest")).overworld().from(Kb).to(Kb,z),ag=(new Location$$module$js$nodes(a,64,ha,"Northeast")).overworld().connect(jb,t),kd=(new Location$$module$js$nodes(a,65,ha,"Southwest")).overworld().connect(jb),bg=(new Location$$module$js$nodes(a,65,ha,"Southeast")).overworld().connect(kd,t),ld=(new Location$$module$js$nodes(a,66,ha,"Lime Tree Valley")).overworld().connect(kd).connect(Ma,Vd),cg=(new Location$$module$js$nodes(a,67,ha,"Lime Tree Lake (Rage)")).cave().connect(ld).trigger(Ec,
L),dg=(new Location$$module$js$nodes(a,68,ja,"Tunnel 1a (entrance)")).cave().connect(bg),eg=(new Location$$module$js$nodes(a,68,ja,"Tunnel 1b (behind wall)")).cave().connect(dg,v),md=(new Location$$module$js$nodes(a,69,ja,"Tunnel 2a (main path, before wall)")).cave().connect(eg);(new Location$$module$js$nodes(a,69,ja,"Tunnel 2b (dead end, antidote)")).cave().connect(md,v).chest(K,25,95);const fg=(new Location$$module$js$nodes(a,69,ja,"Tunnel 2c (main path, after wall)")).cave().connect(md,v),gg=(new Location$$module$js$nodes(a,
70,ja,"Tunnel 3a (last room, before wall)")).cave().connect(fg),hg=(new Location$$module$js$nodes(a,70,ja,"Tunnel 3b (last room, after wall)")).cave().connect(gg,v);(new Location$$module$js$nodes(a,71,ja,"Meadow")).overworld().connect(hg).chest(oc,14);const ig=(new Location$$module$js$nodes(a,72,B,"Tunnel 1a (entrance)")).cave().connect(ag),nd=(new Location$$module$js$nodes(a,72,B,"Tunnel 1b (past wall)")).cave().connect(ig,v);(new Location$$module$js$nodes(a,72,B,"Tunnel 1c (dead end, lysisPlant)")).cave().connect(nd,
v).chest(wa,24);const jg=(new Location$$module$js$nodes(a,73,B,"Tunnel 2 (tiny connector)")).cave().connect(nd),od=(new Location$$module$js$nodes(a,74,B,"Tunnel 3a (upper branch)")).cave().connect(jg);(new Location$$module$js$nodes(a,74,B,"Tunnel 3b (dead end, mimic)")).cave().connect(od,v).chest(N,21,112);const pd=(new Location$$module$js$nodes(a,74,B,"Tunnel 3c (short passage with mimic)")).cave().connect(od,v).chest(N,22,113),kg=(new Location$$module$js$nodes(a,74,B,"Tunnel 3d (lower branch)")).cave().connect(pd,
v);(new Location$$module$js$nodes(a,75,B,"Tunnel 4 (dead end loop)")).cave().connect(kg);const qd=(new Location$$module$js$nodes(a,76,B,"Tunnel 5a (right branch over bridge)")).cave().connect(pd),lg=(new Location$$module$js$nodes(a,76,B,"Tunnel 5b (past wall over bridge)")).cave().connect(qd,v),mg=(new Location$$module$js$nodes(a,77,B,"Tunnel 6a (from left branch)")).cave().connect(qd),ng=(new Location$$module$js$nodes(a,77,B,"Tunnel 6b (reconvergence)")).cave().connect(mg,v).connect(lg),og=(new Location$$module$js$nodes(a,
77,B,"Tunnel 6c (between walls)")).cave().connect(ng,v),pg=(new Location$$module$js$nodes(a,77,B,"Tunnel 6d (under bridge)")).cave().connect(og,v),qg=(new Location$$module$js$nodes(a,78,B,"Tunnel 7a (over second bridge)")).cave().connect(pg),rg=(new Location$$module$js$nodes(a,78,B,"Tunnel 7b (past wall)")).cave().connect(qg,v),sg=(new Location$$module$js$nodes(a,79,B,"Tunnel 8a (under second bridge)")).cave().connect(rg);(new Location$$module$js$nodes(a,79,B,"Tunnel 8b (fog lamp)")).cave().connect(sg,
v).chest(jc,19);const ua=(new Location$$module$js$nodes(a,80,y,"Town")).town().connect(jb),rd=(new Location$$module$js$nodes(a,81,y,"Fishherman Island")).town().connect(ua);(new Location$$module$js$nodes(a,82,ha,"Mesia Shrine")).cave().trigger(Fc).connect(cg,t,Ec);const sd=(new Location$$module$js$nodes(a,84,ia,"Tunnel 1a (entrance)")).cave().connect(jb);(new Location$$module$js$nodes(a,84,ia,"Tunnel 1b (dead end, mimic)")).cave().connect(sd,w).chest(N,19,114);const tg=(new Location$$module$js$nodes(a,
84,ia,"Tunnel 1c (past ice)")).cave().connect(sd,w),ug=(new Location$$module$js$nodes(a,85,ia,"Tunnel 2 (stoned pair)")).cave().connect(tg),Lb=(new Location$$module$js$nodes(a,86,ia,"Tunnel 3 (wide medusa hallways)")).cave().connect(ug,Qe),vg=(new Location$$module$js$nodes(a,87,ia,"Tunnel 4a (left entrance)")).cave().from(Lb,w).chest($b.chest().bonus(),25).trigger(te,ub);(new Location$$module$js$nodes(a,87,ia,"Tunnel 4b (right entrance)")).cave().from(Lb,w).connect(vg,z);(new Location$$module$js$nodes(a,
87,ia,"Tunnel 4c (sword of water)")).cave().connect(Lb,w).chest(L,24);const td=(new Location$$module$js$nodes(a,88,da,"Entrance")).misc(),wg=(new Location$$module$js$nodes(a,89,da,"Level 1")).misc().from(td),xg=(new Location$$module$js$nodes(a,90,da,"Level 2")).misc().from(wg),yg=(new Location$$module$js$nodes(a,91,da,"Level 3")).misc().from(xg),ud=(new Location$$module$js$nodes(a,92,da,"Outside Mesia Room")).misc().from(yg),zg=(new Location$$module$js$nodes(a,93,da,"Outside Dyna Room")).misc().from(ud,
pb);(new Location$$module$js$nodes(a,94,da,"Mesia")).misc().connect(ud).trigger(Ee);const Ag=(new Location$$module$js$nodes(a,95,da,"Dyna")).misc().from(zg).boss(mf);(new Location$$module$js$nodes(a,95,da,"Win")).misc().from(Ag).end();const Mb=(new Location$$module$js$nodes(a,96,ra,"Cabin Beach")).sea().from(rd,Cb),Fa=(new Location$$module$js$nodes(a,96,ra,"South")).sea().connect(Mb,ba),Bg=(new Location$$module$js$nodes(a,96,ra,"Joel Beach")).sea().connect(Fa,ba),vd=(new Location$$module$js$nodes(a,
96,ka,"Outside Lighthouse")).sea();(new Location$$module$js$nodes(a,96,ra,"Altar")).sea().connect(Fa,ba).trigger(Db,mc,lc);const Cg=(new Location$$module$js$nodes(a,96,ra,"North")).sea().to(Fa,ba).from(Fa,ba,Oe).trigger(we,Xe),Dg=(new Location$$module$js$nodes(a,96,ra,"Swan Beach")).sea().connect(Cg,ba);(new Location$$module$js$nodes(a,97,ra,"Cabin")).misc().connect(Mb).trigger(Ic,Cb);(new Location$$module$js$nodes(a,98,ka,"Lighthouse")).misc().connect(vd).trigger(ve,wb);const kb=(new Location$$module$js$nodes(a,
100,y,"Underground Channel 1 (from throne room)")).sea().trigger(Dc),Eg=(new Location$$module$js$nodes(a,100,y,"Underground Channel 2 (to fortune teller)")).sea().connect(kb,t),wd=(new Location$$module$js$nodes(a,100,y,"Underground Channel 3 (from fortune teller)")).sea().connect(kb,z),xd=(new Location$$module$js$nodes(a,100,y,"Underground Channel 4 (asina)")).sea().connect(wd,t),Fg=(new Location$$module$js$nodes(a,100,y,"Underground Channel 5 (dolphin)")).sea().connect(xd,t).trigger(Hc,D,Gc);(new Location$$module$js$nodes(a,
100,y,"Underground Channel 6 (water)")).sea().connect(Fg,ba).connectTo(Fa,ba).chest(nc,17);const Nb=(new Location$$module$js$nodes(a,101,H,"Zombie Town")).town(),Gg=(new Location$$module$js$nodes(a,104,H,"Tunnel 1 (entrance)")).sea().connect(Fa,Jc,ba),yd=(new Location$$module$js$nodes(a,105,H,"Tunnel 2a (start)")).cave().connect(Gg);(new Location$$module$js$nodes(a,105,H,"Tunnel 2b (dead end to left)")).cave().connect(yd,t);const Hg=(new Location$$module$js$nodes(a,105,H,"Tunnel 2c (across first river)")).cave().connect(yd,
t),zd=(new Location$$module$js$nodes(a,105,H,"Tunnel 2d (across second river)")).cave().connect(Hg,t);(new Location$$module$js$nodes(a,105,H,"Tunnel 2e (dead end, magic ring)")).cave().connect(zd,v).chest(W,29);const Ig=(new Location$$module$js$nodes(a,105,H,"Tunnel 2f (stair down)")).cave().connect(zd,v),Jg=(new Location$$module$js$nodes(a,106,H,"Tunnel 3a (main area)")).cave().connect(Ig).connectTo(Nb).chest(wa,25,92),Kg=(new Location$$module$js$nodes(a,106,H,"Tunnel 3b (left area toward items)")).cave().connect(Jg,
v),Lg=(new Location$$module$js$nodes(a,107,H,"Tunnel 4a (right side, mimic)")).cave().connect(Kg).chest(N,14,115);(new Location$$module$js$nodes(a,107,H,"Tunnel 4b (left side, iron necklace)")).cave().connect(Lg,t).chest(he,15);const Ob=(new Location$$module$js$nodes(a,108,la,"Floor 1")).fortress().connect(Nb),Mg=(new Location$$module$js$nodes(a,108,la,"Miniboss")).fortress().boss(cf).connect(Ob);(new Location$$module$js$nodes(a,109,la,"Floor 2a (left stair)")).fortress().connect(Ob).chest(M,19);
const Ng=(new Location$$module$js$nodes(a,109,la,"Floor 2b (right stair)")).fortress().connect(Mg).chest(D,30,85),Og=(new Location$$module$js$nodes(a,110,la,"Floor 3a (toward boss)")).fortress().connect(Ng),Ad=(new Location$$module$js$nodes(a,110,la,"Floor 3b (boss room)")).fortress().connect(Og);(new Location$$module$js$nodes(a,110,la,"Boss")).fortress().boss(Yc).connect(Ad);(new Location$$module$js$nodes(a,110,la,"Floor 3c (back room trap)")).fortress().from(Ad).to(Ob);const Pg=(new Location$$module$js$nodes(a,
112,ka,"Secret Passage")).cave().connectTo(vd),lb=(new Location$$module$js$nodes(a,113,ka,"Town")).town().connect(Bg),ea=(new Location$$module$js$nodes(a,114,Y,"Town")).town().connect(Dg),Qg=(new Location$$module$js$nodes(a,115,Y,"Inside Gate")).overworld().connect(ea),Rg=(new Location$$module$js$nodes(a,115,Y,"Outside Gate")).overworld().from(Qg,pa),mb=(new Location$$module$js$nodes(a,120,ta,"Valley")).overworld().connect(Rg),Pb=(new Location$$module$js$nodes(a,124,x,"Entrance")).overworld().connect(mb),
Sg=(new Location$$module$js$nodes(a,124,x,"Over first river toward Shyron")).overworld().connect(Pb,t),Bd=(new Location$$module$js$nodes(a,124,x,"After first tunnel")).overworld(),Cd=(new Location$$module$js$nodes(a,124,x,"Door to Styx")).overworld().connect(Bd,t),Tg=(new Location$$module$js$nodes(a,124,x,"Dead end (no item)")).overworld(),Ug=(new Location$$module$js$nodes(a,124,x,"Dead end (fruit of lime)")).overworld().chest(rb,26),Vg=(new Location$$module$js$nodes(a,124,x,"Dead end (magic ring)")).overworld().chest(W,
25,101),Dd=(new Location$$module$js$nodes(a,124,x,"Outside tunnel to bow")).overworld();(new Location$$module$js$nodes(a,124,x,"Floating island (bow of sun)")).overworld().connect(Dd,z).chest(qc,24);const Wg=(new Location$$module$js$nodes(a,125,x,"Tunnel 1 (to Shyron)")).cave().connect(Sg),Xg=(new Location$$module$js$nodes(a,126,x,"Outside Shyron")).overworld().connect(Wg);(new Location$$module$js$nodes(a,127,x,"Tunnel 2 (fork)")).cave().connect(Pb).connectTo(Ug).connectTo(Bd);const Ed=(new Location$$module$js$nodes(a,
128,x,"Tunnel 3 (caves)")).cave().connect(Cd).connect(Tg),Fd=(new Location$$module$js$nodes(a,129,x,"Tunnel 4 (left branch of cave)")).cave().connect(Ed);(new Location$$module$js$nodes(a,130,x,"Tunnel 5 (dead end, medical herb)")).cave().connect(Fd).chest(D,15,86);const Yg=(new Location$$module$js$nodes(a,131,x,"Tunnel 6a (left-then-right)")).cave().connect(Fd),Zg=(new Location$$module$js$nodes(a,131,x,"Tunnel 6b (past wall)")).cave().connect(Yg,v),$g=(new Location$$module$js$nodes(a,132,x,"Tunnel 7 (wide hall)")).cave().connect(Zg);
(new Location$$module$js$nodes(a,133,x,"Tunnel 8 (red slimes)")).cave().from($g,v).connectTo(Dd).chest(N,23,116);const ah=(new Location$$module$js$nodes(a,134,x,"Tunnel 9 (right branch, infinite loop)")).cave().connect(Ed),bh=(new Location$$module$js$nodes(a,135,x,"Tunnel 10a (toward magic ring)")).cave().connect(ah);(new Location$$module$js$nodes(a,135,x,"Tunnel 10b (past wall)")).cave().connect(bh,v).connectTo(Vg);const Gd=(new Location$$module$js$nodes(a,136,sa,"Entrance")).fortress().from(Cd,
ic,Fb),ch=(new Location$$module$js$nodes(a,137,sa,"Left branch")).fortress().connect(Gd).chest(N,19,117),dh=(new Location$$module$js$nodes(a,137,sa,"Left branch, past one bridge")).fortress().connect(ch,t),eh=(new Location$$module$js$nodes(a,137,sa,"Left branch, past two bridges")).fortress().connect(dh,t).chest(D,29,87),fh=(new Location$$module$js$nodes(a,137,sa,"Right branch")).fortress().connect(Gd);(new Location$$module$js$nodes(a,137,sa,"Right branch, across water")).fortress().connect(fh,z).chest(N,
20,118).chest(N,21,119).chest(Zd,28);(new Location$$module$js$nodes(a,138,sa,"Upper floor")).fortress().connect(eh).chest(J,27);const Ga=(new Location$$module$js$nodes(a,140,ma,"Town")).town().connect(Xg,Re).trigger(Eb),Oa=(new Location$$module$js$nodes(a,142,ta,"Town")).town().connect(mb),Hd=(new Location$$module$js$nodes(a,143,F,"Draygonia Fortress Basement 1 (front)")).fortress();(new Location$$module$js$nodes(a,143,F,"Draygonia Fortress Basement 2 (power ring)")).fortress().connect(Hd,xa).chest(fe,
15);const Qb=(new Location$$module$js$nodes(a,144,ta,"Desert 1")).overworld().connect(mb),nb=(new Location$$module$js$nodes(a,145,F,"Area 1 (from entrance)")).cave().chest(fc,26),Id=(new Location$$module$js$nodes(a,145,F,"Area 2 (across top bridge)")).cave().connect(nb,t);(new Location$$module$js$nodes(a,145,F,"Area 3 (dead-end across top-right bridge)")).cave().connect(Id,t);const gh=(new Location$$module$js$nodes(a,145,F,"Area 4 (left across middle-right bridge)")).cave().connect(Id,t),hh=(new Location$$module$js$nodes(a,
145,F,"Area 5 (bottom edge)")).cave().connect(gh,t),ih=(new Location$$module$js$nodes(a,145,F,"Area 6 (bottom island)")).cave().connect(hh,t),jh=(new Location$$module$js$nodes(a,145,F,"Area 7 (bottom inner ring)")).cave().connect(ih,t),kh=(new Location$$module$js$nodes(a,145,F,"Area 8 (left outer ring)")).cave().connect(jh,t).connect(nb,t).chest(M,27,100),lh=(new Location$$module$js$nodes(a,145,F,"Area 9 (top left inner ring)")).cave().connect(kh,t),mh=(new Location$$module$js$nodes(a,145,F,"Area 10 (top right inner ring)")).cave().connect(lh,
t);(new Location$$module$js$nodes(a,145,F,"Area 11 (center)")).cave().connect(mh,t).connectTo(Hd);(new Location$$module$js$nodes(a,145,F,"Area 12 (top center islands)")).cave().connect(nb,z).chest(de,28);const nh=(new Location$$module$js$nodes(a,146,aa,"Desert Cave 1")).cave().connect(Qb,z),Ha=(new Location$$module$js$nodes(a,147,aa,"Town")).town(),oh=(new Location$$module$js$nodes(a,148,aa,"Outside Cave")).overworld().connect(Ha),ph=(new Location$$module$js$nodes(a,149,aa,"Desert Cave 2")).cave().connect(oh);
(new Location$$module$js$nodes(a,150,aa,"Meadow")).overworld().connect(nh).connectTo(Ha).trigger(Be,pa,Ze);const Rb=(new Location$$module$js$nodes(a,152,aa,"Desert 2")).overworld().connect(ph),qh=(new Location$$module$js$nodes(a,156,Ca,"Entrance")).fortress().connect(Rb,z),rh=(new Location$$module$js$nodes(a,156,Ca,"Azteca")).fortress().trigger(De),sh=(new Location$$module$js$nodes(a,157,Ca,"Fork")).fortress().connect(qh),Jd=(new Location$$module$js$nodes(a,158,Ca,"Main")).fortress().connect(sh);
(new Location$$module$js$nodes(a,158,Ca,"Treasure Chest (magic ring)")).fortress().connect(Jd).chest(W,27,108);(new Location$$module$js$nodes(a,159,Ca,"Draygon")).fortress().connect(Jd).to(rh).boss(jf);const th=(new Location$$module$js$nodes(a,160,ca,"Entrance")).fortress().connect(Rb,z),uh=(new Location$$module$js$nodes(a,160,ca,"Statues")).fortress().connect(th).boss(kf),vh=(new Location$$module$js$nodes(a,161,ca,"Hall 1")).fortress().connect(uh),Sb=(new Location$$module$js$nodes(a,162,ca,"Branch")).fortress().connect(vh);
(new Location$$module$js$nodes(a,163,ca,"Left Dead End")).fortress().connect(Sb).chest(N,13,120);(new Location$$module$js$nodes(a,164,ca,"Right Dead End")).fortress().connect(Sb);const wh=(new Location$$module$js$nodes(a,165,ca,"Hall 2")).fortress().connect(Sb).chest(tb,26,109),xh=(new Location$$module$js$nodes(a,166,ca,"Draygon 2")).fortress().connect(wh).boss(lf);(new Location$$module$js$nodes(a,167,ca,"Teleporter")).fortress().from(xh).to(td);const Kd=(new Location$$module$js$nodes(a,168,eb,"Entrance")).fortress().connect(Oa,
Fb).trigger(Nc,J),yh=(new Location$$module$js$nodes(a,169,eb,"Main")).fortress().from(Kd,xa),zh=(new Location$$module$js$nodes(a,169,eb,"Boss")).fortress().connect(yh).boss(ef),Ah=(new Location$$module$js$nodes(a,170,eb,"Zebu")).fortress().connect(zh),ob=(new Location$$module$js$nodes(a,171,Z,"Entrance")).fortress().connect(Ah);(new Location$$module$js$nodes(a,171,Z,"Dead End Behind Iron (fruit of power)")).fortress().connect(ob,xa).chest(M,28,98);(new Location$$module$js$nodes(a,171,Z,"Dead End Loop Across Closer Bridges")).fortress().connect(ob,
t).connect(ob,t);const Bh=(new Location$$module$js$nodes(a,171,Z,"Across First Bridge (fruit of repun)")).fortress().connect(ob,t).chest(sb,30,102),Tb=(new Location$$module$js$nodes(a,171,Z,"Across Second Bridge")).fortress().connect(Bh,t);(new Location$$module$js$nodes(a,171,Z,"Dead End Across Two Bridges ()")).fortress().connect(Tb,t).connect(Tb,t).chest(wa,29,93);const Ch=(new Location$$module$js$nodes(a,171,Z,"Across Third Bridge")).fortress().connect(Tb,t),Dh=(new Location$$module$js$nodes(a,
171,Z,"Exit Behind Iron Door")).fortress().connect(Ch,xa),Eh=(new Location$$module$js$nodes(a,172,Z,"Boss")).fortress().connect(Dh).boss(ff),Fh=(new Location$$module$js$nodes(a,172,Z,"Tornel")).fortress().connect(Eh),Ld=(new Location$$module$js$nodes(a,173,Ba,"Lower")).fortress().connect(Fh).chest(tb,26,99).chest(W,27,111),Gh=(new Location$$module$js$nodes(a,174,Ba,"Upper Loop")).fortress().connect(Ld).chest(K,22,96);(new Location$$module$js$nodes(a,174,Ba,"Upper Loop Behind Wall (magic ring)")).fortress().connect(Gh,
xa).chest(W,23,107);const Hh=(new Location$$module$js$nodes(a,175,Ba,"Upper Passage (toward Mado)")).fortress().connect(Ld).chest(W,27,84),Ub=(new Location$$module$js$nodes(a,176,O,"Initial Fork")).fortress(),Ih=(new Location$$module$js$nodes(a,177,O,"Left Branch")).fortress().connect(Ub),Md=(new Location$$module$js$nodes(a,178,O,"Main Area (right branch, over bridges)")).fortress().connect(Ub),Jh=(new Location$$module$js$nodes(a,179,O,"U-shaped Passage (between floors)")).fortress().connect(Md),
Kh=(new Location$$module$js$nodes(a,180,O,"Main Area Lower (under bridge)")).fortress().connect(Ih).connect(Md).connect(Jh),Nd=(new Location$$module$js$nodes(a,180,O,"Behind Iron Wall")).fortress().connect(Kh,xa),Lh=(new Location$$module$js$nodes(a,181,O,"Lower")).fortress().connect(Nd).chest(N,13,121).chest(N,14,122).chest(N,15,123).chest(W,23,88).chest(R,24,110),Mh=(new Location$$module$js$nodes(a,182,O,"Boss Corridor")).fortress().connect(Lh),Nh=(new Location$$module$js$nodes(a,182,O,"Boss")).fortress().connect(Mh,
Fb).boss(hf);(new Location$$module$js$nodes(a,182,O,"Behind Boss (stormBracelet)")).fortress().connect(Nh).chest(Xa,18);const Od=(new Location$$module$js$nodes(a,183,O,"Exit Stairs")).fortress(),Oh=(new Location$$module$js$nodes(a,184,F,"Entrance Back (behind river)")).cave().connect(Od).chest(M,13,90);(new Location$$module$js$nodes(a,184,F,"Entrance Front")).cave().connect(Oh,z).connectTo(Qb).connectTo(nb);const Ph=(new Location$$module$js$nodes(a,185,Ba,"Boss")).fortress().connect(Hh).boss(gf);
(new Location$$module$js$nodes(a,185,Ba,"Asina")).fortress().connect(Ph).connectTo(Ub);(new Location$$module$js$nodes(a,186,O,"Kensu")).fortress().connect(Nd).connectTo(Od).trigger(Ae,zb);(new Location$$module$js$nodes(a,187,ta,"House")).house().connect(Oa).trigger(Ce,pa,zb);(new Location$$module$js$nodes(a,188,ta,"Inn")).shop().connect(Oa,Eb);(new Location$$module$js$nodes(a,190,ta,"Tool Shop")).shop(D,K,R,M).connect(Oa,Eb);(new Location$$module$js$nodes(a,191,ta,"Tavern")).shop().connect(Oa);(new Location$$module$js$nodes(a,
192,X,"Elder House")).house().connect(na).trigger(uc);(new Location$$module$js$nodes(a,193,X,"Rabbit Hut")).house().connect(na).trigger(Ac,zc,fa);(new Location$$module$js$nodes(a,194,X,"Inn")).shop().connect(na);(new Location$$module$js$nodes(a,195,X,"Tool Shop")).shop(D,K,R,M).connect(na);(new Location$$module$js$nodes(a,196,X,"Armor Shop")).shop($d,Yb).connect(na);(new Location$$module$js$nodes(a,197,X,"Student House")).house().connect(na).trigger(vc);(new Location$$module$js$nodes(a,198,za,"Tavern")).house().connect(Na);
(new Location$$module$js$nodes(a,199,za,"Pawn Shop")).shop().connect(Na);(new Location$$module$js$nodes(a,200,za,"Inn")).shop().connect(Na);(new Location$$module$js$nodes(a,201,za,"Armor Shop")).shop(ae,Yb,Xd).connect(Na);(new Location$$module$js$nodes(a,203,za,"Tool Shop")).shop(D,K,R).connect(Na);(new Location$$module$js$nodes(a,205,La,"Elder House")).house().from(gb,fa).trigger(pe,Bb);(new Location$$module$js$nodes(a,206,La,"Mother's House")).house().from(gb,fa).trigger(yc,fa).trigger(oe,Bb);(new Location$$module$js$nodes(a,
207,La,"Tool Shop")).shop(D,K,M).from(gb,fa);(new Location$$module$js$nodes(a,208,La,"Inn")).shop().from(gb,fa);(new Location$$module$js$nodes(a,209,Aa,"Inn")).shop().connect(fb);(new Location$$module$js$nodes(a,210,Aa,"Tool Shop")).shop(R,M,wa).connect(fb);(new Location$$module$js$nodes(a,211,Aa,"Armor Shop")).shop(bc,Zb,$b,qb).connect(fb);const Qh=(new Location$$module$js$nodes(a,212,Aa,"Queen's House")).house().from(fb,Se).trigger(ye,pa,oc);(new Location$$module$js$nodes(a,213,db,"Nadare's")).house().connect(hb).connectTo(Yf).connectTo(Zf).connectTo($f);
(new Location$$module$js$nodes(a,214,y,"Fisherman's House")).house().connect(rd).trigger(Cb,jc,yb,Te);const Vb=(new Location$$module$js$nodes(a,215,y,"Palace Entrance")).house().connect(ua);(new Location$$module$js$nodes(a,216,y,"Fortune Teller Front")).house().connect(ua).trigger(Cc,Bc);(new Location$$module$js$nodes(a,216,y,"Fortune Teller Back")).house().connect(Eg).connect(wd);(new Location$$module$js$nodes(a,217,y,"Pawn Shop")).shop().connect(ua);(new Location$$module$js$nodes(a,218,y,"Armor Shop")).shop(be,
bc,Zb).connect(ua);(new Location$$module$js$nodes(a,220,y,"Inn")).shop().connect(ua);(new Location$$module$js$nodes(a,221,y,"Tool Shop")).shop(D,wa,rb,R).connect(ua);(new Location$$module$js$nodes(a,222,y,"Palace Left")).house().connect(Vb);(new Location$$module$js$nodes(a,223,y,"Palace Throne Room")).house().connect(Vb).to(kb,Me).from(kb).trigger(Bc).trigger(se,Cc,Dc);(new Location$$module$js$nodes(a,224,y,"Palace Right")).house().connect(Vb);(new Location$$module$js$nodes(a,225,y,"Asina's Room")).house().connect(xd).trigger(Gc,
Qc);(new Location$$module$js$nodes(a,226,Aa,"Queen Downstairs")).house().connect(Qh).chest(Va,13);(new Location$$module$js$nodes(a,227,ka,"Elder's House")).house().connect(lb).trigger(Jc);(new Location$$module$js$nodes(a,228,ka,"Shed")).house().connect(lb).to(Pg,kc);(new Location$$module$js$nodes(a,229,ka,"Tool Shop")).shop(D,K,M,wa).connect(lb);(new Location$$module$js$nodes(a,231,ka,"Inn")).shop().connect(lb);const Rh=(new Location$$module$js$nodes(a,232,H,"Zombie Town House")).house().connect(Nb);
(new Location$$module$js$nodes(a,233,H,"Zombie Town Basement")).house().connect(Rh).trigger(ue,Yc);(new Location$$module$js$nodes(a,235,Y,"Tool Shop")).shop(D,K,M,R).connect(ea);(new Location$$module$js$nodes(a,236,Y,"Stom's Hut")).house().connect(ea).trigger(Kc);(new Location$$module$js$nodes(a,237,Y,"Inn")).shop().connect(ea);(new Location$$module$js$nodes(a,238,Y,"Armor Shop")).shop(ce,cc,Yd,ac).connect(ea);(new Location$$module$js$nodes(a,239,Y,"Tavern")).house().connect(ea).trigger(Lc,Kc,Ja);
(new Location$$module$js$nodes(a,240,Y,"Pawn Shop")).shop().connect(ea);(new Location$$module$js$nodes(a,241,Y,"Dance Hall")).house().connect(ea).trigger(Mc,Lc,Ja).trigger(xe,Mc,nc);const Sh=(new Location$$module$js$nodes(a,242,ma,"Temple (pre-massacre)")).fortress().connect(Ga).from(Gb,J,Td).trigger(ze);(new Location$$module$js$nodes(a,242,ma,"Temple (post-massacre)")).fortress().from(Sh,Nc).boss(df);(new Location$$module$js$nodes(a,243,ma,"Training Hall")).house().connect(Ga);(new Location$$module$js$nodes(a,
244,ma,"Hospital")).house().connect(Ga);(new Location$$module$js$nodes(a,245,ma,"Armor Shop")).shop(cc,qb,ac).connect(Ga);(new Location$$module$js$nodes(a,246,ma,"Tool Shop")).shop(D,K,rb,W).connect(Ga);(new Location$$module$js$nodes(a,247,ma,"Inn")).shop().connect(Ga);(new Location$$module$js$nodes(a,248,aa,"Inn")).shop().connect(Ha);(new Location$$module$js$nodes(a,249,aa,"Tool Shop")).shop(K,R,sb,W).connect(Ha);(new Location$$module$js$nodes(a,250,aa,"Elder's House")).house().connect(Ha);(new Location$$module$js$nodes(a,
251,aa,"Pawn Shop")).house().connect(Ha);const Th=[na,Ma,$c,Da,Hb,Ea,Kb,ld,Mb,ea,mb,Pb,Qb,Kd,Rb];for(const a of Th)a.from(Gb,He);return a},shuffle$$module$js$depgraph=(b,a,c,d,e)=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){const f=generate$$module$js$depgraph(d);d.shuffleShops()&&f.shuffleShops(a);const g=f.nodes.filter((a)=>a instanceof Slot$$module$js$nodes&&a.slots&&a.slotName);d||(d=new FlagSet$$module$js$flagset("Sbkm Sct"));var k={};for(var h of g)if(h instanceof Slot$$module$js$nodes&&
h.slots){var n=h.slotType[0];(k[n]=k[n]||[]).push(h)}h=[];n=[];var l={};for(var q of d.flags.S||[]){var p=[];for(var m of q){l[m]=!0;for(var r of k[m])p.push(r),n.push([r,h.length])}h.push(p)}for(var I of"bc")if(l[I])for(q=k[I],m=q.map((a)=>[a.item,a.itemIndex]),a.shuffle(m),r=0;r<q.length;r++)q[r].set(...m[r]);p=(a)=>g.find((b)=>b.vanillaItemName===a);k=p("Sword of Wind");I=p("Sword of Fire");q=p("Ball of Wind");m=p("Ball of Fire");r=p("Statue of Onyx");p=p("Gas Mask");l.k&&a.nextInt(2)&&(I.swap(k),
m.swap(q));l.k&&!a.nextInt(3)&&r.swap(p);l=[1,1,1,1,2,2,2,3,3,3,5,5,8,13];k=[];e&&e.addTasks(10);for(I=0;1E3>I;I++){e&&0==I%100&&(yield new Promise(requestAnimationFrame),e.addCompleted(1));a.shuffle(n);for(var C of h)a.shuffle(C);q=h.map(()=>-1);m=l[a.nextInt(l.length)];r=[];p=(a,b)=>a.itemIndex===b.itemIndex||a.isMimic()&&!b.canHoldMimic()||b.isMimic()&&!a.canHoldMimic()||a.needsChest()&&!b.isChest()||b.needsChest()&&!a.isChest()?!1:!0;for(let a=0;a<m;a++){const [b,c]=n[a],d=h[c][++q[c]];p(b,d)&&
...
shuffle2$$module$js$depgraph=(b,a,c,d,e)=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){const f=generate$$module$js$depgraph(d);d.shuffleShops()&&f.shuffleShops(a);const g=f.integrate();e&&e.addTasks(1E3);for(let k=0;1E3>k;k++){if(yield shuffle3$$module$js$depgraph(f,g,b,a,c,d,e)){console.log(`success after ${k} attempts`);return}e&&(e.addCompleted(1),0===k%50&&(yield new Promise(requestAnimationFrame)))}return shuffle$$module$js$depgraph(b,a,c,d,e)}),shuffle3$$module$js$depgraph=(b,a,
c,d,e,f,g)=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){const g=e&&e.stats;var h=b.nodes.filter((a)=>a instanceof Slot$$module$js$nodes&&a.slots&&a.slotName);const n=new Map(d.shuffle(h.map((a)=>[a,[a.item,a.itemIndex]]))),l=new Set(d.shuffle(h)),q=new Map,p={};g&&g.seeds++;f||(f=new FlagSet$$module$js$flagset("Sbkm Sct"));(f.flags.S||[]).forEach((a,b)=>{for(const c of a)p[c]=b});for(var m of l)q.set(m.item,m);let r=new Set;const I=(a,b)=>{if(null==p[a.slotType?a.slotType[0]:"c"])return a==
b;if(b.isMimic()&&!a.canHoldMimic()||"unique"!=b.item.inventoryRow&&a.requiresUnique||"armor"==b.item.inventoryRow&&!a.isChest())return!1;if(f.guaranteeSword()){if(0===a.slotIndex)return 4>b.item.id;if(4>b.item.id&&(r.add(b),4===r.size))return 0===a.slotIndex}return p[a.slotType?a.slotType[0]:"c"]==p[b.slotType?b.slotType[0]:"c"]};m=a.assumedFill(d,(a,b)=>I(a,q.get(b)));if(null==m){g&&g.endState.key++;return!1;throw Error("Could not fill!");}const C=new Map;for(var V=0;V<m.length;V++){var va=m[V];
if(null!=va){var oa=q.get(a.item(va));va=n.get(oa);n.delete(oa);oa=a.location(V);C.set(oa,va);l.delete(oa)}}V=(a,b,c)=>{c=void 0===c?"":c;for(const c of l)if(I(c,a))return C.set(c,b),l.delete(c),n.delete(a),!0;g&&("traps"==c?g.endState.traps++:"chest"==c?g.endState.chest++:g.endState.misc++);return!1};for(var E of n){const [a,b]=E;if(a.isMimic()&&!V(a,b,"traps"))return!1}for(const a of n){const [b,c]=a;if(b.needsChest()&&!V(b,c,"chest"))return!1}for(const a of n){const [b,c]=a;if(!V(b,c))return!1}for(const a of C){const [b,
c]=a;b.set(...c)}const {win:G,path:Pa}=b.traverse({wanted:h,dfs:!1});if(!G)return g&&g.endState.no_win++,!1;c&&b.write(c);if(!e)return!0;g&&(g.endState.success++,g.analyze(b,a,m));e.items=[];e.route=[];for(const a of Pa){const [c,d]=a;e.route.push(d);h=b.nodes[c];null!=h.slotName&&(E=h.slotName,0>E.indexOf(h.orig)&&(E+=` (normally ${h.vanillaItemName})`),e.items.push({slotIndex:h.slotIndex,itemIndex:h.itemIndex,origName:h.vanillaItemName,slotName:h.slotName,itemName:h.item.name,text:`${h.item.name}: ${E}`}))}return!0});
...
const breakLines$$module$js$util=(b,a)=>{b=b.trim();const c=[];for(;b.length>a;){let d=b.substring(0,a).lastIndexOf(" ");0>d&&(d=a);c.push(b.substring(0,d).trim());b=b.substring(d).trim()}c.push(b.trim());return c};var module$js$util={};module$js$util.Deque=Deque$$module$js$util;module$js$util.IndexedList=IndexedList$$module$js$util;module$js$util.breakLines=breakLines$$module$js$util;const Edge$$module$js$graph={},EdgeT$$module$js$graph={};Edge$$module$js$graph.of=(...b)=>b.map((a)=>a.uid);class Node$$module$js$graph{constructor(b,a){this.graph=b;this.name=a;this.uid=b.nodes.length;b.nodes.push(this)}get nodeType(){return"Node"}toString(){return`${this.nodeType} ${this.name}`}edges(b){return[]}write(b){}}
...
4<=e.sells.length?g.push(d):e.sells.push(d));for(const {shops:b}of[a,c])for(const a of b)a.sells.sort((a,b)=>a.id-b.id)}integrate(b={}){const {tracker:a=!1}=b;var c=!a;const d=new SparseDependencyGraph$$module$js$graph(this.nodes.length),e=[],f=[],g=new Set,k=new Map;var h=[],n=[];const l=[];var q=[];const p=[];b=new Set;for(var m of this.nodes)if(m instanceof Location$$module$js$nodes){if(m.isStart){const [a]=d.addRoute([m.uid]);k.set(a.label,a)}e[m.uid]=[];f[m.uid]=[];for(const a of m.connections)g.has(a)||
(g.add(a),a.bidi&&g.add(a.reverse()));for(const a of m.chests)d.addRoute([a.uid,m.uid])}else if(m instanceof ItemGet$$module$js$nodes&&"Medical Herb"===m.name&&(d.addRoute([m.uid]),d.finalize(m.uid)),m instanceof Option$$module$js$nodes)h.push(m);else if(m instanceof TrackerNode$$module$js$nodes)n.push(m);else if(m instanceof Condition$$module$js$nodes)l.push(m);else if(m instanceof Trigger$$module$js$nodes)q.push(m);else if(m instanceof ItemGet$$module$js$nodes)p.push(m);else if(m instanceof Slot$$module$js$nodes)b.add(m);
else if(!(m instanceof Area$$module$js$nodes))throw Error(`Unknown node type: ${m.nodeType}`);for(const a of g)e[a.from.uid].push(a),f[a.to.uid].push(a);m=(b)=>{for(const c of b){for(const b of c.edges({tracker:a}))d.addRoute(b);d.finalize(c.uid)}};m(h);c&&m(n);m(q);for(c=k.values();!(h=c.next()).done;){h=h.value;n=h.target;for(var r of e[n]){n=[r.to.uid,...h.deps];for(q=r.deps.length-1;0<=q;q--)n.push(r.deps[q].uid);r.from.bossNode&&n.push(r.from.bossNode.uid);for(const a of d.addRoute(n))k.has(a.label)||
k.set(a.label,a)}}m(l);for(r=0;r<this.nodes.length;r++)this.nodes[r]instanceof ItemGet$$module$js$nodes||this.nodes[r]instanceof TrackerNode$$module$js$nodes||d.finalize(r);r=new LocationList$$module$js$nodes(this);for(const a of b)for(const b of d.nodes[a.uid].values())r.addRoute([a.uid,...b]);return r}}
...
g=[...[].concat(...a.flags.map((a)=>[...a])),255],k=[].concat(...a.pits.map((a)=>[...a])),[h,n,l,q,p,m]=yield Promise.all([b.write(c),b.write(d),b.write(e),b.write(f),b.write(g),k.length?b.write(k):null]);c=[h&255,(h>>>8)-192,n&255,(n>>>8)-192,l&255,(l>>>8)-192,q&255,(q>>>8)-192,p&255,(p>>>8)-192,...m?[m&255,(m>>8)-192]:[]];c=yield b.write(c);b.rom[a.mapDataPointer]=c&255;b.rom[a.mapDataPointer+1]=(c>>>8)-192})}allScreens(){const b=new Set,a=this.extended?256:0;for(const c of this.screens)for(const d of c)b.add(this.rom.screens[d+
...
pat:a.spritePatterns[d],pal:a.spritePalettes[d]}}return b}get locationMonsters(){const b={};for(const a of this.locations){if(!a||!a.objects)continue;const c=b["$"+a.id.toString(16).padStart(2,0)]={};for(const b of a.objects)if(!(b[2]&7)){const a=b[2]&128?1:0,d=b[3]+80;c[`${a}:${d.toString(16)}`]=(c[`${a}:${d.toString(16)}`]||0)+1}}return b}static load(b){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){const a=yield pickFile$$module$js$rom();b&&(yield b(a));return new Rom$$module$js$rom(a)})}writeData(){const b=
this;return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){b.prg[246387]=b.prg[246392]=190;b.prg[246399]=b.prg[246404]=191;var a=new Writer$$module$js$rom(b.prg,83192,97792);const c=new Writer$$module$js$rom(b.prg,103417,113408),d=[];for(const e of b.locations)e&&(d.push(e.writeMapData(a)),d.push(e.writeNpcData(c)));yield a.commit();a=yield c.commit();a=new Writer$$module$js$rom(b.prg,Math.max(106496,a),113408);for(var e of b.objects)e.write(a,114176);yield a.commit();e=new Writer$$module$js$rom(b.prg,
123392,123888);for(var f of b.triggers)f.used&&d.push(f.write(e));yield e.commit();f=new Writer$$module$js$rom(b.prg,116602,117085);for(const a of b.npcSpawns)a.used&&d.push(a.write(f));yield f.commit();for(const a of b.tilesets)a.write(b.prg);for(const a of b.tileEffects)a.write(b.prg);for(const a of b.screens)a.write(b.prg);return Promise.all(d).then(()=>{})})}analyzeTiles(){}disjointTilesets(){var b=[];for(var a of this.locations)if(a){var c=a.tileset,d=a.extended?256:0;for(const e of a.screens)for(const a of e)(b[a+
d]||(b[a+d]=new Set)).add(c)}a=Array(256).fill(0).map(()=>new UnionFind$$module$js$unionfind);for(c=0;c<b.length;c++)if(b[c]){d=new Set;for(var e of this.screens[c].tiles)for(const a of e)d.add(a);for(const e of d)a[e].union([...b[c]])}for(b=0;b<a.length;b++)e=a[b].sets().map((a)=>[...a].map((a)=>a.toString(16)).join(" ")).join(" | "),console.log(`Tile ${b.toString(16).padStart(2,0)}: ${e}`)}swapMetatiles(b,...a){var c=new Map,d=seq$$module$js$rom(256);const e=new Map,f=(a)=>Array.isArray(a)?a[0]:
0>a?~a:a;for(var g of a){for(var k=0;k<g.length-1;k++)Array.isArray(g[k])&&(e.set(g[k][0],g[k][1]),g[k]=g[k][0]);for(k=0;k<g.length-1;k++){let a=g[k],b=g[k+1];0>a||0>b||(c.set(b,a),d[b]=a)}}g=new Set;c=new Set;b=new Set(b);for(var h of this.locations)if(h&&b.has(h.tileset)){c.add(h.tileEffects);for(const a of h.allScreens())g.add(a)}for(var n of g)for(var l of n.tiles)for(h=0;h<l.length;h++)l[h]=d[l[h]];for(var q of b){b=this.tilesets[(q&127)>>>2];for(var p of a)for(d=0;d<p.length-1;d++){n=f(p[d]);
l=f(p[d+1]);for(h=0;4>h;h++)b.tiles[h][n]=b.tiles[h][l];b.attrs[n]=b.attrs[l];if(32>l&&b.alternates[l]!=l){if(32<=n)throw Error(`Cannot unflag: ${q} ${n} ${l} ${b.alternates[l]}`);b.alternates[n]=b.alternates[l]}}for(var m of e){const [a,c]=m;b.alternates[a]=c}}for(const d of c){q=this.tileEffects[d-179];for(const c of a)for(p=0;p<c.length-1;p++)m=f(c[p]),b=f(c[p+1]),q.effects[m]=q.effects[b];for(const a of e.keys())q.effects[a]|=8}}}
...
return Math.min(AM$$module$js$random*this.iy,RNMX$$module$js$random)}nextInt(b){return Math.floor(this.next()*b)}shuffle(b){for(let a=b.length;a;){const c=this.nextInt(a--);[b[a],b[c]]=[b[c],b[a]]}return b}}var module$js$random={};module$js$random.Random=Random$$module$js$random;const STATUS$$module$js$version="rc",VERSION$$module$js$version="sg",LABEL$$module$js$version="sg e159b9e",HASH$$module$js$version="e159b9e0c0b9c77fc4fe2bc8bdbd8876ced44926",DATE$$module$js$version=new Date(1558693561E3);var module$js$version={};module$js$version.STATUS=STATUS$$module$js$version;module$js$version.VERSION=VERSION$$module$js$version;module$js$version.LABEL=LABEL$$module$js$version;module$js$version.HASH=HASH$$module$js$version;module$js$version.DATE=DATE$$module$js$version;var $jscompDefaultExport$$module$js$patch={apply(b,a,c){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){let d;a.seed||(a.seed=parseSeed$$module$js$patch("").toString(16),window.location.hash+="&seed="+a.seed);d=a.flags?new FlagSet$$module$js$flagset(a.flags):new FlagSet$$module$js$flagset("Em Gt Mr Rlpt Sbk Sct Sm Tasd");for(const b in a)"false"===a[b]&&(a[b]=!1);yield shuffle$$module$js$patch(b,parseSeed$$module$js$patch(a.seed),d,new FetchReader$$module$js$fetchreader(c))})}};
...
_SAHARA_RABBITS_REQUIRE_TELEPATHY:c.saharaRabbitsRequireTelepathy(),_TELEPORT_ON_THUNDER_SWORD:c.teleportOnThunderSword()},h=new Assembler$$module$js$6502,n=(a)=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){h.assemble(yield d.read(a),a);h.patchRom(b)});var l=Object.keys(k).filter((a)=>k[a]).map((a)=>`define ${a} 1\n`).join("");h.assemble(l,"flags.s");yield n("preshuffle.s");g=new Random$$module$js$random(g);yield shuffle2$$module$js$depgraph(b,g,e,c,f);rescaleShops$$module$js$patch(b,
h,c.bargainHunting()?g:null);l=new Rom$$module$js$rom(b);rescaleMonsters$$module$js$patch(b,l);c.shuffleMonsters()&&shuffleMonsters$$module$js$patch(b,l,g);identifyKeyItemsForDifficultyBuffs$$module$js$patch(l);c.doubleBuffMedicalHerb()?(b[115996]*=2,b[115962]*=3):c.buffMedicalHerb()&&(b[115996]+=16,b[115962]*=2);c.connectLimeTreeToLeaf()&&connectLimeTreeToLeaf$$module$js$patch(l);addCordelWestTriggers$$module$js$patch(l,c);c.disableRabbitSkip()&&fixRabbitSkip$$module$js$patch(l);c.storyMode()&&storyMode$$module$js$patch(l);
c.chargeShotsOnly()&&disableStabs$$module$js$patch(l);c.orbsOptional()&&orbsOptional$$module$js$patch(l);closeCaveEntrances$$module$js$patch(l,c);misc$$module$js$patch(l,c);buffDyna$$module$js$patch(l,c);yield n("postshuffle.s");updateDifficultyScalingTables$$module$js$patch(b,c,h);updateCoinDrops$$module$js$patch(b,c);shuffleRandomNumbers$$module$js$patch(b,g);yield l.writeData();return stampVersionSeedAndHash$$module$js$patch(b,a,c)}),misc$$module$js$patch=(b,a)=>{},buffDyna$$module$js$patch=(b,
a)=>{b.objects[184].collisionPlane=1;b.objects[184].immobile=1;b.objects[185].collisionPlane=1;b.objects[185].immobile=1;b.objects[51].collisionPlane=2},closeCaveEntrances$$module$js$patch=(b,a)=>{b.swapMetatiles([144],[7,[1,0],-194],[14,[2,0],-194],[32,[3,10],-216],[33,[4,10],-216]);b.swapMetatiles([148,156],[104,[1,0],-194],[131,[2,0],-194],[136,[3,10],-216],[137,[4,10],-216]);b.screens[10].tiles[3][8]=1;b.screens[10].tiles[3][9]=2;b.screens[10].tiles[4][8]=3;b.screens[10].tiles[4][9]=4;b.screens[21].tiles[7][9]=
...
c){d[0]=f;return}throw Error(`Could not find flag to replace at ${a}:${c}`);};a.paralysisRequiresPrisonKey()&&(c(20,48,238),c(21,48,238),c(64,0,216),b.locations[64].objects.splice(1,0,[6,6,4,44]),b.locations[64].objects.push([7,7,2,173]))},eastCave$$module$js$patch=(b)=>{},addCordelWestTriggers$$module$js$patch=(b,a)=>{for(const c of b.locations[21].objects)2===c[2]&&(128>c[3]||a.disableTeleportSkip())&&b.locations[20].objects.push([...c])},fixRabbitSkip$$module$js$patch=(b)=>{for(const a of b.locations[40].objects)2===
a[2]&&134===a[3]&&116===a[1]&&(a[0]++,a[1]++)},storyMode$$module$js$patch=(b)=>{const a=b.npcSpawns[203].conditions[166];a.push(~b.npcSpawns[194].conditions[40][0]);a.push(~b.npcSpawns[132].conditions[110][0]);a.push(~b.triggers[26].conditions[1]);a.push(~b.npcSpawns[197].conditions[169][0]);a.push(~b.npcSpawns[198].conditions[172][0]);a.push(~b.npcSpawns[199].conditions[185][0]);a.push(~b.npcSpawns[200].conditions[182][0]);a.push(~b.npcSpawns[203].conditions[159][0]);a.push(512);a.push(513);a.push(514);
a.push(515)},disableStabs$$module$js$patch=(b)=>{for(const a of[8,9,39])b.objects[a].collisionPlane=0},orbsOptional$$module$js$patch=(b)=>{for(const a of[16,20,24,29])b.objects[a].terrainSusceptibility&=-5,b.objects[a].level=2},connectLimeTreeToLeaf$$module$js$patch=(b)=>{const a=b.locations[3];b=b.locations[66];a.screens[5][4]=16;b.screens[1][0]=26;b.screens[2][0]=12;const c=a.entrances.push([239,4,120,5])-1,d=b.entrances.push([16,0,192,1])-1;a.exits.push([79,86,66,d],[79,87,66,d]);b.exits.push([0,
...
7,,60,37,6,106],[155,"b","Draygon",5,6,16,41,,,],[158,"b","Draygon 2",7,6,28,40,,,],[160,"m","Ground Sentry (1)",4,,12,26,,],[161,"m","Tower Defense Mech (2)",5,,16,36,,],[162,"m","Tower Sentinel",,,2,,,],[163,"m","Air Sentry",3,,4,26,,],[165,"b","Vampire 2",3,,12,27,,,],[164,"b","Dyna",6,5,32,,,,],[180,"b","dyna pod",6,5,48,26,,,],[184,"p","dyna counter",15,,,42,,,],[185,"p","dyna laser",15,,,42,,,],[186,"p","dyna bubble",,,,36,,,],[188,"m","vamp2 bat",,,,16,,15],[191,"p","draygon2 fireball",,,,
...
a[3]+80;if(k in UNTOUCHED_MONSTERS$$module$js$patch||!SCALED_MONSTERS$$module$js$patch.has(k)||"m"!=SCALED_MONSTERS$$module$js$patch.get(k).type)continue;var f=b.rom.objects[k];if(!f)continue;const n=a[2]&128?1:0,l=b.spritePatterns[n];var g=f.palettes(!0);f=g.includes(2)?b.spritePalettes[0]:null;g=g.includes(3)?b.spritePalettes[1]:null;d.push({id:k,pat:l,pal2:f,pal3:g,patBank:n});(this.report[`start-${k.toString(16)}`]=this.report[`start-${k.toString(16)}`]||[]).push("$"+b.id.toString(16));c.push(e)}d.length&&
...
this.report["$"+d.id.toString(16).padStart(2,0)]=[];const {maxFlyers:g,nonFlyers:k={},fixedSlots:h={}}=MONSTER_ADJUSTMENTS$$module$js$patch[d.id]||{};let n=h.pat0||null,l=h.pat1||null,q=h.pal2||null,p=h.pal3||null,m=g,r=!1;for(const b of d.objects||[])if(2==(b[2]&7)&&(r=!0),!(b[2]&7)){var a=b[3]+80;126==a||127==a||159==a?l=98:143==a&&(n=97)}if(21==d.id||71==d.id)r=!1;f.push(`Initial pass: ${[r,n,l,q,p].join(", ")}`);a=(a)=>{var b=FLYERS$$module$js$patch.has(a.id),c=MOTHS_AND_BATS$$module$js$patch.has(a.id);
if(b){if(!m)return!1;--m}if(null!=q&&null!=a.pal2&&q!=a.pal2||null!=p&&null!=a.pal3&&p!=a.pal3)return!1;var g=!r||TREASURE_CHEST_BANKS$$module$js$patch.has(a.pat);let h;if(d.rom.objects[a.id].child||RETAIN_SLOTS$$module$js$patch.has(a.id)){const b=(h=a.patSlot?128:0)?l:n;if(null!=b&&b!=a.pat)return!1;if(h)l=a.pat;else if(g)n=a.pat;else return!1;f.push(`  Adding ${a.id.toString(16)}: pat(${h}) <-  ${a.pat.toString(16)}`)}else if(null==n&&g||n==a.pat)n=a.pat,h=0,f.push(`  Adding ${a.id.toString(16)}: pat0 <-  ${a.pat.toString(16)}`);
...
[]).push("$"+d.id.toString(16));b=e[g];c=d.objects[b-13];b in k&&(c[0]+=k[b][0],c[1]+=k[b][1]);c[2]=c[2]&127|h;c[3]=a.id-80;f.push(`    slot ${b.toString(16)}: objData=${c}`);e.splice(g,1);return!0};if(m)for(var c=0;c<Math.min(40,this.monsters.length);c++)FLYERS$$module$js$patch.has(this.monsters[c].id)&&a(this.monsters[c])&&this.monsters.splice(c,1),b.shuffle(this.monsters);for(c=0;c<this.monsters.length&&e.length;c++)if(a(this.monsters[c])){const [a]=this.monsters.splice(c,1);FLYERS$$module$js$patch.has(a.id)||
