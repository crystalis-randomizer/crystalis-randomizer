...
import {Node, NodeId} from './graph.js';
import {
  Area,
  Boss,
  Condition,
...
export const generate = (flags: FlagSet = new FlagSet(), rom?: Rom): WorldGraph => {
  ...
  const item = (id: number, name: string): Item =>
      new Item(graph, id, name);
  const magic = (id: number, name: string): Magic =>
      new Magic(graph, id, name);
  const trigger = (name: string): Trigger =>
      new Trigger(graph, name);
  const condition = (name: string): Condition =>
      new Condition(graph, name);
  const boss = (index: number, name: string, ...deps: Node[]): Boss =>
      new Boss(graph, index, name, ...deps);
  const area = (name: string): Area =>
      new Area(graph, name);
  const location = (id: number, areaName: Area, name: string): Location =>
      new Location(graph, id, areaName, name);
  const sink = (...x: unknown[]): void => {}; // prevent unused code errors
  ...
  const glitch = (opt: Option, items?: (ItemGet | Slot)[], weight?: number) =>
      opt.value ? opt : GLITCH;
  const hard = (opt: Option, items?: (ItemGet | Slot)[], weight?: number) =>
      opt.value ? opt : HARD;
  ...
  const mtSabreWestTornel     = location(0x21, SBRW, 'Tornel Dead End').overworld()
                                  .trigger(talkedToTornelOnMtSabre,
                                           ballOfWind, tornadoBracelet)
  ...
  sink(mtSabreNorthTunnel2b, mtSabreNorthTunnel4, mtSabreNorthTunnel5b, mtSabreNorthLeftCell2,
       mtSabreNorthTunnel9, mtSabreNorthTunnel1, mtSabreNorthTunnel7);
  ...
  sink(fortress2b, fortress2c, fortress2f, fortress3UpperDeadEnd, fortress4End,
       oasisCaveEntrance, fortressAsina, fortressKensu);
  ...
  sink(goaHouse, goaInn, goaToolShop, goaTavern, leafElderHouse, leafRabbitHut, leafInn,
       leafToolShop, leafArmorShop, leafStudentHouse, brynmaerTavern, brynmaerPawnShop,
       brynmaerInn, brynmaerArmorShop, brynmaerToolShop, oakElderHouse, oakMotherHouse,
       oakToolShop, oakInn, amazonesInn, amazonesToolShop, amazonesArmorShop, nadare,
       portoaFishermanHouse, portoaFortuneTeller1, portoaFortuneTeller2, portoaPawnShop,
       portoaArmorShop, portoaInn, portoaToolShop, portoaPalaceLeft, portoaThroneRoom,
       portoaPalaceRight, portoaAsinaRoom, aryllisDownstairs, joelElderHouse, joelShed,
       joelToolShop, joelInn, zombieTownBasement, swanToolShop, swanStomHut, swanInn,
       swanArmorShop, swanTavern, swanPawnShop, swanDanceHall, shyronTemple2,
       shyronTrainingHall, shyronHospital, shyronArmorShop, shyronToolShop, shyronInn,
       saharaInn, saharaToolShop, saharaElderHouse, saharaPawnShop);
...
export const shuffle = async (rom: Rom,
  ...
  const buckets: {[type: string]: Slot[]} = {};
  ...
  const pools: Slot[][] = [];
  const all: [Slot, number][] = [];
  const shuffled: {[type: string]: boolean} = {};
  // let magicPool = null;
  // let keyPool = null;
  ...
  for (const bucket of 'bc') {
    ...
    const slots: Slot[] = buckets[bucket];
    const items: [ItemGet, number][] = slots.map(slot => [slot.item, slot.itemIndex]);
  ...
  const findSlot = (name: string) =>
                   allSlots.find(n => (n as Slot).vanillaItemName === name);

  const swordOfWind = findSlot('Sword of Wind') as Slot;
  const swordOfFire = findSlot('Sword of Fire') as Slot;
  const ballOfWind = findSlot('Ball of Wind') as Slot;
  const ballOfFire = findSlot('Ball of Fire') as Slot;
  const statueOfOnyx = findSlot('Statue of Onyx') as Slot;
  const gasMask = findSlot('Gas Mask') as Slot;
  const keyItems = 'k';
  ...
  for (let i = 0; i < 1000; i++) {
    ...
    const swaps: [Slot, Slot][] = [];

    const canSwap = (s1: Slot, s2: Slot) => {
...
export const shuffle3 = async (graph: WorldGraph,
  ...
  (flags.get('S')).forEach((pool: string, index: number) => {
    for (const type of pool) {
      buckets[type] = index;
    }
  });
  ...
  const fits = (slot: Slot, item: Slot) => {
  ...
  const filling =
      locationList.assumedFill(
          random,
          (slot: Slot, item: ItemGet) => fits(slot, itemToSlot.get(item)!));
  ...
  for (let slot = 0 as NodeId; slot < filling.length; slot++) {
  ...
  const findSlot = (item: Slot, args: [ItemGet, number], type: string = '') => {
    for (const slot of allSlots) {
