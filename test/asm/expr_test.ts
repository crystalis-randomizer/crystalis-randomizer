...
describe('Expr', function() {
  ...
  describe('Expr.parseOnly', function() {
    ...
    it('should return the expression', function() {
      expect(Expr.parseOnly([tnum(1)])).toEqual(num(1));
    });
  ...
  describe('Expr.evaluate', function() {
    it('should preserve numbers', function() {
      expect(Expr.evaluate(num(5))).toEqual(num(5));
    });

    it('should preserve offsets', function() {
      expect(Expr.evaluate(off(5, 0))).toEqual(off(5, 0));
    });

    it('should preserve symbols', function() {
      expect(Expr.evaluate(sym('foo'))).toEqual(sym('foo'));
    });

    it('should preserve expressions on symbols', function() {
      const expr = op('+', sym('foo'), num(1));
      expect(Expr.evaluate(expr)).toBe(expr);
    });

    it('should return perform simple arithmetic', function() {
      expect(Expr.evaluate(op('+', num(5), num(14)))).toEqual(num(19));
    });

    it('should perform binary bitwise operations', function() {
      expect(Expr.evaluate(op('|', num(0x416), num(0x241)))).toEqual(num(0x657));
    });

    it('should perform unary bitwise operations', function() {
      expect(Expr.evaluate(op('<', num(0x416)))).toEqual(num(0x16));
    });

    it('should perform logical operations', function() {
      expect(Expr.evaluate(op('<', num(4), num(2)))).toEqual(num(0));
      expect(Expr.evaluate(op('<', num(2), num(4)))).toEqual(num(1));
    });

    it('should add pure numbers to offsets', function() {
      expect(Expr.evaluate(op('+', off(5, 0), num(1)))).toEqual(off(6, 0));
    });

    it('should evaluate a bank byte', function() {
      const arg = {op: 'num', num: 1, meta: {bank: 2}};
      expect(Expr.evaluate(op('^', arg))).toEqual(num(2));
    });
  });
  ...
  //     expect(Expr.resolve(sym('qux'), resolver)).toEqual(num(5));
  //   });

  //   it('should not resolve a global symbol table entry', function() {
  //     expect(Expr.resolve(sym('baz'), resolver)).toEqual({op: 'sym', num: 6});
  //   });
  ...
  //     expect(Expr.resolve(sym('*'), resolver)).toEqual(off(0x14, 3));
  //   });

  //   it('should substitute fixed offsets with numbers', function() {
  //     expect(Expr.resolve(sym('foo'), resolver)).toEqual(num(0x9023));
  //   });
