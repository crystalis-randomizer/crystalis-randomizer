...
function num(num: number) { return {op: 'num', num, size: 1 + +(num > 255)}; }
function op(op: string, ...args: Expr[]) { return {op, args}; }
function op1(op: string, ...args: Expr[]) { return {op, args, size: 1}; }
function sym(sym: string) { return {op: 'sym', sym}; }
function off(num: number, chunk: number) { return {op: 'off', num, chunk}; }
...
describe('Expr', function() {
  ...
  describe('Expr.parseOnly', function() {
    ...
    it('should return the expression', function() {
      expect(Expr.parseOnly([tnum(1)])).to.eql({op: 'num', num: 1, size: 1});
    });
  ...
  describe('Expr.evaluate', function() {
    it('should return a number for simple numeric expressions', function() {
      expect(Expr.evaluate(num(5))).to.equal(5);
    });

    it('should return a number for simple arithmetic', function() {
      expect(Expr.evaluate(op('+', num(5), num(14)))).to.equal(19);
    });

    it('should return a number for bitwise operations', function() {
      expect(Expr.evaluate(op('<', op('|', num(0x416), num(0x241)))))
          .to.equal(0x57);
    });

    it('should return a number for logical operations', function() {
      expect(Expr.evaluate(op('<', num(4), num(2)))).to.equal(0);
      expect(Expr.evaluate(op('<', num(2), num(4)))).to.equal(1);
    });

    it('should return undefined for a symbol', function() {
      expect(Expr.evaluate(sym('foo'))).to.be.undefined;
    });

    it('should return undefined for expressions on symbols', function() {
      expect(Expr.evaluate(op('+', sym('foo'), num(1)))).to.be.undefined;
    });

    it('should return undefined for an offset', function() {
      expect(Expr.evaluate({op: 'off', num: 5})).to.be.undefined;
    });

    it('should return undefined for expressions on offsets', function() {
      expect(Expr.evaluate(op('+', {op: 'off', num: 5}, num(1))))
          .to.be.undefined;
    });

    it('should return undefined for bank byte', function() {
      expect(Expr.evaluate(op('^', num(1)))).to.be.undefined;
    });
  });
  ...
  describe('Expr.resolve', function() {
    ...
    it('should annotate zp offsets', function() {
      expect(Expr.resolve(sym('bar'), resolver))
         .to.eql({op: 'off', num: 0x37, chunk: 1, size: 1});
    });
  });
});
