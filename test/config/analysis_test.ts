...
function analyze(config: IConfig|string, presets = false): AnalyzeResult {
...
describe('Analyzer', function() {
  ...
  describe('analyze', function() {
    ...
    it('should analyze simple assignments', function() {
      expect(analyze({items: {chargeSpeed: 4}})).to.eql({
        mutations: {
          'items.chargeSpeed': {op: '=', value: fixed(4)},
        },
      });
    });

    it('should fold in a preset', function() {
      const config = {
        presets: ['foo'],
        nested: {
          foo: {items: {chargeSpeed: 6}},
          bar: {items: {chargeWhileWalkingSpeed: 5}},
        },
      };
  ...
  describe('analyzeExpr', function() {
    ...
    it('should recognize definite config property assignments', function() {
      expect(analyze('placement.random_armors = true')).to.eql({
        mutations: {
          'placement.randomArmors': {op: '=', value: fixed(true)},
        },
      });
    });
    ...
    it('should recognize definite op-equals config property assignments', function() {
      expect(analyze('items.charge_speed += 1')).to.eql({
        mutations: {
          'items.chargeSpeed': {op: '+=', value: fixed(1)},
        },
      });
    });
    ...
    it('should recognize definite config assignment to unknown', function() {
      expect(analyze('items.charge_speed = x')).to.eql({
        mutations: {
          'items.chargeSpeed': {op: '=', value: complex},
        },
      });
    });
    ...
    it('should recognize known-probability assignment', function() {
      expect(analyze('items.charge_speed = rand() < 0.4 ? 3 : 4')).to.eql({
        mutations: {
          'items.chargeSpeed': {op: '=', value: pick([0.4, 3], [0.6, 4])},
        },
      });
    });
    ...
    it('should recognize assignment to map', function() {
      expect(analyze('placement.force = {}')).to.eql({
        mutations: {
          'placement.force': {op: '=', value: complex},
        },
      });
    });
    ...
    it('does simple math on rhs', function() {
      expect(analyze('items.charge_speed = 1 + 2')).to.eql({
        mutations: {
          'items.chargeSpeed': {op: '=', value: fixed(3)},
        },
      });
    });
  });
